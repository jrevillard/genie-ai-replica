...
async function createArangoUser() {
  ...
  console.log(
    "This script will create a genie-ai-manager user in the database.",
  );
  ...
  try {
    ...
    console.log(
      `Checking for existing user with loginName: "${managerUser.loginName}"...`,
    );
    ...
    if (existingUser) {
      ...
      console.log(
        `User "${managerUser.loginName}" already exists. No action taken.`,
      );
    } else {
      // 2b. If user does not exist, create it
      console.log(
        `User "${managerUser.loginName}" not found. Creating new user...`,
      );
      const result = await usersCollection.save(managerUser, {
        returnNew: true,
      });
