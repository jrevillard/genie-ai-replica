...
class TestResolveCtx:
    ...
    def test_from_config_only(self):
        ctx = ctm.resolve_ctx(
            self._ENV, {"platform": "gitlab", "host": "gl.example", "project": "g/p"}
        )
    ...
    def test_env_overrides_config(self):
        env = {
            **self._ENV,
            "host_override": "override.example",
            "platform_override": "github",
        }
        ctx = ctm.resolve_ctx(
            env, {"platform": "gitlab", "host": "config.example", "project": "g/p"}
        )
...
class TestListOpenMrs:
    ...
    def test_gitlab_non_json_body_returns_empty(self):
        recorder = _Recorder()
        recorder.add(
            _Recorder.cmd_starts_with("glab"), _FakeProc(0, "<html>oops</html>")
        )
        assert ctm.list_open_mrs(recorder, _gitlab_ctx()) == []
...
class TestCloseOne:
    def test_gitlab_success(self):
        recorder = _Recorder()
        recorder.add(
            _Recorder.cmd_starts_with("glab"), _FakeProc(0, '{"state": "closed"}')
        )
    ...
    def test_github_success(self):
        recorder = _Recorder()
        recorder.add(
            _Recorder.cmd_starts_with("gh"), _FakeProc(0, "✓ Closed pull request #11\n")
        )
...
class TestRunEndToEnd:
    ...
    def _run(
        self, env, tmp_path, *, runner_script=None, config_body=None, run_dir=None
    ):
        repo = tmp_path / "repo"
        ...
        runner_script = [
            ...
            (
                lambda cmd: (
                    cmd[0] == "glab"
                    and cmd[1] == "api"
                    and "merge_requests/17" in cmd[2]
                ),
                _FakeProc(409, stdout="{}", stderr="Merge request is already closed"),
            ),
        ]
        ...
        runner_script = [
            ...
            (
                lambda cmd: (
                    cmd[0] == "glab"
                    and cmd[1] == "api"
                    and "merge_requests/42" in cmd[2]
                ),
                _FakeProc(403, stderr="403 Forbidden"),
            ),
        ]
    ...
    def test_env_settings_override_config(self, tmp_path: Path):
        ...
        (repo / "_bmad" / "custom" / "issue-tracking.yaml").write_text(
            cfg, encoding="utf-8"
        )
