...
class LogsService {
  ...
  async init() {
    ...
    try {
      ...
      logger.error(`Error initializing LogsService: ${error.message}`, {
        stack: error.stack,
      });
      throw error;
    }
  ...
  async getLogsSummary(options = {}) {
    try {
      ...
      logger.info(
        `[SUMMARY-DEBUG] Found ${logFiles.length} files for summary: ${logFiles.join(", ")}`,
      );
      ...
      for (const logFile of logFiles) {
        try {
          ...
          if (logLines.length > MAX_LINES_TO_PROCESS) {
            logger.warn(
              `Too many log lines in ${logFile}, limiting to ${MAX_LINES_TO_PROCESS}`,
            );
          ...
          const parsedLogs = this.parseLogs(
            logLines,
            isErrorLog ? "ERROR" : null,
          );
          allParsedLogs.push(...parsedLogs);
        } catch (fileError) {
          logger.error(
            `Error processing file ${logFile} for summary: ${fileError.message}`,
          );
        }
      }

      logger.info(
        `[SUMMARY-DEBUG] Total parsed entries from all files: ${allParsedLogs.length}`,
      );

      // Filter logs for the specified date
      const filteredLogs = allParsedLogs.filter((log) => log.date === date);
      logger.info(
        `[SUMMARY-DEBUG] Filtered logs for date ${date}: ${filteredLogs.length}`,
      );
      ...
      logger.info(
        `[SUMMARY-DEBUG] Filtered counts: ${errorLogs.length} ERRORs, ${warningLogs.length} WARNs`,
      );

      const errorLogsSummary = this.groupLogs(errorLogs);
      const warningLogsSummary = this.groupLogs(warningLogs);
      logger.info(
        `[SUMMARY-DEBUG] Grouped summary: ${errorLogsSummary.length} error types, ${warningLogsSummary.length} warning types.`,
      );
      ...
      logger.info(
        `[SUMMARY-DEBUG] Final summary object:`,
        JSON.stringify(finalResult, null, 2),
      );

      return finalResult;
    } catch (error) {
      logger.error(`Error in getLogsSummary: ${error.message}`, {
        stack: error.stack,
      });
      throw error;
    }
  ...
  async searchLogs(options = {}) {
    try {
      ...
      if (
        !this.getDateRange ||
        !this.getLogFilesInRange ||
        !this.readLogFile ||
        !this.parseLogs
      ) {
        logger.error(
          "One or more required methods are undefined in LogsService",
          {
      ...
      for (const file of logFiles) {
        try {
          ...
          if (logLines.length > MAX_LINES_TO_PROCESS) {
            logger.warn(
              `Too many log lines in ${file} (${logLines.length}), limiting to ${MAX_LINES_TO_PROCESS}`,
            );
            logLines = logLines.slice(0, MAX_LINES_TO_PROCESS);
          }
      ...
      logger.info(
        `Parsed ${allLogs.length} total log entries from ${totalLinesProcessed} lines before filtering`,
      );
      ...
      if (searchParams.level && searchParams.level.trim() !== "") {
        ...
        filteredLogs = filteredLogs.filter(
          (log) =>
            ...
            (targetLevel === "WARN" && log.level === "WARNING"),
        );
        logger.debug(
          `After level filter (${targetLevel}): ${filteredLogs.length} logs`,
        );
      }
      ...
      if (searchParams.service && searchParams.service.trim() !== "") {
        ...
        logger.debug(
          `After service filter (${searchParams.service}): ${filteredLogs.length} logs`,
        );
      }
      ...
      if (searchParams.term && searchParams.term.trim() !== "") {
        filteredLogs = filteredLogs.filter(
          (log) =>
            log.message &&
            log.message.toLowerCase().includes(searchParams.term.toLowerCase()),
        );
        logger.debug(
          `After term filter (${searchParams.term}): ${filteredLogs.length} logs`,
        );
      }
      ...
      logger.debug(
        `Log search response: ${formattedLogs.length} logs returned out of ${allLogs.length} total`,
      );
      ...
      logger.error(`Error in searchLogs: ${error.message}`, {
        stack: error.stack,
      });
      throw error;
    }
    ...
    try {
      ...
      try {
        ...
        if (stats.size > MAX_LOG_FILE_SIZE) {
          logger.warn(
            `Log file too large (${Math.round(stats.size / 1024 / 1024)}MB), reading first ${Math.round(MAX_LOG_FILE_SIZE / 1024 / 1024)}MB`,
          );
  ...
  async getLogFilesInRange(startDate, endDate, includeArchived = true) {
    try {
      if (!isValidDateStr(startDate) || !isValidDateStr(endDate)) {
        logger.warn("getLogFilesInRange.invalid_date_params", {
          startDate,
          endDate,
        });
        return [];
      ...
      if (startDate <= today && today <= endDate) {
        ...
        for (const file of currentLogs) {
          ...
          try {
            ...
            logger.debug(
              `Current log not accessible: ${filePath}, error: ${error.message}`,
            );
          }
        }
      }
      ...
      if (includeArchived) {
        for (const file of files) {
          const dateMatch = file.match(
            /^(combined|error)-(\d{4}-\d{2}-\d{2})\.log(?:\.\d+)?(?:\.gz)?$/,
          );
          if (!dateMatch) {
          ...
          if (fileDate >= startDate && fileDate <= endDate) {
            ...
            try {
              ...
              logger.debug(
                `Could not access log file ${filePath}: ${error.message}`,
              );
            }
          } else {
            logger.debug(
              `File ${file} date ${fileDate} out of range ${startDate} to ${endDate}`,
            );
          }
        }
      }
      ...
      logFiles.sort((a, b) => {
        ...
        if (
          aFile.startsWith("combined.log") ||
          aFile.startsWith("combined1.log") ||
          aFile.startsWith("error.log")
        )
          return -1;
        if (
          bFile.startsWith("combined.log") ||
          bFile.startsWith("combined1.log") ||
          bFile.startsWith("error.log")
        )
          return 1;

        const aMatch = aFile.match(
          /^(combined|error)-(\d{4}-\d{2}-\d{2})\.log(?:\.(\d+))?(?:\.gz)?$/,
        );
        const bMatch = bFile.match(
          /^(combined|error)-(\d{4}-\d{2}-\d{2})\.log(?:\.(\d+))?(?:\.gz)?$/,
        );
      ...
      if (logFiles.length === 0) {
        logger.warn(
          `No log files found for date range ${startDate} to ${endDate}`,
        );
      } else {
        logger.info(
          `Found ${logFiles.length} log files in range ${startDate} to ${endDate}: ${logFiles.join(", ")}`,
        );
      }
      ...
      for (const filePath of logFiles) {
        try {
          const stats = await fs.stat(filePath);
          logger.debug(
            `File ${filePath}: size=${stats.size} bytes, modified=${stats.mtime}`,
          );
        } catch (error) {
  ...
  groupLogs(logs) {
    ...
    const summaryPatterns = [
      ...
      {
        regex: /disk space below threshold/i,
      ...
      for (let index = 0; index < logLines.length; index++) {
        try {
          ...
          if (trimmedLine.includes("[DEBUG]: Skipping unparseable log line:")) {
            const embeddedMatch = trimmedLine.match(
              /\[DEBUG\]: Skipping unparseable log line: (.+)/,
            );
            if (embeddedMatch) {
              const embeddedLine = embeddedMatch[1].trim();
              if (embeddedLine && !processedLines.has(embeddedLine)) {
                const embeddedLogs = this.parseLogs(
                  [embeddedLine],
                  defaultLevel,
                );
          ...
          const dateTimeOnlyMatch = trimmedLine.match(
          ...
          logger.debug(
            `Line ${index} does not match any format: "${trimmedLine.substring(0, 100)}..."`,
          );
          currentLog = null;
        } catch (lineError) {
  ...
  async debugYesterdayLogs() {
    try {
      ...
      logger.info(
        `Found ${logFiles.length} files for yesterday: ${logFiles.join(", ")}`,
      );
      try {
