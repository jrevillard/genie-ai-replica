...
export default {
  ...
  computed: {
    ...
    // Compute if the user has admin role (legacy `role` string and/or Keycloak-style `roles` array)
    isAdmin() {
      if (!this.currentUser) {
        return false;
      }
    ...
    getCurrentUserFromStorage() {
      try {
        ...
        if (userStr) {
          ...
          const role = this.currentUser.role || (this.currentUser.user && this.currentUser.user.role);
