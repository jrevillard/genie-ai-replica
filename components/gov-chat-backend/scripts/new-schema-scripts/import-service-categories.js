...
async function createServiceCategoriesCollection(importConfig) {
  try {
    ...
    if (exists) {
      console.log(
        "✓ serviceCategories collection already exists - using existing collection",
      );
      return collection;
    }
    ...
    if (!importConfig.createCollection) {
      throw new Error(
        "serviceCategories collection does not exist and CREATE_COLLECTION=false",
      );
    }
...
async function createServicesCollection(importConfig) {
  try {
    ...
    if (exists) {
      console.log(
        "✓ services collection already exists - using existing collection",
      );
      return collection;
    }
    ...
    if (!importConfig.createCollection) {
      throw new Error(
        "services collection does not exist and CREATE_COLLECTION=false",
      );
    }
...
async function createCategoryServicesCollection(importConfig) {
  try {
    ...
    if (exists) {
      console.log(
        "✓ categoryServices edge collection already exists - using existing collection",
      );
      return collection;
    }
    ...
    if (!importConfig.createCollection) {
      throw new Error(
        "categoryServices edge collection does not exist and CREATE_COLLECTION=false",
      );
    }
    ...
    console.error(
      "✗ Error with categoryServices edge collection:",
      error.message,
    );
    throw error;
  }
...
async function createServiceCategoryTranslationsCollection(importConfig) {
  try {
    ...
    if (exists) {
      console.log(
        "✓ serviceCategoryTranslations collection already exists - using existing collection",
      );
      return collection;
    }
    ...
    if (!importConfig.createCollection) {
      throw new Error(
        "serviceCategoryTranslations collection does not exist and CREATE_COLLECTION=false",
      );
    }
    ...
    console.log(
      "✓ serviceCategoryTranslations collection created with indexes",
    );
    return newCollection;
  } catch (error) {
    console.error(
      "✗ Error with serviceCategoryTranslations collection:",
      error.message,
    );
    throw error;
  }
...
async function createServiceCategoryTranslationsEdgeCollection(importConfig) {
  try {
    ...
    if (exists) {
      console.log(
        "✓ serviceCategoryTranslationsEdge collection already exists - using existing collection",
      );
      return collection;
    }
    ...
    if (!importConfig.createCollection) {
      throw new Error(
        "serviceCategoryTranslationsEdge collection does not exist and CREATE_COLLECTION=false",
      );
    }
    ...
    console.error(
      "✗ Error with serviceCategoryTranslationsEdge collection:",
      error.message,
    );
    throw error;
  }
...
async function createServiceTranslationsCollection(importConfig) {
  try {
    ...
    if (exists) {
      console.log(
        "✓ serviceTranslations collection already exists - using existing collection",
      );
      return collection;
    }
    ...
    if (!importConfig.createCollection) {
      throw new Error(
        "serviceTranslations collection does not exist and CREATE_COLLECTION=false",
      );
    }
    ...
    console.error(
      "✗ Error with serviceTranslations collection:",
      error.message,
    );
    throw error;
  }
...
async function createServiceTranslationsEdgeCollection(importConfig) {
  try {
    ...
    if (exists) {
      console.log(
        "✓ serviceTranslationsEdge collection already exists - using existing collection",
      );
      return collection;
    }
    ...
    if (!importConfig.createCollection) {
      throw new Error(
        "serviceTranslationsEdge collection does not exist and CREATE_COLLECTION=false",
      );
    }
    ...
    console.error(
      "✗ Error with serviceTranslationsEdge collection:",
      error.message,
    );
    throw error;
  }
...
async function readImportFile(importConfig) {
  try {
    ...
    if (!importData.metadata || !importData.data) {
      throw new Error(
        "Invalid import file structure - missing metadata or data",
      );
    }
    ...
    if (importData.metadata.exportVersion === "4.0") {
      ...
      serviceCategoryTranslationsData =
        importData.data.serviceCategoryTranslations || [];
      serviceCategoryTranslationsEdgeData =
        importData.data.serviceCategoryTranslationsEdge || [];
      serviceTranslationsData = importData.data.serviceTranslations || [];
      serviceTranslationsEdgeData =
        importData.data.serviceTranslationsEdge || [];
      console.log(
        "✓ Format v4.0 detected - importing all collections including translations",
      );
    } else if (
      importData.metadata.exportVersion === "3.0" &&
      importData.data.categoryServices
    ) {
      ...
      serviceTranslationsEdgeData = [];
      console.log(
        "✓ Format v3.0 detected - importing serviceCategories, services, and categoryServices edges",
      );
    } else if (
      importData.metadata.exportVersion === "2.0" &&
      importData.data.serviceCategories
    ) {
      ...
      serviceTranslationsEdgeData = [];
      console.log(
        "✓ Format v2.0 detected - importing serviceCategories and services (no edges)",
      );
      ...
      serviceTranslationsEdgeData = [];
      console.log(
        "⚠ Old format (v1.0) detected - importing serviceCategories only",
      );
    } else {
      throw new Error(
        "Invalid import file data structure - cannot determine format",
      );
    }

    if (
      !Array.isArray(serviceCategoriesData) ||
      !Array.isArray(servicesData) ||
      !Array.isArray(categoryServicesData) ||
      !Array.isArray(serviceCategoryTranslationsData) ||
      !Array.isArray(serviceCategoryTranslationsEdgeData) ||
      !Array.isArray(serviceTranslationsData) ||
    ...
    console.log(
      `  - Source database: ${importData.metadata.sourceDatabase || "unknown"}`,
    );
    console.log(
      `  - Export date: ${importData.metadata.exportDate || "unknown"}`,
    );
    console.log(
      `  - Export version: ${importData.metadata.exportVersion || "1.0"}`,
    );
    console.log(
      `  - ServiceCategories: ${serviceCategoriesData.length} documents`,
    );
    console.log(`  - Services: ${servicesData.length} documents`);
    console.log(`  - CategoryServices: ${categoryServicesData.length} edges`);
    console.log(
      `  - ServiceCategoryTranslations: ${serviceCategoryTranslationsData.length} documents`,
    );
    console.log(
      `  - ServiceCategoryTranslationsEdge: ${serviceCategoryTranslationsEdgeData.length} edges`,
    );
    console.log(
      `  - ServiceTranslations: ${serviceTranslationsData.length} documents`,
    );
    console.log(
      `  - ServiceTranslationsEdge: ${serviceTranslationsEdgeData.length} edges`,
    );
    console.log(
      `  - Total: ${
        serviceCategoriesData.length +
        servicesData.length +
        categoryServicesData.length +
        serviceCategoryTranslationsData.length +
        serviceCategoryTranslationsEdgeData.length +
        serviceTranslationsData.length +
        serviceTranslationsEdgeData.length
      } items`,
    );
  ...
  try {
    ...
    importData.serviceCategories.forEach((doc, index) => {
      ...
      if (missingFields.length > 0) {
        errors.push(
          `ServiceCategory ${index}: Missing required fields: ${missingFields.join(", ")}`,
        );
      }
      ...
      if (doc.order !== undefined && typeof doc.order !== "number") {
        warnings.push(
          `ServiceCategory ${index}: order field should be a number, got: ${typeof doc.order}`,
        );
      }
    });
    ...
    importData.services.forEach((doc, index) => {
      const missingFields = serviceRequiredFields.filter(
        (field) =>
          !Object.prototype.hasOwnProperty.call(doc, field) ||
          doc[field] === null ||
          doc[field] === undefined,
      ...
      if (missingFields.length > 0) {
        errors.push(
          `Service ${index}: Missing required fields: ${missingFields.join(", ")}`,
        );
      }
      ...
      if (doc.order !== undefined && typeof doc.order !== "number") {
        warnings.push(
          `Service ${index}: order field should be a number, got: ${typeof doc.order}`,
        );
      }
      ...
      if (doc.categoryId) {
        ...
        if (!categoryKeySet.has(categoryRef)) {
          warnings.push(
            `Service ${index}: categoryId '${doc.categoryId}' (extracted: '${categoryRef}') not found in serviceCategories`,
          );
        }
      }
    });
    ...
    importData.categoryServices.forEach((doc, index) => {
      const missingFields = edgeRequiredFields.filter(
        (field) =>
          !Object.prototype.hasOwnProperty.call(doc, field) ||
          doc[field] === null ||
          doc[field] === undefined,
      ...
      if (missingFields.length > 0) {
        errors.push(
          `CategoryServices edge ${index}: Missing required fields: ${missingFields.join(", ")}`,
        );
      }
      ...
      if (doc._from) {
        ...
        if (!categoryKeySet.has(fromKey)) {
          warnings.push(
            `CategoryServices edge ${index}: _from '${doc._from}' references non-existent serviceCategory`,
          );
        }
      }
      ...
      if (doc._to) {
        ...
        if (!serviceKeySet.has(toKey)) {
          warnings.push(
            `CategoryServices edge ${index}: _to '${doc._to}' references non-existent service`,
          );
        }
      }
      ...
      if (edgeKeySet.has(edgeKey)) {
        warnings.push(
          `CategoryServices edge ${index}: Duplicate edge from ${doc._from} to ${doc._to}`,
        );
      } else {
    ...
    const categoryTranslationRequiredFields = [
      "_key",
      "serviceCategoryId",
      "languageCode",
      "translation",
    ...
    importData.serviceCategoryTranslations.forEach((doc, index) => {
      const missingFields = categoryTranslationRequiredFields.filter(
        (field) =>
          !Object.prototype.hasOwnProperty.call(doc, field) ||
          doc[field] === null ||
          doc[field] === undefined,
      ...
      if (missingFields.length > 0) {
        errors.push(
          `ServiceCategoryTranslation ${index}: Missing required fields: ${missingFields.join(", ")}`,
        );
      }
      ...
      if (doc._key) {
        if (categoryTranslationKeySet.has(doc._key)) {
          errors.push(
            `ServiceCategoryTranslation ${index}: Duplicate _key: ${doc._key}`,
          );
        } else {
      ...
      if (doc.serviceCategoryId && !categoryKeySet.has(doc.serviceCategoryId)) {
        warnings.push(
          `ServiceCategoryTranslation ${index}: serviceCategoryId '${doc.serviceCategoryId}' not found in serviceCategories`,
        );
      }
    });
    ...
    importData.serviceCategoryTranslationsEdge.forEach((doc, index) => {
      ...
      if (missingFields.length > 0) {
        errors.push(
          `ServiceCategoryTranslationsEdge ${index}: Missing required fields: ${missingFields.join(", ")}`,
        );
      }
      ...
      if (doc._from) {
        ...
        if (!categoryKeySet.has(fromKey)) {
          warnings.push(
            `ServiceCategoryTranslationsEdge ${index}: _from '${doc._from}' references non-existent serviceCategory`,
          );
        }
      }
      ...
      if (doc._to) {
        ...
        if (!categoryTranslationKeySet.has(toKey)) {
          warnings.push(
            `ServiceCategoryTranslationsEdge ${index}: _to '${doc._to}' references non-existent serviceCategoryTranslation`,
          );
        }
      }
      ...
      if (categoryTranslationEdgeKeySet.has(edgeKey)) {
        warnings.push(
          `ServiceCategoryTranslationsEdge ${index}: Duplicate edge from ${doc._from} to ${doc._to}`,
        );
      } else {
    ...
    const serviceTranslationRequiredFields = [
      "_key",
      "serviceId",
      "languageCode",
      "translation",
    ...
    importData.serviceTranslations.forEach((doc, index) => {
      const missingFields = serviceTranslationRequiredFields.filter(
        (field) =>
          !Object.prototype.hasOwnProperty.call(doc, field) ||
          doc[field] === null ||
          doc[field] === undefined,
      ...
      if (missingFields.length > 0) {
        errors.push(
          `ServiceTranslation ${index}: Missing required fields: ${missingFields.join(", ")}`,
        );
      }
      ...
      if (doc._key) {
        if (serviceTranslationKeySet.has(doc._key)) {
          errors.push(
            `ServiceTranslation ${index}: Duplicate _key: ${doc._key}`,
          );
        } else {
      ...
      if (doc.serviceId && !serviceKeySet.has(doc.serviceId)) {
        warnings.push(
          `ServiceTranslation ${index}: serviceId '${doc.serviceId}' not found in services`,
        );
      }
    });
    ...
    importData.serviceTranslationsEdge.forEach((doc, index) => {
      ...
      if (missingFields.length > 0) {
        errors.push(
          `ServiceTranslationsEdge ${index}: Missing required fields: ${missingFields.join(", ")}`,
        );
      }
      ...
      if (doc._from) {
        ...
        if (!serviceKeySet.has(fromKey)) {
          warnings.push(
            `ServiceTranslationsEdge ${index}: _from '${doc._from}' references non-existent service`,
          );
        }
      }
      ...
      if (doc._to) {
        ...
        if (!serviceTranslationKeySet.has(toKey)) {
          warnings.push(
            `ServiceTranslationsEdge ${index}: _to '${doc._to}' references non-existent serviceTranslation`,
          );
        }
      }
      ...
      if (serviceTranslationEdgeKeySet.has(edgeKey)) {
        warnings.push(
          `ServiceTranslationsEdge ${index}: Duplicate edge from ${doc._from} to ${doc._to}`,
        );
      } else {
    ...
    if (warnings.length > 0) {
      ...
      console.log(
        `  - ServiceCategories: ${importData.serviceCategories.length} valid documents`,
      );
      console.log(
        `  - Services: ${importData.services.length} valid documents`,
      );
      console.log(
        `  - CategoryServices: ${importData.categoryServices.length} valid edges`,
      );
      console.log(
        `  - ServiceCategoryTranslations: ${importData.serviceCategoryTranslations.length} valid documents`,
      );
      console.log(
        `  - ServiceCategoryTranslationsEdge: ${importData.serviceCategoryTranslationsEdge.length} valid edges`,
      );
      console.log(
        `  - ServiceTranslations: ${importData.serviceTranslations.length} valid documents`,
      );
      console.log(
        `  - ServiceTranslationsEdge: ${importData.serviceTranslationsEdge.length} valid edges`,
      );
    }
...
async function checkExistingData(collections) {
  try {
    ...
    console.log(
      `  - serviceCategoryTranslations: ${counts[3].count} existing documents`,
    );
    console.log(
      `  - serviceCategoryTranslationsEdge: ${counts[4].count} existing edges`,
    );
    console.log(
      `  - serviceTranslations: ${counts[5].count} existing documents`,
    );
    console.log(
      `  - serviceTranslationsEdge: ${counts[6].count} existing edges`,
    );

    console.log(
      "✓ Import will skip any existing documents and only add new ones",
    );
...
async function importDocumentsForCollection(
  collection,
  documents,
  collectionName,
  importConfig,
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        console.log(
          `  - Found ${existingKeys.size} existing ${collectionName} edges`,
        );
      } catch (error) {
      ...
      try {
        ...
        console.log(
          `  - Found ${existingKeys.size} existing ${collectionName} documents`,
        );
      } catch (error) {
    ...
    const documentsToImport = documents.filter((doc) => {
      if (
        ...
        if (existingKeys.has(edgeKey)) {
          console.log(
            `  - Skipping existing edge from ${doc._from} to ${doc._to}`,
          );
          skippedCount++;
          return false;
        }
        ...
        if (existingKeys.has(doc._key)) {
          console.log(
            `  - Skipping existing ${collectionName} ${doc._key}: ${doc.nameEN || doc.translation || "document already exists"}`,
          );
          skippedCount++;
          return false;
        }
    ...
    if (documentsToImport.length === 0) {
      console.log(
        `✓ All ${collectionName} documents already exist - nothing to import`,
      );
      return { importedCount: 0, errorCount: 0, skippedCount, errors: [] };
    }

    // Clean documents based on schema requirements
    const cleanedDocs = documentsToImport.map((doc) =>
      cleanDocument(doc, collectionName, importConfig),
    );
    ...
    for (let i = 0; i < cleanedDocs.length; i += importConfig.batchSize) {
      ...
      const totalBatches = Math.ceil(
        cleanedDocs.length / importConfig.batchSize,
      );

      console.log(
        `Processing ${collectionName} batch ${batchNumber}/${totalBatches} (${batch.length} documents)...`,
      );
      ...
      try {
        ...
        for (let j = 0; j < batch.length; j++) {
          try {
            ...
            if (
              ...
              console.log(
                `    ✓ Edge ${batch[j]._from} → ${batch[j]._to} imported`,
              );
            } else {
              console.log(
                `    ✓ ${collectionName} ${batch[j]._key}: ${batch[j].nameEN || batch[j].translation || "imported"}`,
              );
            }
            importedCount++;
          } catch (docError) {
            if (
              ...
              console.error(
                `    ✗ Failed edge ${batch[j]._from} → ${batch[j]._to}:`,
                docError.message,
              );
            } else {
              console.error(
                `    ✗ Failed ${collectionName} ${batch[j]._key}:`,
                docError.message,
              );
            }
            console.error(
              `    Document was:`,
              JSON.stringify(batch[j], null, 2),
            );
            errorCount++;
            errors.push(
              `${collectionName} ${batch[j]._key || "edge"}: ${docError.message}`,
            );
          }
        }
        ...
        console.log(
          `  ✓ ${collectionName} batch ${batchNumber} completed - Progress: ${progress}% (${importedCount}/${cleanedDocs.length})`,
        );
      } catch (batchError) {
        console.error(
          `  ✗ ${collectionName} batch ${batchNumber} failed:`,
          batchError.message,
        );
        errorCount += batch.length;
        errors.push(
          `${collectionName} batch ${batchNumber}: ${batchError.message}`,
        );
      }
    }
...
async function importAllDocuments(collections, importData, importConfig) {
  try {
    ...
    const serviceCategoryTranslationsResult =
      await importDocumentsForCollection(
    ...
    const serviceCategoryTranslationsEdgeResult =
      await importDocumentsForCollection(
    ...
    const totalImported =
      categoriesResult.importedCount +
      servicesResult.importedCount +
      categoryServicesResult.importedCount +
      serviceCategoryTranslationsResult.importedCount +
      serviceCategoryTranslationsEdgeResult.importedCount +
      serviceTranslationsResult.importedCount +
      serviceTranslationsEdgeResult.importedCount;
    const totalSkipped =
      categoriesResult.skippedCount +
      servicesResult.skippedCount +
      categoryServicesResult.skippedCount +
      serviceCategoryTranslationsResult.skippedCount +
      serviceCategoryTranslationsEdgeResult.skippedCount +
      serviceTranslationsResult.skippedCount +
      serviceTranslationsEdgeResult.skippedCount;
    const totalErrors =
      categoriesResult.errorCount +
      servicesResult.errorCount +
      categoryServicesResult.errorCount +
      serviceCategoryTranslationsResult.errorCount +
      serviceCategoryTranslationsEdgeResult.errorCount +
      serviceTranslationsResult.errorCount +
    ...
    console.log(
      `✓ ServiceCategories imported: ${categoriesResult.importedCount}`,
    );
    console.log(
      `✓ ServiceCategories skipped: ${categoriesResult.skippedCount}`,
    );
    console.log(`✓ Services imported: ${servicesResult.importedCount}`);
    console.log(`✓ Services skipped: ${servicesResult.skippedCount}`);
    console.log(
      `✓ CategoryServices edges imported: ${categoryServicesResult.importedCount}`,
    );
    console.log(
      `✓ CategoryServices edges skipped: ${categoryServicesResult.skippedCount}`,
    );
    console.log(
      `✓ ServiceCategoryTranslations imported: ${serviceCategoryTranslationsResult.importedCount}`,
    );
    console.log(
      `✓ ServiceCategoryTranslations skipped: ${serviceCategoryTranslationsResult.skippedCount}`,
    );
    console.log(
      `✓ ServiceCategoryTranslationsEdge imported: ${serviceCategoryTranslationsEdgeResult.importedCount}`,
    );
    console.log(
      `✓ ServiceCategoryTranslationsEdge skipped: ${serviceCategoryTranslationsEdgeResult.skippedCount}`,
    );
    console.log(
      `✓ ServiceTranslations imported: ${serviceTranslationsResult.importedCount}`,
    );
    console.log(
      `✓ ServiceTranslations skipped: ${serviceTranslationsResult.skippedCount}`,
    );
    console.log(
      `✓ ServiceTranslationsEdge imported: ${serviceTranslationsEdgeResult.importedCount}`,
    );
    console.log(
      `✓ ServiceTranslationsEdge skipped: ${serviceTranslationsEdgeResult.skippedCount}`,
    );
...
async function verifyImport(collections, originalData) {
  try {
    ...
    console.log(
      `  - Expected serviceCategories: ${originalData.serviceCategories.length}`,
    );
    ...
    console.log(
      `  - Expected categoryServices edges: ${originalData.categoryServices.length}`,
    );
    console.log(`  - Actual categoryServices edges: ${counts[2].count}`);
    console.log(
      `  - Expected serviceCategoryTranslations: ${originalData.serviceCategoryTranslations.length}`,
    );
    console.log(`  - Actual serviceCategoryTranslations: ${counts[3].count}`);
    console.log(
      `  - Expected serviceCategoryTranslationsEdge: ${originalData.serviceCategoryTranslationsEdge.length}`,
    );
    console.log(
      `  - Actual serviceCategoryTranslationsEdge: ${counts[4].count}`,
    );
    console.log(
      `  - Expected serviceTranslations: ${originalData.serviceTranslations.length}`,
    );
    console.log(`  - Actual serviceTranslations: ${counts[5].count}`);
    console.log(
      `  - Expected serviceTranslationsEdge: ${originalData.serviceTranslationsEdge.length}`,
    );
    console.log(`  - Actual serviceTranslationsEdge: ${counts[6].count}`);

    // Sample verification for serviceCategories
    const categorySampleSize = Math.min(
      3,
      originalData.serviceCategories.length,
    );
    const categorySampleKeys = originalData.serviceCategories
      .slice(0, categorySampleSize)
      .map((doc) => doc._key);
    ...
    const serviceSampleKeys = originalData.services
      .slice(0, serviceSampleSize)
      .map((doc) => doc._key);
    ...
    for (const key of serviceSampleKeys) {
      try {
        ...
        if (doc) {
          ...
          console.log(
            `  ✓ service ${key}: ${doc.nameEN} (category: ${doc.categoryId})`,
          );
        }
      } catch {
    ...
    for (const edgeSample of edgeSamples) {
      try {
        ...
        if (results.length > 0) {
          ...
          console.log(
            `  ✗ edge ${edgeSample._from} → ${edgeSample._to} not found`,
          );
        }
      } catch (error) {
        console.log(
          `  ✗ edge ${edgeSample._from} → ${edgeSample._to} verification failed: ${error.message}`,
        );
      }
    }

    // Sample verification for serviceCategoryTranslations
    const categoryTranslationSampleSize = Math.min(
      3,
      originalData.serviceCategoryTranslations.length,
    );
    const categoryTranslationSampleKeys =
      originalData.serviceCategoryTranslations
        .slice(0, categoryTranslationSampleSize)
        .map((doc) => doc._key);
    ...
    for (const key of categoryTranslationSampleKeys) {
      try {
        ...
        if (doc) {
          ...
          console.log(
            `  ✓ serviceCategoryTranslation ${key}: ${doc.translation} (${doc.languageCode})`,
          );
        }
      } catch {
    ...
    const categoryTranslationEdgeSampleSize = Math.min(
      3,
      originalData.serviceCategoryTranslationsEdge.length,
    );
    const categoryTranslationEdgeSamples =
      originalData.serviceCategoryTranslationsEdge.slice(
        0,
        categoryTranslationEdgeSampleSize,
      );
    ...
    const serviceTranslationSampleSize = Math.min(
      3,
      originalData.serviceTranslations.length,
    );
    const serviceTranslationSampleKeys = originalData.serviceTranslations
      .slice(0, serviceTranslationSampleSize)
      .map((doc) => doc._key);
    ...
    for (const key of serviceTranslationSampleKeys) {
      try {
        ...
        if (doc) {
          ...
          console.log(
            `  ✓ serviceTranslation ${key}: ${doc.translation} (${doc.languageCode})`,
          );
        }
      } catch {
    ...
    const serviceTranslationEdgeSampleSize = Math.min(
      3,
      originalData.serviceTranslationsEdge.length,
    );
    const serviceTranslationEdgeSamples =
      originalData.serviceTranslationsEdge.slice(
        0,
        serviceTranslationEdgeSampleSize,
      );
    ...
    try {
      ...
      relResults.forEach((result) => {
        ...
        if (result.translations.length > 0) {
          console.log(
            `    Translations: ${result.translations.map((t) => `${t.language}: ${t.translation}`).join(", ")}`,
          );
        }
      });
    } catch (error) {
    ...
    const categorySuccess =
      categorySampleSize === 0 || categoryVerifiedCount === categorySampleSize;
    const serviceSuccess =
      serviceSampleSize === 0 || serviceVerifiedCount === serviceSampleSize;
    const edgeSuccess =
      edgeSampleSize === 0 || edgeVerifiedCount === edgeSampleSize;
    const categoryTranslationSuccess =
      categoryTranslationSampleSize === 0 ||
      categoryTranslationVerifiedCount === categoryTranslationSampleSize;
    const categoryTranslationEdgeSuccess =
      categoryTranslationEdgeSampleSize === 0 ||
      categoryTranslationEdgeVerifiedCount ===
        categoryTranslationEdgeSampleSize;
    const serviceTranslationSuccess =
      serviceTranslationSampleSize === 0 ||
      serviceTranslationVerifiedCount === serviceTranslationSampleSize;
    const serviceTranslationEdgeSuccess =
      serviceTranslationEdgeSampleSize === 0 ||
      serviceTranslationEdgeVerifiedCount === serviceTranslationEdgeSampleSize;
    const overallSuccess =
      categorySuccess &&
      serviceSuccess &&
      edgeSuccess &&
      categoryTranslationSuccess &&
      categoryTranslationEdgeSuccess &&
      serviceTranslationSuccess &&
      serviceTranslationEdgeSuccess;

    console.log(
      `\n${overallSuccess ? "✓" : "✗"} Verification ${overallSuccess ? "passed" : "failed"}`,
    );
    console.log(
      `  - ServiceCategories: ${categoryVerifiedCount}/${categorySampleSize} samples found`,
    );
    console.log(
      `  - Services: ${serviceVerifiedCount}/${serviceSampleSize} samples found`,
    );
    console.log(
      `  - CategoryServices edges: ${edgeVerifiedCount}/${edgeSampleSize} samples found`,
    );
    console.log(
      `  - ServiceCategoryTranslations: ${categoryVerifiedCount}/${categoryTranslationSampleSize} samples found`,
    );
    console.log(
      `  - ServiceCategoryTranslationsEdge: ${categoryTranslationEdgeVerifiedCount}/${categoryTranslationEdgeSampleSize} samples found`,
    );
    console.log(
      `  - ServiceTranslations: ${serviceVerifiedCount}/${serviceTranslationSampleSize} samples found`,
    );
    console.log(
      `  - ServiceTranslationsEdge: ${serviceTranslationEdgeVerifiedCount}/${serviceTranslationEdgeSampleSize} samples found`,
    );
...
async function executeImport(dbConfig, importConfig) {
  console.log(
    "=== ArangoDB ServiceCategories, Services & Translations Data Import ===\n",
  );
  ...
  try {
    ...
    if (
      importData.serviceCategories.length === 0 &&
      importData.services.length === 0 &&
      importData.categoryServices.length === 0 &&
      importData.serviceCategoryTranslations.length === 0 &&
      importData.serviceCategoryTranslationsEdge.length === 0 &&
      importData.serviceTranslations.length === 0 &&
    ...
    const collections = {
      ...
      serviceCategoryTranslations:
        await createServiceCategoryTranslationsCollection(importConfig),
      serviceCategoryTranslationsEdge:
        await createServiceCategoryTranslationsEdgeCollection(importConfig),
      serviceTranslations:
        await createServiceTranslationsCollection(importConfig),
      serviceTranslationsEdge:
        await createServiceTranslationsEdgeCollection(importConfig),
    };
    ...
    // Perform import
    const importResult = await importAllDocuments(
      collections,
      importData,
      importConfig,
    );
    ...
    // Verify import
    const verificationSuccess = await verifyImport(
      collections,
      importData,
      importConfig,
    );
    ...
    console.log(
      `✓ ServiceCategories imported: ${importResult.serviceCategories.importedCount}`,
    );
    console.log(
      `✓ ServiceCategories skipped: ${importResult.serviceCategories.skippedCount}`,
    );
    ...
    console.log(
      `✓ CategoryServices edges imported: ${importResult.categoryServices.importedCount}`,
    );
    console.log(
      `✓ CategoryServices edges skipped: ${importResult.categoryServices.skippedCount}`,
    );
    console.log(
      `✓ ServiceCategoryTranslations imported: ${importResult.serviceCategoryTranslations.importedCount}`,
    );
    console.log(
      `✓ ServiceCategoryTranslations skipped: ${importResult.serviceCategoryTranslations.skippedCount}`,
    );
    console.log(
      `✓ ServiceCategoryTranslationsEdge imported: ${importResult.serviceCategoryTranslationsEdge.importedCount}`,
    );
    console.log(
      `✓ ServiceCategoryTranslationsEdge skipped: ${importResult.serviceCategoryTranslationsEdge.skippedCount}`,
    );
    console.log(
      `✓ ServiceTranslations imported: ${importResult.serviceTranslations.importedCount}`,
    );
    console.log(
      `✓ ServiceTranslations skipped: ${importResult.serviceTranslations.skippedCount}`,
    );
    console.log(
      `✓ ServiceTranslationsEdge imported: ${importResult.serviceTranslationsEdge.importedCount}`,
    );
    console.log(
      `✓ ServiceTranslationsEdge skipped: ${importResult.serviceTranslationsEdge.skippedCount}`,
    );
    ...
    console.log(
      `${importResult.totals.errorCount > 0 ? "⚠" : "✓"} Total errors: ${importResult.totals.errorCount}`,
    );
    console.log(
      `${verificationSuccess ? "✓" : "✗"} Verification: ${verificationSuccess ? "passed" : "failed"}`,
    );

    const success =
      (importResult.totals.importedCount > 0 ||
        importResult.totals.skippedCount > 0) &&
      importResult.totals.errorCount === 0 &&
      verificationSuccess;
    ...
    if (success) {
      ...
      console.log(
        "2. Update your application configuration to use the new database",
      );
      console.log(
        "3. Verify the parent-child relationships between serviceCategories and services work correctly",
      );
      console.log(
        "4. Test that categoryServices edges properly link categories to their services",
      );
      console.log(
        "5. Verify translation data is accessible in all supported languages",
      );
...
async function main() {
  ...
  const IMPORT_CONFIG = {
    inputFile:
      ...
      "./exports/serviceCategoriesAndServices_export_2025-06-19T14-55-00.json",
  ...
  executeImport(dbConfig, importConfig)
    .then((success) => {
