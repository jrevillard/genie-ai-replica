...
describe("ChatHistoryService", () => {
  ...
  describe("getUserConversations", () => {
    it("should return paginated user conversations", async () => {
      ...
      const result = await chatHistoryService.getUserConversations("user-1", {
        userKey: "user-1",
      });
  ...
  describe("addMessage", () => {
    ...
    it("should throw when conversationId is missing", async () => {
      await expect(
        chatHistoryService.addMessage({ content: "Hello", sender: "user" }),
      ).rejects.toThrow();
    });
  });
    ...
    it("should return paginated messages", async () => {
      mockDb.query
        .mockResolvedValueOnce(
          createMockCursor([{ _key: "msg-1", content: "Hi" }]),
        )
        .mockResolvedValueOnce(createMockCursor([1]));
  ...
  describe("updateConversation", () => {
    it("should update conversation properties", async () => {
      ...
      const result = await chatHistoryService.updateConversation("conv-1", {
        title: "Updated",
      });
  ...
  describe("folder CRUD", () => {
    describe("createFolder", () => {
      ...
      it("should throw when userId is missing", async () => {
        await expect(
          chatHistoryService.createFolder({ name: "No User" }),
        ).rejects.toThrow();
      });
    });
    ...
    describe("deleteFolder", () => {
      it("should throw ForbiddenError when user lacks permission", async () => {
        mockDb.query.mockResolvedValue(createMockCursor([]));
        await expect(
          chatHistoryService.deleteFolder(
            "folder-1",
            "user-2",
            false,
            "user-2",
        ...
        mockDb.query
          ...
          .mockResolvedValueOnce(
            createMockCursor([
              {
                _key: "fc-1",
                _from: "folders/folder-1",
                _to: "conversations/conv-1",
              },
            ]),
          ) // existing link
  ...
  describe("searchConversations", () => {
    it("should return matching conversations", async () => {
      ...
      const result = await chatHistoryService.searchConversations(
        "user-1",
        "tax",
      );
      expect(result).toBeDefined();
  ...
  describe("getRecentConversations", () => {
    it("should return recent conversations for user", async () => {
      ...
      const result = await chatHistoryService.getRecentConversations(
        "user-1",
        5,
        "user-1",
      );
  ...
  describe("updateQueryResponseTime", () => {
    it("should update response time for a query", async () => {
      ...
      const result = await chatHistoryService.updateQueryResponseTime(
        "query-1",
        2500,
      );
    ...
    it("should throw when queryId is missing", async () => {
      await expect(
        chatHistoryService.updateQueryResponseTime(null, 1000),
      ).rejects.toThrow("queryId and responseTime are required");
    });

    it("should throw when responseTime is missing", async () => {
      await expect(
        chatHistoryService.updateQueryResponseTime("query-1"),
      ).rejects.toThrow("queryId and responseTime are required");
    });
    ...
    it("should accept zero as valid responseTime", async () => {
      const mockQueries = createMockCollection();
      mockQueries.update.mockResolvedValueOnce({
        new: { _key: "query-1", responseTime: 0 },
      });
      mockDb.collection.mockImplementation((name) => {
        if (name === "queries") return mockQueries;
        return createMockCollection();
  ...
  describe("getFolderPath", () => {
    ...
    it("should throw when folder not found", async () => {
      ...
      await expect(chatHistoryService.getFolderPath("missing")).rejects.toThrow(
        "Not found",
      );
    });
  });
  ...
  describe("deleteFolder", () => {
    it("should delete folder with contents when user has permission", async () => {
      mockDb.query
        ...
        .mockResolvedValueOnce(
          createMockCursor([{ _key: "fc-1", _to: "conversations/conv-1" }]),
