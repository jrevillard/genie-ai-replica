...
class AdminDashboardService {
   ...
   */
  // `runSecurityScan` was removed: the legacy implementation read
  // log files via `fs.readFile`, used `Math.random()` to fabricate
  // vulnerabilities, and never reached the OTel VL pipeline. The
  // live route (`admin-routes.js:317-331`) and the controller
  // (`adminController.runSecurityScan`) now both delegate to
  // `securityScanService.runSecurityScan(logsService)` which queries
  // VictoriaLogs via the MELT adapter.
