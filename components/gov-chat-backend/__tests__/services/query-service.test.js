...
function createMockQueryData(overrides = {}) {
  return {
    ...
    context: {
      categoryLabel: "Taxes & Revenue",
      serviceLabels: ["Tax Payment"],
    },
    ...overrides,
  };
}
...
describe("QueryService", () => {
  ...
  describe("getQuery", () => {
    ...
    it("should propagate DB errors", async () => {
      mockQueriesCollection.document.mockRejectedValueOnce(
        new Error("Not found"),
      );
      await expect(queryService.getQuery("invalid")).rejects.toThrow(
        "Not found",
      );
    });
  });
  ...
  describe("addFeedback", () => {
    ...
    it("should throw if rating is missing", async () => {
      await expect(
        queryService.addFeedback("query-1", { comment: "no rating" }),
      ).rejects.toThrow("Feedback rating is required");
    });

    it("should continue if analytics update fails", async () => {
      mockAnalyticsService.recordFeedback.mockRejectedValueOnce(
  ...
  describe("updateQueryResponseTime", () => {
    it("should update response time", async () => {
      ...
      const result = await queryService.updateQueryResponseTime(
        "query-1",
        1200,
      );
    ...
    it("should throw on negative responseTime", async () => {
      await expect(
        queryService.updateQueryResponseTime("query-1", -1),
      ).rejects.toThrow("Invalid response time");
    });

    it("should throw on non-number responseTime", async () => {
      await expect(
        queryService.updateQueryResponseTime("query-1", "fast"),
      ).rejects.toThrow("Invalid response time");
    });
  });
  ...
  describe("setQueryCategory", () => {
    ...
    it("should include serviceId when provided", async () => {
      ...
      mockDb.query.mockResolvedValueOnce(createMockCursor([]));
      const result = await queryService.setQueryCategory(
        "query-1",
        "cat-1",
        "svc-1",
      );
  ...
  describe("getMockOpeaResponse", () => {
    ...
    it("should add generic document when no specific label matches", () => {
      ...
      const result = queryService.getMockOpeaResponse(queryData);
      expect(result.metadata.source_documents.length).toBeGreaterThan(0);
      expect(result.metadata.source_documents[0].document_id).toContain(
        "doc_generic_",
      );
    });
  });
  ...
  describe("createQuery", () => {
    it("should throw when userId is missing", async () => {
      ...
      delete data.userId;
      await expect(queryService.createQuery(data)).rejects.toThrow(
  ...
  describe("initStreamQuery", () => {
    ...
    it("should throw when messages and text are both missing", async () => {
      ...
      await expect(queryService.initStreamQuery(data, {})).rejects.toThrow(
        "Missing required query data",
      );
    });
  ...
  describe("finalizeStreamQuery", () => {
    it("should update query with response and mark as answered", async () => {
      await queryService.finalizeStreamQuery("q1", "Response text", 500, {
    ...
    it("should continue if analytics recording fails", async () => {
      mockAnalyticsService.recordQuery.mockRejectedValueOnce(
  ...
  describe("saveQueryWithCriteria", () => {
    ...
    it("should throw when userId is missing", async () => {
      ...
      await expect(
        queryService.saveQueryWithCriteria(queryData),
      ).rejects.toThrow("Missing required query data");
    });
  ...
  describe("getSavedQueries", () => {
    ...
    it("should propagate DB errors", async () => {
      ...
      await expect(queryService.getSavedQueries("user-1")).rejects.toThrow(
        "DB error",
      );
    });
  });
  ...
  describe("getQueryRecommendations", () => {
    ...
    it("should combine popular queries when insufficient recommendations", async () => {
      const recentQueries = [
        { _key: "q1", text: "query 1", categoryId: "cat-1" },
      ];
  ...
  describe("createConversationFromQuery", () => {
    ...
    it("should create conversation from query", async () => {
      ...
      expect(
        queryService.chatHistoryService.createConversationFromQuery,
      ).toHaveBeenCalledWith("query-1", "user-1", {
        title: "Custom Title",
        responseText: "Response",
        tags: ["tag1"],
      });
    ...
    it("should throw when chatHistoryService is not set", async () => {
      ...
      await expect(
        queryService.createConversationFromQuery("query-1"),
    ...
    it("should throw NotFoundError when query does not exist", async () => {
      ...
      await expect(
        queryService.createConversationFromQuery("invalid"),
  ...
  describe("getConversationsForQuery", () => {
    ...
    it("should throw when chatHistoryService is not set", async () => {
      queryService.chatHistoryService = null;
      await expect(
        queryService.getConversationsForQuery("query-1"),
      ).rejects.toThrow("Chat history service is not set");
    });
  ...
  describe("markQueryAsAnswered", () => {
    ...
    it("should throw on invalid query ID", async () => {
      ...
      await expect(
        queryService.markQueryAsAnswered("undefined", 100),
      ).rejects.toThrow("Invalid query ID provided");
    });
    ...
    it("should throw NotFoundError when query does not exist", async () => {
      ...
      await expect(
        queryService.markQueryAsAnswered("invalid", 100),
      ).rejects.toThrow("Query not found");
    });
    ...
    it("should propagate other errors", async () => {
      ...
      await expect(
        queryService.markQueryAsAnswered("query-1", 100),
  ...
  describe("linkQueryToMessage", () => {
    ...
    it("should link query to message", async () => {
      ...
      expect(
        queryService.chatHistoryService.linkQueryToConversation,
      ).toHaveBeenCalledWith("query-1", "conv-1", "msg-1", {
        responseType: "primary",
        confidenceScore: 0.95,
      });
    ...
    it("should throw NotFoundError when message does not exist", async () => {
      mockDb.query.mockResolvedValueOnce(createMockCursor([]));
      await expect(
        queryService.linkQueryToMessage("query-1", "invalid"),
