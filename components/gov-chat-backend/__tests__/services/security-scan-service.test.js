...
describe("SecurityScanService", () => {
  ...
  describe("generateRecommendations", () => {
    it("should generate login recommendations", () => {
      ...
      expect(
        result.some((r) => r.title === "Improve Authentication Security"),
      ).toBe(true);
    });
    ...
    it("should generate disabled account recommendation", () => {
      ...
      expect(result.some((r) => r.title === "Review Disabled Accounts")).toBe(
        true,
      );
    });
    ...
    it("should generate critical vulnerability recommendation", () => {
      ...
      expect(result.some((r) => r.title === "Fix Critical Server Errors")).toBe(
        true,
      );
    });
    ...
    it("should generate database recommendation", () => {
      ...
      expect(result.some((r) => r.title === "Resolve Database Issues")).toBe(
        true,
      );
    });
    ...
    it("should generate CORS recommendation", () => {
      const result = securityScanService.generateRecommendations(
        ...
        {
          critical: [],
          medium: [{ type: "cross_domain_misconfiguration" }],
          low: [],
        },
      );
      expect(result.some((r) => r.title === "Fix CORS Configuration")).toBe(
        true,
      );
    });
    ...
    it("should generate IP blocking recommendation", () => {
      ...
      expect(result.some((r) => r.title === "Review IP Blocking Events")).toBe(
        true,
      );
    });
    ...
    it("should generate 401 recommendation", () => {
      ...
      expect(
        result.some((r) => r.title === "Address Unauthorized Access Attempts"),
      ).toBe(true);
    });
    ...
    it("should generate JWT recommendation", () => {
      ...
      expect(
        result.some((r) => r.title === "Fix Authentication Token Issues"),
      ).toBe(true);
    });
    ...
    it("should generate sensitive file access recommendation", () => {
      ...
      expect(
        result.some((r) => r.title === "Secure Sensitive File Access"),
      ).toBe(true);
    });
    ...
    it("should generate header recommendation", () => {
      ...
      expect(result.some((r) => r.title === "Implement Security Headers")).toBe(
        true,
      );
    });
    ...
    it("should generate leakage recommendation", () => {
      ...
      expect(
        result.some((r) => r.title === "Prevent Information Leakage"),
      ).toBe(true);
    });
    ...
    it("should generate low severity recommendations", () => {
      ...
      expect(
        result.some((r) => r.title === "Review Non-Critical File Access"),
      ).toBe(true);
      expect(result.some((r) => r.title === "Fix Missing Resources")).toBe(
        true,
      );
      expect(
        result.some((r) => r.title === "Monitor Registration Attempts"),
      ).toBe(true);
      expect(result.some((r) => r.title === "Optimize Log Processing")).toBe(
        true,
      );
    });
    ...
    it("should always include regular maintenance recommendation", () => {
      ...
      expect(
        result.some((r) => r.title === "Regular Security Maintenance"),
      ).toBe(true);
    });
  });
  ...
  describe("loginIssues", () => {
    ...
    it("should throw when logsService is null", async () => {
      await expect(securityScanService.loginIssues(null)).rejects.toThrow(
        "LogsService is required",
      );
    });
  ...
  describe("checkFailedLogins", () => {
    it("should return cached failed logins when available", async () => {
      const cachedData = {
        failedLoginDetails: [
          { timestamp: "2026-05-26", message: "Failed login" },
        ],
  ...
  describe("checkSuspiciousActivities", () => {
    it("should return cached suspicious activities when available", async () => {
      const cachedData = {
        suspiciousDetails: [
          { timestamp: "2026-05-26", message: "Suspicious activity" },
        ],
        vulnerabilityDetails: { critical: [], medium: [], low: [] },
      };
      mockFs.stat.mockResolvedValueOnce({ mtime: new Date() });
