...
### DW-325: Mobile auth has no recovery path — one failed token refresh wipes the session and every later request 401s as a "Connection error".
origin: incident-investigation 2026-09-22 (repeated "Connection error" reported while testing quick-help parity on the el_salvador flavour)
location: mobile/genie_ai_mobile/lib/services/auth/auth_notifier.dart:501-528, mobile/genie_ai_mobile/lib/services/auth/auth_interceptor.dart:26-49
severity: high
reason: Realm `genie` issues 5-minute access tokens (`accessTokenLifespan=300`) with a 30-minute SSO idle timeout (`ssoSessionIdleTimeout=1800`), so a 401 is routine rather than exceptional — it happens every ~5 minutes of use. What follows is the defect. (1) `AuthNotifier.refreshToken()` treats ANY non-network refresh failure as fatal: it calls `_tokenStorage.deleteAll()` and sets state unauthenticated (auth_notifier.dart:516-528); only `_networkErrorClassifier.isNetworkError(e)` preserves tokens. (2) The interceptor then reads the token back and gets null (auth_interceptor.dart:110), so it throws `AuthException('Session expired')` (auth_interceptor.dart:42-49) — that exception is the user-visible "Connection error" on the chat. (3) There is no path back: `send()` sets the Authorization header only `if (token != null)` (auth_interceptor.dart:26-29) and the 401 branch also requires `token != null` (line 40), so every later request goes out unauthenticated and returns a bare 401 — the app stays broken until a manual sign-in and never self-heals. Observed trigger: idling the app past the 30-minute SSO timeout kills the refresh token, and the next 401 ends the session permanently (prod 2026-09-22 11:46-12:12 — Keycloak `LOGOUT_ERROR ... session_expired` for client `genie-mobile-el-salvador`, then repeated `USER_INFO_REQUEST_ERROR ... reason="Token is not active"` on every request). Compounding factor: `_isRefreshing` returns immediately on a concurrent call (auth_notifier.dart:359), so a colliding refresh completes having done nothing and the interceptor retries with the stale token into a second 401 (auth_interceptor.dart:60-68). Asymmetry that hides the failure: the REST path recovers on its own (`ApiService` logs "401 — retried after token refresh" then 200), but the SSE chat stream has no retry — so chat is the only surface reporting it and the rest of the app looks healthy. Fix direction (needs its own branch + review; larger and riskier than a label fix): do not `deleteAll()` on the first refresh failure, only on a definitive `invalid_grant`; give the SSE chat the same retry-after-refresh the REST path has; refresh proactively at ~4 minutes rather than reactively on 401; and decide at deployment level whether 5-minute access tokens suit a mobile client (raising `accessTokenLifespan` / the SSO idle timeout is a config decision, not an app fix).
status: open
