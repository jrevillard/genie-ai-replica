...
Read `{spec_folder}/stories.yaml`. If the file does not exist or fails to parse, fall through to the **sprint-status fallback** below — the bmad-loop orchestrator's tracking convention uses `sprint-status.yaml` + flat per-story spec files in `{spec_folder}/stories/` instead of `stories.yaml`. Only HALT with status `blocked` and blocking condition `no stories.yaml found` if BOTH `stories.yaml` AND the sprint-status fallback also fail to resolve the story.

Find the entry whose `id` equals `{story_id}`; if none matches, HALT with status `blocked` and blocking condition `story id not found in stories.yaml`. Take only that entry's `title` and `description` — never read the checkpoint fields or `invoke_dev_with`; those are the caller's orchestration fields, not build-auto's.

**Sprint-status fallback** (bmad-loop orchestrator convention): if `{spec_folder}/sprint-status.yaml` exists, parses, AND has a top-level key matching `{story_id}` (the value is the story's status — e.g. `ready-for-dev`, `in-progress`, `done`), treat the story as resolved. Read `title` from the matching flat spec file at `{spec_folder}/stories/{story_id}-*.md` (id-prefix match). If both `stories.yaml` AND `sprint-status.yaml` are missing or the story id resolves in neither, HALT with status `blocked` and blocking condition `no stories.yaml found` (covers both failure modes — the fallback never silently produces an unresolved entry). sprint-status.yaml is orchestrator-owned and read-only to build-auto — never write it; never revert a change to it.
...
One entry per invocation: never read another story (whether from `stories.yaml` or sprint-status), and never advance to a different story id regardless of outcome.
