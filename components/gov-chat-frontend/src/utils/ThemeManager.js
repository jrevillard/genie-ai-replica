...
class ThemeManager {
  ...
  /**
   * Get current theme information, reading colors from CSS variables
  ...
  getThemeInfo() {
    return {
      ...
      borderColor: this._getCssVar(
        '--border-color',
        this.isDarkMode ? 'rgba(255, 255, 255, 0.2)' : 'rgba(0, 0, 0, 0.2)'
      ),
      gridColor: this._getCssVar(
        '--border-light',
        this.isDarkMode ? 'rgba(255, 255, 255, 0.15)' : 'rgba(0, 0, 0, 0.1)'
      ),
  ...
  getDialogTheme() {
    return {
      modal: {
        titleColor: this._getCssVar('--text-primary', this.isDarkMode ? '#f0f0f0' : '#333333'),
