...
class AqlToSqlTranslator {
  ...
  _translateNestedFor(sql, params) {
    ...
    if (match) {
      ...
      const joinCondition = filter2.match(
        new RegExp(`${escapeRegExp(docVar)}\\._id\\s*==\\s*${escapeRegExp(edgeVar)}\\._(to|from)`, 'i') // nosemgrep: eslint.detect-non-literal-regexp
      );
      if (joinCondition) {
  ...
  _translateTopLet(sql, params) {
    ...
    while ((match = current.match(/LET\s+(\w+)\s*=\s*(.+?)(?=\s+LET\s+|\s+RETURN\s+|$)/i))) {
      ...
      returnExpr = returnExpr.replace(new RegExp(`\\b${escapeRegExp(varName)}\\b`, 'g'), varName); // nosemgrep: eslint.detect-non-literal-regexp
  ...
  _translateForRemove(sql, params) {
    ...
    if (match) {
      const varName = match[1];
      const coll = match[2];
      const filters = this._translateExpression(match[3]).replace(new RegExp(`${escapeRegExp(varName)}\\.`, 'gi'), ''); // nosemgrep: eslint.detect-non-literal-regexp
  ...
  _translateCoreForLoop(sql, params) {
    ...
    if (hasAgg && !isGlobalAgg && groupVar) {
      if (returnContent === groupVar) {
        ...
        selectClause = selectClause.replace(new RegExp(`\\b${escapeRegExp(groupVar)}\\b`), groupExpr); // nosemgrep: eslint.detect-non-literal-regexp
      }
    }

    postCollectLets.forEach(({ letVar, translatedExpr }) => {
      selectClause = selectClause.replace(new RegExp(`\\b${escapeRegExp(letVar)}\\b`), translatedExpr); // nosemgrep: eslint.detect-non-literal-regexp
    });
  ...
  _translateExpression(expr) {
    ...
    do {
      ...
      for (const [key, value] of Object.entries(this.functionMap)) {
        const regex = new RegExp(`${escapeRegExp(key)}\\s*\\(\\s*([^)]*)\\s*\\)`, 'gi'); // nosemgrep: eslint.detect-non-literal-regexp
