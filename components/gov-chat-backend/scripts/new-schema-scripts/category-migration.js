...
async function createServiceCategoryTranslationsCollection() {
  try {
    ...
    const translationsCollection = await db.createCollection(
      "serviceCategoryTranslations",
      {
        ...
        schema: {
          message:
            "Service category translation document does not match schema",
    ...
    console.log(
      "✓ serviceCategoryTranslations collection created successfully",
    );
    return translationsCollection;
  } catch (error) {
    console.error(
      "✗ Error creating serviceCategoryTranslations collection:",
      error,
    );
    throw error;
  }
...
async function createServiceCategoryTranslationsEdgeCollection() {
  try {
    ...
    const edgeCollection = await db.createCollection(
    ...
    console.log(
      "✓ serviceCategoryTranslationsEdge collection created successfully",
    );
    return edgeCollection;
  } catch (error) {
    console.error(
      "✗ Error creating serviceCategoryTranslationsEdge collection:",
      error,
    );
    throw error;
  }
...
async function createServiceTranslationsEdgeCollection() {
  try {
    ...
    console.error(
      "✗ Error creating serviceTranslationsEdge collection:",
      error,
    );
    throw error;
  }
...
async function migrateServiceCategoryTranslations() {
  try {
    ...
    if (!exists) {
      console.log(
        "⚠ serviceCategories collection does not exist - skipping migration",
      );
      return 0;
    }
    ...
    console.log(
      `Found ${serviceCategories.length} service categories to process`,
    );

    for (const category of serviceCategories) {
      console.log(
        `Processing serviceCategory: ${category._key} (${category.nameEN || "no nameEN"})`,
      );
      ...
      for (const lang of languageMappings) {
        ...
        if (translation && translation.trim() !== "") {
          try {
            ...
            migratedCount++;
            console.log(
              `  ✓ Migrated serviceCategory ${lang.code}: ${translation}`,
            );
          } catch (insertError) {
            if (insertError.code === 1210 || insertError.errorNum === 1210) {
              // Unique constraint violation
              console.log(
                `  ⚠ Translation already exists for serviceCategory ${category._key} - ${lang.code}`,
              );
            } else {
              console.error(
          ...
          console.log(
            `  - Skipped serviceCategory ${lang.code}: empty or missing translation`,
          );
        }
      }
    }
    ...
    console.log(
      `✓ ServiceCategory translation migration completed. ${migratedCount} translations migrated.`,
    );
    return migratedCount;
  } catch (error) {
    console.error(
      "✗ Error during serviceCategory translation migration:",
      error,
    );
    throw error;
  }
...
async function migrateServiceTranslations() {
  try {
    ...
    for (const service of services) {
      console.log(
        `Processing service: ${service._key} (${service.nameEN || "no nameEN"})`,
      );
        ...
        if (translation && translation.trim() !== "") {
          try {
            ...
            if (insertError.code === 1210 || insertError.errorNum === 1210) {
              // Unique constraint violation
              console.log(
                `  ⚠ Translation already exists for service ${service._key} - ${lang.code}`,
              );
            } else {
              console.error(
                `  ✗ Error inserting translation for service ${service._key} - ${lang.code}:`,
                insertError,
              );
            }
          }
          ...
          console.log(
            `  - Skipped service ${lang.code}: empty or missing translation`,
          );
        }
      }
    }
    ...
    console.log(
      `✓ Service translation migration completed. ${migratedCount} translations migrated.`,
    );
    return migratedCount;
  } catch (error) {
...
async function verifyMigration() {
  try {
    ...
    const serviceCategoriesCollection = db.collection("serviceCategories");
    const servicesCollection = db.collection("services");
    const categoryTranslationsCollection = db.collection(
      "serviceCategoryTranslations",
    );
    const serviceTranslationsCollection = db.collection("serviceTranslations");
    const categoryEdgeCollection = db.collection(
      "serviceCategoryTranslationsEdge",
    );
    ...
    const totalCategoryTranslations =
      await categoryTranslationsCollection.count();
    const totalServiceTranslations =
      await serviceTranslationsCollection.count();
    ...
    console.log(
      `  - ServiceCategory translations: ${totalCategoryTranslations.count}`,
    );
...
async function executeTranslationMigration() {
  console.log(
    "=== ArangoDB Service Category & Services Translation Migration ===\n",
  );
  ...
  // --- Confirmation Prompt ---
  console.log(
    "This script will migrate translation data in an ArangoDB database.",
  );
  ...
  try {
    ...
    console.log(
      `✓ ServiceCategory translations migrated: ${categoryMigratedCount}`,
    );
    console.log(`✓ Service translations migrated: ${serviceMigratedCount}`);
    console.log(
      `✓ Total translations migrated: ${categoryMigratedCount + serviceMigratedCount}`,
    );
    console.log(
    ...
    console.log(
      "2. Update your CRUD services to use both serviceCategoryTranslations and serviceTranslations",
    );
    console.log(
      "3. Add new language support as needed for both serviceCategories and services",
    );
    console.log(
      "4. Once migration is complete, you can remove nameEN, nameFR, nameSW from both collection schemas",
    );
...
async function addNewServiceCategoryTranslation(
  serviceCategoryKey,
  languageCode,
  translation,
) {
  try {
    ...
    console.log(
      `✓ Added serviceCategory translation: ${languageCode} - ${translation}`,
    );
    return result;
  } catch (error) {
...
async function addNewServiceTranslation(serviceKey, languageCode, translation) {
  try {
    ...
    console.log(
      `✓ Added service translation: ${languageCode} - ${translation}`,
    );
    return result;
  } catch (error) {
