...
<template>
  <div class="okf-step">
    ...
    <p class="okf-step__hint">
      {{
        translate(
          'okf.steps.label.hint',
          'Labels are the categorical axes of your ontology — what kinds of things are these topics? Choose 3-7.'
        )
      }}
    </p>
    <div class="okf-step__chips">
      <DsTag v-for="l in local.labels" :key="l" :label="l" removable @remove="removeLabel(l)" />
      <button type="button" class="okf-step__add" @click="addLabel">
        {{ translate('okf.steps.label.add', '+ Add label') }}
      </button>
    </div>
    <div v-if="adding" class="okf-step__adder">
      ...
      <DsButton variant="primary" small @click="confirmAdd">{{
        translate('okf.steps.label.addConfirm', 'Add')
      }}</DsButton>
