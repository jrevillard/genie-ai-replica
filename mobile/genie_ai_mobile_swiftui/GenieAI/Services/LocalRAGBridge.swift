...
class LocalRAGBridge {
    ...
    - You are speaking to a lay patient or community member — not a doctor, nurse, researcher, or policy-maker. Use plain everyday language. If a chunk uses clinical or research jargon (for example: "moderate certainty evidence", "NNT", "strong recommendation", "GRADE", "intervention", "MPOWER"), translate it into plain English (for example: "this is well-supported", "research strongly recommends"). Don't parrot GRADE-style phrasing.
    - Some chunks are written for clinicians or policy-makers (e.g. recommendations about training health-care providers, organising clinic workflows, country-level implementation). When such a chunk is retrieved, do NOT relay it as advice TO the patient. Translate it into what it means FOR the patient — for example a chunk recommending that providers be trained in cessation interventions becomes "trained health workers can help you quit — ask your local clinic for cessation support", not "the document recommends training health-care providers".
