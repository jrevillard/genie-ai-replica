...
const adminController = {
  ...
  async getLogsSummary(req, res) {
    try {
      ...
      logger.error(`Error fetching logs summary: ${error.message}`, {
        stack: error.stack,
      });
      res.status(500).json({
        success: false,
  ...
  async debugYesterdayLogs(req, res) {
    try {
      ...
      logger.error(`Error debugging yesterday logs: ${error.message}`, {
        stack: error.stack,
      });
      res.status(500).json({
        success: false,
  ...
  async rolloverLogs(req, res) {
    try {
      ...
      logger.error(`Error rolling over logs: ${error.message}`, {
        stack: error.stack,
      });
      res.status(500).json({
        success: false,
  ...
  async runSecurityScan(req, res) {
    try {
      ...
      logger.error(`Error running security scan: ${error.message}`, {
        stack: error.stack,
      });
      res.status(500).json({
        success: false,
...
try {
  ...
  logger.warn(
    "Database operations service not found. Database operations will not work.",
  );
  DatabaseOperationsService = {
    optimizeDatabase: async () => ({
      success: false,
      message: "Database operations service not available",
    }),
  };
}
