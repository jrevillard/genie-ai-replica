    ...
    function processProperties(properties) {
      ...
      for (const [key, value] of Object.entries(properties)) {
        ...
        if (
          ...
          newValue.items.properties = processProperties(
            newValue.items.properties,
          );
        }
  ...
  async createCollections(collections) {
    ...
    for (const collectionSchema of collections) {
      try {
        ...
        if (
          collectionSchema.properties.schema &&
          collectionSchema.properties.schema.rule
        ) {
          try {
            console.log(`  Processing schema for ${collectionSchema.name}`);

            // Convert schema format to ArangoDB format
            const convertedSchemaRule = this.convertSchemaFormat(
              collectionSchema.properties.schema.rule,
            );
            console.log(
              `  Converted schema rule:`,
              JSON.stringify(convertedSchemaRule, null, 2),
            );
            ...
            options.schema = {
              ...
              message:
                collectionSchema.properties.schema.message ||
                "Document does not match schema",
            };

            console.log(
              `  Adding schema validation to ${collectionSchema.name}`,
            );
          } catch (schemaError) {
            console.error(
              `  Schema conversion failed for ${collectionSchema.name}: ${schemaError.message}`,
            );
        ...
        if (error.code === 1207 || error.errorNum === 1207) {
          ...
          console.error(
            `Error creating collection ${collectionSchema.name}: ${error.message}`,
          );
    ...
    for (const collectionSchema of collections) {
      ...
      for (const indexSchema of collectionSchema.indexes) {
        ...
        if (indexSchema.type === "edge") {
          console.log(
            `Skipping managed edge index on ${collectionSchema.name}`,
          );
          continue;
        }
        ...
        try {
          ...
          if (indexSchema.estimates !== undefined)
            indexOptions.estimates = indexSchema.estimates;
          if (indexSchema.minLength !== undefined)
            indexOptions.minLength = indexSchema.minLength;
          if (indexSchema.geoJson !== undefined)
            indexOptions.geoJson = indexSchema.geoJson;
          if (indexSchema.constraint !== undefined)
            indexOptions.constraint = indexSchema.constraint;
          if (indexSchema.expireAfter !== undefined)
            indexOptions.expireAfter = indexSchema.expireAfter;
          if (indexSchema.cacheEnabled !== undefined)
            indexOptions.cacheEnabled = indexSchema.cacheEnabled;
          if (indexSchema.storedValues && indexSchema.storedValues.length > 0)
            indexOptions.storedValues = indexSchema.storedValues;
          if (indexSchema.inBackground !== undefined)
            indexOptions.inBackground = indexSchema.inBackground;

          await collection.ensureIndex(indexOptions);
          console.log(
            `Created index on ${collectionSchema.name}: ${indexSchema.fields.join(", ")}`,
          );
        } catch (error) {
          console.error(
            `Error creating index on ${collectionSchema.name}: ${error.message}`,
          );
        }
      }
    }
  ...
  async createGraphs(graphs) {
    ...
    for (const graphSchema of graphs) {
      try {
        ...
        const edgeDefs = Array.isArray(graphSchema.edgeDefinitions)
          ? graphSchema.edgeDefinitions
          : [];
        const orphanColls = Array.isArray(graphSchema.orphanCollections)
          ? graphSchema.orphanCollections
          : [];
        ...
        if (error.code === 1925) {
          ...
          console.error(
            `Error creating graph ${graphSchema.name}:`,
            error.message,
          );
        }
      }
    ...
    for (const analyzerSchema of analyzers) {
      try {
        ...
        if (error.code === 1650) {
          ...
          console.error(
            `Error creating analyzer ${analyzerSchema.name}:`,
            error,
          );
        }
      }
    ...
    for (const functionSchema of functions) {
      try {
        ...
        if (error.code === 1582) {
          ...
          console.error(
            `Error creating function ${functionSchema.name}:`,
            error,
          );
        }
      }
