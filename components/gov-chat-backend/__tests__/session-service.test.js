...
function createMockCollection() {
  return {
    ...
    ensureIndex: jest
      .fn()
      .mockResolvedValue({ id: "idx-1", name: "idx-active-lastActiveTime" }),
  };
}
...
describe("SessionService", () => {
  ...
  describe("cleanupExpiredSessions", () => {
    ...
    it("should clean up userSessions and sessionQueries edges", async () => {
      ...
      const queryEdgeCursor = createMockCursor([
        "sessionQueries/edge-1",
        "sessionQueries/edge-2",
      ]);
      mockDb.query
  ...
  describe("_removeSessionEdges", () => {
    ...
    it("should remove edges from both userSessions and sessionQueries", async () => {
      ...
      const removed =
        await sessionService._removeSessionEdges("sessions/session-1");
