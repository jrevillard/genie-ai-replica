  ...
  >
    ...
    <DsTable :columns="columns" :rows="logs" :loading="loading">
      ...
      <template #cell-action="{ row }">
        ...
        <!-- D-G (David, 2026-09-07): the import method is part of the
             audit trail. Rendered only when the row carries it (old rows
             and non-import rows stay unchanged). -->
        <DsPill v-if="isMethod(row.method)" variant="info" class="okf-logs__method">{{ row.method }}</DsPill>
      </template>
      <template #cell-description="{ row }">
        {{ row.description || '—' }}
        <span v-if="importCounts(row)" class="okf-logs__counts">{{ importCounts(row) }}</span>
      </template>
    </DsTable>
...
export default {
  ...
  methods: {
    ...
    // D-G helpers — every access guarded: old rows carry none of these.
    isMethod(m) {
      return ['heuristics', 'llm', 'hybrid'].includes(m);
    },
    importCounts(row) {
      const parts = [];
      if (typeof row.concepts_total === 'number') {
        parts.push(this.translate('okf.logs.concepts', 'concepts: {n}').replace('{n}', String(row.concepts_total)));
      }
      if (typeof row.labeled === 'number') {
        parts.push(this.translate('okf.logs.labeled', 'labeled: {n}').replace('{n}', String(row.labeled)));
      }
      if (typeof row.fallback_count === 'number' && row.fallback_count > 0) {
        parts.push(
          this.translate('okf.logs.fallbacks', 'LLM fallbacks: {n}').replace('{n}', String(row.fallback_count))
        );
      }
      return parts.join(' · ');
    },
