...
class UserProfileService {
  constructor() {
    ...
    if (!fs.existsSync(this.uploadDir)) {
      ...
      logger.info("UserProfileService.created_upload_dir", {
        path: this.uploadDir,
      });
    }
  ...
  async init() {
    ...
    try {
      ...
      logger.error(`Error initializing UserProfileService: ${error.message}`, {
        stack: error.stack,
      });
      throw error;
    }
  ...
  async updateUserProfile(userKey, profileData, files = {}) {
    ...
    try {
      logger.info("UserProfileService.update_user_profile_start", {
        userKey,
        incomingKeys: Object.keys(profileData),
      });
      ...
      if (typeof profileData === "string") {
        try {
          ...
          logger.error("UserProfileService.parse_profile_data_failed", {
            userKey,
            error: error.message,
          });
          profileData = {};
      ...
      const strippedFields = Object.keys(profileData).filter((k) =>
        JIT_PROTECTED_FIELDS.includes(k),
      );
      if (strippedFields.length > 0) {
        logger.warn("UserProfileService.stripped_jit_fields", {
          userKey,
          strippedFields,
        });
      ...
      const completeUser = await this.users.document(userKey, {
        graceful: true,
      });

      const returnedKeys = Object.keys(completeUser);
      const returnedHasPersonalIdentification =
        !!completeUser.personalIdentification;
      ...
      if (user.customSettings && typeof user.customSettings === "object") {
        ...
        for (const key in user.customSettings) {
          if (!user[key]) {
            ...
            logger.debug("UserProfileService.merged_custom_key", {
              userKey,
              key,
            });
          }
  ...
  async userExists(userKey) {
    ...
    try {
      ...
      logger.debug("UserProfileService.user_exists", {
        userKey,
        durationMs: Date.now() - startTime,
      });
      return true;
  ...
  async process(userKey, profileData, files) {
    const startTime = Date.now();
    logger.info("UserProfileService.process_profile_data_start", {
      userKey,
      incomingKeys: Object.keys(profileData),
    });
    ...
    for (const key in profileData) {
      if (key !== "_key") {
        ...
        logger.debug("UserProfileService.copied_key_to_processed", {
          userKey,
          key,
          dataType: typeof profileData[key],
        });
      }
    }
    ...
    for (const key in profileData) {
      if (
        ...
        if (profileData[key] !== null && typeof profileData[key] === "object") {
          ...
          logger.debug("UserProfileService.added_to_custom_settings", {
            userKey,
            customKey: key,
          });
        }
  ...
  async storeFile(file, userKey, fieldName) {
    const startTime = Date.now();
    try {
      logger.debug("UserProfileService.store_file_start", {
        userKey,
        fieldName,
      });
      ...
      if (!fs.existsSync(userDir)) {
        ...
        logger.info("UserProfileService.created_user_directory", {
          userKey,
          path: userDir,
        });
      }
  ...
  async resetUserData(userKey) {
    ...
    try {
      ...
      try {
        ...
        logger.info("UserProfileService.user_document_updated_with_overwrite", {
          userKey,
        });
      }
