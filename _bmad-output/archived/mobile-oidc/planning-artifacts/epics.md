...
**Given** `SettingsComponent` uses `PasswordProxy` for password change and account deletion
**When** `password_proxy.dart` is deleted
**Then** the "Change Password" button/menu is removed from `SettingsComponent`
**And** the account deletion flow (password input + `_userService.deleteAccount()`) is removed from `SettingsComponent`
**And** the `PasswordProxy` import and field are removed from `settings_component.dart`

**Given** account management is no longer handled in-app
**When** the user views settings
**Then** a "Manage Account" link opens the Keycloak account console (`{keycloakUrl}/realms/{realm}/account/`) in the system browser
...
**Given** `password_reset_initiate_screen.dart` and `password_reset_confirm_screen.dart` were removed in Story 6.2
**When** no remaining code references these screens
**Then** their i18n keys are cleaned up from locale files if orphaned
