...
describe("Fixtures", () => {
  ...
  describe("createMockAdmin", () => {
    it("returns user with admin roles", () => {
      ...
      expect(admin.resource_access).toEqual({
        "genie-app": { roles: ["admin"] },
      });
    });
  ...
  describe("shared-lib mock", () => {
    ...
    it("SecurityMiddleware has applySecurityMiddleware jest fn", () => {
      ...
      expect(typeof SecurityMiddleware.applySecurityMiddleware).toBe(
        "function",
      );
      expect(SecurityMiddleware.applySecurityMiddleware._isMockFunction).toBe(
        true,
      );
    });
  });
