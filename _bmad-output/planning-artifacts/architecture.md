...
| Flutter flavors | Build system | Per-deployment build via `lib/config/` + `--dart-define=FLAVOR=<name>` | New |
...
**Decision: Flavor config via `lib/config/` + `--dart-define`**

- Rationale: No code generation (`flavorizr`), no `build_runner`. A `lib/config/` directory with one Dart file per flavor, selected via `--dart-define=FLAVOR=<name>` at build time. Simple, transparent, no tooling dependency.
