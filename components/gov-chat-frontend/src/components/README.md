...
* [Component Interactions](https://www.google.com/search?q=%23component-interactions)  
...
* **AdminDashboard.vue**: A comprehensive, multi-tab administration panel for system monitoring and content management. It includes:  
  * **System Health**: Real-time monitoring of services and resource usage.  
  * **Knowledge Hierarchy Management**: A full CRUD interface for creating, reading, updating, and deleting service categories and their underlying services. Includes management of multilingual translations.  
  * **Document Management**: A dashboard for managing uploaded documents, viewing their metadata, and controlling their lifecycle (ingest, retract, delete).  
  * **Database Management**: Tools for re-indexing, backing up, and optimizing the database.  
  * **Log Management**: An interface for viewing and searching system logs.  
  * **Security**: A dashboard for running security scans and viewing vulnerabilities.  
  * **User Management**: A list of all users with search and edit capabilities.  
...
* **UploadFilesDialog.vue**: A dialog providing a drag-and-drop interface for uploading multiple files, with validation for allowed file types.  
* **AddFromLinkDialog.vue**: A dialog for ingesting a document by providing a public URL to be crawled.  
* **FileDetailsDialog.vue**: A detailed dialog for viewing and editing a file's metadata (name, author, labels) and managing its lifecycle (ingest, retract, delete) based on its current status.
...
1. **Parent-Child Communication**:  
   * Props flow down from parent to child components (e.g., AdminDashboard passing a fileId to FileDetailsDialog).  
   * Events flow up from child to parent components (e.g., UploadFilesDialog emitting a files-uploaded event to AdminDashboard).  
2. **Event Bus**:  
   * Components communicate across the hierarchy using a shared eventBus for cross-component communication.  
   * Key events include treeNodeSelected (from ServiceTreePanelComponent to ChatBotComponent), knowledge-hierarchy-updated (from AdminDashboard to ServiceTreePanelComponent), and chat-deleted.  
3. **Vuex Store**:  
...
* A comprehensive **Admin Dashboard** with real-time system health monitoring, content management, and system tools.  
* **Knowledge Hierarchy Management** with full CRUD capabilities for categories, services, and multilingual translations.  
* **Document Management** with features for uploading files (direct and via URL), editing metadata (name, author, labels), and controlling the data lifecycle through ingestion and retraction.  
