    ...
    try {
      ...
      if (backendMode === 'test-mode') {
        ...
        if (backendMode === 'single-message') {
          ...
          // AgriConnect guardrail - filter non-agriculture questions
          const nonAgriPatterns = [
            /king|queen|prince|princess|royal|monarch|palace/i,
            /president|prime minister|minister|parliament|government leader|politician/i,
            /weather forecast|climate change|global warming/i,
            /celebrity|actor|singer|musician|footballer|sport/i,
            /history of|capital of|population of|currency of/i,
            /who is|what is the name of|tell me about [a-z]+ [a-z]+(?! farm| crop| pest| disease| soil| seed| harvest)/i
          ];
          const agriKeywords = /farm|crop|soil|seed|plant|harvest|pest|disease|fertiliz|maize|bean|poone|linaoa|livestock|cattle|sheep|goat|irrigat|weed|yield|extension|agricultur|ministry of agri/i;
          
          const isNonAgri = nonAgriPatterns.some(p => p.test(queryText)) && !agriKeywords.test(queryText);
          
          if (isNonAgri) {
            logger.info('[DEBUG] Non-agriculture question detected. Returning guardrail response.');
            throw {
              isGuardrail: true,
              guardrailMessage: "I am Keletso, your agricultural advisor for Lesotho. I can only assist with farming and agriculture questions. Please ask me about maize, beans, pests, weather or farming programs.",
            };
          }
          
          const systemPrompt = process.env.CHATQNA_SYSTEM_PROMPT || '';
          opeaPayload = {
            messages: systemPrompt 
              ? [{ role: 'system', content: systemPrompt }, { role: 'user', content: queryText }]
      ...
      if (error.isGuardrail) {
        logger.info('[DEBUG] Guardrail response returned to user.');
        return {
          response: error.guardrailMessage,
          metadata: { source_documents: [], confidence_score: 1.0 }
        };
      }
