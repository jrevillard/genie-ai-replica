...
from comps.cores.proto.api_protocol_genieai import ChatCompletionRequest, RetrievalRequest, RetrievalRequestArangoDB
...
class OpeaArangoRetriever(OpeaComponent):
    ...
    async def invoke(
        ...
        filter_data = input_dict.get("context", {}) # previously called filter
        filter_strategy = input_dict.get("filter_strategy", "OR").upper() # Set it as an environment variable 💛
        ...
        if labels_to_filter:
            ...
            # bind_vars["labels"] = labels_to_filter
            if logflag:
                logger.info(f"Applying filter strategy '{filter_strategy}' with labels: {labels_to_filter}")
        ...
        if logflag:
            logger.info(f"Graph name: {graph_name}, Start Collection name: {collection_name}, label filter clause: {aql_filter_clause}")
        ...
        try:
            vector_db = ArangoVector(
                ...
                #return_metadata_fields=["file_id", "file_path"]
            )
        except Exception as e:
            if logflag:
        ...
        try:
            if input.search_type == "similarity_score_threshold":
                ...
                print(f'🫠🫠🫠 Doing similarity score with threshold: {input} 🫠🫠🫠 ')
                ...
                results = await vector_db.amax_marginal_relevance_search(
                    ...
                    use_approx=use_approx_search,
                    aql_filter=aql_filter_clause,  # might need to change to filter_clause as per: https://github.com/arangoml/langchain-arangodb/blob/a1d9a4c064413d5aedc2263540b852f6938421e9/libs/arangodb/langchain_arangodb/vectorstores/arangodb_vector.py#L581
                    # bind_vars=bind_vars,
                )
        ...
        if logflag:
            ...
            #             RETURN {{_key: doc._key, file_id: doc.file_id}}
            #     """
            #     bind_vars = {"chunk_ids": chunk_ids}
            #     cursor = self.db.aql.execute(aql, bind_vars=bind_vars)
            #     file_id_map = {doc["_key"]: doc.get("file_id") for doc in cursor}
            #     file_ids = list(set(file_id_map.values()))
            # if logflag:
            #     logger.info(f"Retrieved file IDs after similarity search: {file_ids}")
        ...
        if enable_traversal:
            ...
            neighborhoods = self.fetch_neighborhoods(
                ...
                collection_name=collection_name, # In the search step before, collection_name was set based on search_start. Here it seems the same collection_name.
                traversal_max_depth=traversal_max_depth,
                traversal_max_returned=traversal_max_returned,
                traversal_score_threshold=traversal_score_threshold,
            ...
            for r in search_res:
                ...
                if neighborhood:
                    if search_start == 'chunk': 
                        ...
                        # file_ids.extend([neighborhood[0]['file_id']] if neighborhood and 'file_id' in neighborhood[0] else [])
                    else: # search_start == 'node'
                        r['doc'].page_content += "\n------\nRELATED INFORMATION:\n------\n"
                        ...
                        # file_ids.extend([list(neighborhood[0].values())[0]['file_id']] if neighborhood and list(neighborhood[0].values())[0] and 'file_id' in list(neighborhood[0].values())[0] else [])
            
            # file_ids = list(set(file_ids)) # unique file IDs
        ...
        if logflag:
            logger.info(f"Final results of retrievers/src/integrations/arangodb_genieai.py: {search_res}")
            # logger.info(f"Final file IDs of retrievers/src/integrations/arangodb_genieai.py: {file_ids}")

        return search_res
        # return search_res, file_ids
