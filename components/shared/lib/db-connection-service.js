...
class DatabaseService {
  ...
  async _performActiveRecovery(name, originalError) {
    ...
    try {
      ...
      logger.info(`[DB_RECOVERY] ‚úÖ ACTIVE RECOVERY SUCCESSFUL for ${name}`);
      ...
      logger.info(`[DB_RECOVERY] Ì†ΩÌ¥Ñ Updated ${proxyCount} existing proxies with fresh connection`);
      ...
      logger.error(`[DB_RECOVERY] ‚ùå Recovery attempt ${attempts + 1} failed for ${name}: ${recoveryError.message}`);
    }
  }
