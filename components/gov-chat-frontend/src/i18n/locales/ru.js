export default {
  countries: {
    AF: 'Афганистан',
    AL: 'Албания',
    DZ: 'Алжир',
    AD: 'Андорра',
    AO: 'Ангола',
    AG: 'Антигуа и Барбуда',
    AR: 'Аргентина',
    AM: 'Армения',
    AU: 'Австралия',
    AT: 'Австрия',
    AZ: 'Азербайджан',
    BS: 'Багамы',
    BH: 'Бахрейн',
    BD: 'Бангладеш',
    BB: 'Барбадос',
    BY: 'Беларусь',
    BE: 'Бельгия',
    BZ: 'Белиз',
    BJ: 'Бенин',
    BT: 'Бутан',
    BO: 'Боливия',
    BA: 'Босния и Герцеговина',
    BW: 'Ботсвана',
    BR: 'Бразилия',
    BN: 'Бруней',
    BG: 'Болгария',
    BF: 'Буркина-Фасо',
    BI: 'Бурунди',
    CV: 'Кабо-Верде',
    KH: 'Камбоджа',
    CM: 'Камерун',
    CA: 'Канада',
    CF: 'Центральноафриканская Республика',
    TD: 'Чад',
    CL: 'Чили',
    CN: 'Китай',
    CO: 'Колумбия',
    KM: 'Коморы',
    CG: 'Конго',
    CD: 'Конго, Демократическая Республика',
    CR: 'Коста-Рика',
    CI: 'Кот-д’Ивуар',
    HR: 'Хорватия',
    CU: 'Куба',
    CY: 'Кипр',
    CZ: 'Чехия',
    DK: 'Дания',
    DJ: 'Джибути',
    DM: 'Доминика',
    DO: 'Доминиканская Республика',
    EC: 'Эквадор',
    EG: 'Египет',
    SV: 'Сальвадор',
    GQ: 'Экваториальная Гвинея',
    ER: 'Эритрея',
    EE: 'Эстония',
    ET: 'Эфиопия',
    FJ: 'Фиджи',
    FI: 'Финляндия',
    FR: 'Франция',
    GA: 'Габон',
    GM: 'Гамбия',
    GE: 'Грузия',
    DE: 'Германия',
    GH: 'Гана',
    GR: 'Греция',
    GD: 'Гренада',
    GT: 'Гватемала',
    GN: 'Гвинея',
    GW: 'Гвинея-Бисау',
    GY: 'Гайана',
    HT: 'Гаити',
    HN: 'Гондурас',
    HU: 'Венгрия',
    IS: 'Исландия',
    IN: 'Индия',
    ID: 'Индонезия',
    IR: 'Иран',
    IQ: 'Ирак',
    IE: 'Ирландия',
    IL: 'Израиль',
    IT: 'Италия',
    JM: 'Ямайка',
    JP: 'Япония',
    JO: 'Иордания',
    KZ: 'Казахстан',
    KE: 'Кения',
    KI: 'Кирибати',
    KP: 'КНДР',
    KR: 'Южная Корея',
    KW: 'Кувейт',
    KG: 'Кыргызстан',
    LA: 'Лаос',
    LV: 'Латвия',
    LB: 'Ливан',
    LS: 'Лесото',
    LR: 'Либерия',
    LY: 'Ливия',
    LI: 'Лихтенштейн',
    LT: 'Литва',
    LU: 'Люксембург',
    MG: 'Мадагаскар',
    MW: 'Малави',
    MY: 'Малайзия',
    MV: 'Мальдивы',
    ML: 'Мали',
    MT: 'Мальта',
    MH: 'Маршалловы Острова',
    MR: 'Мавритания',
    MU: 'Маврикий',
    MX: 'Мексика',
    FM: 'Микронезия',
    MD: 'Молдова',
    MC: 'Монако',
    MN: 'Монголия',
    ME: 'Черногория',
    MA: 'Марокко',
    MZ: 'Мозамбик',
    MM: 'Мьянма',
    NA: 'Намибия',
    NR: 'Науру',
    NP: 'Непал',
    NL: 'Нидерланды',
    NZ: 'Новая Зеландия',
    NI: 'Никарагуа',
    NE: 'Нигер',
    NG: 'Нигерия',
    MK: 'Северная Македония',
    NO: 'Норвегия',
    OM: 'Оман',
    PK: 'Пакистан',
    PW: 'Палау',
    PS: 'Палестина',
    PA: 'Панама',
    PG: 'Папуа — Новая Гвинея',
    PY: 'Парагвай',
    PE: 'Перу',
    PH: 'Филиппины',
    PL: 'Польша',
    PT: 'Португалия',
    QA: 'Катар',
    RO: 'Румыния',
    RU: 'Россия',
    RW: 'Руанда',
    KN: 'Сент-Китс и Невис',
    LC: 'Сент-Люсия',
    VC: 'Сент-Винсент и Гренадины',
    WS: 'Самоа',
    SM: 'Сан-Марино',
    ST: 'Сан-Томе и Принсипи',
    SA: 'Саудовская Аравия',
    SN: 'Сенегал',
    RS: 'Сербия',
    SC: 'Сейшелы',
    SL: 'Сьерра-Леоне',
    SG: 'Сингапур',
    SK: 'Словакия',
    SI: 'Словения',
    SB: 'Соломоновы Острова',
    SO: 'Сомали',
    ZA: 'Южная Африка',
    SS: 'Южный Судан',
    ES: 'Испания',
    LK: 'Шри-Ланка',
    SD: 'Судан',
    SR: 'Суринам',
    SZ: 'Эсватини',
    SE: 'Швеция',
    CH: 'Швейцария',
    SY: 'Сирия',
    TJ: 'Таджикистан',
    TZ: 'Танзания',
    TH: 'Таиланд',
    TL: 'Восточный Тимор',
    TG: 'Того',
    TO: 'Тонга',
    TT: 'Тринидад и Тобаго',
    TN: 'Тунис',
    TR: 'Турция',
    TM: 'Туркменистан',
    TV: 'Тувалу',
    UG: 'Уганда',
    UA: 'Украина',
    AE: 'Объединенные Арабские Эмираты',
    GB: 'Великобритания',
    US: 'Соединенные Штаты Америки',
    UY: 'Уругвай',
    UZ: 'Узбекистан',
    VU: 'Вануату',
    VA: 'Ватикан',
    VE: 'Венесуэла',
    VN: 'Вьетнам',
    YE: 'Йемен',
    ZM: 'Замбия',
    ZW: 'Зимбабве'
  },
  admin: {
    huduma: 'Genie AI',
    dashboard: 'Панель управления',
    system: 'Система',
    settings: 'Настройки',
    overview: 'Обзор',
    database: 'База данных',
    logs: 'Журналы',
    userManagement: 'Управление пользователями',
    systemAdministration: 'Администрирование системы',
    manage: 'Управлять',
    loading: 'Загрузка...',
    close: 'Закрыть панель управления',
    edit: 'Редактировать',
    systemUptime: 'Время работы системы',
    avgResponseTime: 'Среднее время ответа',
    errorRate: 'Уровень ошибок',
    activeUsers: 'Активные пользователи',
    monthlyActiveUsers: 'Ежемесячные активные пользователи (MAU)',
    fromLastMonth: 'с прошлого месяца',
    today: 'Сегодня',
    tabs: {
      overview: 'Состояние системы',
      hierarchy: 'Иерархия знаний',
      documents: 'Управление документами',
      database: 'База данных',
      logs: 'Журналы',
      security: 'Безопасность',
      users: 'Пользователи'
    },
    contentManagement: 'УПРАВЛЕНИЕ КОНТЕНТОМ',
    knowledgeHierarchy: 'Иерархия знаний',
    documentManagement: 'Управление документами',
    noLogsFound: 'Журналы за сегодня не найдены',
    invalidLogsResponse: 'Недопустимая структура ответа сводки журналов',
    logsSummaryError: 'Не удалось загрузить сводку журналов',
    buttons: {
      save: 'Сохранить',
      cancel: 'Отмена',
      switch: 'Все равно переключить',
      discard: 'Отменить'
    },
    confirm: {
      defaultTitle: 'Подтвердить',
      defaultMessage: 'Вы уверены?'
    },
    systemHealthStatus: 'Статус состояния системы',
    runDiagnostics: 'Запустить диагностику',
    resourceUsage: 'Использование ресурсов',
    services: {
      apiServices: 'Службы API',
      database: 'База данных',
      cache: 'Кэш',
      storage: 'Хранилище',
      messageQueue: 'Очередь сообщений',
      externalApi: 'Внешний API'
    },
    resources: {
      cpu: 'Использование процессора',
      memory: 'Использование памяти',
      storage: 'Использование хранилища',
      network: 'Пропускная способность сети'
    },
    databaseManagement: 'Управление базой данных',
    databaseSize: 'Размер базы данных',
    totalTables: 'Всего таблиц',
    logManagement: 'Управление журналами',
    rolloverLogs: 'Ротация журналов',
    searchLogs: 'Поиск в журналах',
    logTime: 'Время',
    logLevel: 'Уровень',
    logService: 'Служба',
    logMessage: 'Сообщение',
    showingEntries: 'Показано {start}-{end} из {total} записей',
    logLevels: {
      error: 'ОШИБКА',
      warn: 'ПРЕДУПРЕЖДЕНИЕ',
      info: 'ИНФО',
      debug: 'ОТЛАДКА'
    },
    logMessages: {
      connectionTimeout: 'Тайм-аут соединения с внешним провайдером',
      lowDiskSpace: 'Место на диске ниже порога 10%',
      userRoleUpdated: 'Роль пользователя обновлена для admin@huduma.ai'
    },
    userName: 'Имя',
    userEmail: 'Электронная почта',
    userRole: 'Роль',
    userStatus: 'Статус',
    userActions: 'Действия',
    roleAdministrator: 'Администратор',
    roleUser: 'Пользователь',
    statusActive: 'Активен',
    dbActions: {
      backup: 'Резервное копирование',
      optimize: 'Оптимизировать',
      backupDesc: 'Создать резервную копию базы данных',
      optimizeDesc: 'Оптимизировать производительность запросов'
    },
    operations: {
      backupTitle: 'Результаты резервного копирования базы данных',
      optimizeTitle: 'Результаты оптимизации базы данных',
      optimizeResults: 'Результаты оптимизации',
      collection: 'Коллекция',
      status: 'Статус',
      indexSuggestions: 'Предложения по индексам',
      backupDetails: 'Детали резервного копирования',
      backupFile: 'Файл резервной копии',
      backupLocation: 'Местоположение',
      backupSize: 'Размер',
      errorDetails: 'Детали ошибки',
      close: 'Закрыть',
      resultsTitle: 'Результаты операции',
      reindexDatabase: {},
      backupDatabase: {
        success: 'Резервное копирование базы данных успешно завершено',
        error: 'Ошибка при резервном копировании базы данных',
        loading: 'Резервное копирование базы данных...'
      },
      optimizeDatabase: {
        success: 'Оптимизация базы данных успешно завершена',
        error: 'Ошибка при оптимизации базы данных',
        loading: 'Оптимизация базы данных...'
      },
      rolloverLogs: {
        success: 'Ротация журналов успешно завершена',
        loading: 'Ротация журналов...'
      },
      searchLogs: {
        success: 'Поиск в журналах завершен',
        loading: 'Поиск в журналах...'
      },
      runDiagnostics: {
        success: 'Диагностика успешно завершена',
        loading: 'Выполняется диагностика...'
      },
      runSecurityScan: {
        success: 'Сканирование безопасности успешно завершено',
        loading: 'Выполняется сканирование безопасности...'
      },
      cancel: 'Отмена',
      save: 'Сохранить изменения'
    },
    userManagementSection: {
      totalUsers: 'Всего пользователей',
      newUsers: 'Новые пользователи (месяц)',
      searchUsers: 'Поиск пользователей...',
      clearSearch: 'Очистить поиск',
      search: 'Поиск',
      usersFound: 'пользователей найдено',
      showAllUsers: 'Показать всех пользователей',
      searching: 'Поиск...',
      searchingUsers: 'Поиск пользователей...',
      noUsersFound: 'Пользователи, соответствующие критериям поиска, не найдены.',
      noUsers: 'Нет доступных пользователей.',
      showing: 'Показано',
      of: 'из',
      previous: 'Предыдущая',
      next: 'Следующая',
      searchFields: {
        all: 'Все поля',
        name: 'Имя',
        email: 'Электронная почта',
        role: 'Роль'
      },
      userSearch: {
        resultsFound: 'Найдено {total} пользователей, соответствующих "{term}"',
        error: 'Ошибка при поиске пользователей'
      }
    },
    logsSection: {
      errorLogs: 'Журналы ошибок',
      warningLogs: 'Журналы предупреждений',
      noErrorLogs: 'Сегодня не зарегистрировано журналов ошибок.',
      noWarningLogs: 'Сегодня не зарегистрировано журналов предупреждений.',
      infoLogsNote:
        'Информационные журналы не отображаются в сводке. Используйте функцию поиска, чтобы просмотреть все типы журналов.',
      searchResults: 'Последние результаты поиска',
      entriesFound: 'записей найдено',
      viewAllResults: 'Просмотреть все результаты',
      logType: 'Тип',
      logCount: 'Количество',
      logTypes: {
        connectionTimeout: 'Тайм-аут соединения',
        databaseFailed: 'Ошибка запроса к базе данных',
        authFailed: 'Ошибка аутентификации',
        lowDiskSpace: 'Место на диске ниже порога',
        slowQuery: 'Низкая производительность запроса',
        rateLimit: 'Приближение к лимиту скорости'
      },
      logSearch: {
        noResultsFound: 'Не найдено журналов, соответствующих вашим критериям поиска',
        resultsFound: 'Найдено {count} записей журнала'
      }
    },
    security: {
      title: 'Мониторинг безопасности',
      securityScan: 'Сканирование безопасности',
      runningSecurityScan: 'Выполняется сканирование...',
      lastSecurityScan: 'Последнее сканирование безопасности',
      vulnerabilitiesFound: 'Обнаруженные уязвимости',
      daysAgo: 'дней назад',
      critical: 'критический',
      medium: 'средний',
      low: 'низкий',
      failedLoginAttempts: 'Неуспешные попытки входа (24ч)',
      suspiciousActivities: 'Подозрительные действия (24ч)',
      securityRecommendations: 'Рекомендации по безопасности',
      criticalVulnerabilities: 'Критические уязвимости',
      mediumVulnerabilities: 'Средние уязвимости',
      lowVulnerabilities: 'Низкие уязвимости',
      authenticationIssues: 'Проблемы аутентификации',
      suspiciousActivityLogs: 'Журналы подозрительных действий',
      securityProbeAttempts: 'Обнаружены попытки зондирования безопасности',
      sensitiveFileAccess: 'обнаружены попытки доступа к конфиденциальным файлам или конечным точкам',
      envFileAccess: 'Попытки доступа к файлам окружения',
      envFileAccessDesc: 'обнаружены попытки доступа к файлам .env',
      gitRepoAccess: 'Попытки доступа к репозиторию Git',
      gitRepoAccessDesc: 'обнаружены попытки доступа к файлам репозитория Git',
      recommendedAction: 'Рекомендуемое действие',
      rateLimitRecommendation:
        'Рассмотрите внедрение ограничения скорости, блокировку IP-адресов для постоянных нарушителей и убедитесь, что сервер должным образом защищён.',
      envFileRecommendation:
        'Убедитесь, что файлы окружения недоступны из веб-каталогов, и серверные конфигурации должным образом блокируют доступ к конфиденциальным файлам.',
      gitRepoRecommendation: 'Убедитесь, что каталоги .git должным образом защищены и недоступны из веб.',
      noVulnerabilitiesFound: 'Уязвимости не найдены',
      systemSecure: 'Ваша система выглядит безопасной. Продолжайте регулярно отслеживать.',
      loadingScan: 'Загрузка результатов сканирования...',
      occurrences: 'Случаи',
      firstSeen: 'Впервые замечено',
      lastSeen: 'Последний раз замечено',
      timestamp: 'Временная метка',
      message: 'Сообщение',
      service: 'Служба',
      type: 'Тип',
      severity: 'Серьёзность',
      url: 'URL',
      matchedTerm: 'Соответствующий термин',
      lineNumber: 'Номер строки',
      lineNumbers: 'Номера строк',
      showLess: 'Показать меньше',
      showMore: 'Показать все проблемы',
      lastScanJustNow: 'Только что'
    },
    hierarchy: {
      title: 'Управление иерархией знаний (примечание: всегда на английском - добавить переводы)',
      addCategory: 'Добавить новую категорию',
      loading: 'Загрузка иерархии...',
      addService: 'Добавить услугу',
      editCategory: 'Редактировать категорию',
      deleteCategory: 'Удалить категорию',
      editService: 'Редактировать услугу',
      deleteService: 'Удалить услугу',
      empty: 'Категории не найдены. Нажмите "Добавить новую категорию", чтобы начать.',
      formTitleCreateCategory: 'Создать новую категорию',
      formTitleAddService: 'Добавить услугу в "{categoryName}"',
      formTitleEditCategory: 'Редактировать категорию: "{itemName}"',
      formTitleEditService: 'Редактировать услугу: "{itemName}"',
      nameEnLabel: 'Название (Английский)',
      translationsTitle: 'Переводы для отображения',
      loadingTranslations: 'Загрузка переводов...',
      selectLang: 'Выберите язык',
      translationPlaceholder: 'Введите перевод',
      deleteTranslation: 'Удалить перевод',
      addTranslation: '+ Добавить перевод',
      saveSuccess: 'Элемент иерархии успешно сохранен.',
      saveError: 'Не удалось сохранить элемент иерархии.',
      loadError: 'Не удалось загрузить иерархию знаний.',
      loadTranslationsError: 'Не удалось загрузить переводы.',
      confirmCancelTitle: 'Несохраненные изменения',
      confirmCancelEdit: 'У вас есть несохраненные изменения. Вы уверены, что хотите отменить?',
      confirmDeleteTitleCategory: 'Удалить категорию?',
      confirmDeleteTitleService: 'Удалить услугу?',
      confirmDeleteCategory: 'Вы уверены, что хотите удалить категорию "{itemName}"? Это действие необратимо.',
      confirmDeleteService: 'Вы уверены, что хотите удалить услугу "{itemName}"? Это действие необратимо.',
      deleteSuccessCategory: 'Категория успешно удалена.',
      deleteSuccessService: 'Услуга успешно удалена.',
      deleteErrorCategory: 'Не удалось удалить категорию.',
      deleteErrorService: 'Не удалось удалить услугу.',
      duplicateLangError: 'В переводах найдены дублирующиеся языки. Пожалуйста, удалите их.'
    },
    documents: {
      title: 'Управление документами',
      addLink: 'Добавить по ссылке',
      uploadFiles: 'Загрузить файлы',
      searchPlaceholder: 'Поиск по имени файла...',
      allStatuses: 'Все статусы',
      statusPending: 'Ожидание',
      statusIngested: 'Обработано',
      statusRetracted: 'Изъято',
      statusIngesting: 'Обрабатывается',
      statusError: 'Ошибка обработки',
      statusWarning: 'Обработано с предупреждениями',
      ingestSelected: 'Обработать выбранные',
      colFileName: 'Имя файла',
      colStatus: 'Статус',
      colLabels: 'Метки',
      colUploadDate: 'Дата загрузки',
      colSize: 'Размер',
      loading: 'Загрузка документов...',
      empty: 'Документы не найдены.',
      loadError: 'Не удалось загрузить документы.',
      confirmIngestTitle: 'Подтвердить пакетную обработку',
      ingest: 'Обработать',
      confirmIngestSelected: 'Вы уверены, что хотите обработать {count} выбранный(х) файл(ов)?',
      ingestQueuedSuccess: '{count} файл(ов) добавлен(ы) в очередь на обработку.',
      ingestQueuedError: 'Произошла ошибка во время процесса пакетной обработки.',
      uploadSuccessMultiple: '{count} файл(ов) успешно загружен(ы).',
      linkSubmitSuccess: 'Успешно просканировано и сохранено "{fileName}".',
      actionSuccess: 'Действие "{action}" с файлом {fileId} прошло успешно.',
      metadataUpdateSuccess: 'Метаданные для файла {fileId} обновлены.'
    },
    queryInspector: {
      search: 'Поиск',
      searchPlaceholder: 'Поиск текста запроса...',
      minConfidence: 'Мин. достоверность',
      maxConfidence: 'Макс. достоверность',
      from: 'От',
      to: 'До',
      reset: 'Сброс',
      colTime: 'Время',
      colQuestion: 'Вопрос пользователя',
      colConfidence: 'Достоверность',
      colResponseTime: 'Время отклика',
      colSources: 'Источники',
      colFeedback: 'Отзывы',
      colActions: 'Действия',
      inspect: 'Проверить',
      noResults: 'Запросы, соответствующие вашим фильтрам, не найдены.',
      prev: 'Назад',
      pageInfo: 'Страница {current} из {total} (всего {count})',
      next: 'Далее',
      backToList: 'Вернуться к списку',
      detailTitle: 'Инспектор запросов',
      sectionQuestion: 'Вопрос пользователя',
      user: 'Пользователь',
      time: 'Время',
      responseTime: 'Время отклика',
      mode: 'Режим',
      sectionContext: 'Контекст (метки отправлены в RAG)',
      category: 'Категория',
      serviceLabels: 'Сервисные метки',
      language: 'Язык',
      sectionMessages: 'Сообщения, отправленные в конвейер',
      sectionDocuments: 'Найденные документы (результаты Vector DB)',
      score: 'Оценка',
      labels: 'Метки',
      viewDoc: 'Просмотр документа',
      noDocuments: 'Исходные документы для этого запроса не найдены.',
      sectionResponse: 'Ответ LLM',
      confidenceScore: 'Оценка достоверности',
      retrievalConfidenceScore: 'Достоверность поиска',
      selfConfidence: 'Уверенность (LLM)',
      sectionFeedback: 'Отзывы пользователей',
      rating: 'Рейтинг',
      comment: 'Комментарий',
      providedAt: 'Предоставлено',
      loadError: 'Не удалось загрузить запросы.'
    }
  },
  auth: {
    errors: {
      tokenExpired: 'Ваша сессия истекла. Пожалуйста, войдите снова.',
      tokenInvalid: 'Ваша сессия недействительна. Пожалуйста, войдите снова.',
      insufficientRoles: 'У вас нет необходимых разрешений. Свяжитесь с вашим администратором.',
      serviceUnavailable: 'Служба аутентификации временно недоступна. Пожалуйста, попробуйте позже.',
      provisioningFailed: 'Произошла системная ошибка. Пожалуйста, попробуйте позже.',
      default: 'Произошла ошибка'
    }
  },
  charts: {
    tooltip: {
      title: 'Дата',
      totalQueries: 'Всего запросов',
      uniqueUsers: 'Уникальные пользователи'
    },
    day: 'День',
    noData: 'Нет данных',
    notAvailable: 'Н/Д',
    topQueries: 'Популярные запросы',
    categoryDistribution: 'Использование по областям знаний',
    usageTrend: 'Тренд использования'
  },
  analytics: {
    title: 'Аналитика данных и инсайты',
    loading: 'Загрузка аналитических данных...',
    period: 'Период времени',
    charts: {
      satisfactionHeatmap: 'Удовлетворённость по областям знаний',
      satisfactionSubtitle: 'Процентные оценки с течением времени'
    },
    timePeriods: {
      week4: '4 недели назад',
      week3: '3 недели назад',
      week2: '2 недели назад',
      week1: 'Прошлая неделя',
      current: 'Текущий'
    },
    errors: {
      loading: 'Не удалось загрузить данные об удовлетворённости. Пожалуйста, попробуйте снова.'
    },
    status: {
      loading: 'Загрузка...',
      noData: 'Нет данных за этот период',
      error: 'Ошибка загрузки аналитических данных',
      online: 'Система онлайн',
      offline: 'Система оффлайн',
      responseTime: 'Среднее время ответа',
      queueLength: 'Очередь',
      uptime: 'Время работы'
    },
    periods: {
      daily: 'Ежедневно',
      weekly: 'Еженедельно',
      monthly: 'Ежемесячно',
      allTime: 'За всё время'
    },
    metrics: {
      totalQueries: 'Всего запросов',
      uniqueUsers: 'Уникальные пользователи',
      avgResponseTime: 'Среднее время ответа',
      satisfaction: 'Удовлетворённость пользователей'
    },
    table: {
      rank: 'Ранг',
      query: 'Запрос',
      count: 'Количество',
      avgTime: 'Сред. время'
    },
    chartLabels: {
      categories: 'Области',
      serviceCategories: 'Области знаний',
      byUsage: 'по использованию',
      category: 'Область'
    },
    tooltips: {
      selectPeriod: 'Выбрать период времени',
      selectDate: 'Выбрать дату',
      exportData: 'Экспортировать данные'
    },
    gauge: {
      historical: 'Исторические тренды',
      vsPrevious: 'по сравнению с предыдущим периодом',
      target: 'Цель'
    },
    ratings: {
      poor: 'Плохо',
      average: 'Средне',
      good: 'Хорошо',
      excellent: 'Отлично'
    },
    slower: 'медленнее',
    faster: 'быстрее',
    percentage: 'Процент',
    note: 'Ниже приведены статистика использования и сводки отзывов пользователей.',
    usageStats: 'Статистика использования',
    peakUsers: 'Пиковые одновременные пользователи',
    activeChats: 'Активные чаты',
    usageTrend: 'Тренд использования',
    topQueries: 'Популярные запросы',
    feedbackSamples: 'Образцы отзывов пользователей',
    close: 'Закрыть',
    chartComingSoon: 'Интерактивная визуализация графиков скоро появится...',
    retry: 'Повторить',
    dailyQueries: 'Ежедневные запросы',
    export: 'Экспортировать данные',
    dateRange: 'Диапазон дат',
    startDate: 'Дата начала',
    endDate: 'Дата окончания',
    rank: 'Ранг',
    query: 'Запрос',
    count: 'Количество',
    avgTime: 'Сред. время',
    serviceUsage: 'Использование по областям знаний',
    usageTrends: 'Тренды использования',
    week: 'Прошлая неделя',
    month: 'Прошлый месяц',
    quarter: 'Прошлый квартал',
    year: 'Прошлый год',
    uniqueUsers: 'Уникальные пользователи',
    satisfactionRate: 'Уровень удовлетворённости',
    satisfactionAnalysis: 'Анализ удовлетворённости пользователей'
  },
  brandName: 'Genie AI: Ваш цифровой правительственный помощник',
  nav: {
    menu: 'Меню',
    analytics: 'Аналитика',
    userProfile: 'Профиль пользователя',
    settings: 'Настройки',
    logout: 'Выйти',
    profile: 'Мой профиль',
    toggleSidebar: 'Переключить боковую панель',
    changeLanguage: 'Изменить язык',
    administration: 'Администрирование',
    languages: {
      english: 'Английский',
      french: 'Французский',
      swahili: 'Суахили'
    }
  },
  services: {
    eCitizen: 'Портал eCitizen',
    taxFiling: 'Система подачи налогов',
    idApplication: 'Заявка на ID',
    businessReg: 'Регистрация бизнеса',
    drivingLicense: 'Водительские права'
  },
  sidebar: {
    governmentServices: 'Области знаний',
    chatHistory: 'История чатов',
    searchPlaceholder: 'Поиск по областям знаний...',
    createFolder: 'Создать папку',
    editFolder: 'Редактировать папку',
    folderName: 'Имя папки',
    folderNamePlaceholder: 'Введите имя папки',
    moveChat: 'Переместить чат',
    moveChatTo: 'Переместить чат в',
    selectFolder: 'Выбрать папку',
    deleteChat: 'Удалить чат',
    deleteChatConfirm: 'Вы уверены, что хотите удалить этот чат?',
    deleteChatWarning: 'Это действие нельзя отменить.',
    renameChat: 'Переименовать чат',
    chatTitle: 'Название чата',
    chatTitlePlaceholder: 'Введите название для этого чата',
    deleteFolder: 'Удалить папку',
    deleteFolderConfirm: "Вы уверены, что хотите удалить папку '{name}'?",
    chatsMoveWarning: 'Все чаты в этой папке будут перемещены в папку по умолчанию.',
    weatherTitle: 'Прогноз погоды',
    weatherLoading: 'Загрузка данных о погоде...',
    weatherError: 'Не удалось загрузить данные о погоде. Пожалуйста, попробуйте снова.',
    weatherLocationLoading: 'Загрузка местоположения...',
    weatherErrorDefault: 'Не удалось получить данные о погоде.',
    weatherGeolocationUnsupported: 'Геолокация не поддерживается вашим браузером.',
    collapse: 'Свернуть боковую панель',
    expand: 'Развернуть боковую панель',
    weatherRefresh: 'Обновить погоду',
    weatherConditions: {
      clear: 'Ясно',
      partlycloudy: 'Частично облачно',
      cloudy: 'Облачно',
      overcast: 'Пасмурно',
      rain: 'Дождь',
      shower: 'Ливень',
      thunderstorm: 'Гроза',
      snow: 'Снег'
    },
    folders: 'Папки',
    emptyFolder: 'Эта папка пуста. Переместите сюда беседы из меню чата.',
    title: 'Информация и ресурсы',
    noChats: 'Нет недавних чатов',
    relatedDocs: 'Связанные документы',
    noDocuments: 'Нет связанных документов',
    faq: 'Часто задаваемые вопросы',
    tab: {
      all: 'Все чаты',
      folders: 'Папки',
      starred: 'Избранные',
      archived: 'Архивированные'
    },
    savedChats: 'Сохранённые чаты',
    chats: 'Чаты',
    allChats: 'Все чаты',
    noFolder: 'Без папки',
    starredChats: 'Избранные чаты',
    archivedChats: 'Архивированные чаты',
    noStarredChats: 'Пока нет избранных бесед. Добавьте беседу в избранное, чтобы она появилась здесь.',
    noArchivedChats: 'Пока нет архивированных бесед.',
    noSearchResults: 'Не найдено бесед для "{term}"',
    loadingChats: 'Загрузка бесед...',
    errorLoadingConversations: 'Не удалось загрузить беседы. Пожалуйста, попробуйте снова.',
    errorLoadingUser: 'Данные пользователя неполные. Пожалуйста, перезагрузите страницу.',
    errorNoUser: 'Данные пользователя отсутствуют. Пожалуйста, перезагрузите страницу.',
    retry: 'Повторить',
    message: 'сообщение',
    messages: 'сообщения',
    created: 'Создано',
    updated: 'Обновлено',
    star: 'Добавить в избранное',
    unstar: 'Убрать из избранного',
    archive: 'Архивировать',
    chatStarred: 'Беседа добавлена в избранное',
    chatUnstarred: 'Беседа убрана из избранного',
    chatArchived: 'Беседа архивирована',
    chatUnarchived: 'Беседа разархивирована',
    errorUpdatingChat: 'Ошибка обновления беседы',
    chatRenamed: 'Беседа успешно переименована',
    errorRenamingChat: 'Ошибка переименования беседы',
    chatDeleted: 'Беседа успешно удалена',
    errorDeletingChat: 'Ошибка удаления беседы',
    chatMoved: 'Беседа успешно перемещена',
    errorMovingChat: 'Ошибка перемещения беседы',
    noPreview: 'Предпросмотр недоступен',
    searchConversations: 'Поиск бесед...',
    selectFolderInstruction: 'Выберите папку для просмотра её бесед',
    folderCreated: 'Папка успешно создана',
    errorCreatingFolder: 'Ошибка создания папки',
    errorLoadingFolders: 'Ошибка загрузки папок',
    folderUpdated: 'Папка успешно обновлена',
    errorUpdatingFolder: 'Ошибка обновления папки',
    folderDeleted: 'Папка успешно удалена',
    errorDeletingFolder: 'Ошибка удаления папки',
    chatRemovedFromFolders: 'Беседа удалена из папки',
    errorRemovingChat: 'Ошибка удаления беседы из папки',
    id: 'ID',
    labels: 'Метки',
    confidence: 'Уверенность',
    unknown: 'Неизвестный'
  },
  leftPanel: {
    cat1: {
      name: '1. Идентификация и гражданская регистрация',
      children:
        'Свидетельства о рождении,Национальные удостоверения личности,Паспорта,Свидетельства о браке и смерти,Регистрация избирателей'
    },
    cat2: {
      name: '2. Здравоохранение и социальные услуги',
      children:
        'Доступ к общественному здравоохранению,Программы вакцинации,Медицинская страховка,Пособия по инвалидности,Социальная помощь и продовольственная поддержка'
    },
    cat3: {
      name: '3. Образование и обучение',
      children:
        'Государственные школы и университеты,Стипендии и студенческие кредиты,Программы обучения для взрослых,Онлайн-образовательные ресурсы'
    },
    cat4: {
      name: '4. Трудоустройство и трудовые услуги',
      children:
        'Поиск работы и пособия по безработице,Защита работников и трудовые права,Правила безопасности на рабочем месте,Обучение навыкам и стажировки'
    },
    cat5: {
      name: '5. Налоги и доходы',
      children:
        'Подача налоговых деклараций и возврат налогов,Оплата налогов на имущество,Соблюдение налоговых требований для бизнеса,Таможенные пошлины и импорт'
    },
    cat6: {
      name: '6. Общественная безопасность и правосудие',
      children:
        'Полиция и службы экстренной помощи,Суды и юридическая помощь,Сообщения о преступлениях,Законы о защите прав потребителей'
    },
    cat7: {
      name: '7. Транспорт и мобильность',
      children:
        'Водительские права и регистрация транспортных средств,Общественный транспорт и инфраструктура,Нарушения ПДД и штрафы,Программы безопасности дорожного движения'
    },
    cat8: {
      name: '8. Жильё и городское развитие',
      children:
        'Государственная жилищная помощь,Регистрация недвижимости и земельные записи,Жилищные кредиты и субсидии,Зонирование и разрешения на строительство'
    },
    cat9: {
      name: '9. Коммунальные услуги и окружающая среда',
      children:
        'Водоснабжение и электричество,Управление отходами и переработка,Экологические нормы,Инициативы по возобновляемой энергии'
    },
    cat10: {
      name: '10. Бизнес и торговля',
      children:
        'Регистрация и лицензирование бизнеса,Торговые правила и разрешения,Гранты и стимулы для малого бизнеса,Поддержка электронной коммерции и цифрового бизнеса'
    },
    cat11: {
      name: '11. Социальное обеспечение и пенсии',
      children: 'Пенсионные пособия,Управление пенсионными фондами,Пособия для выживших,Пенсии по инвалидности'
    },
    cat12: {
      name: '12. Сообщество и гражданское участие',
      children:
        'Голосование и выборы,Общественные отзывы и жалобы граждан,Волонтёрство и общественные программы,Участие в местном самоуправлении'
    }
  },
  settings: {
    title: 'Настройки',
    manageMyAccount: 'Управление аккаунтом',
    manageMyAccountDesc: 'Управляйте профилем, настройками безопасности и параметрами учетной записи.',
    save: 'Сохранить',
    close: 'Закрыть',
    saveSettings: 'Сохранить настройки',
    savingSettings: 'Сохранение ваших настроек...',
    settingsSaved: 'Настройки успешно сохранены!',
    saveSuccess: 'Настройки успешно сохранены',
    saveError: 'Ошибка сохранения настроек',
    display: 'Отображение',
    displayLanguage: 'Язык отображения',
    languages: {
      english: 'Английский',
      french: 'Французский',
      swahili: 'Суахили'
    },
    theme: 'Тема',
    themes: {
      light: 'Светлая',
      dark: 'Тёмная',
      system: 'Системная'
    },
    fontSize: 'Размер шрифта',
    notifications: 'Уведомления',
    emailUpdates: 'Обновления по электронной почте',
    soundNotifications: 'Звуковые уведомления',
    accountManagement: 'Управление учётной записью',
    resetUserData: 'Сбросить данные пользователя',
    resetUserDataDesc: 'Это удалит все данные вашего профиля и историю чатов.',
    standardAccount: 'Стандартная учётная запись',
    cancel: 'Отмена',
    account: {
      title: 'Учётная запись',
      resetDescription: 'Это удалит все данные вашего профиля и историю чатов.',
      confirmReset: 'Вы уверены, что хотите сбросить все ваши данные? Это действие нельзя отменить.',
      resetComplete: 'Ваши данные были сброшены.'
    },
    language: {
      title: 'Язык',
      selectLabel: 'Язык отображения'
    },
    appearance: {
      title: 'Внешний вид',
      lightTheme: 'Светлая',
      darkTheme: 'Тёмная',
      systemTheme: 'Системная'
    },
    processing: 'Обработка...',
    userDataReset: 'Данные вашего профиля успешно сброшены.',
    failedToResetUserData: 'Не удалось сбросить данные вашего профиля. Пожалуйста, попробуйте снова позже.',
    unableToLoadUser: 'Не удалось загрузить информацию о пользователе. Пожалуйста, попробуйте снова.',
    retry: 'Повторить',
    user: 'Пользователь',
    loadingUserInfo: 'Загрузка информации о пользователе...',
    resetUserDataTitle: 'Сброс данных пользователя',
    confirmResetUserData:
      'Вы уверены, что хотите сбросить все данные вашего профиля? Это удалит всю информацию вашего профиля и историю чатов, но сохранит ваши учетные данные.',
    reset: 'Сбросить',
    deleteAccount: 'Удалить мой аккаунт',
    deleteAccountDesc: 'Безвозвратно удалить ваш аккаунт и все данные. Это действие нельзя отменить.',
    deleteAccountTitle: 'Удаление аккаунта',
    confirmDeleteAccount:
      'Вы уверены, что хотите удалить свой аккаунт? Это действие необратимо и не может быть отменено. Все ваши данные будут удалены.',
    delete: 'Удалить',
    accountDeleted: 'Ваш аккаунт был удалён.',
    failedToDeleteAccount: 'Не удалось удалить аккаунт. Пожалуйста, попробуйте позже.'
  },
  userProfile: {
    title: 'Профиль пользователя',
    privacyInfo:
      'Предоставляя больше информации, вы получите более точные и значимые ответы от чат-бота. Пожалуйста, ознакомьтесь с нашей',
    privacyPolicyLink: 'Политикой конфиденциальности',
    tabComingSoon: 'Эта вкладка находится в разработке и скоро будет доступна.',
    saveSuccess: 'Профиль успешно сохранён',
    loadError: 'Ошибка загрузки данных профиля',
    saveError: 'Ошибка сохранения профиля',
    confirmSaveTitle: 'Сохранить профиль',
    confirmSave: 'Вы уверены, что хотите сохранить эти изменения?',
    confirmCancel: 'У вас есть несохранённые изменения. Вы уверены, что хотите отменить?',
    confirmLeave: 'Вы уверены, что хотите уйти? Все несохранённые изменения будут потеряны.',
    confirmDiscardChanges: 'Отменить несохранённые изменения?',
    uploadPhoto: 'Загрузить фото',
    uploadFile: 'Загрузить файл',
    photoRequirements: 'Фотография должна быть чёткой, актуальной и показывать ваше лицо полностью',
    biometricRequirements: 'Принимаются только официальные файлы биометрических данных',
    documentUpload: 'Загрузка документа',
    validationTitle: 'Пожалуйста, исправьте следующие ошибки:',
    profileIcon: 'Иконка профиля',
    change: 'Изменить',
    chooseProfileIcon: 'Выбрать иконку профиля',
    presetIcons: 'Предустановленные иконки',
    upload: 'Загрузить',
    initials: 'Инициалы',
    clickToUpload: 'Нажмите для загрузки',
    useThisImage: 'Использовать это изображение',
    useInitials: 'Использовать инициалы',
    tabs: {
      tab1: 'Данные личной идентификации',
      tab2: 'Гражданская регистрация и документация',
      tab3: 'Информация об адресе и проживании',
      tab4: 'Документы удостоверения личности и путешествий',
      tab5: 'Медицинские и здравоохранительные записи',
      tab6: 'Данные о занятости и экономике',
      tab7: 'Образовательные и академические записи',
      tab8: 'Финансовые и налоговые данные',
      tab9: 'Социальное обеспечение и благосостояние',
      tab10: 'Криминальные и юридические записи',
      tab11: 'Транспорт и мобильность',
      tab12: 'Гражданское и политическое участие'
    },
    actions: {
      cancel: 'Отмена',
      save: 'Сохранить профиль',
      previous: 'Предыдущая',
      next: 'Следующая',
      saving: 'Сохранение...'
    },
    fields: {
      fullName: 'Полное имя (включая псевдонимы)',
      dob: 'Дата рождения',
      gender: 'Пол',
      nationality: 'Национальность',
      maritalStatus: 'Семейное положение',
      photograph: 'Фотография',
      biometric: 'Отпечатки пальцев / Биометрические данные',
      birthCert: 'Свидетельство о рождении',
      deathCert: 'Свидетельство о смерти',
      marriageDivorce: 'Записи о браке / разводе',
      adoption: 'Записи об усыновлении',
      citizenship: 'Документы о гражданстве / натурализации',
      immigration: 'История иммиграции и виз',
      currentAddress: 'Текущий адрес проживания',
      previousAddresses: 'Предыдущие адреса',
      postalCode: 'Почтовый индекс',
      country: 'Страна',
      residencyStatus: 'Статус проживания',
      homeOrRental: 'Собственность или детали аренды',
      utilityBills: 'Коммунальные счета, связанные с адресом',
      landRecords: 'Записи о земельной собственности',
      idCard: 'Номер национального удостоверения личности',
      passport: 'Детали паспорта',
      driversLicense: 'Водительские права',
      voterId: 'Идентификатор избирателя',
      ssn: 'Номер социального страхования / национального страхования',
      militaryRecords: 'Записи о военной службе',
      medicalHistory: 'Медицинская история и состояния здоровья',
      vaccinations: 'Записи о вакцинации',
      insuranceDetails: 'Детали медицинской страховки',
      disability: 'Статус инвалидности',
      bloodType: 'Группа крови',
      organDonor: 'Статус донора органов',
      prescriptions: 'Рецепты и полученные лечения',
      mentalHealth: 'История психического здоровья',
      eHistory: 'История занятости',
      currentEmployer: 'Детали текущего работодателя',
      workPermits: 'Разрешения на работу и трудовые контракты',
      certifications: 'Профессиональные сертификаты и лицензии',
      unemployment: 'Статус безработицы и полученные пособия',
      tin: 'Идентификационный номер налогоплательщика (TIN)',
      businessAffiliations: 'Собственность бизнеса и связи с компаниями',
      schools: 'Посещённые школы и университеты',
      education: 'Образование',
      degrees: 'Дипломы, степени и сертификаты',
      performance: 'Академическая успеваемость и результаты тестов',
      academicRecords: 'Академические записи',
      scholarships: 'Полученные стипендии и финансовая помощь',
      incomeTax: 'Записи о подоходном налоге',
      bankAccounts: 'Банковские и финансовые счета',
      propertyTax: 'Оплаты налога на имущество',
      businessTax: 'Налоговые декларации бизнеса',
      pensionContrib: 'Взносы и снятия с пенсии',
      loanAid: 'Записи о кредитах и государственной помощи',
      pensionStatus: 'Статус пенсии и взносы',
      childcare: 'Поддержка по уходу за детьми',
      foodAssistance: 'Продовольственная помощь / программы благосостояния',
      housingAssistance: 'Жилищная помощь',
      policeRecords: 'Полицейские записи (криминальная история, аресты, обвинения)',
      courtCases: 'История судебных дел',
      finesPenalties: 'Штрафы и наказания',
      paroleProbation: 'Статус условно-досрочного освобождения или испытательного срока',
      citizenshipRevocation: 'Отзыв гражданства (если применимо)',
      vehicleReg: 'Детали регистрации транспортного средства',
      trafficViolations: 'Нарушения ПДД и штрафы',
      licenseHistory: 'История водительских прав и подтверждений',
      publicTransportCard: 'Использование карты общественного транспорта',
      voterRegistration: 'Детали регистрации избирателей',
      electionHistory: 'История участия в выборах',
      partyMembership: 'Членство в политической партии',
      militaryStatus: 'Статус военной службы или призыва',
      publicServiceRoles: 'Роли в государственной службе'
    },
    placeholders: {
      fullName: 'Введите ваше полное законное имя',
      nationality: 'Введите вашу национальность',
      searchCountries: 'Поиск стран...',
      selectCountry: 'Выберите страну',
      searchDisciplines: 'Поиск дисциплин...',
      selectDiscipline: 'Выберите дисциплину',
      searchDegrees: 'Поиск степеней...',
      selectDegree: 'Выберите степень'
    },
    validation: {
      nameRequired: 'Полное имя обязательно',
      dobRequired: 'Дата рождения обязательна',
      dobFuture: 'Дата рождения не может быть в будущем',
      requiredField: '{field} обязательно',
      invalidFormat: 'Недействительный формат для {field}',
      futureDate: 'Дата не может быть в будущем',
      invalidSelection: 'Пожалуйста, выберите действительный вариант'
    },
    errors: {
      savingFailed: 'Не удалось сохранить профиль',
      loadingFailed: 'Не удалось загрузить данные профиля',
      invalidForm: 'Пожалуйста, заполните все обязательные поля',
      invalidFileType: 'Недействительный тип файла',
      fileTooLarge: 'Файл слишком большой',
      countryLoadError: 'Ошибка загрузки стран',
      countryUpdateError: 'Ошибка обновления страны'
    },
    notifications: {
      saveSuccess: 'Профиль успешно сохранён',
      saved: 'Frontmatter сохранен',
      saveFailed: 'Не удалось сохранить профиль',
      loadSuccess: 'Профиль успешно загружен',
      loadFailed: 'Не удалось загрузить профиль',
      fieldRequired: 'Это поле обязательно',
      uploadSuccess: 'Файл успешно загружен',
      uploadFailed: 'Не удалось загрузить файл',
      formChanged: 'У вас есть несохранённые изменения',
      sessionExpired: 'Ваша сессия истекла, пожалуйста, войдите снова',
      profileUpdated: 'Ваш профиль обновлён',
      profileIncomplete: 'Ваш профиль неполный'
    },
    instructions: {
      fullNameHelp: 'Введите ваше полное законное имя, как указано в официальных документах',
      dobHelp: 'Введите дату рождения в формате ММ/ДД/ГГГГ',
      uploadPhotoHelp: 'Загрузите недавнюю фотографию. Файл должен быть в формате JPG, PNG или GIF и менее 2 МБ',
      documentHelp: 'Принимаемые форматы: PDF, JPG, PNG (максимум 5 МБ)',
      requiredFields: 'Поля, отмеченные *, обязательны',
      selectFromList: 'Пожалуйста, выберите вариант из списка',
      nextTab: 'Перейти к следующему разделу',
      previousTab: 'Вернуться к предыдущему разделу',
      saveInstructions: 'Нажмите Сохранить, чтобы сохранить ваши данные',
      cancelInstructions: 'Нажмите Отмена, чтобы отменить изменения'
    },
    completionStatus: {
      notStarted: 'Не начато',
      inProgress: 'В процессе',
      complete: 'Завершено',
      percentComplete: '{percent}% завершено',
      tabsCompleted: '{completed} из {total} разделов завершено',
      tabComplete: 'Раздел завершён!'
    },
    gender: {
      male: 'Мужской',
      female: 'Женский',
      other: 'Другое',
      preferNot: 'Предпочитаю не указывать'
    },
    maritalStatus: {
      single: 'Холост/Не замужем',
      married: 'Женат/Замужем',
      divorced: 'Разведён/Разведена',
      widowed: 'Вдовец/Вдова',
      other: 'Другое'
    },
    residencyStatuses: {
      citizen: 'Гражданин',
      permanentResident: 'Постоянный резидент',
      temporaryResident: 'Временный резидент',
      other: 'Другое'
    },
    employmentStatuses: {
      employed: 'Занят',
      selfEmployed: 'Самозанятый',
      unemployed: 'Безработный',
      student: 'Студент',
      retired: 'Пенсионер',
      homemaker: 'Домохозяин/Домохозяйка',
      other: 'Другое'
    },
    educationLevels: {
      primary: 'Начальное образование',
      secondary: 'Среднее образование',
      highSchool: 'Средняя школа',
      vocational: 'Профессиональное обучение',
      associate: 'Ассоциативная степень',
      bachelor: 'Бакалавр',
      master: 'Магистр',
      doctoral: 'Докторская степень',
      professional: 'Профессиональная степень',
      other: 'Другое'
    },
    proficiencyLevels: {
      native: 'Носитель языка',
      fluent: 'Свободный',
      advanced: 'Продвинутый',
      intermediate: 'Средний',
      basic: 'Базовый'
    },
    bloodTypes: {
      aPositive: 'A (II) Rh+',
      aNegative: 'A (II) Rh-',
      bPositive: 'B (III) Rh+',
      bNegative: 'B (III) Rh-',
      abPositive: 'AB (IV) Rh+',
      abNegative: 'AB (IV) Rh-',
      oPositive: 'O (I) Rh+',
      oNegative: 'O (I) Rh-',
      unknown: 'Неизвестно'
    },
    languages: {
      english: 'Английский',
      french: 'Французский',
      swahili: 'Суахили',
      arabic: 'Арабский',
      spanish: 'Испанский',
      portuguese: 'Португальский',
      chinese: 'Китайский',
      japanese: 'Японский',
      german: 'Немецкий',
      russian: 'Русский',
      hindi: 'Хинди',
      urdu: 'Урду',
      other: 'Другое'
    },
    yesNo: {
      yes: 'Да',
      no: 'Нет'
    },
    select: 'Пожалуйста, выберите',
    existingFile: 'Существующий файл',
    noMatchingCountries: 'Не найдено соответствующих стран',
    noMatchingDisciplines: 'Не найдено соответствующих дисциплин',
    noMatchingDegrees: 'Не найдено соответствующих степеней',
    educationOptions:
      'Бухгалтерский учёт,Аэрокосмическая инженерия,Аграрные науки,Антропология,Архитектура,История искусств,Искусственный интеллект,Астрономия,Астрофизика,Биохимия,Биомедицинская инженерия,Биотехнология,Бизнес-администрирование,Химическая инженерия,Химия,Гражданская инженерия,Коммуникации,Компьютерная инженерия,Компьютерные науки,Управление строительством,Уголовное правосудие,Кибербезопасность,Наука о данных,Стоматология,Экономика,Образование,Электротехника,Начальное образование,Английская литература,Экологическая инженерия,Экологические науки,Дизайн моды,Киноведение,Финансы,Изобразительное искусство,Пищевая наука,Криминалистика,Игровой дизайн,География,Геология,Графический дизайн,Управление здравоохранением,История,Гостиничный менеджмент,Человеческие ресурсы,Промышленный дизайн,Промышленная инженерия,Информационные системы,Информационные технологии,Дизайн интерьеров,Международный бизнес,Международные отношения,Журналистика,Право,Библиотековедение,Лингвистика,Менеджмент,Маркетинг,Материаловедение,Математика,Механическая инженерия,Медиа-исследования,Медицина,Метеорология,Микробиология,Музыка,Нанотехнологии,Сестринское дело,Питание,Трудотерапия,Океанография,Нефтяная инженерия,Фармацевтика,Философия,Фотография,Физическое воспитание,Физиотерапия,Физика,Политология,Психология,Государственное управление,Общественное здравоохранение,Связи с общественностью,Робототехника,Среднее образование,Социальная работа,Социология,Программная инженерия,Специальное образование,Спортивный менеджмент,Статистика,Системная инженерия,Театральное искусство,Туризм,Градостроительство,Ветеринарная медицина,Веб-разработка,Биология дикой природы,Зоология',
    degreeOptions:
      'Ассоциативная степень,Бакалавр искусств (BA),Бакалавр наук (BS),Бакалавр инженерии (BEng),Бакалавр делового администрирования (BBA),Бакалавр изобразительных искусств (BFA),Бакалавр образования (BEd),Бакалавр медицины (MBBS),Бакалавр права (LLB),Магистр искусств (MA),Магистр наук (MS),Магистр делового администрирования (MBA),Магистр инженерии (MEng),Магистр изобразительных искусств (MFA),Магистр образования (MEd),Магистр права (LLM),Магистр общественного здравоохранения (MPH),Доктор философии (PhD),Доктор медицины (MD),Доктор образования (EdD),Доктор делового администрирования (DBA),Доктор юриспруденции (JD),Профессиональный диплом,Технический диплом,Профессиональный сертификат,Сертификат магистра,Диплом магистра,Постдокторантура',
    aria: {
      tabList: 'Разделы формы профиля',
      nextButton: 'Перейти к следующему разделу',
      prevButton: 'Вернуться к предыдущему разделу',
      closeButton: 'Закрыть форму профиля',
      saveButton: 'Сохранить данные профиля',
      requiredField: 'Обязательное поле',
      dropdownSelect: 'Выберите вариант'
    }
  },
  chatbot: {
    streamingError: 'Ответ был прерван. Повторите попытку.',
    aiGeneratedNoDocs: 'Сгенерировано ИИ — не основано на документах библиотеки',
    welcomeMessage: 'Добро пожаловать! Чем я могу помочь вам сегодня?',
    configMismatchWarning:
      'Несоответствие конфигурации: {warnings}. Пожалуйста, проверьте Быструю помощь и настройки иерархии знаний.',
    categoryNotFound: 'Категория "{label}" не найдена в иерархии знаний',
    serviceLabelMismatch:
      'Сервис "{label}" использует метку интерфейса, которая может не соответствовать иерархии знаний',
    noFilterWarning: 'Нет активного контекстного фильтра — запрос будет выполнен без фильтрации.',
    placeholder: 'Введите ваш запрос здесь...',
    sendButton: 'Отправить',
    fileReceived: 'Файл успешно получен.',
    fileUploadError: 'Ошибка загрузки файла.',
    processingError: 'Ошибка обработки вашего запроса.',
    attachFile: 'Прикрепить файл',
    fileTooLarge: 'Файл слишком большой. Максимальный размер {maxSize}.',
    saveChat: 'Сохранить чат',
    chatSaved: 'Чат сохранён',
    chatTitle: 'Название чата',
    chatTitlePlaceholder: 'Введите название для этого чата',
    selectFolder: 'Выберите папку',
    newChat: 'Новый чат',
    clearContext: 'Очистить контекст и начать новый разговор',
    unsavedChanges: 'У вас есть несохранённые изменения. Вы уверены, что хотите начать новый чат?',
    whatCanIHelp: 'Чем я могу помочь вам сегодня?',
    queryContext: 'Контекст запроса',
    contextAdded: 'Контекст добавлен к вашему запросу',
    contextRemoved: 'Контекст удалён из вашего запроса',
    sessionUpdated: 'Сессия обновлена',
    newChatStarted: 'Начата новая беседа',
    loadError: 'Не удалось загрузить историю чата',
    responsePrefix: 'Я получил ваше сообщение',
    withContext: 'с контекстом',
    conversationLoaded: 'Беседа успешно загружена!',
    saveConfirmTitle: 'Сохранить существующую беседу',
    saveConfirmMessage: 'Сохранить существующую беседу?',
    loadConfirmTitle: 'Загрузить существующую беседу',
    loadConfirmMessage:
      'У вас есть несохранённые изменения. Хотите ли вы отменить их и загрузить выбранную беседу или сначала сохранить текущую беседу?',
    loadAndDiscard: 'Загрузить и отменить',
    saveAndLoad: 'Сохранить и загрузить',
    saveAndStartNew: 'Сохранить и начать новый',
    discardAndStartNew: 'Отменить и начать новый',
    newChatTitle: 'Начать новый чат',
    exportChat: 'Экспортировать чат',
    exportFilename: 'Имя файла',
    exportFilenamePlaceholder: 'Введите имя файла',
    exportButton: 'Экспортировать',
    exportSuccess: 'Чат успешно экспортирован',
    exportError: 'Не удалось экспортировать чат. Пожалуйста, попробуйте снова.',
    removeItem: 'Удалить'
  },
  quickhelp: {
    applyForIDPrompt:
      "Выступайте в роли компетентного эксперта по регистрации актов гражданского состояния Кении. Объясните шаги для получения национального удостоверения личности (Maisha Namba) или замены утерянного. ВАЖНО: Предоставьте четкий список необходимых документов (например, свидетельство о рождении, копии удостоверений личности родителей) и порекомендуйте пользователю обратиться в ближайший Huduma Centre или офис Registrar of Persons. ПРАВИЛО: Всегда называйте приложение 'Genie AI'.",
    payTaxesPrompt:
      "Выступайте в роли консультанта по вопросам KRA (Налоговое управление Кении). Объясните процесс подачи налоговых деклараций, оформления KRA PIN или сброса пароля на портале iTax. ВАЖНО: Напомните пользователю о крайнем сроке подачи годовых деклараций 30 июня и расскажите, как подать нулевую декларацию (Nil return) при отсутствии дохода. ПРАВИЛО: Всегда называйте приложение 'Genie AI'.",
    startBusinessPrompt:
      "Выступайте в роли бизнес-консультанта по услугам портала eCitizen. Помогите пользователю пройти процедуру резервирования наименования компании и регистрации предприятия в Кении. ВАЖНО: Разъясните действующие тарифы на проверку названия и регистрацию, а затем направьте пользователя на официальный портал eCitizen для оформления заявки. ПРАВИЛО: Всегда называйте приложение 'Genie AI'.",
    findHealthcarePrompt:
      "Выступайте в роли консультанта по медицинским услугам. Предоставьте информацию о переходе от NHIF к SHIF (Социальный фонд медицинского страхования) и порядке регистрации. ВАЖНО: Укажите официальные коды USSD (например, *263#) или ссылки на веб-сайты для регистрации и разъясните преимущества системы государственного медицинского страхования. ПРАВИЛО: Всегда называйте приложение 'Genie AI'.",
    educationServicesPrompt:
      "Выступайте в роли консультанта по вопросам образования. Расскажите об учебной программе CBC, регистрации в NEMIS или распределении в университеты через KUCCPS. ВАЖНО: Объясните, как родители могут проверить результаты национальных экзаменов по SMS или через портал KNEC после их публикации. ПРАВИЛО: Всегда называйте приложение 'Genie AI'.",
    transportLicensesPrompt:
      "Выступайте в роли консультанта по услугам NTSA. Объясните процедуру продления водительских прав, прохождения техосмотра транспортных средств или управления учетной записью TIMS. ВАЖНО: Расскажите, как войти на портал eCitizen NTSA для подачи заявления на смарт-удостоверение (Smart DL) или записи на техосмотр. ПРАВИЛО: Всегда называйте приложение 'Genie AI'.",
    housingProgramsPrompt:
      "Выступайте в роли консультанта по жилищным программам. Разъясните процесс регистрации в программе доступного жилья (Boma Yangu) и порядок внесения добровольных взносов. ВАЖНО: Направьте пользователя на портал Boma Yangu для просмотра проектов и объясните критерии отбора для распределения жилья. ПРАВИЛО: Всегда называйте приложение 'Genie AI'.",
    findJobsPrompt:
      "Выступайте в роли карьерного консультанта по государственной службе. Помогите пользователю создать профиль и подать заявку на вакансии через портал Public Service Commission (PSC). ВАЖНО: Посоветуйте подготовить документы об образовании и регулярно проверять сайт PSC или местные печатные издания на предмет объявлений MyGov. ПРАВИЛО: Всегда называйте приложение 'Genie AI'.",
    justChatPrompt:
      "Выступайте в роли дружелюбного местного собеседника. Будьте вежливы, отзывчивы и демонстрируйте глубокие знания кенийской культуры и повседневной жизни. ВАЖНО: Напомните пользователю, что, хотя вы можете побеседовать на любую тему, ваша главная задача — помочь сориентироваться в государственных услугах Кении, таких как **удостоверения личности**, **налоги** и **регистрация бизнеса**. ПРАВИЛО: Всегда называйте приложение 'Genie AI'."
  },
  common: {
    cancel: 'Отмена',
    create: 'Создать',
    save: 'Сохранить',
    move: 'Переместить',
    delete: 'Удалить',
    ok: 'ОК',
    ingest: 'Обработать',
    retract: 'Изъять',
    loading: 'Загрузка...',
    refresh: 'Обновить'
  },
  feedback: {
    title: 'Отзыв',
    button: 'Отзыв',
    positive: 'Положительный отзыв',
    negative: 'Отрицательный отзыв',
    promptText: 'Был ли этот ответ полезен?',
    placeholder: 'Дополнительные комментарии...',
    submit: 'Отправить отзыв',
    close: 'Отмена',
    thankYouMessage: 'Спасибо за ваш отзыв!',
    submitting: 'Отправка...',
    error: 'Что-то пошло не так. Пожалуйста, попробуйте снова.',
    success: 'Спасибо за ваш отзыв!'
  },
  responseRating: {
    title: 'Помогите нам улучшиться',
    note: 'Ваш отзыв будет использован для лучшей настройки чат-бота и улучшения ответов со временем.',
    chatbotResponse: 'Ответ чат-бота:',
    ratingLabels: {
      1: 'Бесполезно',
      2: 'Слегка полезно',
      3: 'Умеренно полезно',
      4: 'Очень полезно',
      5: 'Изменяет жизнь'
    },
    additionalComments: 'Дополнительные комментарии...',
    submit: 'Отправить',
    cancel: 'Отмена'
  },
  upload: {
    alt: 'Загрузить'
  },
  details: {
    loading: 'Загрузка сведений о файле...',
    title: 'Сведения о файле',
    close: 'Закрыть',
    fileName: 'Имя файла',
    author: 'Автор',
    labels: 'Метки',
    selectAll: 'Выбрать все',
    loadingLabels: 'Загрузка меток...',
    status: 'Статус',
    fileId: 'ID файла',
    fileType: 'Тип файла',
    viewFile: 'Просмотреть файл',
    visitLink: 'Перейти по внешней ссылке',
    openFile: 'Открыть файл в новой вкладке',
    fileSize: 'Размер файла',
    uploadDate: 'Дата загрузки',
    hash: 'Хеш SHA256',
    tabs: {
      details: 'Детали',
      ingestionLog: 'Журнал обработки',
      detailsHint:
        'Метаданные этого файла: имя, автор и метки иерархии знаний, используемые ИИ для маршрутизации вопросов. Для выполнения ingest требуется хотя бы одна метка.',
      dashboardHint:
        'Мониторинг обхода сайта для этого файла в реальном времени: скорость, очередь, ошибки и прогресс. После успешного обхода нижний колонтитул преобразует его в репозиторий OKF.',
      crawlLogHint:
        'Постраничный журнал обхода сайта: какие URL были получены, перенаправлены или завершились ошибкой. Записывается во время выполнения обхода.',
      ingestionLogHint:
        'Поэтапный прогресс подготовки файла для ответов ИИ: разбиение на фрагменты (chunking), разметка, создание эмбеддингов и индексация. Ошибки здесь точно указывают, на чем остановился процесс ingest.'
    },
    log: {
      killActions: 'Действия по остановке:',
      killDocument: 'Остановить этот документ',
      killProcess: 'Остановить процесс обработки',
      timestamp: 'Временная метка',
      level: 'Уровень',
      stage: 'Этап',
      message: 'Сообщение',
      loadingLogs: 'Загрузка журналов...',
      noLogs: 'Журналы не найдены.'
    },
    buttons: {
      saveMetadata: 'Сохранить метаданные',
      retract: 'Изъять',
      ingest: 'Обработать'
    },
    notifications: {
      loadError: 'Не удалось загрузить сведения о файле.',
      validationError: 'Имя файла и автор обязательны для заполнения.',
      saveSuccess: 'Метаданные успешно обновлены.',
      saveError: 'Не удалось сохранить метаданные.',
      tokenError: 'Токен аутентификации не найден в данных пользователя.',
      viewError: 'Не удалось загрузить файл для просмотра.',
      ingestSuccess: 'Файл успешно добавлен в очередь на обработку.',
      ingestError: 'Не удалось запустить процесс обработки.',
      retractSuccess: 'Файл успешно изъят.',
      retractError: 'Не удалось изъять файл.',
      deleteSuccess: 'Файл успешно удален.',
      deleteError: 'Не удалось удалить файл.',
      ingestLabelRequired: 'Пожалуйста, выберите хотя бы одну метку перед обработкой.',
      ingestSaving: 'Сохранение метаданных перед обработкой...',
      ingestSaveFailed: 'Не удалось сохранить метаданные. Обработка отменена.',
      logError: 'Не удалось получить журналы обработки.',
      killDocSent: 'Запрос на остановку этого документа отправлен.',
      killProcSent: 'Запрос на остановку процесса обработки отправлен.'
    },
    confirm: {
      ingest: 'Вы уверены, что хотите обработать этот файл? Это запустит конвейер обработки данных.',
      retract: 'Вы уверены, что хотите изъять этот файл?',
      delete: 'Вы уверены, что хотите безвозвратно удалить этот файл? Это действие нельзя отменить.',
      ingestTitle: 'Подтвердить обработку',
      retractTitle: 'Подтвердить изъятие',
      deleteTitle: 'Подтвердить удаление',
      killDocTitle: 'Остановить обработку документа',
      killDoc:
        'Вы уверены, что хотите остановить задачу обработки для ЭТОГО документа? Процесс попытается выполнить безопасный откат.',
      killProcTitle: 'Остановить ВЕСЬ процесс обработки',
      killProc:
        'ВНИМАНИЕ: Это остановит всю службу обработки в бэкенде, что затронет ВСЕ файлы, находящиеся в данный момент в обработке. Вы абсолютно уверены?'
    }
  },
  uploadDialog: {
    title: 'Загрузить файлы',
    dropzone: 'Перетащите файлы сюда или нажмите для выбора',
    allowedTypesLabel: 'Разрешенные типы:',
    buttonUpload: 'Загрузить {count} файл(ов)',
    remove: 'Удалить',
    uploading: 'Загрузка...',
    notifications: {
      dropError: 'Можно перетаскивать только файлы. Убедитесь, что вы перетаскиваете действительный файл с компьютера.',
      typeNotAllowed: 'Тип файла "{extension}" не разрешен.',
      shortcutUnsupported: 'Файлы ярлыков (.url) не поддерживаются. Пожалуйста, перетащите сам файл.',
      duplicate: 'Файл "{fileName}" уже добавлен.',
      uploadSuccess: 'Файл {fileName} успешно загружен',
      uploadFailed: 'Не удалось загрузить файл {fileName}.'
    }
  },
  okf: {
    versions: {
      bundle: 'Пакет',
      col: {
        concepts: 'Концепции',
        curator: 'Куратор',
        minted: 'Создано',
        tag: 'Тег',
        trigger: 'Триггер',
        version: 'Версия'
      },
      current: 'текущая',
      loadFailed: 'Не удалось загрузить версии',
      none: 'Версии пока не созданы — публикация создаст v1.',
      notServing: 'Не обслуживается',
      publish: 'Создать новую версию',
      published: "Версия v{'{'}v{'}'} опубликована — пакет {'{'}f{'}'} сохранен в репозитории документов.",
      serving: 'Поглощено (обслуживается)',
      title: 'Версии'
    },
    shell: {
      back: 'Панель управления Studio',
      building: 'Сборка…',
      buildingHint: 'Исходный файл все еще обрабатывается',
      delete: {
        body: 'Это действие безвозвратно удалит репозиторий, его концепты, проиндексированный контент, граф и артефакты пакета.',
        confirm: 'Удалить',
        title: 'Удалить репозиторий'
      },
      export: 'Экспорт .zip',
      logs: 'Журналы',
      pii: {
        ack: 'Подтвердить отмеченные сущности и опубликовать',
        note: 'Отмеченные сущности входят в состав публикуемого контента. Если вы проверили их (например, официальные контакты), подтвердите и продолжайте.'
      },
      publish: {
        body: "При выполнении publish создается версия v{'{'}n{'}'}, а пакет \"{'{'}file{'}'}\" сохраняется в репозитории документов, заменяя любой предыдущий zip. Новая версия не перейдет в serving, пока вы не выполните Ingest.",
        confirm: 'Опубликовать',
        inProgress: 'Публикация — экспорт и передача пакета…',
        title: 'Опубликовать'
      },
      readonly: 'ТОЛЬКО ДЛЯ ЧТЕНИЯ',
      serving: 'Обслуживание',
      state: {
        ingested: 'Поглощено'
      },
      tab: {
        editor: 'Редактор',
        wizard: 'Мастер'
      },
      version: "v{'{'}n{'}'}",
      versions: 'Версии',
      deleteLabel: 'Удалить'
    },
    logs: {
      labeled: "с метками: {'{'}n{'}'}",
      fallbacks: "резервных вызовов LLM: {'{'}n{'}'}",
      concepts: "концептов: {'{'}n{'}'}",
      col: {
        action: 'Действие',
        description: 'Описание',
        user: 'Пользователь',
        when: 'Дата и время'
      },
      count: "{'{'}n{'}'} записей",
      loadFailed: 'Не удалось загрузить журнал активности.',
      none: 'Активность пока не зафиксирована — действия с репозиторием появятся здесь по мере их выполнения.',
      title: 'Журнал активности'
    },
    graph: {
      aria: 'Граф концептов',
      building: 'Подготовка графа…',
      concepts: 'концепции',
      empty: 'Концептов пока нет — нечего отображать на графе.',
      fit: 'Вписать граф',
      hub: 'Показать или скрыть индексный узел (его ссылки на оглавление представляют структуру, а не знания)',
      layouting: 'Компоновка…',
      legend: 'индекс',
      links: 'ссылки',
      zoomIn: 'Увеличить',
      zoomOut: 'Уменьшить',
      card: {
        links: "{'{'}n{'}'} связей",
        chunks: "{'{'}n{'}'} фрагм.",
        failed: 'ошибка индексации',
        flagged: 'отмеченные сущности',
        hub: 'Индексный узел',
        pending: 'еще не проиндексировано'
      }
    },
    import: {
      title: 'Импорт документов в новый OKF-репозиторий',
      hint: 'Документы становятся ОДНИМ репозиторием — сегментируются, связываются и размечаются вместе. Ничего не публикуется; далее обычный поток: проверка → публикация → загрузка.',
      name: 'Имя репозитория',
      namePh: 'напр. Пакет политик',
      domain: 'Предметная область (домен)',
      strategy: 'Стратегия классификации',
      stratHeuristics: 'Эвристика (быстро, без LLM)',
      stratLlm: 'Классификация LLM',
      stratHybrid: 'Гибридная',
      files: 'Выбранные документы',
      servingBadge: 'обслуживает свободный RAG',
      alreadyBadge: 'уже в другом OKF-репозитории',
      servingTip:
        'Этот документ сейчас обслуживает свободный корпус — новый репозиторий нельзя загрузить, пока он не отозван.',
      alreadyTip: 'Этот документ уже является источником другого OKF-репозитория.',
      servingWarn:
        '{n} документ(ов) всё ещё обслуживают свободный корпус. Импорт выполнится, но этот репозиторий нельзя загрузить, пока они не отозваны.',
      cancel: 'Отмена',
      importing: 'Импорт…',
      go: 'Импортировать',
      dupRepo: 'Такое имя репозитория уже существует — выберите другое.',
      failed: 'Не удалось начать импорт. Проверьте документы и попробуйте снова.'
    },
    editor: {
      addConcept: {
        bodyLabel: 'Markdown (вставьте или оставьте пустым)',
        bodyPh: '# Заголовок\n\nВставьте markdown сюда — frontmatter будет добавлен автоматически.',
        create: 'Создать файл',
        failed: 'Не удалось создать файл.',
        title: 'Добавить файл концепта',
        titleLabel: 'Заголовок',
        titlePh: 'например, Дикая природа Мары',
        updateIndex: 'Добавить в оглавление индекса'
      },
      autocorrect: {
        blankBefore: '(пусто)',
        blankBadge: 'нет frontmatter — полное предложение',
        applyField: 'Применить',
        apply: 'Применить исправления',
        body: 'Запланированные исправления frontmatter для всех концептов. Основной текст файлов никогда не изменяется.',
        button: 'Автокоррекция',
        clean: 'Исправления не требуются — все данные frontmatter соответствуют стандарту.',
        col: {
          field: 'Поле',
          after: 'После',
          before: 'До',
          concept: 'Концепция'
        },
        failed: 'Ошибка сканирования.',
        scanning: 'Сканирование…',
        title: 'Автоисправление (только frontmatter)'
      },
      concepts: {
        originTip: 'Как курировался этот концепт',
        add: 'Добавить концепцию',
        addLabel: 'Установить метку',
        delete: 'Удалить файл',
        empty: 'Файлов пока нет — добавьте концепт или выполните повторное разделение из источника.',
        filter: 'Фильтр файлов',
        failedCard: {
          title: 'Ошибка загрузки',
          problem: 'Проблема',
          noError: 'Помечен как ошибочный без указания причины.',
          fixLabel: 'Как исправить',
          attempts: "Attempts: {'{'}n{'}'}",
          when: "Last attempt {'{'}when{'}'}",
          fix: {
            reaper:
              'Рабочий процесс загрузки прекратил ожидание в пределах льготного окна (слив был перегружен) — содержимое не повреждено. Исправление: внесите небольшое изменение в файл и сохраните — он встанет в очередь заново; либо отзовите и заново загрузите весь репозиторий.',
            dataprep:
              'Служба подготовки контента не смогла обработать эту загрузку. Исправление: измените и сохраните для повторной постановки в очередь; при повторении проверьте состояние службы dataprep.',
            generic:
              'Индексация не удалась. Исправление: измените содержимое и сохраните для повторной постановки в очередь, либо отзовите и заново загрузите весь репозиторий.'
          }
        },
        indexBadge: 'индекс',
        label: 'Концепции',
        loading: 'Загрузка…',
        resplit: 'Разделить повторно',
        flagged: 'отмечено',
        flaggedTip:
          'Концепты с отмеченными сущностями — откройте каждый, удалите или измените сущность, сохраните (повторное сканирование выполняется автоматически); либо подтвердите их при publish.',
        piiBadge: 'PII',
        piiTip:
          'Отмеченные сущности: {k}. Откройте, удалите или измените их, затем сохраните — повторное сканирование запустится автоматически.',
        piiTipBare: 'Отмеченные сущности — откройте, просмотрите и сохраните для повторного сканирования.',
        sourceLabel: 'Исходный документ',
        sourceView: 'Открыть исходный документ',
        bulkAccept: 'Принять все',
        bulkRedact: 'Скрыть все',
        bulkRemove: 'Удалить всё',
        loadProgress: 'Загрузка файлов: {done}/{total}'
      },
      delete: {
        body: 'Это действие навсегда удалит файл, его проиндексированные фрагменты и связи в графе.',
        confirm: 'Удалить',
        title: 'Удалить файл'
      },
      label: 'Редактор репозитория',
      loadFailed: 'Не удалось загрузить этот концепт.',
      loadingConcept: 'Загрузка концепта…',
      meta: {
        label: 'Метаданные концепта',
        labelLabel: 'Метка (Иерархия знаний)',
        noLabel: 'Нет метки',
        none: 'Концепт не выбран',
        saveFailed: 'Не удалось сохранить метаданные',
        saved: 'Метаданные сохранены',
        status: 'Статус индекса',
        title: 'Заголовок',
        trust: 'Уровень доверия',
        type: 'Тип',
        typePlaceholder: 'Выберите тип…'
      },
      pane: {
        files: 'Файлы',
        graph: 'Граф'
      },
      pickConcept: 'Выберите концепт из списка, чтобы начать редактирование.',
      resplit: {
        body: 'Это действие удалит все текущие концепты и производный граф, а затем повторно импортирует исходный файл с новым разделением. Все правки концептов будут утрачены.',
        confirm: 'Разделить повторно',
        failed: 'Повторное разделение не удалось — проверьте ссылку на исходный файл.',
        modeA: 'Один концепт на весь обход',
        modeAHint: 'Мега-концепт — весь контент объединяется в один концепт',
        modeB: 'Один концепт на страницу',
        modeBHint: 'Разделение по маркерам краулера `## Source:` (рекомендуется)',
        modeC: 'Использовать извлечение тем с помощью LLM',
        modeCHint: 'История 10.6 — скоро появится',
        title: 'Разделить заново из источника'
      },
      save: 'Сохранить',
      saved: 'Сохранено',
      saving: 'Сохранение…',
      unsaved: 'Несохраненные изменения',
      view: {
        rendered: 'Отрисовано',
        source: 'Источник'
      },
      viewToggle: 'Вид редактора',
      paneLabel: 'Панель просмотра',
      saveFailed: 'Ошибка сохранения — повторите попытку',
      piiBulk: {
        title: {
          redact: 'Скрыть весь отмеченный контент',
          remove: 'Удалить весь отмеченный контент',
          accept: 'Принять все отмеченные сущности'
        },
        body: {
          redact:
            'Основной текст каждого отмеченного концепта заменяется уведомлением об изъятии данных. Отменить действие невозможно.',
          remove: 'Основной текст каждого отмеченного концепта очищается. Отменить действие невозможно.',
          accept:
            'Все отмеченные сущности помечены как проверенные и сохраненные — они не будут отмечены снова до повторного сканирования.'
        },
        scope: "Затронуто концептов: {'{'}n{'}'}.",
        confirm: 'Применить',
        failed: 'Массовое действие над PII завершилось ошибкой — повторите попытку.'
      },
      actions: {
        bulkPii: 'Применение массового действия к PII',
        deleting: 'Удаление'
      }
    },
    dashboard: {
      bulk: {
        body: 'Каждый репозиторий проходит полную входную проверку (проверка PII, индексация, соответствие) при публикации. Результаты по репозиториям:',
        pending: 'Подтвердите, чтобы выполнить publish для выбранных репозиториев.',
        publishConfirm: "Опубликовать {'{'}n{'}'}",
        title: 'Опубликовать выбранные репозитории'
      },
      card: {
        actions: "Действия для {'{'}name{'}'}",
        building: 'Сборка…',
        buildingAria: 'Сборка — исходный файл все еще обрабатывается',
        delete: 'Удалить',
        export: 'Экспорт',
        ingesting: 'Поглощение…',
        logs: 'Журналы',
        rename: 'Переименовать',
        versions: 'Версии'
      },
      delete: {
        body: 'Это действие безвозвратно удалит репозиторий, его концепты, проиндексированный контент, граф и артефакты пакета. Отменить операцию невозможно.',
        confirm: 'Удалить',
        title: 'Удалить репозиторий'
      },
      filter: {
        domain: 'Фильтр по предметной области',
        allDomains: 'Все предметные области'
      },
      new: '+ Новый репозиторий',
      pii: {
        ack: 'Подтвердить отмеченные сущности и опубликовать',
        note: 'Отмеченные сущности являются частью публикуемого контента. Если вы проверили их (например, официальные контактные данные), подтвердите и продолжайте.'
      },
      publish: {
        body: "При выполнении publish создается версия v{'{'}n{'}'}, а пакет \"{'{'}file{'}'}\" сохраняется в репозитории документов, заменяя предыдущий zip. Новая версия не перейдет в serving, пока вы не выполните Ingest.",
        confirm: 'Опубликовать',
        title: 'Опубликовать'
      },
      search: 'Поиск...',
      select: "Выбрать {'{'}name{'}'} для массовой публикации",
      stage: {
        drainFailed: 'Ошибка ingest',
        redraining: 'Повторная очистка…',
        building: 'Сборка…',
        draft: 'Черновик',
        inReview: 'На проверке',
        ingested: "Выполнен ingest для v{'{'}n{'}'}",
        published: "Опубликована v{'{'}n{'}'}",
        retracted: 'Отозвано',
        stepOf: 'Step '
      },
      title: 'Репозитории',
      topics: 'темы',
      lane: {
        draft: 'Импорт',
        in_review: 'На проверке',
        published: 'Опубликовано',
        ingested: 'Поглощено',
        retracted: 'Отозвано'
      },
      laneEmpty: {
        draft: 'Пока ничего не импортировано',
        in_review: 'Нет объектов на проверке',
        published: 'Пока нет опубликованных репозиториев',
        ingested: 'Пока ничего не поглощено',
        retracted: 'Ничего не отозвано'
      }
    },
    build: {
      drainFailed: {
        more: "+ еще {'{'}n{'}'}",
        fixPath: 'Повторите ingest для новой попытки: retract → создать версию → ingest.'
      },
      curation: {
        fallbacks: 'резервных вызовов LLM',
        curated: 'Курируется',
        typed: 'Типизировано',
        labeled: 'Размечено',
        described: 'Описано'
      },
      batches: 'Пакетов сохранено',
      bytes: 'Прочитано из источника',
      conceptsIndexed: 'концептов проиндексировано',
      elapsed: {
        hr: "Запущено {'{'}n{'}'} ч назад",
        lt1: 'Запущено меньше минуты назад',
        min: "Запущено {'{'}n{'}'} мин назад"
      },
      hint: {
        import: 'Репозиторий остается в статусе Import до завершения конвертации файла.',
        rag: 'Индекс RAG строится — версия переходит в статус serving после индексации каждого концепта.',
        serving: 'Эта версия находится в статусе serving для трафика RAG — выполните retract для внесения изменений.',
        review: 'На этапе review — рецензент утверждает, затем куратор выполняет publish и ingest.',
        retracted: 'Выведено из эксплуатации — submit → review → approve → publish → ingest для возврата к serving.',
        idle: 'Импорт завершен — выполните submit на review для продолжения рабочего процесса.'
      },
      pages: 'Страниц обработано',
      serving: {
        graph: 'Граф serving',
        version: 'Версия serving'
      },
      stage: {
        indexing: 'Индексирование',
        serving: 'Обслуживание',
        label: 'Этап'
      },
      title: {
        import: 'Прогресс импорта',
        rag: 'Прогресс ingest',
        serving: 'Статус serving',
        review: 'Статус review',
        retracted: 'Статус retract',
        idle: 'Статус репозитория'
      },
      lifecycle: {
        importing: 'Импорт',
        reviewing: 'На проверке',
        ingesting: 'Поглощение',
        serving: 'Обслуживание',
        retracted: 'Отозвано'
      },
      row: {
        subject: 'Предметная область',
        topics: 'Темы',
        lastVersion: 'Последняя версия'
      }
    },
    create: {
      zipOnly: 'Выберите файл пакета .zip.',
      stagedFile: "Подготовлено: {'{'}name{'}'}",
      staged: 'Пакет подготовлен — нажмите «Создать репозиторий» для начала импорта.',
      openExisting: 'Открыть существующий репозиторий',
      name: 'Имя репозитория',
      importing: 'Импорт пакета…',
      importHint: 'Или импортируйте существующий zip-пакет как новый репозиторий.',
      importDefaultName: 'Импортированный репозиторий',
      failed: 'Не удалось создать репозиторий',
      duplicateInline: 'Репозиторий с таким именем уже существует. Откройте его или выберите другое имя.',
      duplicate:
        'Репозиторий с таким именем уже существует — откройте его из панели управления или выберите другое имя.',
      domain: 'Предметная область',
      create: 'Создать репозиторий',
      title: 'Новый репозиторий OKF',
      hint: 'Создает пустой репозиторий с файлом index.md, который редактируется в редакторе Studio.',
      domainPlaceholder: 'Выберите предметную область…',
      domainRequired: 'Сначала выберите предметную область — ее нельзя изменить после создания.',
      classLabel: 'Классификация концептов',
      classHeuristics: 'Эвристика (по умолчанию)',
      classLlm: 'С поддержкой LLM',
      classHybrid: 'Гибридный'
    },
    fm: {
      errKeyCurated: 'Зарезервированное имя поля.',
      errKeyDuplicate: 'Имя поля используется дважды.',
      errJson: 'Неверный JSON',
      errNumber: 'Значение должно быть числом',
      kind: {
        json: 'JSON',
        array: 'Список',
        boolean: 'Истина/Ложь',
        number: 'Число',
        string: 'Текст'
      },
      removeKey: 'Удалить поле',
      kindLabel: 'Тип значения',
      keyPh: 'имя поля',
      fullHint: 'Каждое поле frontmatter доступно для редактирования. Добавляйте и удаляйте ключи без ограничений.',
      errRemovalPending: 'Удаление полей требует серверного контракта слияния и удаления (скоро появится).',
      errKeyRequired: 'Каждому полю требуется имя.',
      arrayPh: 'значения через запятую',
      addKey: 'Добавить поле',
      saved: 'Frontmatter сохранен',
      label: 'Метаданные (frontmatter)',
      tipLabel: 'Что такое Frontmatter?',
      edit: 'Изменить',
      empty: 'Frontmatter пока отсутствует — редактирование добавит тип, заголовок и метки.',
      type: 'Тип',
      noType: 'Нет типа',
      titleLabel: 'Заголовок',
      labelsLabel: 'Метка (Иерархия знаний)',
      noLabel: 'Нет метки',
      descriptionLabel: 'Описание',
      save: 'Сохранить frontmatter',
      saveFailed: 'Не удалось сохранить frontmatter'
    },
    glossary: {
      concept:
        'Элемент вашего репозитория — обычно отдельная страница или тема. Каждый концепт содержит структурированный frontmatter, считываемый ассистентом, и текст markdown, на основе которого формируются ответы.',
      repository:
        'Коллекция курируемых концептов в рамках одной предметной области. Она преобразуется в пакет OKF, который вы публикуете, версионируете и делаете доступным для ассистента.',
      subjectArea:
        'К какой области относятся эти знания? Предметная область группирует ваш репозиторий и определяет доступные метки. Ее нельзя изменить после создания.',
      selectSubjectArea: 'Выберите предметную область…',
      subjectAreaMissing:
        'Предметная область этого репозитория отсутствует в иерархии знаний — отображаются все метки.',
      label:
        'Категория из иерархии знаний, сообщающая ассистенту о сущности данного концепта. С помощью меток ответы находят подходящий контент.',
      bundle:
        'Экспортируемый zip-архив репозитория — его концепты, структура и метаданные в одном файле. Пакеты используются для переноса репозиториев между системами.',
      version:
        'Фиксированный снимок состояния репозитория на момент выполнения publish. Редактирование продолжается в следующей версии — опубликованные версии неизменны.',
      serving:
        'Эта версия активна: ассистент обращается к ее контенту при ответах. Выполните retract для внесения изменений.',
      ingestion:
        'Этап, на котором опубликованная версия подготавливается для генерации ответов: текст разбивается, векторизуется и связывается. До завершения этого процесса контент ассистенту недоступен.',
      classification:
        'Метод определения сущности каждого концепта (тема, объект, процесс…). Эвристика анализирует страницу автоматически; опция LLM работает медленнее, но справляется со сложными страницами.',
      resplit:
        'Повторное разбиение источника на концепты другим методом — например, по одному концепту на страницу вместо одного на весь сайт.',
      saveFailed: 'Ошибка сохранения',
      importFailedTitle: 'Импорт завершился ошибкой — исходный файл не удалось преобразовать.',
      importFailedHint: 'Удалите этот репозиторий и импортируйте исходный файл снова.',
      frontmatter:
        'Структурированные метаданные в начале каждого файла — тип, заголовок, метки. Ассистент использует их, чтобы понимать предмет каждого концепта.'
    },
    md: {
      toolbar: 'Форматирование',
      bold: 'Жирный',
      italic: 'Курсив',
      h1: 'Заголовок 1',
      h2: 'Заголовок 2',
      h3: 'Заголовок 3',
      bullet: 'Маркированный список',
      numbered: 'Нумерованный список',
      link: 'Вставить ссылку',
      code: 'Встроенный код',
      table: 'Вставить таблицу',
      split: 'Разделить',
      rendering: 'Отрисовка…'
    },
    narrative: {
      intro:
        'Репозиторий OKF — это структурированная версионированная коллекция тем, на которые могут ссылаться ответы в чате. Рассматривайте его как облегченный онтологический слой для вашей области: метки задают категории, темы определяют сущности, а источники обеспечивают происхождение данных.',
      step0:
        'Репозиторий OKF — это курируемая версионированная коллекция тем. Представьте ее как облегченный онтологический слой вашей области: метки определяют категории, темы определяют сущности, а источники подтверждают происхождение. После публикации ответы чата цитируют эти темы по идентификаторам.',
      step1:
        'Три способа инициализации репозитория OKF: извлечь темы из обхода сайта, получить их из ранее загруженных документов или начать с чистого листа. Клонирование существующего репозитория копирует его темы и позволяет продолжить работу в новой ветке.',
      step2:
        'Каждый документ становится источником тем. Модуль генерации анализирует их, извлекает кандидатов в темы и предлагает иерархию. На следующем шаге вы проверите каждую тему — фиксация изменений произойдет только после вашего утверждения.',
      step3:
        'Мы считываем ваши источники и предлагаем темы. Темы группируются по выбранным вами категориям меток. Модуль генерации работает консервативно: он предпочитает сформировать меньше четких тем, чем множество зашумленных.',
      step4:
        'Метки — это категориальные оси вашей онтологии: к какому типу сущностей относится данная тема? Выберите от 3 до 7 меток, отражающих основные оси; модуль генерации использует их как каркас для иерархии тем.',
      step5:
        'Это ключевой этап работы. Каждая тема — это небольшая цитируемая единица знаний. Темы получают название, описание и наследуют выбранные метки. Ваша задача — обеспечить точность: переименовать неясные заголовки, объединить дубликаты и исключить темы, не подтвержденные источниками.',
      step6:
        'Мы запускаем проверку соответствия: есть ли название у каждой темы? Корректны ли метки? Сохранены ли привязки к источникам? Блокирующие проблемы необходимо устранить до публикации; предупреждения можно просто подтвердить.',
      step7:
        'Некоторые предупреждения можно исправить автоматически: привести статусы к допустимым значениям перечислений, заполнить отсутствующие поля стандартными значениями. Примените нужные исправления и вернитесь к куратору для проверки.',
      step8:
        'Сводка публикуемых данных: темы, метки, источники и нерешенные вопросы. После вашего утверждения репозиторий перейдет на этап публикации.',
      step9:
        'Публикация создает версию v1 этого репозитория. Каждое последующее изменение — новая тема, обновленное описание, повторная проверка устаревших данных — публикуется как v2, v3 и далее. Старые версии навсегда остаются доступными для цитирования.',
      docMgmtEntry:
        'Создайте новый репозиторий OKF из выбранных документов. Документы станут источниками; вы сможете просмотреть и назвать созданные ими темы до публикации.',
      crawlSegment:
        'Репозиторий OKF — это структурированная версионированная коллекция тем, на которые могут ссылаться ответы в чате.',
      emptyDashboard:
        'Вы еще не создали ни одного репозитория OKF. Репозиторий OKF представляет собой структурированный цитируемый набор тем, к которому обращаются ответы чата.',
      labels:
        'Метки служат категориальными осями вашей онтологии — они отвечают на вопрос: «К какому типу сущностей относится эта тема?»',
      hide: 'Скрыть',
      whatIsThis: 'Что это?'
    },
    common: {
      close: 'Закрыть',
      cancel: 'Отмена',
      save: 'Сохранить',
      clear: 'Очистить',
      delete: 'Удалить'
    },
    studio: {
      title: 'OKF Studio',
      help: 'Справка',
      helpTitle: 'О программе OKF Studio',
      helpBody:
        'Репозитории OKF являются облегченным онтологическим слоем: метки определяют категории, темы определяют сущности, а источники подтверждают происхождение. После публикации ответы чата цитируют темы по идентификаторам и показывают их источники.',
      view: {
        dashboard: 'Панель управления',
        wizard: 'Мастер настройки'
      },
      clonedFrom: 'Склонировано из {name} · версия {version}',
      context: {
        title: 'Репозиторий',
        untitled: 'Репозиторий без названия',
        trust: 'Уровень доверия',
        sources: 'Источники',
        concepts: 'Концептов на данный момент',
        stale: 'устарело'
      },
      status: {
        draft: 'в работе',
        inReview: 'на рассмотрении',
        published: 'опубликовано'
      },
      stage: {
        inReview: 'В процессе review',
        stepOf: 'Шаг {n} из 10',
        draft: 'Черновик'
      },
      dashboard: {
        title: 'Репозитории',
        new: '+ Новый репозиторий',
        search: 'Поиск…',
        empty: 'Здесь пока нет репозиториев.',
        topics: 'тем',
        select: 'Выбрать {name} для массовой публикации',
        stale: 'устарело',
        stage: {
          inReview: 'В процессе review',
          stepOf: 'Шаг {n} из 10',
          draft: 'Черновик'
        },
        bulk: {
          selected: 'Выбрано: {n}',
          publish: 'Опубликовать выбранные',
          title: 'Опубликовать выбранные репозитории',
          body: 'После публикации последующие ответы чата смогут использовать их. Для каждого репозитория ниже указан итоговый результат.',
          results: 'Результаты',
          publishConfirm: 'Опубликовать {n}',
          col: {
            name: 'Имя',
            status: 'Статус',
            topics: 'Темы'
          },
          status: {
            ready: 'Готово'
          }
        }
      },
      draft: {
        notReady: 'Черновики пока нельзя сохранить на сервере; сохранено локально.'
      },
      mode: {
        basic: 'Базовый',
        expert: 'Эксперт'
      }
    },
    wizard: {
      status: {
        published: 'опубликовано',
        inReview: 'на проверке',
        draft: 'в процессе'
      },
      exit: 'Вернуться в панель управления',
      context: {
        untitled: 'Репозиторий без названия',
        trust: 'Доверие',
        title: 'Репозиторий',
        stale: 'устарело',
        sources: 'Источники',
        concepts: 'Концептов на данный момент'
      },
      label: 'Мастер OKF Studio',
      back: 'Назад',
      continue: 'Продолжить',
      publish: 'Опубликовать репозиторий'
    },
    steps: {
      entry: {
        title: 'Имя репозитория и предметная область',
        hint: 'Задайте понятное имя для этого репозитория OKF и выберите его предметную область.',
        nameLabel: 'Имя репозитория',
        namePh: 'например, Транспортные разрешения Нидерландов',
        domainLabel: 'Предметная область'
      },
      choose: {
        title: 'С чего должен начинаться этот репозиторий OKF?',
        hint: 'Выберите способ начального заполнения этого репозитория. Вы сможете изменить его позже.',
        source: {
          documents: {
            title: 'Документы',
            desc: 'Извлечь темы из документов, которые вы уже загрузили.'
          },
          crawl: {
            title: 'Обход веб-сайта',
            desc: 'Обойти веб-сайт и предложить темы на основе его страниц.'
          },
          manual: {
            title: 'Чистый лист',
            desc: 'Начать с нуля и написать темы самостоятельно.'
          },
          clone: {
            title: 'Клон существующего репозитория',
            desc: 'Создать ответвление тем и структуры из другого репозитория OKF.'
          }
        }
      },
      input: {
        title: 'Входные данные',
        documents: 'Выберите документы для формирования начального списка тем.',
        crawl: 'Выберите веб-сайт(ы) для обхода.',
        manual: 'Вы сможете составить темы непосредственно на следующем шаге.',
        clone: 'Выберите существующий репозиторий для клонирования.',
        placeholder: 'Интерфейс шага появится в следующей истории.'
      },
      produce: {
        title: 'Генерация тем',
        hint: 'Мы анализируем ваши источники и предлагаем темы.',
        progress: 'Модуль генерации работает...',
        placeholder: 'Служба генерации появится в следующей истории.'
      },
      label: {
        title: 'Выбор меток',
        hint: 'Метки — это категориальные оси вашей онтологии: к какому типу относятся эти темы? Выберите от 3 до 7.',
        add: '+ Добавить метку',
        addConfirm: 'Добавить',
        placeholder: 'например, Разрешения'
      },
      curate: {
        title: 'Просмотр тем',
        hint: 'Каждая тема — это небольшая цитируемая единица знаний. Отредактируйте заголовок и описание.',
        placeholder: 'Дерево куратора и редактор появятся в истории 3-8.'
      },
      validate: {
        title: 'Поиск проблем',
        hint: 'Мы выполняем проверку соответствия. Блокирующие проблемы должны быть устранены до публикации.',
        placeholder: 'Панель валидации появится в истории 3-8.'
      },
      autocorrect: {
        title: 'Автоисправление',
        hint: 'Некоторые предупреждения можно исправить автоматически. Примените подтвержденные исправления, затем вернитесь к куратору для проверки.',
        placeholder: 'Служба автоисправления появится в истории 10.2.'
      },
      review: {
        title: 'Проверка',
        hint: 'Сводка публикуемых данных.',
        repo: 'Репозиторий',
        topics: 'Темы',
        labels: 'Метки',
        sources: 'Источники'
      },
      publish: {
        title: 'Опубликовать этот репозиторий',
        hint: 'Публикация создает версию v1 этого репозитория.',
        nameOk: 'Имя репозитория задано',
        labelsOk: 'Метки выбраны',
        topicsOk: 'Темы проверены'
      }
    },
    trust: {
      tier: {
        unverified: 'не проверено',
        'machine-confirmed': 'подтверждено автоматически',
        'human-reviewed': 'проверено человеком'
      }
    },
    repos: {
      retract: {
        notReady: 'Интерфейс для операции retract появится в истории 3.9.'
      },
      manifest: {
        notReady: 'Служба манифеста пакетов появится в следующей истории.'
      }
    },
    docs: {
      createRepo: 'Создать репозиторий OKF',
      gate: {
        emptySelection: 'Выберите хотя бы один документ',
        alreadyInOkf: 'Исключить документы, которые уже находятся в репозитории OKF',
        alreadyIngested: 'Исключить документы, для которых уже выполнен ingest',
        servingWarn:
          '{n} выбранных документов всё ещё обслуживают свободный корпус — новый репозиторий нельзя загрузить, пока они не отозваны.'
      },
      produce: {
        notReady: 'Интеграция с модулем генерации появится в следующей истории.'
      }
    },
    crawl: {
      domainLabel: 'Предметная область',
      domainPlaceholder: 'Выберите предметную область…',
      classLabel: 'Классификация концептов',
      classHeuristics: 'Эвристика (по умолчанию)',
      classHeuristicsHint:
        'Быстрая классификация на основе правил — без затрат на LLM, подходит для структурированных сайтов.',
      classLlm: 'С поддержкой LLM',
      classLlmHint:
        'LLM курирует каждый концепт — тип, метку иерархии знаний и описание. Гораздо точнее и полнее эвристики; требует дополнительного времени на концепт.',
      classHybrid: 'Гибридный',
      classHybridHint:
        'Сначала эвристика; LLM проверяет неопределенные случаи и заполняет пробелы. Баланс времени и полноты.',
      targetLabel: 'Куда направить результат?',
      target: {
        freeform: 'Обход в свободный корпус',
        okfRepo: 'Репозиторий OKF'
      },
      targetHint: 'Мы покажем найденные темы перед сохранением данных.',
      createOkfFromCrawl: 'Создать репозиторий OKF из этого обхода',
      splitLabel: 'Разделение концептов',
      splitA: 'Один концепт на весь обход',
      splitB: 'Один концепт на страницу (рекомендуется)',
      splitC: 'Использовать извлечение тем с помощью LLM',
      splitCHint: 'История 10.6 — скоро появится',
      splitBHint: 'Каждая обойденная страница становится отдельным концептом — ИИ может цитировать отдельные страницы.',
      splitAHint: 'Весь обход становится одним большим концептом — оптимально для небольших сайтов.',
      progressDownload: 'Загрузка собранного контента...',
      progressSplit: 'Разделение на концепты...',
      progressIngest: 'Добавление концептов (пакет [i] из [n])...',
      postCrawlHint:
        'После завершения обхода вы можете преобразовать его в репозиторий OKF на вкладке «Панель управления» файла.',
      creating: 'Создание репозитория OKF...',
      createOk: 'Репозиторий OKF создан. Открытие Studio для курирования.',
      createOkRenamed: 'Репозиторий OKF создан с именем "[name]". Открытие Studio для курирования.',
      createFailed: 'Не удалось создать репозиторий OKF из этого обхода.',
      conversionsTitle: 'Репозитории, созданные на основе этого обхода',
      convQueued: 'В очереди...',
      convDownloading: 'Загрузка обхода...',
      convSplitting: 'Разделение на концепты...',
      convAdding: 'Добавление концептов (обработано страниц: [p])...',
      convDone: 'Создано (страниц: [p])',
      convFailed: 'Ошибка',
      createStarted: 'Создание "[name]" — следите за прогрессом ниже.',
      modelTier: {
        label: 'Модель',
        options: {
          'local-small': 'Локальная · малая',
          'local-medium': 'Локальная · средняя',
          'sovereign-large': 'Суверенная · большая'
        }
      },
      gate: {
        notSucceeded: 'Обход должен завершиться перед созданием репозитория OKF.',
        alreadyInOkf: 'Этот обход уже находится в репозитории OKF.'
      }
    },
    curator: {
      placeholder: 'Выберите тему слева для просмотра и редактирования.',
      search: 'Поиск тем',
      noTopics: 'Тем пока нет.',
      labels: {
        title: 'Настроить метки',
        body: 'Метки — это категориальные оси вашей онтологии: к какому типу сущностей относится эта тема?',
        edit: 'Настроить метки',
        add: 'Добавить',
        addPh: 'например, Разрешения',
        remove: 'Удалить',
        empty: 'Меток пока нет.',
        notReady: 'Метки сохранены локально — синхронизация с сервером скоро появится.'
      },
      frontmatter: {
        label: 'Метаданные (frontmatter)',
        edit: 'Редактировать frontmatter',
        dialogTitle: 'Редактировать frontmatter',
        okfVersion: 'okf_version',
        status: 'lifecycle.status',
        staleAfter: 'lifecycle.stale_after',
        trustTier: 'trust_tier',
        attestation: 'attestation',
        sources: 'provenance.sources'
      },
      showSource: 'Показать источник',
      issue: {
        missingType: 'У концепта отсутствует тип.',
        badActor: 'Субъект источника должен начинаться с agent:/human:/tool:/process:.'
      },
      saveBlocked: 'Сохранение заблокировано: сначала устраните проблемы соответствия.'
    },
    validation: {
      frozen:
        "Контент зафиксирован на версии {'{'}v{'}'} — режим только для чтения. Выполните retract для рабочей версии, чтобы внести изменения.",
      none: 'Нет',
      expertHint:
        'Переключитесь в режим эксперта, чтобы просмотреть исходный JSON проверки, отфильтровать по серьезности и переопределить результаты.',
      headline: {
        blockers: 'Блокирующих проблем: {n} — устраните их перед публикацией',
        warnings: 'Пунктов, требующих вашего внимания: {n}',
        ok: 'Все в порядке. Замечаний нет.'
      },
      summary: '{clean} без замечаний · {warnings} требуют проверки · {blockers} блокирующих',
      formatter: {
        notReady: 'Модуль форматирования появится в истории 4.2b.'
      },
      run: {
        notReady: 'Интеграция валидации появится в следующей истории.'
      }
    },
    pii: {
      panel: 'Отмеченные сущности',
      nFlagged: 'Найдено {n}',
      rescan: 'Пересканировать',
      scanning: 'Сканирование…',
      clean: 'Отмеченных объектов нет — этот concept чист.',
      how: 'Решение: удалите или измените текст (например, укажите должность вместо имени), затем сохраните — повторное сканирование обновит этот список.',
      descFallback:
        'Здесь были обнаружены персональные данные (GDPR, ст. 4(1) — относящиеся к идентифицируемому лицу).',
      scanError: 'Сканирование недоступно',
      type: {
        PERSON: 'Имя человека',
        LOCATION: 'Местоположение',
        DATE_TIME: 'Дата или время',
        EMAIL_ADDRESS: 'Адрес электронной почты',
        PHONE_NUMBER: 'Номер телефона',
        IP_ADDRESS: 'IP-адрес',
        NRP: 'Национальный регистрационный номер',
        BD_NATIONAL_ID: 'Номер национального удостоверения личности'
      },
      desc: {
        PERSON: 'Обнаружено имя человека. Имена могут напрямую идентифицировать физическое лицо.',
        LOCATION:
          'Обнаружено название места. В сочетании с другими данными это может помочь идентифицировать человека.',
        DATE_TIME: 'Обнаружена дата или время. Даты могут способствовать идентификации человека.',
        EMAIL_ADDRESS: 'Обнаружен адрес электронной почты. Он напрямую идентифицирует контактное лицо.',
        PHONE_NUMBER: 'Обнаружен номер телефона. Он напрямую идентифицирует контактное лицо.',
        IP_ADDRESS: 'Обнаружен IP-адрес. Он может идентифицировать устройство или домохозяйство.',
        NRP: 'Обнаружен национальный регистрационный номер. Он точно идентифицирует человека.',
        BD_NATIONAL_ID: 'Обнаружен номер национального удостоверения. Он точно идентифицирует человека.'
      },
      allClear: 'Чисто',
      redactFile: 'Скрыть весь файл',
      confirmRedactFile: 'Подтверждение: скрыть файл полностью?',
      locateTip: 'Показать этот фрагмент в редакторе',
      file: {
        label: 'Весь файл:',
        redact: 'Скрыть файл',
        remove: 'Удалить тело',
        accept: 'Принять всё',
        confirmRedact: 'Подтверждение: скрыть весь файл?',
        confirmRemove: 'Подтверждение: удалить весь текст документа?'
      },
      action: {
        redact: 'Скрыть',
        replace: 'Заменить',
        remove: 'Удалить',
        accept: 'Принять',
        replacement: 'Текст замены',
        apply: 'Применить',
        cancel: 'Отмена'
      },
      resolved: {
        title: 'Обработано в этом файле',
        redacted: 'Скрыто',
        replaced: 'Заменено',
        removed: 'Удалено',
        accepted: 'Принято',
        fileRedacted: 'Файл скрыт',
        kept: 'сохранено в тексте',
        removedWord: 'удалено',
        wholeFile: 'Весь текст файла скрыт'
      }
    },
    lifecycle: {
      submit: 'Отправить на проверку',
      approve: 'Утвердить',
      publish: 'Опубликовать',
      ingest: 'Поглотить',
      retract: 'Отозвать'
    }
  }
};
