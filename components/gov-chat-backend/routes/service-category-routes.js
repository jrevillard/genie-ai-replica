...
module.exports = (serviceCategoryService) => {
  if (
    ...
    logger.error(
      "Invalid serviceCategoryService provided to service-category-routes",
    );
    throw new Error(
  ...
  logger.debug(
    "serviceCategory-routes initialized with serviceCategoryService",
  );
  ...
  router.get("/categories/detailed", async (req, res) => {
    ...
    try {
      const locale = req.query.locale || "en";
      logger.info(
        `Fetching all DETAILED service categories with locale: ${locale}`,
      );
      const categories =
        await serviceCategoryService.getAdminAllCategoriesWithServices(locale);
      logger.info(
        `Fetched ${categories.length} detailed categories in ${Date.now() - start}ms`,
      );
      res.json(categories);
    } catch (error) {
  ...
  router.get("/categories/:categoryId", async (req, res, next) => {
    ...
    try {
      const locale = req.query.locale || "en";
      logger.info(
        `Fetching category ${req.params.categoryId} with locale: ${locale}`,
      );
      ...
      logger.error(
        `Error getting category ${req.params.categoryId}: ${error.message}`,
        {
          stack: error.stack,
          durationMs: Date.now() - start,
        },
  ...
  router.get("/:categoryId/translations", async (req, res) => {
    ...
    try {
      ...
      const translations =
        await serviceCategoryService.getCategoryTranslations(categoryId);
      logger.info(
        `Fetched ${translations.length} translations for category ${categoryId} in ${Date.now() - start}ms`,
      );
      res.json(translations);
    } catch (error) {
      logger.error(
        `Error getting translations for category ${req.params.categoryId}: ${error.message}`,
        {
          stack: error.stack,
          durationMs: Date.now() - start,
        },
  ...
  router.get("/services/:serviceId/translations", async (req, res) => {
    ...
    try {
      ...
      const translations =
        await serviceCategoryService.getServiceTranslations(serviceId);
      logger.info(
        `Fetched ${translations.length} translations for service ${serviceId} in ${Date.now() - start}ms`,
      );
      res.json(translations);
    } catch (error) {
      logger.error(
        `Error getting translations for service ${req.params.serviceId}: ${error.message}`,
        {
          stack: error.stack,
          durationMs: Date.now() - start,
        },
  ...
  router.get("/search", async (req, res) => {
    ...
    try {
      ...
      logger.info(
        `Searching categories and services with query: "${query}" and locale: ${locale}`,
      );
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      ...
      if (!payload || !payload.nameEN) {
        return res
          .status(400)
          .json({ message: "Payload with nameEN is required" });
      }
  ...
  router.delete("/:categoryId", async (req, res) => {
    ...
    try {
      ...
      const exists = await serviceCategoryService.categoryExists(
        req.params.categoryId,
      );
      if (!exists) {
        ...
        return res
          .status(404)
          .json({ message: `Category ${req.params.categoryId} not found` });
      }
      await serviceCategoryService.deleteCategory(req.params.categoryId);
      logger.info(
        `Category ${req.params.categoryId} deleted successfully in ${Date.now() - start}ms`,
      );
      res.json({
      ...
      logger.error(
        `Error deleting category ${req.params.categoryId}: ${error.message}`,
        {
          stack: error.stack,
          durationMs: Date.now() - start,
        },
  ...
  router.delete("/services/:serviceId", async (req, res) => {
    ...
    try {
      ...
      logger.info(
        `Service ${serviceId} deleted successfully in ${Date.now() - start}ms`,
      );
      res
        .status(200)
      ...
      logger.error(
        `Error deleting service ${req.params.serviceId}: ${error.message}`,
        {
          stack: error.stack,
          durationMs: Date.now() - start,
        },
  ...
  router.post("/init", async (req, res) => {
    ...
    try {
      ...
      const result =
        await serviceCategoryService.initializeDefaultCategoriesAndServices();
      logger.info(
        `Default categories initialized successfully in ${Date.now() - start}ms`,
      );
      res.json(result);
    } catch (error) {
  ...
  router.post("/:categoryId/services", async (req, res) => {
    ...
    try {
      ...
      const newService =
        await serviceCategoryService.createServiceWithTranslations(
          categoryId,
          payload,
        );
      logger.info(
        `Service created successfully for category ${categoryId} in ${Date.now() - start}ms`,
      );
      res.status(201).json(newService);
    } catch (error) {
      logger.error(
        `Error creating service for category ${req.params.categoryId}: ${error.message}`,
        {
          stack: error.stack,
          durationMs: Date.now() - start,
        },
  ...
  router.put("/:categoryId", async (req, res) => {
    ...
    try {
      ...
      const result =
        await serviceCategoryService.updateCategoryWithTranslations(
          categoryId,
          payload,
        );
      logger.info(
        `Category ${categoryId} updated successfully in ${Date.now() - start}ms`,
      );
      res.status(200).json(result);
    } catch (error) {
      logger.error(
        `Error updating category ${req.params.categoryId}: ${error.message}`,
        {
          stack: error.stack,
          durationMs: Date.now() - start,
        },
  ...
  router.put("/services/:serviceId", async (req, res) => {
    ...
    try {
      ...
      const result = await serviceCategoryService.updateServiceWithTranslations(
        serviceId,
        payload,
      );
      logger.info(
        `Service ${serviceId} updated successfully in ${Date.now() - start}ms`,
      );
      res.status(200).json(result);
    } catch (error) {
      logger.error(
        `Error updating service ${req.params.serviceId}: ${error.message}`,
        {
          stack: error.stack,
          durationMs: Date.now() - start,
        },
