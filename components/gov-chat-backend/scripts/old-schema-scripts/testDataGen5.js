...
`;
            ...
            try {
                ...
                if (nowResult.recentQueries.some(q => q.nowCompare === "after now")) {
                    ...
                    const createFixFunction = `CREATE OR REPLACE FUNCTION fixed_date_now() RETURNS DATE RETURN DATE_ISO8601(DATE_NOW())`;
...
`;
            ...
            // Test query to check authentication fields
            console.log('Testing authentication fields on users...');
            const authQuery = `
RETURN {
totalUsers: LENGTH(FOR u IN users RETURN 1),
usersWithLoginName: LENGTH(FOR u IN users FILTER HAS(u, "loginName") RETURN 1),
usersWithEmail: LENGTH(FOR u IN users FILTER HAS(u, "email") RETURN 1),
usersWithAccessToken: LENGTH(FOR u IN users FILTER HAS(u, "accessToken") && u.accessToken != null RETURN 1),
usersWithNullToken: LENGTH(FOR u IN users FILTER HAS(u, "accessToken") && u.accessToken == null RETURN 1),
emailAsLogin: LENGTH(FOR u IN users FILTER HAS(u, "loginName") && u.loginName == u.email RETURN 1),
usernameAsLogin: LENGTH(FOR u IN users FILTER HAS(u, "loginName") && u.loginName != u.email RETURN 1),
sample: (
FOR u IN users 
  LIMIT 1 
  RETURN { 
    ...
    hasToken: HAS(u, "accessToken") && u.accessToken != null,
    created: u.createdAt,
    updated: u.updatedAt
  }
)[0]
...
`;
            ...
            try {
                const authCursor = await db.query(authQuery);
                const authResult = await authCursor.next();
                console.log('Authentication field statistics:');
                console.log(JSON.stringify(authResult, null, 2));

                // Verify our targets were met
                if (authResult.usersWithLoginName === authResult.totalUsers &&
                    authResult.usersWithEmail === authResult.totalUsers) {
                    console.log('✅ All users have required loginName and email fields');
                } else {
                    console.log('⚠️ Some users are missing required authentication fields');
                }

                // Check token distribution
                const tokenPercentage = (authResult.usersWithAccessToken / authResult.totalUsers * 100).toFixed(1);
                if (tokenPercentage >= 75 && tokenPercentage <= 85) {
                    console.log(`✅ Token distribution correct: ${tokenPercentage}% of users have active tokens (target: 80%)`);
                } else {
                    console.log(`⚠️ Token distribution incorrect: ${tokenPercentage}% of users have active tokens (target: 80%)`);
                }

                // Check login name distribution
                const emailLoginPercentage = (authResult.emailAsLogin / authResult.totalUsers * 100).toFixed(1);
                if (emailLoginPercentage >= 65 && emailLoginPercentage <= 75) {
                    console.log(`✅ Login type distribution correct: ${emailLoginPercentage}% use email as login (target: 70%)`);
                } else {
                    console.log(`⚠️ Login type distribution incorrect: ${emailLoginPercentage}% use email as login (target: 70%)`);
                }
            } catch (err) {
                console.error('Error testing authentication fields:', err.message);
            }
