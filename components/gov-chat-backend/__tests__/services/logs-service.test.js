...
jest
  ...
  .mockReturnValue(jest.fn().mockResolvedValue(Buffer.from("decompressed")));
...
describe("LogsService", () => {
  ...
  describe("fileExists", () => {
    ...
    it("should throw for non-existent file", async () => {
      ...
      await expect(logsService.fileExists("/missing")).rejects.toThrow(
        "File not found",
      );
    });
    ...
    it("should re-throw non-ENOENT errors", async () => {
      mockFs.access.mockRejectedValueOnce(new Error("Permission denied"));
      await expect(logsService.fileExists("/forbidden")).rejects.toThrow(
        "Permission denied",
      );
    });
  });
  ...
  describe("readLogFile", () => {
    it("should read regular log file", async () => {
      ...
      const result = await logsService.readLogFile(
        "/logs/combined-2026-05-26.log",
      );
    ...
    it("should read and decompress .gz files", async () => {
      ...
      const result = await logsService.readLogFile(
        "/logs/combined-2026-05-26.log.gz",
      );
    ...
    it("should throw on read error", async () => {
      ...
      await expect(logsService.readLogFile("/logs/broken.log")).rejects.toThrow(
        "IO error",
      );
    });
  });
  ...
  describe("getLogFilesInRange", () => {
    ...
    it("should return empty when logs directory does not exist", async () => {
      mockFs.access.mockRejectedValueOnce(new Error("ENOENT"));
      const result = await logsService.getLogFilesInRange(
        "2026-05-20",
        "2026-05-26",
      );
      expect(result).toEqual([]);
  ...
  describe("detectService", () => {
    it("should detect EmailService", () => {
      expect(logsService.detectService("EmailService sent email")).toBe(
        "Email Service",
      );
    });

    it("should detect DatabaseService", () => {
      expect(logsService.detectService("DatabaseService query failed")).toBe(
        "Database Service",
      );
    });

    it("should detect AuthService", () => {
      expect(logsService.detectService("AuthService login")).toBe(
        "Auth Service",
      );
    });
  ...
  describe("extractDateFromFilename", () => {
    it("should extract date from combined log filename", () => {
      expect(
        logsService.extractDateFromFilename("combined-2026-05-26.log"),
      ).toBe("2026-05-26");
    });

    it("should extract date from error log filename", () => {
      expect(logsService.extractDateFromFilename("error-2026-05-25.log")).toBe(
        "2026-05-25",
      );
    });
  ...
  describe("groupLogs", () => {
    ...
    it("should handle unknown events", () => {
      const logs = [
        { message: "Something completely unique happened", service: "System" },
      ];
      const result = logsService.groupLogs(logs);
      expect(result).toHaveLength(1);
    });
  ...
  describe("searchLogs", () => {
    ...
    it("should filter by service", async () => {
      ...
      expect(
        result.logs.every((log) => log.service.toLowerCase().includes("auth")),
      ).toBe(true);
    });
    ...
    it("should respect limit parameter", async () => {
      ...
      const manyLogs = Array(50)
        ...
        .map((l, i) =>
          l.replace("10:00:00", `10:${String(i).padStart(2, "0")}:00`),
        )
        .join("\n");
