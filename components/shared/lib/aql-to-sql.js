...
/**
 * Escape regex special characters in a string interpolated into a RegExp
 * source. Interpolated values are AQL identifiers (\w+ captures) today, so
 * metacharacters cannot appear in practice — this keeps the dynamic RegExp
 * construction crash-proof if that invariant ever changes.
 */
...
class AqlToSqlTranslator {
  constructor() {
    ...
    this.functionMap = {
      ...
      LIKE: 'like',
      HAS: (args) =>
        `(${args[0]}.${args[1].replace(/['"]/g, '')} IS NOT NULL AND ${args[0]}.${args[1].replace(/['"]/g, '')} != {})`,
      CONCAT: 'concat',
      DOCUMENT: (args) => {
      ...
      DATE_SUBTRACT: (args) =>
        args.length === 3
          ? `date_add(${args[0]}, '${args[2].replace(/['"]/g, '')}', -${args[1]})`
          : `DATE_SUBTRACT(${args.join(', ')})`,
      DATE_ADD: (args) =>
        args.length === 3
          ? `date_add(${args[0]}, '${args[2].replace(/['"]/g, '')}', ${args[1]})`
          : `DATE_ADD(${args.join(', ')})`,
  ...
  translateQuery(aqlQuery, bindVars = {}) {
    ...
    if (sql.toUpperCase().startsWith('RETURN ')) {
      const returnExpr = sql.replace(/^RETURN/i, '').trim();
      const processed = returnExpr.replace(/\{([^}]+)\}/, (m, fields) => {
        return fields
          .split(',')
          .map((f) => {
            const split = f
  ...
  isGraphTraversal(sql) {
    return (
      sql.match(/FOR\s+\w+\s+IN\s+\w+\s+FILTER\s+\w+\._(from|to)\s*==/i) &&
  ...
  _translateNestedFor(sql, params) {
    ...
    const match = sql.match(
      /FOR\s+(\w+)\s+IN\s+(\w+)\s+FILTER\s+(.+?)\s+FOR\s+(\w+)\s+IN\s+(\w+)\s+FILTER\s+(.+?)(?:\s+RETURN\s+(.+)|$)/i
    );
    if (match) {
      ...
      const ret =
        match[7] || `{ _id: ${docVar}._id, nameEN: ${docVar}.nameEN, relevanceScore: ${edgeVar}.relevanceScore }`;
      const joinCondition = filter2.match(
        new RegExp(`${escapeRegExp(docVar)}\\._id\\s*==\\s*${escapeRegExp(edgeVar)}\\._(to|from)`, 'i')
      );
      if (joinCondition) {
        const direction = joinCondition[1];
        filter1 = filter1.replace(new RegExp(`${escapeRegExp(edgeVar)}\\.`, 'g'), `${edgeVar}.`);
        filter2 = filter2.replace(joinCondition[0], '');
        if (filter2.trim().startsWith('AND')) {
          filter2 = filter2.trim().substring(3).trim();
        }
        filter2 = filter2.replace(new RegExp(`${escapeRegExp(docVar)}\\.`, 'g'), `${docVar}.`);
  ...
  _translateTopLet(sql, params) {
    ...
    while ((match = current.match(/LET\s+(\w+)\s*=\s*(.+?)(?=\s+LET\s+|\s+RETURN\s+|$)/i))) {
      ...
      returnExpr = returnExpr.replace(new RegExp(`\\b${escapeRegExp(varName)}\\b`, 'g'), varName);
      current = current.substring(match.index + match[0].length).trim();
    }
    const translatedReturn = returnExpr.replace(/\{([^}]+)\}/, (m, fields) => {
      return fields
        .split(',')
        .map((f) => {
          const split = f
  ...
  _translateGraphTraversal(sql, params) {
    ...
    if (edgeMatch) {
      ...
      if (docMatch && docMatch[2] === edgeVar) {
        ...
        const remMatch = postDoc.match(
          /(?:\s+FILTER\s+(.+?))?(?:\s+SORT\s+(.+?))?(?:\s+LIMIT\s+(.+?))?\s+RETURN\s+(.+)/i
        );
        if (remMatch) {
          const filter = remMatch[1]
            ? ` AND ${this._translateExpression(remMatch[1]).replace(new RegExp(`${escapeRegExp(docVar)}\\.`, 'g'), docVar + '.')}`
            : '';
  ...
  _translateForRemove(sql, params) {
    ...
    if (match) {
      const varName = match[1];
      const coll = match[2];
      const filters = this._translateExpression(match[3]).replace(new RegExp(`${escapeRegExp(varName)}\\.`, 'gi'), '');
  ...
  _translateComplexLet(sql, params) {
    ...
    const finalSql = `WITH ${varName} AS (${translatedSub}) ${translatedRemaining.sql}`;
  ...
  _translateCoreForLoop(sql, params) {
    ...
    for (const clause of clauseList.filter((c) => c.type !== 'FOR' && c.type !== 'RETURN')) {
      if (clause.type === 'FILTER') {
        ...
        if (match) {
          ...
          finalSql = finalSql.replace(new RegExp(`${escapeRegExp(varName)}\\.`, 'g'), '');
          type = 'update';
        }
    ...
    if (hasAgg && !isGlobalAgg && groupVar) {
      if (returnContent === groupVar) {
        ...
        selectClause = selectClause.replace(new RegExp(`\\b${escapeRegExp(groupVar)}\\b`), groupExpr);
      }
    }

    postCollectLets.forEach(({ letVar, translatedExpr }) => {
      selectClause = selectClause.replace(new RegExp(`\\b${escapeRegExp(letVar)}\\b`), translatedExpr);
    });
    ...
    while (pos < sql.length) {
      if (depth === 0) {
        for (const kw of keywords) {
          if (
            upperSql.substr(pos, kw.length) === kw &&
            (pos + kw.length === sql.length || !/[a-zA-Z0-9_]/.test(sql[pos + kw.length]))
          ) {
            if (currentKeyword) {
  ...
  _translateExpression(expr) {
    ...
    do {
      ...
      for (const [key, value] of Object.entries(this.functionMap)) {
        const regex = new RegExp(`${escapeRegExp(key)}\\s*\\(\\s*([^)]*)\\s*\\)`, 'gi');
  ...
  translateInsert(collectionName, doc, options = {}) {
    logger.debug(
      `[AQL_TRANSLATE_INSERT] Starting translation for INSERT in ${collectionName} with doc: ${JSON.stringify(doc)}`
    );
  ...
  translateUpdate(collectionName, key, newData, options = {}) {
    logger.debug(
      `[AQL_TRANSLATE_UPDATE] Starting translation for UPDATE in ${collectionName} with key: ${key}, newData: ${JSON.stringify(newData)}`
    );
  ...
  translateReplace(collectionName, key, newData, options = {}) {
    logger.debug(
      `[AQL_TRANSLATE_REPLACE] Starting translation for REPLACE in ${collectionName} with key: ${key}, newData: ${JSON.stringify(newData)}`
    );
    const params = { key, newData: JSON.stringify(newData) };
