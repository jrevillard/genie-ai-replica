...
beforeEach(async () => {
  ...
  mockDb._reset();
  // The REPO-GONE guard (2026-09-04) reads the repo doc before every flush —
  // seed a live repo so conversions flow (tests that delete it exercise the guard).
  mockDb.collection('okf_repositories').save({ _key: 'r1', repo_id: 'r1', name: 'Conversion Repo' });
...
describe('sanitizeCrawlBody', () => {
  ...
  test('escaped brackets in alt text no longer leak raw images (live crawl 2026-09-03)', () => {
    ...
    expect(conv.sanitizeCrawlBody('![\\[icon\\]](https://up.example/i.png)')).toBe(
      '[[icon]](https://up.example/i.png)'
    );
  });
...
describe('mode B — per-page streaming conversion', () => {
  ...
  test('splits pages, sanitizes, ingests and writes the index LAST', async () => {
    ...
    // Page 1: title from the URL segment (ruling: URL name FIRST, heading second).
    ...
    expect(index[0].body).toContain('[two](./b-example-two.md)');
...
describe('directive (David, 2026-09-04): a conversion whose repo is gone self-terminates', () => {
  test('(b) repo deleted mid-conversion → flush throws REPO_GONE → conversion failed', async () => {
    // Program the source stream so the run reaches the first flush, THEN the
    // guard discovers the repo is gone.
    mockSource('# Crawl\n\n## Source: /a\n\nA page');
    // The beforeEach seeded repo 'r1'; delete it before the conversion runs.
    mockDb.collection('okf_repositories').remove('r1');
    const started = await conv.startConversion({ repo_id: 'r1', file_id: 'f1', split_mode: 'B' });
    expect(started.status).toBe('queued');
    const terminal = await waitForTerminal();
    expect(terminal.status).toBe('failed');
    expect(terminal.error).toContain('deleted mid-conversion');
    expect(ingestService.ingestRepoConcepts).not.toHaveBeenCalled(); // never imported into a ghost
  });
});

describe('TITLE RULING (David 2026-09-06 — page name, never raw link text)', () => {
  test('(1) URL last path segment unslugified wins', () => {
    expect(conv.deriveTitleFromUrl('https://en.wikipedia.org/wiki/100_metres_hurdles')).toBe('100 metres hurdles');
    expect(conv.deriveTitleFromUrl('https://www.gov.example/services/registration%20of%20birth.html')).toBe(
      'registration of birth'
    );
  });

  test('root/index URLs yield nothing (fallback to heading)', () => {
    expect(conv.deriveTitleFromUrl('https://en.wikipedia.org/')).toBe('');
    expect(conv.deriveTitleFromUrl('https://x.example/index.html')).toBe('');
  });

  test('(2) first heading cleaned of markdown tokens', () => {
    expect(conv.deriveTitleFromBody('# Battle of Waterloo\nbody')).toBe('Battle of Waterloo');
    expect(conv.deriveTitleFromBody('## [Russo-Ukrainian War](https://x.example/w) — overview')).toBe(
      'Russo-Ukrainian War — overview'
    );
  });

  test('the garbage class David sampled is NEVER a title', () => {
    // Link UNWRAPS to its anchor text — the URL is metadata, never a name.
    expect(conv.cleanTitle('[ISO](https://en.wikipedia.org/wiki/I)')).toBe('ISO');
    // The raw markdown form with the URL can never appear as a title.
    expect(conv.cleanTitle('[ISO](https://en.wikipedia.org/wiki/I)')).not.toContain('https://');
    expect(conv.cleanTitle('%')).toBe('%');
    expect(conv.cleanTitle('`33.`')).toBe('33.');
  });
});
