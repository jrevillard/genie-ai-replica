...
- A curated document base built from **CENTA** publications (Centro Nacional de Tecnología Agropecuaria y Forestal — the Salvadoran national agricultural technology centre), **MAG** advisories (Ministerio de Agricultura y Ganadería — the Ministry of Agriculture and Livestock), and reference material from **FAO** (the Food and Agriculture Organization of the United Nations) and **OIRSA** (Organismo Internacional Regional de Sanidad Agropecuaria — the international regional animal and plant health organisation for Central America), plus the **OPEA** services (Open Platform for Enterprise AI, the project's AI service layer) that index and retrieve them.
- Live agricultural data for El Salvador and the region: satellite-derived crop health (NDVI — Normalized Difference Vegetation Index), pest and disease alerts (curated advisories and iNaturalist community observations), commodity market prices (FAO/WFP, USDA, local ministries), post-harvest loss estimates (FAO SDG 12.3.1), and agricultural input prices (US BLS Producer Price Index for pesticides and ag chemicals, fertilizer benchmarks).
...
- **Use the Insights shortcuts.** The `Crop Health` and `Pest Alerts` cards on the home screen open the live dashboards for El Salvador and let the AI interpret what the numbers mean for your situation.
...
The `Crop Health` dashboard shows vegetation health (NDVI) for El Salvador's 14 departments, updated roughly monthly. NDVI is a satellite-derived indicator from the **WFP VAM** programme (World Food Programme Vulnerability Analysis and Monitoring), distributed via **HDX** (the Humanitarian Data Exchange, open data licence CC BY 4.0). Values are normalised against the long-term baseline for each department, so a "good" department means vegetation is at or near its historical norm, not an absolute threshold.
...
- The badge reflects how far the reading sits from the baseline; it does **not** predict crop failure. The badge values are `Good`, `Moderate`, and `Warning`.
- The `Last updated` timestamp is the server's `fetchedAt` field from the API response — it shows when the data was last refreshed, not the current wall-clock time.
...
The `Pest Alerts` section covers El Salvador's most economically important pests and diseases, with the affected departments and the time of year when activity is highest.

- The advisory copy is **curated from MAG and CENTA publications and OIRSA regional alerts**, with community sightings cross-referenced from **iNaturalist** (Creative Commons observations).
- Each alert lists the scientific name, the EPPO code (the international pest-coding standard maintained by the European and Mediterranean Plant Protection Organization), the affected crops (in English and Spanish), the departments currently affected, and the seasonal window when monitoring is most important.
...
- `Maize, Beans & Grains` — unit **USD/quintal**. Quintal is the Central American farm-gate measure: 1 quintal ≈ 46 kg. Source data in USD/kg is converted to USD/quintal at 45.97 kg. Sources include FAO/WFP, MAG, and regional wholesale markets. The series includes white maize, yellow maize, beans, rice, sorghum, wheat and others — both domestic and international benchmark series are shown side by side so you can see whether a local price moves with or against the regional benchmark.
- `Crop Protection` — unit **Producer Price Index**. The US Bureau of Labor Statistics Producer Price Index for **pesticide and agricultural chemical manufacturing**, plus a **pesticide import parity** index. Index values are relative to a base period (for example, 2016 = 100), not absolute prices — the trend shows input-cost direction, not a price level.
- `Fruits & Veggies` — units **USD/kg**, **USD/lb**, and **USD/dozen**. Wholesale-market series from MAG, CENTA, and FAO. Tomatoes, onions, cucumbers, potatoes, and similar staples. Prices vary by variety and season; the chart shows whichever series the data feed returned for that week.
- `Livestock` — unit **USD/kg**. Cattle (`BEEF`) and broiler chicken (`CHICKEN`) farm-gate prices. Sources include the Salvadoran livestock exchanges and FAO regional series.
- `Fertilizer` — units **USD/short ton** (907.18 kg, the US convention) and **USD/metric tonne** (1,000 kg, the international convention). Urea (`URE`), DAP, TSP, MOP, and similar. Series follow US Gulf and Brazil benchmark prices; the unit on the chart depends on which benchmark is selected for that series.
- `Apiary & Honey` — unit **USD/kg**. Honey wholesale series, both Salvadoran and regional benchmarks.
- `Aquaculture` — units **USD/lb** (0.4536 kg) and **USD/kg**. Tilapia farm-gate and wholesale series from regional producers and FAO. Very large numbers are formatted as thousands (for example, `12.5K`).
- `Harvest & Storage` — unit **% of harvest**. A modelled regional statistic from **FAO SDG 12.3.1** — the share of the Central American food harvest, by mass, lost between harvest and retail. This is **not** a price; it is a loss indicator. A reading of `8.3` means about 8 of every 100 kg of food grown never reaches a consumer.
...
## **What are the Estimated and Actual markers in the chart?**

Some series carry a quality marker per point. The markers are written `(est.)` in the legend.
...
Caveat chips flag data limitations so you can interpret the chart honestly. They appear above every chart dialog. The chip labels are exactly: `Bundled snapshot`, `Saved data X old`, and `Updated X ago`.
...
- The curated CENTA, MAG, FAO, OIRSA, and OPEA document base.
...
## **Why does the chatbot sometimes answer that it does not have enough information?**
...
- The selected Quick Help or service context doesn't include any documents on the topic you asked about.
...
If you find a chart whose numbers look wrong, an FAQ answer that is out of date, or a feature that would help you in the field, mention it in your next chat session — AgroGenio forwards feedback to the project maintainers. For access, account, or login issues, contact your MAG or CENTA administrator or the project deployment team.
