...
const { retractRepoGraph, dropRepoGraphsForRepo } = require('../services/graph-retract-service');
...
// ── D-C (#981): retract-side graph teardown — graphs ONLY, meta rows KEPT ──
describe('graph-retract-service.dropRepoGraphsForRepo (retract teardown, D-C #981)', () => {
  beforeEach(() => mockDb._reset());

  test('drops the graph + member collections but NEVER touches meta/files/versions/manifest rows', async () => {
    seedRepo(GRAPH);
    seedGraphCollections();
    const querySpy = mockDb.query; // no REMOVE query may run in this path
    const dropped = await dropRepoGraphsForRepo(REPO);
    expect(dropped.length).toBeGreaterThan(0);
    expect(dropped.some((d) => d.includes('graph definition + member collections'))).toBe(true);
    expect(querySpy).not.toHaveBeenCalled();
    // the registry doc survives (the repo stays visible/editable)
    const repo = await mockDb.collection('okf_repositories').document(REPO);
    expect(repo.repo_id).toBe(REPO);
  });

  test('retracted-but-graph-bearing orphan cleanup: a MISSING repo doc is not an error (silent [])', async () => {
    // The Kenya v4 one-time cleanup runs server-side; the unit contract here is
    // that a missing registry doc yields no candidates — [] — never a throw.
    const dropped = await dropRepoGraphsForRepo('does-not-exist');
    expect(dropped).toEqual([]);
    expect(mockDb.route).not.toHaveBeenCalled();
  });
});
