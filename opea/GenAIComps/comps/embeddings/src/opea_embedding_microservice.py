...
logflag = os.getenv("LOGFLAG", True) # Change the default to False later
...
async def embedding(input: EmbeddingRequest) -> EmbeddingResponse:
    ...
    try:
        ...
        if logflag:
            logger.info(f"🥹🥹🥹Embedding microservice output: {embedding_response}🥹🥹🥹")
        print(f'🥹🥹🥹Embedding microservice output: {embedding_response}🥹🥹🥹')
