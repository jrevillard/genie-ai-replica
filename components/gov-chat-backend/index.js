    ...
    components: {
      schemas: {
            ...
            results: {
              ...
              items: {
                ...
                properties: {
                  ...
                  snippet: {
                    type: "string",
                    description: "Text snippet containing the search match",
                  },
                  matchType: {
                    type: "string",
                    description: "Type of match (title, message, etc.)",
                  },
                },
              },
            },
        ...
        ConversationStats: {
          ...
          properties: {
            ...
            starredCount: {
              type: "integer",
              description: "Number of starred conversations",
            },
            archivedCount: {
              type: "integer",
            ...
            conversationsByCategory: {
              ...
              items: {
                ...
                properties: {
                  ...
                  count: {
                    type: "integer",
                    description: "Number of conversations in this category",
                  },
                },
              },
            },
      ...
      securitySchemes: {
        KeycloakOAuth2: {
          type: "oauth2",
          description:
            "Keycloak OAuth2 authentication (Authorization Code + PKCE)",
          flows: {
            authorizationCode: {
...
const connectSrcUrls = (
  ...
  `'self' http://localhost:3000 ws://localhost:3000 ${process.env.KEYCLOAK_URL}`
).split(" ");
...
function createApp({ services = {} } = {}) {
  ...
  app.use(
    morgan(
      ":method :url :status :response-time ms - Headers: :req[content-type] :req[user-agent]",
      {
        stream: {
          write: (message) => {
  ...
  try {
    ...
    logger.info(
      "Helmet middleware applied with CSP from environment variables",
    );
  } catch (error) {
...
function registerRoutes(app, services) {
  ...
  for (const config of ROUTE_CONFIGS) {
    ...
    const extraService = config.extraServiceName
      ? services[config.extraServiceName]
      : null;

    // Skip routes that require a service when it's not provided
    if (config.serviceName && !service) {
      logger.debug(
        `Skipping ${config.file}: service ${config.serviceName} not provided`,
      );
      continue;
    }
    ...
    try {
      ...
      if (!routeInstance || typeof routeInstance.use !== "function") {
        throw new Error(
          `Route instance for ${config.file} is not a valid Express router`,
        );
      }
    ...
    for (const routePath of config.paths) {
      try {
        ...
        logger.info(
          `${config.file.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())} Module: LOADED`,
        );
        logger.debug(
        ...
        console.error(
          `[ERROR] Failed to mount route ${config.file} at ${routePath}:`,
          {
            message: error.message || "No message",
            stack: error.stack || "No stack",
            rawError: JSON.stringify(error, Object.getOwnPropertyNames(error)),
        ...
        logger.warn(
          `Skipping ${config.file} at ${routePath}: route not mounted`,
        );
      }
    }
  }
...
async function initializeServices() {
  ...
  if (isProduction) {
    ...
    const missingSecrets = requiredSecrets.filter(
      (key) =>
        !process.env[key] ||
        process.env[key].includes("default") ||
        process.env[key].includes("change"),
    );
    if (missingSecrets.length > 0) {
      logger.error("Missing or insecure secrets in production:", {
        missing: missingSecrets,
      });
      throw new Error(
        `Production requires secure values for: ${missingSecrets.join(", ")}`,
      );
    }
  }
  ...
  try {
    const defaultConnection = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () =>
            reject(new Error("Pre-initialization connection test timed out")),
          30000,
        ),
      ),
    ]);
    ...
    try {
      ...
      logger.error("Failed to get ArangoDB version:", {
        ...
        rawError: JSON.stringify(
          versionError,
          Object.getOwnPropertyNames(versionError),
        ),
  ...
  try {
    userProfileService = await importService(
      "UserProfileService",
      "./services/user-profile-service",
    );
    adminDashboardService = await importService(
      "AdminDashboardService",
      "./services/admin-dashboard-service",
    );
    analyticsService = await importService(
      "AnalyticsService",
      "./services/analytics-service",
    );
    queryService = await importService(
      "QueryService",
      "./services/query-service",
    );
    chatHistoryService = await importService(
      "ChatHistoryService",
      "./services/chat-history-service",
    );
    serviceCategoryService = await importService(
    ...
    weatherService = await importService(
      "WeatherService",
      "./services/weather-service",
    );
    securityScanService = await importService(
      "SecurityScanService",
      "./services/security-scan-service",
    );
    translationService = await importService(
      "TranslationService",
    ...
    const serviceMap = {
      serviceCategoryService: {
        instance: serviceCategoryService,
      ...
      adminDashboardService: {
        instance: adminDashboardService,
      ...
      databaseOperationsService: {
        instance: databaseOperationsService,
        name: "DatabaseOperationsService",
      },
      queryService: { instance: queryService, name: "QueryService" },
      chatHistoryService: {
        instance: chatHistoryService,
      ...
      securityScanService: {
        instance: securityScanService,
        name: "SecurityScanService",
      },
      translationService: {
    ...
    for (const [key, { instance, name }] of Object.entries(serviceMap)) {
      try {
        ...
        logger.error(`Failed to validate service ${name}:`, {
          ...
          rawError: JSON.stringify(
            validationError,
            Object.getOwnPropertyNames(validationError),
          ),
    ...
    if (
      ...
      typeof services.adminDashboardService.getSystemHealth !== "function"
    ) {
    ...
    try {
      ...
      if (
        ...
        services.queryService.setChatHistoryService(
          services.chatHistoryService,
        );
      ...
      if (
        ...
        services.chatHistoryService.setAnalyticsService(
          services.analyticsService,
        );
      ...
      if (
        ...
        typeof services.adminDashboardService.setSecurityScanService ===
          "function"
      ) {
        services.adminDashboardService.setSecurityScanService(
          services.securityScanService,
        );
