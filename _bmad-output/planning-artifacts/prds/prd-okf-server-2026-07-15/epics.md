...
| FR-44 | Epic 1 (Story 1.7) + Epic 10 (Story 10.7) | Retrieval mode governance (legacy/okf_only/hybrid) + live admin controls — *added 2026-09-10, ADR-okf-039* |
...
> **2026-09-10 amendment — retrieval mode governance ([ADR-okf-039](../../../../../docs/adr/okf-039-retrieval-mode-governance.md), FR-44):** the whole epic is **operator-gated by mode** — `legacy` (default) runs today's retriever byte-for-byte and skips every story below; `okf_only` / `hybrid` engage the fan-out only when ≥1 OKF graph is serving. Tiered option-(d) fan-out per graph (curated-spine tier-1 ≤2 hops over `source='author'` edges via the indexed `concept_id` join; extracted-relation tier-2 ≤1 hop capped), candidate provenance `{hop, edge_provenance}`, utility-cost union gate, and KH label federation are specced in the ADR; config is runtime-live from the OKF Studio tab (Story 10.7).
...
> **2026-09-10 amendment (mode gate + tiered fan-out — ADR-okf-039 D1/D4/D5):** the fan-out path engages **only** when the runtime retrieval mode is `okf_only`/`hybrid` **and** ≥1 OKF graph is serving (Story 1.7 config read); `legacy` mode never reaches this code. Within each OKF graph, expansion is **tiered**: tier-1 = curated spine (chunk hit → `concept_id` indexed join to the concept-root vertex → `source='author'` edges ≤ `spine_max_hops`); tier-2 = extracted relations ≤ `extracted_hop_cap`, capped. Every candidate carries `{hop, edge_provenance: vector|bm25|author|parser|label}`; the merged pool passes the utility-cost gate (D5) with hard caps (`candidate_cap_per_graph`/`candidate_cap_global`) before fusion. Regression guard: the single-graph legacy call and `legacy` mode produce **identical results and spans** to pre-Epic-1 code (CI-asserted on seed fixtures).
...
> **2026-09-10 amendment (utility-cost co-deliverable — strategy §5 Phase A, ADR-okf-039 D5):** the graph-set cut is a **utility-cost selection**: `R_g` from discovery scores normalized against their own distribution, graph-level complementarity `K_g` (clone lineage + concept fingerprint + same-domain), fan-out cost `C_g`, greedy include while `V_g > 0` — with `MAX_FANOUT_GRAPHS` and the ≤20ms gate as hard stops. `legacy` mode skips the router entirely (no added latency on the production path); `okf_only`/`hybrid` run it per the Story 1.7 config.
...
### Story 1.7: Retrieval mode governance — runtime config + engagement gate *(NEW 2026-09-10; gated with Epic 1; config service + okf-server API legs ungated)*
As a **platform operator**,
I want **retrieval to run legacy-only until I explicitly enable OKF grounding — and to switch legacy / okf_only / hybrid live, without a redeploy**,
So that **legacy chat is provably untouched by Epic 1 and OKF adoption is an explicit, audited operator act**.
**Acceptance Criteria:** **Given** the `okf_system_config` collection (doc `_key='retrieval'`), **When** ChatQnA resolves the effective retrieval config, **Then** okf-server serves `GET /api/okf/retrieval-config` returning `{config, serving_graph_count, serving_repo_ids}` (env vars are boot defaults; DB overrides; ≤30s TTL cache; last-known-good on failure), **and** the fan-out path engages only when `mode ≠ legacy` AND `serving_graph_count ≥ 1` — `legacy` mode runs the single-`graph_name` path with no router, no authz enumeration, no new spans (byte-identical behavior, CI-asserted), **and** `okf_only` with zero serving graphs yields zero OKF contribution + a structured span warning (never a silent legacy fallback), **and** `PUT /api/okf/retrieval-config` (admin-scoped) validates mode/caps, bumps `revision`, and writes the before→after to `okf_audit_logs`. *(FR-44; ADR-okf-039 D1–D3; ADR-okf-029 audit; config-service + API legs are ungated Node work that can land ahead of the bump.)*
...
### Story 10.7: Retrieval controls — live mode + config card in the OKF Studio tab *(NEW 2026-09-10; ungated frontend + wiring; depends on Story 1.7's API)*
As an **administrator**,
I want **to see and change the live retrieval posture (legacy / okf_only / hybrid) from the OKF Studio tab**,
So that **OKF grounding is something I switch on deliberately — and can tune or revoke — while the system serves traffic**.
**Acceptance Criteria:** **Given** the OKF Studio tab, **When** the admin opens the **Retrieval card**, **Then** it shows the effective config (`mode`, revision, `updated_by/at`), the **live serving-graph count** + repo list (so `okf_only` with zero serving repos is visibly a foot-gun before it is chosen), a mode selector with consequence text per choice, an "Advanced" disclosure for the caps/params (`max_fanout_graphs`, `spine_max_hops`, `extracted_hop_cap`, candidate caps, `utility_gate`, `label_federation`), and the config **audit trail** (from `okf_audit_logs`), **and** saving issues an admin-scoped `PUT /api/okf/retrieval-config` with a before→after confirmation dialog, surfaces validation errors inline, and refreshes the card from the GET (≤30s live effect). DS primitives + tokens only (no raw elements — the zero-inconsistency rule); every state served by a `repoOkfService` method. *(FR-44; ADR-okf-039 D7; UX paradigms per Epic 3/10; depends on Story 1.7.)*
