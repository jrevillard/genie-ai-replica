...
describe('crawlerToOkfService — convertCrawlToOkf (202 trigger)', () => {
  ...
  it('passes the classification strategy through and defaults to heuristics', async () => {
    mockCreated();
    await crawlerToOkfService.convertCrawlToOkf({ fileId: 'f-1', classification: 'hybrid' });
    expect(mockPost.mock.calls[0][1].classification).toBe('hybrid');
    mockCreated();
    await crawlerToOkfService.convertCrawlToOkf({ fileId: 'f-2' });
    expect(mockPost.mock.calls[1][1].classification).toBe('heuristics');
  });
