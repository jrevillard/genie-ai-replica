...
- **Knowledge Hierarchy** — the curated, bounded, steward-gated service-category taxonomy that the labeler resolves concept labels against (`_fetch_all_labels` → `GET /api/service-categories/categories`). It grows deliberately via **Label Onboarding** (FR-36), never automatically. `service-categories` is the canonical store; the document-repository `labels` collection is one-way-synced from it ([ADR-okf-034](../../../../docs/adr/okf-034-knowledge-hierarchy-canonical-store.md)). **Two-level usage rule (David, 2026-09-05):** the hierarchy's **level 1 (categories)** categorizes repositories — the **Subject Area** in the UI, bound at creation; its **level 2 (services)** labels the underlying topic files (concepts). Concept labels are selected **only** from level 2.
...
An authenticated steward (`tools-admin`) can **create, read/list, update, and delete** repositories. Creating a repository mints `repo_id` and reserves `graph_name = OKF_{repo_id}` (graph created on first ingest); the repository is bound to a **Subject Area (domain)** — the **Knowledge Hierarchy level 1 (categories)**, selected in the UI at creation and immutable thereafter (the ACL scope is born slugged from it). Deleting a repository cascades — retracts its entire graph + metadata, audited, with a confirmation + retention grace. Realizes UJ-1, UJ-3. ([ADR-okf-014](../../../../docs/adr/okf-014-repository-model.md), [ADR-okf-002](../../../../docs/adr/okf-002-shared-graph-multi-tenancy.md))
**Consequences:**
...
Concepts are parsed (frontmatter → metadata; header-aware chunking preserving header context) and indexed into the repository's own graph `OKF_{repo_id}` via dataprep — reusing TEI embeddings and the `{graph}_SOURCE/_ENTITY/_HAS_SOURCE/_LINKS_TO` schema, with additive OKF metadata (`concept_id`, `bundle_version`, `source_type:"okf"`). On bundle settle (every concept terminal), the okf-server writes a **bundle manifest** (`okf_bundle_manifest`, one doc per repo): the concept list, the author-stated links, the root (`type: index`), deterministic summary stats, and lineage — the tier-1 discovery index for FR-35 ([ADR-okf-035](../../../../docs/adr/okf-035-bundle-manifest-and-author-graph.md)). The bundle zip's doc-repo doc follows a real **state machine** `Pending → Ingesting → Ingested | Ingestion Error`, owned by the okf-server completion-callback controller (never born 'Ingested'). Realizes UJ-1, UJ-2. ([ADR-okf-010](../../../../docs/adr/okf-010-okf-markdown-loader-location.md), [ADR-okf-013](../../../../docs/adr/okf-013-graph-name-wiring.md))
**Consequences:**
...
- A settled repository is self-describing: any client reads its manifest to learn its concepts, links, and root without a meta-collection scan.

#### FR-7: Structural link graph (dual-source: author + parser)
Edges are **dual-source**: (a) the bundle author's frontmatter `links: [...]` array — mirrored into `OKF_{repo_id}_LINKS_TO` at settle with `source='author'` (authoritative structure; weight; within-repo/G22-enforced); and (b) parser-derived Markdown cross-links — resolved to concept IDs with the link's **anchor text** (`label`), keying (`file_id`/`repo_id`), `source='parser'`. Broken links are tolerated (OKF §5). Graph walks read either source; the `source` field discriminates ([ADR-okf-035](../../../../docs/adr/okf-035-bundle-manifest-and-author-graph.md)). Realizes UJ-2.
**Creation-time link authoring (David, 2026-09-05):** when a repository is created from a crawl or other cross-linked markdown, the links between topics are part of the expected processing: the **crawl source preserves inter-page links** (crawler requirement), and the **conversion resolves them into OKF v0.2 frontmatter `links: [...]`** between the repository's concepts (label = anchor text). Self-references are dropped; targets outside the crawled corpus are **dropped from the graph** (they remain as plain markdown body text). Imported bundles may also carry wiki-style body links (`[[path|label]]`) and `relations` frontmatter, which conversion resolves into `links: [...]` the same way — so every OKF repository is **born with its author graph**.
**Spine completion (2026-09-10, verified live on the first serving graph):** the author graph must be **retrieval-ready**, not merely present — (a) author edges carry the curator's `label` (anchor text) and declared `kind` in BOTH edge writers (settle mirror + drain rewrite; the live graph had `label: null`); (b) concept-root vertices are enriched with `title` + `labels` (KH) so spine walks self-describe; (c) a persistent index on the serving `_SOURCE.concept_id` makes the chunk→root join O(log n) — the join stays attribute-based (ADR-okf-039 D4 rejects per-chunk edges). Existing graphs self-heal on the next settle/re-ingest ([ADR-okf-035](../../../../docs/adr/okf-035-bundle-manifest-and-author-graph.md) revision 2026-09-10, [ADR-okf-039](../../../../docs/adr/okf-039-retrieval-mode-governance.md) D4).
**Consequences:**
- An agent can traverse a concept's neighbors/backlinks via parameterized graph traversal, retaining *why* concepts link (anchor text) and *who declared* the link (author vs parser).
...
#### FR-34: Async import pipeline (write-side orchestration)
The OKF Server owns an **async, per-concept, idempotent** import pipeline: `POST /api/okf/repos/:repo_id/import` (import is the creation word; the old `/ingest` concept route is retired with 410) → the write-side orchestrator resolves the repo, derives `graph_name`/ACL labels/`bundle_version`, unzips the bundle (zip of `.md`), and per concept parses → UPSERTs `okf_concepts_meta` → runs conformance → PII → content-hash dedup → leaves the meta row at `index_status='parsed'` (the rows themselves are the queue — the drain worker claims only repos whose RAG ingest the lifecycle has explicitly armed) → returns **202** (the HTTP call never blocks on dataprep). An ingestion worker (Redis Streams, concurrency 1 / configurable, DLQ) drains `Pending` jobs through doc-repo → dataprep → per-repo graph creation, writes `_LINKS_TO` edges, and transitions `index_status`; a sweeper reconciles orphan chunks. There is **no distributed transaction** — compensation is via the sweeper + per-concept `index_status`. Realizes the store→pending→worker→graph model; [ADR-okf-021](../../../../docs/adr/okf-021-write-side-orchestration.md), [ADR-okf-022](../../../../docs/adr/okf-022-node-python-dataprep-handoff.md).
**Consequences:**
...
A repository has an explicit lifecycle state machine (`draft` — or `register` for crawl-born repos — `→ review → approve → publish → retracted`), driven by five steward actions: **submit / approve / publish / ingest / retract** — David's six-step contract: **Import → Edit → Review/Approve → Publish → Ingest → Retract**. Vocabulary rule (David, 2026-09-04): **import is the creation word, ingest is the RAG word** — concepts are *imported* (POST `/import`); *ingesting* is the RAG step that arms the drain and, at completion, promotes the version to serving (`ingested_version` set; "serving" is not a state). `version` is a publish side-effect, never a state (ADR-030/031). A repository with a running conversion is **building**: it refuses every transition until the conversion settles (retract exempt). Only `published` (and, for RAG, serving) content is served to agents. Realizes UJ-1, UJ-3. ([ADR-okf-030](../../../../docs/adr/okf-030-lifecycle-state-machine.md))
**Consequences:**
- `review`/`draft` content is invisible to non-steward agent queries.
- State transitions are auditable (who/when) and role-restricted; invalid transitions return 409 with the allowed next states.
...
Before a repository is indexed, a **pre-ingest dry-run** surfaces the **minimal label gap** — the labels this repo's concepts need that are not yet in the Knowledge Hierarchy — and a **steward wizard** (Analyze → Cluster → Review → Apply) curates them with smart defaults and a live before/after coverage preview. It is **bounded by construction**: gap-only input (only under-labeled concepts), lexical + embedding variant auto-merge (no duplicates / no synonym bloat), frequency + confidence floors, a learned per-domain denylist, a FAIL-not-truncate hard cap, and the FR-32 steward gate (no auto-create). The hierarchy grows deliberately with exactly the labels a repo needs — never "every imaginable word." **Label flow rule (David, 2026-09-05):** concept labels (Knowledge Hierarchy **level 2 — services**) are selected in the wizard/editor **before publish**, and they **label the data at dataprep ingestion**: every label write lands in the meta row's `ingest_labels`, the drain posts them as `fileLabels`, and dataprep folds them into every chunk's labeling (LLM context, fallback set, final chunk labels) — a label edit made any time before the ingest transition is honored at the next drain without re-embedding. **Bounding rule (David, 2026-09-05):** the labels offered for a repository's concepts are only the services **under the repository's Subject Area** (its KH level-1 category); legacy domains with no KH match fall back to the full tree. **Import categorization (David, 2026-09-05):** the import process attempts to determine each concept's OKF type (`topic`/`entity`/`event`/`source`), via a **user-selected strategy — Heuristics (default), LLM-assisted, or Hybrid** — written into frontmatter and refined during editing. Realizes the bounded-hierarchy principle; [ADR-okf-033](../../../../docs/adr/okf-033-label-onboarding.md), [ADR-okf-034](../../../../docs/adr/okf-034-knowledge-hierarchy-canonical-store.md).
**Consequences:**
...
**Mode governance (2026-09-10):** the fan-out path is **operator-gated, never automatic** — the retrieval mode (`legacy | okf_only | hybrid`, default `legacy`) decides whether the legacy `GRAPH` path, the OKF serving graphs, or both are served; `legacy` mode runs today's path unchanged (no router, no fan-out machinery), and fan-out engages only when the mode allows it **and** at least one OKF graph is serving (FR-44, [ADR-okf-039](../../../../docs/adr/okf-039-retrieval-mode-governance.md)).
**Consequences:**
...
A **Graph Router** selects the **relevant** subset of the caller's authorized graphs before retrieval: it detects the query domain (domain binding) and ranks candidate repos via **tier-1 manifest discovery** (`POST /api/okf/repos/discovery`): label-overlap + name/domain token scoring over the settled `okf_bundle_manifest` index — O(repos), not a meta-collection scan ([ADR-okf-035](../../../../docs/adr/okf-035-bundle-manifest-and-author-graph.md) supersedes the BM25-over-meta approach). It intersects the result with the authorized set and caps it at `MAX_FANOUT_GRAPHS` (default 5, configurable). Selection is **distinct from authorization** (the Authz Resolver, FR-18, produces the authorized set; the router selects within it). The selection-latency budget (≤20ms) is a **gate, enforced in CI** against seed fixtures, not an aspiration. The retriever then fans out across the selected set in a single query where the schema allows, with 2-level RRF fusion (FR-24). Realizes the multi-repo scaling requirement; [ADR-okf-024](../../../../docs/adr/okf-024-graph-selection-router.md) (tier-1 mechanism per ADR-okf-035).
**Consequences:**
...
**Utility-cost selection (2026-09-10, co-deliverable of Story 1.3):** the router's graph-set cut is a **utility-cost selection** — relevance-normalized graph ranking with graph-level complementarity and fan-out cost, greedy with hard caps ([strategy: utility-cost adaptive context selection](../../../utility-cost-context-selection-strategy-2026-08-18.md) §5 Phase A) — and the fused candidate pool is cut by the same framework at chunk level (per-candidate `V = U − C > 0`, graph provenance as utility prior), behind `utility_gate.enabled` and never in `legacy` mode ([ADR-okf-039](../../../../docs/adr/okf-039-retrieval-mode-governance.md) D5).

#### FR-44: Retrieval mode governance & live admin controls
The deployment's retrieval posture — `legacy` (free-form `GRAPH` corpus only, the default; today's path byte-for-byte), `okf_only` (OKF serving graphs only), or `hybrid` (both, fused) — is **operator-owned runtime configuration**, not code: stored in `okf_system_config` (env vars are boot defaults only), read by ChatQnA/retriever with a ≤30s TTL cache (last-known-good on failure), written via an admin-scoped, validated, **audited** okf-server API, and manipulated live from the **OKF Studio admin tab** (mode selector with consequence text, live serving-graph count, cap editors, audit trail — DS primitives only). Fan-out engages only when `mode ≠ legacy` **and** ≥1 OKF graph is serving; `okf_only` with zero serving graphs yields zero OKF contribution plus a structured warning, never a silent legacy fallback. Realizes the coexistence requirement that legacy ingestions and serving OKF graphs can coexist under explicit operator control. ([ADR-okf-039](../../../../docs/adr/okf-039-retrieval-mode-governance.md))
**Consequences:**
- Merging Epic 1 cannot change production chat behavior — `legacy` is the default and skips the router and fan-out machinery entirely.
- A deployment can pilot OKF grounding in `okf_only` before enabling `hybrid`, with no redeploy (live config, ≤30s effect, audited).
- Mode/cap changes are traceable (actor, before→after, revision) in the same audit spine as every steward action.
