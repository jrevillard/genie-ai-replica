...
const createPasswordResetTokens = async (db, totalUsers) => {
    console.log('Creating password reset tokens for 5% of users...');
    ...
    try {
        ...
        try {
            ...
            console.log('passwordResetTokens collection does not exist, creating it...');
        }

        if (!collectionExists) {
            await db.createCollection('passwordResetTokens');
            console.log('Created passwordResetTokens collection');
            ...
            console.log('Created unique index on token field');

            await resetTokensCollection.ensureIndex({
                type: 'persistent',
                fields: ['expiresAt']
            });
            console.log('Created index on expiresAt field');

            await resetTokensCollection.ensureIndex({
                type: 'persistent',
                fields: ['used']
            });
            console.log('Created index on used field');
        } else {
            console.log('passwordResetTokens collection already exists');
            // Truncate collection
            await resetTokensCollection.truncate();
            console.log('Truncated passwordResetTokens collection');
        }

        // Calculate 5% of users
        const tokenCount = Math.ceil(totalUsers * 0.05);
        console.log(`Creating ${tokenCount} tokens for ${totalUsers} users`);
        ...
        for (let i = 0; i < tokenCount; i++) {
            ...
            let userKey;
            do {
                userKey = randomInt(1, totalUsers);
            } while (userKeys.includes(userKey));
        ...
        for (let i = 0; i < tokenCount; i++) {
            ...
            if (i % 10 === 0) {
                console.log(`Created ${i} password reset tokens so far...`);
            }
        }

        console.log(`Created ${tokenCount} password reset tokens successfully`);
        ...
        try {
            ...
            console.error('Error getting token statistics:', err.message);
        }

    } catch (err) {
        console.error('Error creating password reset tokens:', err.message);
    }
};

// Create required indexes for improved query performance - updated with authentication indexes
const createIndexes = async (db) => {
    console.log('Creating necessary indexes for query performance...');

    const indexConfigs = [
        // Authentication-related indexes
        { collection: 'users', fields: ['loginName'], name: 'idx_users_loginName', type: 'persistent', unique: true },
        { collection: 'users', fields: ['email'], name: 'idx_users_email', type: 'persistent', unique: true },
        { collection: 'users', fields: ['accessToken'], name: 'idx_users_accessToken', type: 'persistent', unique: false },
        // Password reset token indexes
        { collection: 'passwordResetTokens', fields: ['userId'], name: 'idx_reset_tokens_userId', type: 'persistent' },
        { collection: 'passwordResetTokens', fields: ['token'], name: 'idx_reset_tokens_token', type: 'persistent', unique: true },
        { collection: 'passwordResetTokens', fields: ['expiresAt'], name: 'idx_reset_tokens_expiresAt', type: 'persistent' },
        { collection: 'passwordResetTokens', fields: ['used'], name: 'idx_reset_tokens_used', type: 'persistent' },
...
const generateTestData = async () => {
    try {
        ...
        const passwordResetTokens = await ensureCollection('passwordResetTokens');
        ...
        await passwordResetTokens.truncate();
        ...
        // Generate users with authentication properties (200 instead of 2000, still more than max monthly users to simulate churn)
        console.log('Creating users with authentication properties...');
        ...
        for (let i = 0; i < totalUsers; i++) {
            ...
            // Create a loginName (either use email or username format)
            const useEmailAsLogin = Math.random() > 0.3; // 70% use email as login
            const loginName = useEmailAsLogin ? email : `user_${userKey}`;

            // Randomly determine if user has an active token (80% of users)
            const hasActiveToken = Math.random() > 0.2;
            const accessToken = hasActiveToken ? generateAccessToken() : null;
        ...
        console.log(`Created ${totalUsers} users with authentication properties`);
        ...
        // Create password reset tokens for 5% of users
        await createPasswordResetTokens(db, totalUsers);
...
- All users have loginName field
- 80% of users have accessToken
- Sessions: ${totalSessions}
- Queries: ${totalQueries}
- Analytics Records: ~${Math.round(totalQueries * 1.25)} (queries + feedback)
- Password Reset Tokens: ${Math.ceil(totalUsers * 0.05)} (5% of users)
...
`;
            ...
            // Test for password reset tokens
            console.log('Testing password reset tokens...');
            const resetTokensQuery = `
RETURN {
    totalTokens: LENGTH(FOR t IN passwordResetTokens RETURN 1),
    activeTokens: LENGTH(FOR t IN passwordResetTokens FILTER t.used == false && t.expiresAt >= "${createCompatibleTimestamp(new Date())}" RETURN 1),
    expiredTokens: LENGTH(FOR t IN passwordResetTokens FILTER t.used == false && t.expiresAt < "${createCompatibleTimestamp(new Date())}" RETURN 1),
    usedTokens: LENGTH(FOR t IN passwordResetTokens FILTER t.used == true RETURN 1),
...
`;
            ...
            try {
                const resetTokensCursor = await db.query(resetTokensQuery);
                const resetTokensStats = await resetTokensCursor.next();
                console.log('Password reset tokens statistics:');
                console.log(JSON.stringify(resetTokensStats, null, 2));

                // Verify our target was met
                const expectedTokens = Math.ceil(totalUsers * 0.05);
                if (resetTokensStats.totalTokens === expectedTokens) {
                    console.log(`✅ Password reset tokens target achieved: ${resetTokensStats.totalTokens} tokens (target: ${expectedTokens})`);
                } else {
                    console.log(`⚠️ Password reset tokens target not met: ${resetTokensStats.totalTokens} tokens (target: ${expectedTokens})`);
                }
            } catch (err) {
                console.error('Error testing password reset tokens:', err.message);
            }
