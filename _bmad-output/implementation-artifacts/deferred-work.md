...
## Deferred from: code review of 4-1-two-keycloak-roles-and-require-role (2026-08-31)

- **Double authenticate per tools request** — `index.js` mounts `keycloakAuthMiddleware.authenticate` app-level for `keycloakAuth: true` routers (index.js:930) AND `tools-routes.js:19` adds `router.use(authenticate)` inside the router → token verified + user provisioned twice on every `/api/admin/tools/*` request. Pre-existing (line unchanged by story 4-1); idempotent, cost is one extra verifyToken+provisionUser round per request. Fix opportunity: drop the router-level `router.use(authenticate)` (keep the index.js-level one) when story 4-2 touches the mount, or dedupe in a middleware cleanup.
- **admin-routes debug middleware logs full request headers including bearer token** — `routes/admin-routes.js:34-40` logs `{ headers: req.headers, ... }` at info level for every request traversing `/api/admin`, which includes the Authorization header. Pre-existing; violates the project's "sensitive logs auto-masked, do not bypass" rule. After the 4-1 mount-order fix, tools traffic no longer traverses this middleware, but all other `/api/admin/*` traffic still does. Fix: mask or drop headers from that log line.

## Deferred from: code review of 2-7-degradation-and-transparent-insufficiency (2026-08-31)

- **Blocking `search_sync` on the event loop** — `genie-ai-overlay/workflows/tools/web_search.py:35` does a synchronous 10s-timeout `requests.get` inside `align_outputs` (async orchestration path), stalling ALL concurrent requests on the chatqna instance when SearXNG is slow. Pre-existing (identical in pre-2-7 code); the async `search()` variant already exists in the same module. Fix when wiring governance/timing budgets (2-4 territory) — swap to `await backend.search(...)` at the seams.
- **Mid-stream client disconnect loses the metadata SSE event** — degradation (and confidence/source docs) ride the post-token `metadata` event emitted by `_stream_with_metadata` after the token loop; a client disconnect mid-stream cancels the generator and the notice is never sent. Pre-existing channel behavior; matters most for AC2/AC5 (the "I can't answer" notices). Fix options: emit degradation earlier (before tokens) or via a final non-SSE channel — decide in 2-8 (SSE contract).
