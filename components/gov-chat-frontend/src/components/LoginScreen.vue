  ...
  <div class="login-container" :data-theme="theme">
    <div class="login-card">
      ...
      <div class="register-account">
        <p>{{ $t('login.noAccount') }} <router-link to="/register" class="register-link">{{ $t('login.registerNow')
            }}</router-link></p>
      </div>
      ...
      <div v-if="savedAccounts.length > 0" class="saved-accounts">
        ...
        <div class="accounts-container">
          <div v-for="account in savedAccounts" :key="account.id" class="account-item"
            @click="loginWithSavedAccount(account)">
            <div class="account-left">
              <div class="account-initials">{{ account.name.charAt(0) }}{{ account.name.split(' ')[1]?.charAt(0) || ''
                }}</div>
...
.saved-accounts h3 {
  font-size: 15px;
  color: #fff; /* Change from #ddd to #fff */
  margin: 0 0 10px 0;
  font-weight: 500;
}
