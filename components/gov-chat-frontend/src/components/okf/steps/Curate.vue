  ...
  <div class="okf-step-curate">
    ...
    <div class="okf-step-curate__detail">
      <div v-if="selectedConcept" class="okf-step-curate__detail-inner">
        ...
        <p v-if="labelError" class="okf-step-curate__label-error">{{ labelError }}</p>
  ...
  methods: {
    async onLabelsSave(labels) {
      ...
      try {
        ...
        // Labels NEVER vanish silently (David, 2026-09-05): a REPO_READ_ONLY
        // 409 on a serving repo must surface "retract to edit", not disappear.
        const code = (err && (err.code || (err.data && err.data.error))) || '';
        this.labelError = okfRepoOps.friendlyLifecycleError(
          code,
          err && err.message,
          this.translate('okf.glossary.saveFailed', 'Save failed')
        );
        // Roll the local mirrors back — the chips must not show a label the
        // server refused.
        if (c) c.labels = previous.slice();
        this.selectedLabels = previous.slice();
      }
    }
  }
};
