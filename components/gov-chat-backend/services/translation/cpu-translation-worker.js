...
async function initWorker() {
  ...
  try {
    ...
    logger.error(`[CPU-WORKER] Initialization failed: ${error.message}`, {
      stack: error.stack,
    });
...
async function handleTranslate(messageId, texts, sourceCode, targetCode) {
  ...
  try {
    ...
    logger.info(
      `[CPU-WORKER] Translated ${texts.length} texts in ${duration}ms`,
    );
...
parentPort.on("message", async (message) => {
  ...
  try {
    ...
    logger.error(`[CPU-WORKER] Message handler error: ${error.message}`, {
      stack: error.stack,
    });
...
process.on("uncaughtException", (error) => {
  logger.error(`[CPU-WORKER] Uncaught exception: ${error.message}`, {
    stack: error.stack,
  });
  parentPort.postMessage({
    type: "error",
