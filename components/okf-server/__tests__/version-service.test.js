...
describe('versionService.mintVersion (ADR-031: repo-level, monotonic, INSERT-only)', () => {
  test('first mint: null → 1, manifest snapshotted with stored hashes, repo bumped + tagged', async () => {
    ...
    expect(writeAudit).toHaveBeenCalledWith(
      expect.objectContaining({ action: 'repo.version_mint', actor: 'steward-1', repo_id: REPO })
    );
  });
  ...
  test('concurrent-mint race: unique violation on first insert → re-read, retry once, succeed', async () => {
    seedRepo({ version: 4 });
    programQuery([{ concept_id: 'x', title: 'X', content_hash: 'h', index_status: 'indexed' }]);
    programQuery([{ concept_id: 'x', title: 'X', content_hash: 'h', index_status: 'indexed' }]); // retry re-snapshots
    // First save collides with a concurrent mint (unique [repo_id, bundle_version]).
    // Simulate the winner having bumped the store to 5 — the retry's document()
    // re-read then mints 6.
    // Detach the ORIGINAL implementation (getMockImplementation returns the
    // underlying fn — calling it never re-enters the mock; capturing the jest.fn
    // itself would recurse infinitely).
    ...
    versionsHandle.save.mockImplementation(async (doc) => {
      if (!collided) {
        collided = true;
        mockDb._stores.okf_repositories[REPO].version = 5; // the concurrent winner
