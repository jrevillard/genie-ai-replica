...
// metrics-middleware.js — Express middleware that records custom HTTP metrics (shared).
// Instruments: http_requests_total (counter), http_request_duration_seconds (histogram).
// Metrics flow via the OTel MeterProvider (configured in tracing.js) → OTLP → Collector
// → VictoriaMetrics. Uses route template patterns to prevent cardinality explosion.
