  ...
  <div class="okf-shell">
    <header class="okf-shell__header">
      ...
      <span v-if="repo.version" class="okf-shell__version">
        ...
        <DsInfoTip
          :text="
            translate(
              'okf.glossary.version',
              'A frozen snapshot of a repository at publish time. Editing continues on the next version — published versions never change.'
            )
          "
        />
      </span>
      <DsPill v-if="serving" variant="success">
        {{ translate('okf.shell.serving', 'Serving') }}
        <DsInfoTip
          :text="
            translate(
              'okf.glossary.serving',
              'This version is live: the assistant reads its content when answering. Retract it to make changes.'
            )
          "
        />
      </DsPill>
      ...
      <div class="okf-shell__actions">
        ...
        <!-- INGESTION ⓘ (David, 2026-09-06): only when the next transition IS
             the ingest step — the tip describes that one transition. -->
        <DsInfoTip
          v-if="!building && lifecycleAction === 'ingest'"
          :text="
            translate(
              'okf.glossary.ingestion',
        ...
        >
          {{ translate('okf.shell.deleteLabel', 'Delete') }}
        </DsButton>
      </div>
    </header>
    <p v-if="actionError" class="okf-shell__error">{{ actionError }}</p>

    <!-- BUILDING PREVIEW (David, 2026-09-05): while the source file is still
      converting there is NO reviewable content yet — render the live import
      progress card instead of the wizard/editor (whose concepts/manifest/
      links/drafts fetches would all fire against a half-built repo) and
      swap to the real tabs the moment the conversion reaches a terminal
      state. A FAILED conversion shows an actionable error instead. -->
    <div v-if="converting" class="okf-shell__building">
      <BuildProgressCard :repo="repo" />
    </div>
    <div v-else-if="conversionFailed" class="okf-shell__building-failed">
      <p class="okf-shell__building-failed-title">
        {{ translate('okf.glossary.importFailedTitle', 'The import failed — the source file could not be converted.') }}
      </p>
      <p v-if="repo.conversion && repo.conversion.error" class="okf-shell__error">{{ repo.conversion.error }}</p>
      <p class="okf-shell__building-failed-hint">
        {{ translate('okf.glossary.importFailedHint', 'Delete this repository and import the source file again.') }}
      </p>
    </div>
    <DsTabs v-else :tabs="subTabs" :model-value="subTab" @update:model-value="onSubTab">
      <template #default>
        <!-- :key = REPO SWITCH REMOUNTS (David, 2026-09-06, cross-repo
             stale-state class): without it the editor/wizard instances are
             REUSED across repos — openEditor never re-runs, per-repo local
             state (selection, concept lists, wizard steps) leaks, and
             one repo's id can pair with another repo's concept (the
             observed GET /repos/<old>/concepts/<new-concept> 404s). -->
        <OkfStudioWizard
          v-show="subTab === 'wizard'"
    ...
    >
      ...
      <div v-if="piiBlocked" class="okf-shell__pii">
        <!-- CORRECTION PATH (David, 2026-09-09: "where do I go from here?"):
             the flagged files are CLICKABLE — one click opens the file in the
             editor, where the Flagged entities panel shows every occurrence
             in context. Saving re-scans; the list refreshes on next publish. -->
        <template v-if="flaggedConcepts.length">
          <p class="okf-shell__pii-note">
            {{ translate('okf.shell.pii.reviewHint', 'Click a flagged file to correct it in the editor:') }}
          </p>
          <ul class="okf-shell__pii-list">
            <li v-for="c in flaggedConcepts" :key="c.concept_id">
              <button type="button" class="okf-shell__pii-link" @click="openFlaggedConcept(c.concept_id)">
...
export default {
  ...
  computed: {
    ...mapGetters('okf', ['editorSubTab', 'repoById', 'conceptsByRepo']),
    // CORRECTION PATH: the repo's flagged concepts (from the fetched list —
    // pii_state rides the list projection), shown clickable in the dialog.
    flaggedConcepts() {
      return this.conceptsByRepo(this.repoId).filter((c) => c && c.pii_state === 'hit');
    },
    subTab() {
    ...
    converting() {
      // BUILDING PREVIEW: the source file is still converting — no reviewable
      // content exists yet, so the tabs stay replaced by the progress card.
      // Terminal vocabulary single-sourced with isBuilding ('done'|'failed').
      const c = this.repo.conversion;
      return !!(c && !['done', 'failed'].includes(c.status));
    },
    conversionFailed() {
      const c = this.repo.conversion;
  ...
  mounted() {
    // Same store poll as the dashboard (the 11-hour ghost fix): while the
    // repo builds, refresh the list every 5s so the building panel swaps to
    // the real editor the moment the conversion reaches a terminal state.
    this.pollTimer = setInterval(this.pollBuilding, 5000);
  },
  beforeUnmount() {
    if (this.pollTimer) {
  ...
  methods: {
    async pollBuilding() {
      ...
      if (!okfRepoOps.isBuilding(this.repo)) return;
      this.pollBusy = true;
      try {
    ...
    async onLifecycle() {
      ...
      if (!res.ok) {
        this.actionError = okfRepoOps.friendlyLifecycleError(res.code, res.message);
        return;
      }
      // P0-UI (David, 2026-09-08, verbatim): "when the Ingest button is
      // clicked, there must be some reaction/response/indication" — the
      // editor CLOSES and the dashboard takes over: the card sits in the
      // Published lane with the Re-draining chip + spinner, and the store
      // poll keeps it fresh until the drain settles.
      if (action === 'ingest') {
        this.$emit('refresh');
        this.$emit('back');
        return;
    ...
    // One click = the flagged file opens in the editor (the Flagged entities
    // panel shows every occurrence); the publish dialog closes.
    openFlaggedConcept(conceptId) {
      this.publishOpen = false;
...
.okf-shell__building-failed {
  ...
  background: var(--warning-bg, var(--bg));
}
.okf-shell__building-failed-title {
  margin: 0;
  font-size: var(--text-sm);
  font-weight: 600;
  color: var(--fg);
}
.okf-shell__building-failed-hint {
  margin: 0;
  font-size: var(--text-sm);
  color: var(--muted);
}
