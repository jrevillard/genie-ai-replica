...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Inspect installed BMad manifests or materialize and repair {project-root}/_bmad."
        )
    )
...
def doctor(
    ...
    *,
    ...
    return {
        ...
        "answers_added": [
            {"module": question.module, "key": question.key} for question in pending
        ],
        "modules": [
            {
                **selection,
        ...
        "legacy_leftovers": [
            ...
            if (project_root / "_bmad")
            .joinpath(*PurePosixPath(relative).parts)
            .exists()
        ],
...
def discover_installed_copies(skill_root: Path) -> tuple[InstalledCopy, ...]:
    ...
    for sibling in siblings:
        ...
        try:
            ...
            raise Exception(
                f"cannot read installed manifest {path}: {error}"
            ) from error
        manifests.append(
            InstalledCopy(
                sibling.name,
...
def parse_packaged_manifest(path: Path, raw: bytes) -> ParsedManifest:
    ...
    if prefix == "github:":
        ...
        if len(github_parts) < 3 or any(not part for part in github_parts):
            raise Exception(
                f"packaged manifest {path} field 'update_source' github source must name owner/repo/path"
            )
    if prefix == "https://" and any(character.isspace() for character in update_source):
        raise Exception(
            f"packaged manifest {path} field 'update_source' must be a valid HTTPS URL"
        )
...
def parse_manifest_questions(
    ...
    for index, question in enumerate(value):
        ...
        if not key or any(not part or part != part.strip() for part in key.split(".")):
            raise Exception(
                f"packaged manifest {path} field {field}.key must be a non-empty dotted key"
            )
        if key == module or key.startswith(f"{module}."):
            raise Exception(
                f"packaged manifest {path} field {field}.key {key!r} must not start with module {module!r}"
            )
        conflict = conflicting_question_key(seen, key)
        if conflict is not None:
            raise Exception(
                f"packaged manifest {path} config question key {key!r} conflicts with {conflict!r}"
            )
        seen.append(key)
...
def read_declared_script(
    ...
    if not resolved.is_file():
        raise Exception(
            f"packaged manifest {manifest} declared script {relative.as_posix()!r} is not a file"
        )
    try:
...
def update_report(project_root: Path, skill_root: Path) -> dict[str, object]:
    ...
    for module in sorted(grouped):
        copy_reports = [
            update_copy_report(project_root, copy_item) for copy_item in grouped[module]
        ]
...
def source_manifest_location(project_root: Path, copy_item: InstalledCopy) -> str:
    ...
    if update_source.startswith("https://"):
        try:
            ...
            raise Exception(
                f"invalid update_source {update_source!r} in {copy_item.manifest}: {error}"
            ) from error
        return urllib.parse.urlunsplit(
            parsed._replace(
                path=(parsed.path.rstrip("/") + f"/{quoted_skill}/{quoted_manifest}")
            )
        )
            ...
            selections.append(
                {
                    **base,
                    "state": "blocked",
                    "reason": (
                        "conflicting installed copies include an unordered dev or non-SemVer version"
                    ),
                }
            )
            continue
            ...
            selections.append(
                {
                    **base,
                    "state": "blocked",
                    "reason": (
                        f"installed copies disagree at the highest orderable release {highest.parsed.version}"
                    ),
                }
            )
            continue
...
def declared_scripts_tree(
    ...
    by_path = {
        PurePosixPath(*relative.parts[1:]): content for relative, content in scripts
    }
...
def has_path(data: object, path: tuple[str, ...], source: Path) -> bool:
    ...
    for part in path:
        if not isinstance(current, dict):
            raise Exception(
                f"cannot inspect {'.'.join(path)}: parent value in {source} is not a table"
            )
...
def flatten_module_answers(
    ...
    for key, value in values.items():
        ...
        if not isinstance(value, str):
            raise Exception(
                f"--module-answers {source} value modules.{module}.{dotted} must be a string"
            )
        identifier = (module, dotted)
        if identifier in answers:
            raise Exception(
                f"--module-answers {source} defines modules.{module}.{dotted} more than once"
            )
...
def validate_module_answers(
    ...
    *,
    ...
    if extra:
        module, key = extra[0]
        raise Exception(
            f"{source_label} contains modules.{module}.{key}, which is not a pending question"
        )
    missing = [
        question
        for question in pending
...
def output_folder(config_text: str) -> str:
    folder = (
        tomllib.loads(config_text).get("core", {}).get("output_folder", "_bmad-output")
    )
    prefix = "{project-root}/"
    folder = folder.removeprefix(prefix)
...
def toml_string(value: str) -> str:
    ...
    escaped = "".join(
        replacements.get(character, toml_control(character)) for character in value
    )
    return f'"{escaped}"'
