...
async function conceptStatus(req, res) {
  try {
    ...
    const { file_id, status, chunk_count, error, repo_id: repoHint } = req.body || {};
    ...
    if (st === 'ingested') {
      ...
      // Failure clarity (David, 2026-09-09): prefer the CAUSE dataprep now
      // sends; the generic wrapper is only the legacy fallback.
      const detail = typeof error === 'string' && error.trim() ? error.trim().slice(0, 400) : null;
      await conceptMetaService.upsertConceptMeta(
        repo_id,
        { concept_id, repo_id },
        {
          patch: {
            index_status: 'failed',
            last_error: detail ? `${status}: ${detail}` : `${status} (chunks=${chunk_count ?? 0})`
          }
        }
      );
    }
