...
// EXTERNAL collections (David, 2026-09-09: "create the required indexes and
// ensure them on startup") — owned by OTHER services, so index-only: this
// bootstrap must NEVER create them. `files` is queried by okf-server on hot
// paths (the mint Pending-zip gate and the ingest worker filter by repo_id
// on every poll); without the index those full-scan the files collection.
const EXTERNAL_INDEXES = {
  files: [{ type: 'persistent', fields: ['repo_id'] }]
};
...
async function ensureCollections() {
  return withSpan('okf.db.ensureCollections', async (span) => {
    ...
    // EXTERNAL collections: index them only when they exist (another service
    // owns the schema and creates the collection itself).
    for (const [name, idxs] of Object.entries(EXTERNAL_INDEXES)) {
      const col = db.collection(name);
      if (await col.exists()) {
