...
const adminUser = {
  ...
  accessToken:
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiIyMTQwIiwiaWF0IjoxNzU5ODE4NTk0LCJleHAiOjE3NTk5MDQ5OTR9.Q_D7mCXORPTCfFu0RTjUFMNtgx-PKjEr4WNrfxyCv3Q",
  refreshToken:
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiIyMTQwIiwidG9rZW5WZXJzaW9uIjozLCJpYXQiOjE3NTk4MTg1OTQsImV4cCI6MTc2MDQyMzM5NH0.yu4Rif5ieMbWpcHRZUeK8g5Pj7TBgD9BeRfNDdfPDrU",
  role: "Admin",
};
...
async function createAdminUser() {
  ...
  try {
    ...
    if (!dbExists) {
      console.error(
        `Error: Database "${dbConfig.databaseName}" does not exist.`,
      );
      return;
    }
    console.log(
      `Successfully connected to database: "${dbConfig.databaseName}"`,
    );
    ...
    console.log(
      `Checking for existing user with loginName: "${adminUser.loginName}"...`,
    );
    ...
    if (existingUser) {
      ...
      console.log(
        `User "${adminUser.loginName}" already exists. No action taken.`,
      );
    } else {
      // 2b. If user does not exist, create it
      console.log(
        `User "${adminUser.loginName}" not found. Creating new user...`,
      );
