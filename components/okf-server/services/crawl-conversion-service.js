 ...
 * splits on `## Source:` markers, sanitizes, batches and imports IN-PROCESS
...
/** Serialized-bytes budget per import flush (mirrors the old client logic). */
const BATCH_BYTES = 4 * 1024 * 1024;
/** Concept count per flush — the import service caps ONE request at 200; the
 * in-process call still funnels through the 200-cap helper, so respect it. */
...
/** Mode A ceiling: the whole file must fit ONE import payload (50mb chain). */
    ...
    await db.query(
      ...
      FOR r IN ${COLLECTION}
        ...
        UPDATE r WITH { conversion: MERGE(r.conversion, {
          ...
          finished_at: DATE_ISO8601(DATE_NOW())
        })} IN ${COLLECTION}
    `,
 ...
 * cross-link map pass and the rewrite+import pass see IDENTICAL pages
 ...
 * and are NOT link material (David's rule 3: true externals stay plain
 * markdown, no link entries). When `collected` is passed, every RESOLVED
 * cross-link is recorded there as { target, label } (anchor text) EXCLUDING
 * self-references — the author-graph source the converter writes into the
 * page's frontmatter links[].
 */
function rewriteCrossLinks(body, pageMap, ownPath, collected) {
  if (!body || !pageMap || pageMap.size === 0) return body;
  // TITLE-ATTR TOLERANCE (live audit 2026-09-08): the crawler's markdown puts
  // a title attribute on every link — `](url "title")` — and the old regex
  // demanded `)` right after the URL, so it resolved ZERO of 91,212 scanned
  // links (6,367 in-crawl targets blocked solely by the title attribute).
  return String(body).replace(/\]\((https?:\/\/[^)\s]+?)(?:\s+"[^"]*")?\)/g, (m, linkUrl) => {
    const target = pageMap.get(crawlUrlKey(String(linkUrl).replace(/\\([()])/g, '$1')));
    if (!target) return m;
    if (collected && target !== ownPath) {
      // The stored label is filled by the parser's body scan (collectLabel
      // walks the markdown tokens — nested images/headings included — so the
      // anchor text is REAL there). The frontmatter links[] entry carries the
      // target; the persist-time dedupe (concept-meta-service) merges the two
      // copies and prefers the labeled one.
      collected.push({ target: './' + target, label: '' });
    }
    return `](./${target})`;
  });
}

/** Dedupe collected links by target (first label wins) — the frontmatter
 * links[] is a CURATED list, not a clickstream. */
function dedupeLinks(collected) {
  const seen = new Set();
  const out = [];
...
async function downloadToTempFile(repoId, res, cap, onBytes) {
  // UNIQUE temp name (live-caught 2026-09-09): a repo_id-keyed path collided
  // ACROSS parallel jest workers sharing os.tmpdir() — one file's awaited
  // unlink deleted another file's in-flight download (ENOENT mid-read). pid +
  // timestamp makes every conversion's temp file its own.
  const tmpPath = path.join(os.tmpdir(), `okf-conv-${repoId}-${process.pid}-${Date.now()}.md`);
...
/** Slug a URL/path to a concept path (same shape as the import service's). */
...
/** Clean a derived title: UNWRAP links (keep the anchor text — a heading
 * like '## [Russo-Ukrainian War](…) — overview' keeps its name; the URL is
 * metadata, not a name), unslugify, strip markdown tokens, collapse
 * whitespace, cap length. Titles feed the file list, search and dedup —
 * they must be human names, never raw markdown (David, 2026-09-06). */
function cleanTitle(raw) {
  return String(raw || '')
    .replace(/\[([^\]]*)\]\([^)]*\)/g, '$1') // [text](url) -> text (the URL is never a name)
    .replace(/%20/g, ' ')
    .replace(/_/g, ' ')
    .replace(/[#`*>~[\]]/g, '')
    .replace(/\s+/g, ' ')
...
/** (1) The source URL's last path segment, unslugified: '100_metres_hurdles'
 * -> '100 metres hurdles'. Empty for root/query-only URLs. */
function deriveTitleFromUrl(url) {
  if (!url) return '';
  try {
    const u = new URL(url);
    const seg = u.pathname.split('/').filter(Boolean).pop() || '';
    if (!seg || /^index\.(html?|php)$/i.test(seg)) return '';
    return cleanTitle(decodeURIComponent(seg).replace(/\.(html?|php|aspx?)$/i, ''));
  } catch {
    return '';
  }
}

/** (2) The page's first H1/heading. */
...
async function runConversion(job) {
  const { repo_id, file_id, url, split_mode, requested_name, classification, actor } = job;
  ...
  // 'failed' with the reason — never linger as an active conversion patching
  // a nonexistent doc (patchConversion silently no-ops). Checked per flush:
  // the cheapest reliable point (Document read per batch, not per page).
  ...
  const flushBatch = async (concepts) => {
    ...
    // skipCuration: the curation PASS runs once at conversion end (below) —
  ...
  try {
    ...
    if (split_mode === 'A') {
      ...
      await iteratePages(tmpPath, async ({ url, body }) => {
        ...
        // AUTHOR LINKS (David's link-resolution spec, 2026-09-05): resolved
        // in-crawl hyperlinks become the page's frontmatter links[] — the
        // author graph the editor graph + serving graph both build on.
        // Self-references and externals never enter the list.
        const resolvedLinks = [];
        const rewritten = rewriteCrossLinks(body, pageMap, conceptPath, resolvedLinks);
        // TITLE PRIORITY (David, 2026-09-06): URL name -> first heading ->
        // concept path. Never raw link text or arbitrary markdown.
        const title =
          deriveTitleFromUrl(url) || deriveTitleFromBody(rewritten) || cleanTitle(conceptPath.replace(/\.md$/, ''));
        ...
        pending.push({
          ...
          frontmatter: {
            type: 'topic',
            title,
            ...(resolvedLinks.length > 0 ? { links: dedupeLinks(resolvedLinks) } : {}),
            sources: url
    ...
    if (pagePaths.length > 0) {
      ...
      await ingestRepoConcepts(repo_id, { concepts: index, classification, skipCuration: true }, actor);
      batchesDone += 1;
    }

    // CURATION PASS ONCE PER CONVERSION (live audit 2026-09-08): labels,
    // descriptions and the type taxonomy through the SAME shared finalize the
    // zip flow uses (multi-format end goal — one curation entry, every
    // adapter). Visible on the card: the conversion stage reads 'curating'
    // while the pass runs, then the record closes 'done'. Fail-soft: the
    // conversion completes even when the pass fails; repo.curation carries
    // the honest counters.
    if (classification) {
      await patchConversion(repo_id, { status: 'adding', stage: 'curating' });
      try {
        ...
        logger.warn('Crawl curation pass failed (non-fatal)', { repo_id: repo_id, error: err.message });
      }
    }
    ...
    if (tmpPath) {
      // AWAITED (live-caught 2026-09-09): a fire-and-forget unlink outlived the
      // runner — the next conversion of the same repo downloaded a fresh temp
      // file while the previous run's unlink deleted it mid-read (ENOENT).
...
async function startConversion({
  ...
  // ONE conversion per repo at a time (live-caught via test flake 2026-09-09):
  // `live` is keyed by repo_id — a second startConversion for the SAME repo
  // OVERWROTE the in-flight runner, dropping its promise from the registry.
  // Refuse instead: concurrent conversions of one repo would interleave batch
  // flushes + conversion-record patches and corrupt the progress record.
  if (live.has(repo_id)) {
    const e = new Error('a conversion is already running for this repository — wait for it to finish');
    e.code = 'CONVERSION_IN_FLIGHT';
    e.status = 409;
    throw e;
  }
  ...
  const queued = {
    ...
    url: url || null,
    split_mode,
    classification: classification || null, // TYPE INFERENCE provenance
    error: null,
  ...
  const job = {
    ...
    split_mode,
    requested_name,
    // TYPE INFERENCE + curation provenance. Was DROPPED here (live audit
    // 2026-09-08: crawl imports ran with no classification — zero type
    // inference, zero curation, zero LLM calls; the queued record showed
    // the mode while the import never saw it).
    classification,
...
module.exports = {
  ...
  isTerminal,
  sanitizeCrawlBody,
  maxSourceBytesFromEnv,
  MODE_A_SOURCE_LIMIT_BYTES,
  sweepInterruptedOnce, // (c) run at SERVER BOOT (index.js), not lazily on the first conversion
  deriveTitleFromUrl,
  deriveTitleFromBody,
