...
class ServiceTreeProxy {
  ...
  Future<List<dynamic>> getAllCategories({String locale = 'en'}) async {
    final res = await _api.get('services/categories', params: {'locale': locale});
    return jsonDecode(res.body);
  }

  Future<List<dynamic>> getAdminCategories({String locale = 'en'}) async {
    final res = await _api.get('service-categories/categories/detailed', params: {'locale': locale});
    return jsonDecode(res.body);
  }

  Future<List<dynamic>> getCategoryServices(String categoryId, {String locale = 'en'}) async {
    final res = await _api.get('services/categories/$categoryId', params: {'locale': locale});
    return jsonDecode(res.body)['children'] ?? [];
  }

  Future<Map<String, dynamic>> searchServices(String query, {String locale = 'en'}) async {
    final res = await _api.get('services/search', params: {'query': query, 'locale': locale});
    return jsonDecode(res.body);
  }
  ...
  Future<Map<String, dynamic>> createService(String catId, Map<String, dynamic> payload) async {
