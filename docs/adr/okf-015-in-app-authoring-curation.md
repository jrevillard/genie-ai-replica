...
- A **Markdown concept editor**: frontmatter form (`type` required + `title`/`description`/`resource`/`tags`/`timestamp`) + Markdown body editor (reuse `marked`/`DOMPurify`) + **link picker** (from the repository's concept tree) + **live OKF §9 validation** + PII pre-check.
...
- **Mitigations**: reuse `marked`/`DOMPurify`; link picker from the concept tree; live §9 validation guides authors.

## References
Production spec §5.2, §7; ADR-okf-007 (expanded).
