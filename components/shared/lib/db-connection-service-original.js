...
class DatabaseService {
  ...
  _getOrCreateProxy(connectionName, targetDb) {
        ...
        get(target, prop) {
          ...
          if (databaseMethods.includes(prop) || typeof currentDb[prop] === 'function') {
            return async function (...args) {
              return await self._executeWithRetry(connectionName, async (db) => db[prop](...args), prop);
            };
          }
    ...
    logger.debug(
      `[DB_PROXY] Created self-healing proxy for ${connectionName}. Total proxies: ${this._activeProxies.get(connectionName).size}`
    );
      ...
      {},
      {
        get(target, prop) {
          ...
          if (cursorMethods.includes(prop)) {
            return async function (...args) {
              const result = await self._executeCollectionMethodWithRetry(connectionName, collectionName, prop, args);
          ...
          if (allCollectionMethods.includes(prop) || typeof currentCollection[prop] === 'function') {
            return async function (...args) {
              return await self._executeCollectionMethodWithRetry(connectionName, collectionName, prop, args);
            };
          }
    ...
    return new Proxy(targetRoute, {
      get(target, prop) {
        ...
        if (routeMethods.includes(prop)) {
          return async function (...args) {
            return await self._executeWithRetry(connectionName, async () => target[prop](...args), `route.${prop}`);
          };
        }
    ...
    return new Proxy(
      {},
      {
        get(target, prop) {
          const currentConnectionInfo = self._connections.get(connectionName);
          const currentDb = currentConnectionInfo ? currentConnectionInfo.db : null;
          const currentVertexCollection = currentDb
            ? currentDb.graph(graphName).vertexCollection(collectionName)
            : targetVertexCollection;
    ...
    return new Proxy(
      {},
      {
        get(target, prop) {
          const currentConnectionInfo = self._connections.get(connectionName);
          const currentDb = currentConnectionInfo ? currentConnectionInfo.db : null;
          const currentEdgeCollection = currentDb
            ? currentDb.graph(graphName).edgeCollection(collectionName)
            : targetEdgeCollection;
  ...
  async _executeWithRetry(connectionName, operation, operationName = 'operation') {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        logger.debug(
          `[DB_EXECUTE] Error details: ${JSON.stringify({
            code: error.code,
            name: error.name,
  ...
  async _createNewConnectionWithRetry(name, config = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        lastError = error;
        logger.error(
          `[DB_CONNECTION] Connection attempt ${attempt}/${maxRetries} failed for ${name}: ${error.message}`
        );
  ...
  async _performActiveRecovery(name, originalError) {
    ...
    logger.info(
      `[DB_RECOVERY] Starting ACTIVE RECOVERY for ${name} (attempt ${attempts + 1}/${this.RECOVERY_RETRY_ATTEMPTS})`
    );
    ...
    try {
      ...
      await this._createNewConnectionWithRetry(name, storedConfig);
  ...
  _isConnectionError(error) {
    ...
    const isConnectionError = connectionErrors.some(
      (errCode) => code.includes(errCode.toLowerCase()) || message.includes(errCode.toLowerCase())
    );
  ...
  async _closeConnection(name) {
    ...
    if (this._connections.has(name)) {
      ...
      logger.info(
        `[DB_CLOSE] Connection ${name} stats: Age: ${Math.round(age / 60000)}min, Idle: ${Math.round(idleTime / 60000)}min`
      );
  ...
  getRecoveryStatus() {
    ...
    for (const [name] of this._connectionConfigs.entries()) {
      ...
      recoveryInfo[name] = {
        ...
        nextRecoveryAllowed: inCooldown
          ? new Date(lastAttempt + this.FAILED_RECOVERY_COOLDOWN).toISOString()
          : 'immediately',
        activeProxies: this._activeProxies.get(name)?.size || 0
      };
    }
  ...
  async executeWithRetry(operation, connectionName = 'default', maxRetries = 3) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        logger.error(
          `[DB_EXECUTE] Attempt ${attempt}/${maxRetries} failed for ${connectionName} after ${executionTime}ms: ${error.message}`
        );
  ...
  _createConnectionProxy(db, connectionName, connectionConfig) {
    logger.warn(
      `[DB_PROXY] Using deprecated _createConnectionProxy for ${connectionName}. Consider using the new self-healing proxy.`
    );
              ...
              try {
                ...
                if (self._isConnectionError(error) && attempt < maxRetries) {
                  ...
                  try {
                    ...
                    logger.error(
                      `[DB_PROXY] Failed to refresh connection for ${connectionName}: ${reconnectError.message}`
                    );
                    if (attempt >= maxRetries) {
    ...
    return {
      ...
      recovery: {
        ...
        lastAttempt: this._lastRecoveryAttempt.get(connectionName)
          ...
          : null,
        inCooldown:
          this._lastRecoveryAttempt.get(connectionName) &&
  ...
  getHealthSummary() {
    ...
    return {
      ...
      recoveryStatus: {
        ...
        cooldownConnections: Array.from(this._lastRecoveryAttempt.entries()).filter(
          ([, lastAttempt]) => now - lastAttempt < this.FAILED_RECOVERY_COOLDOWN
        ).length
      }
    };
  }
  ...
  async performFullHealthCheck() {
    ...
    logger.info(
      `[DB_HEALTH] Health check complete: ${results.summary.healthy}/${results.summary.total} healthy, ${results.summary.recovered} recovered`
    );
