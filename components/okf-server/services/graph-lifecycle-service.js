...
async function renameGraph(repo, fromGraph, toGraph, { allowDropTarget = false } = {}) {
  ...
  // BORN AT INGEST ONLY (David, 2026-09-09): does the repo actually HAVE graph
  // content? A rename of a never-ingested (or retracted) repo must NOT spawn
  // an empty shell graph — live: three bare OKF_*_v1 shells leaked from Studio
  // renames, polluting the graph namespace (and the fan-out retriever). The
  // real graph is born at drain time (dataprep ensure-graph) under the
  // CURRENT working name; step 3 recreates the definition only when content
  // actually moved.
  const hadContent = await graphExists(db, fromGraph);
  ...
  //    ONLY when the repo actually has graph content (born at ingest;
  //    never-ingested / retracted repos stay graph-less through a rename).
  if (hadContent && !(await db.graph(toGraph).exists())) {
