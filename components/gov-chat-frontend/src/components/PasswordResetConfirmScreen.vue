...
<template>
  <div class="password-reset-confirm-container" :data-theme="activeTheme">
    <div class="password-reset-confirm-card">
      <div class="logo">
        ...
        <h1 class="app-name">
          {{ $config && $config.app ? $config.app.title : $t('app.name', 'MEWA Bangladesh') }}
        </h1>
      </div>
...
export default {
  ...
  data() {
    return {
      // Local copy of the `theme` prop: the MutationObserver below syncs it from
      // <html data-theme>, and a prop must not be assigned from inside the component.
  ...
  methods: {
    ...
    observeThemeChanges() {
      console.log('[RESET_CONFIRM] Setting up MutationObserver, initial theme:', this.theme, new Date().toISOString());
        ...
        mutations.forEach((mutation) => {
          if (mutation.attributeName === 'data-theme') {
            ...
            if (newTheme !== this.activeTheme) {
              console.log('[RESET_CONFIRM] Updating component theme to:', newTheme);
              this.activeTheme = newTheme;
              const title = document.querySelector('.password-reset-confirm-card .app-name');
              console.log(
