...
> **Hallucination sweep (2026-09-17):** Several items cite `file:line` references that do not exist in the target file (file shorter than cited line, or content at cited line does not match the claim). Those items are struck through below with a note explaining the discrepancy. The underlying finding (where verifiable) is preserved without the hallucinated citation.
...
- **OTel collector config: duplicate header comment block** — the "Note: Docker services use the fluentd logging driver…" block appears twice (lines 31 and 43), producing two copies. Structural merge error. (BH3 #121, AA3 #175) `[configs/otel/otel-collector-config.yaml]` — CITATION CORRECTED: file has 441 lines; cited ~1531-1553 were hallucinated. Finding preserved (real duplicate at lines 31+43).
...
- ~~**`service-logs.json` `trace_id` dropdown enumerates every distinct trace** — `trace_id:*` returns tens of thousands of rows; Grafana hangs after a few hours of traffic. Scope to `_time:>now-15m` or cap `max: 1000`. (BH3 #128, ECH3 #150) `[configs/grafana/provisioning/dashboards/service-logs.json:1458-1469]`~~ — HALLUCINATED CITATION: file has 223 lines; cited lines 1458-1469 do not exist. The `trace_id` variable at lines 180-193 DOES use `query: "trace_id:*"` (which is the issue); finding preserved, citation wrong.
...
- ~~**`LOG_DROPPED_REASON` enum self-violated** — `victorialogs-transport.js` passes raw `'invalid_timestamp'` string to a counter whose own comment forbids raw strings. (AA1 #46) `[components/shared/lib/victorialogs-transport.js:176]`~~ — HALLUCINATED CITATION: line 176 is a comment about `millisToHrTime` conversion; `invalid_timestamp` appears at lines 189 and 204.
- ~~**`service-logs.json` level dropdown excludes Python WARNING/CRITICAL/NOTICE** — silently filters out Python logs at those severities. (ECH3 #149) `[configs/grafana/provisioning/dashboards/service-logs.json:1458]`~~ — HALLUCINATED CITATION: file has 223 lines; cited line 1458 does not exist.
- **OTel collector severity_text regex runs on JSON bodies** — no guard; rule fires on every record even when it returns empty. (BH3 #124) `[configs/otel/otel-collector-config.yaml:stamp_log_metadata_from_msg]` — FINDING PRESERVED; the cited processor `transform/stamp_log_metadata_from_msg` exists at line 203 (file has 441 lines, not the cited line range).
- ~~**OTel collector ISO 8601 timestamps with `T` separator don't match `[YYYY-MM-DD HH:MM:SS]` regex** — ISO-timestamped log bodies keep redundant prefix in VL `_msg`. (ECH3 #155) `[configs/otel/otel-collector-config.yaml:1774-1775]`~~ — HALLUCINATED CITATION: file has 441 lines; cited lines 1774-1775 do not exist.
- ~~**OTel collector empty regex match → Concat empty body** — malformed access log results in blank `_msg` in VL. (ECH3 #156) `[configs/otel/otel-collector-config.yaml:1738-1744,1759-1764]`~~ — HALLUCINATED CITATION: file has 441 lines; cited line ranges 1738-1744 and 1759-1764 do not exist.
