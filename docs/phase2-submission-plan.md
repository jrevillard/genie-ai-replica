...
**Last updated:** 2026-05-19. *Jury-facing deliverables summary: [`PHASE2-PROTOTYPE.md`](../PHASE2-PROTOTYPE.md).*
...
|---|---|---|---|---|
| 1 | RAG Ingestion Pipeline | ~7 NCD documents + Gambia health-services info | **6 documents end-to-end** (1,668 source chunks): WHO Tobacco, WHO Hypertension, BHBM mHypertension, BHBM mTobaccoCessation, Gambia NCD Policy 2012-2016, Gambia cessation clinical guidelines 2016. WHO HEARTS, Mental Health Gap, ASAM Alcohol, BHBM mActive/mBreathe/mDiabetes/mCervical, Gambia healthcare-services info — **not yet sourced** (covered by M2 below). See [`PHASE2-PROTOTYPE.md`](../PHASE2-PROTOTYPE.md) §3.1 for the per-file ingestion table. | M1 done. M2 (~6 docs still to source) outstanding. |
| 2 | LLM-generated derived patient documents | Patient FAQs / fictive chats / if-then rules / risk questionnaires per NCD | **8 documents ingested** (221 chunks): FAQ + fictive-chat + if-then-rules + risk-questionnaire for both hypertension and tobacco cessation. Labelled `audience:patient`, `derived:true`, `human_reviewed:false`. Sources at [`corpus/derived/`](../corpus/derived/). | ✅ for the v1 scope (2 NCDs). Human review by a clinician still required before any are surfaced to patients. |
...
| 7 | WhatsApp Bot backend | Django + Meta Cloud API | **DELIVERED** at `components/whatsapp-bot/` (Node.js/TypeScript, not Django — documented deviation). Webhook + inbound → RAG → outbound flow, conversation persistence via Drizzle/PostgreSQL, campaign templates, signature-verified Meta payloads, internal-auth-gated push API. Containerised (`Dockerfile` + `docker-compose.yml`). Meta Cloud API test number wired up. | ✅ |
...
| 9 | LLM-as-a-judge testing | "Array of LLMs different from Granite" | **DONE.** Harness in `tests/llm-judge/`, both pipelines, 29 cases, transcript dumps, 7-axis structured-output rubric (gpt-4o-mini; `patient_friendliness` axis added 2026-05-17). Latest re-run on the expanded corpus (2026-05-19, web target): **patient_friendliness 4.95/5, answer_relevance 5.00/5, factuality 4.71/5** across the 21 informational cases. See `tests/llm-judge/FINDINGS.md` and [`PHASE2-PROTOTYPE.md`](../PHASE2-PROTOTYPE.md) §4. | ✅ |
...
|---|---|---|---|---|
| M1 | ~~**Ingest the remaining NCD documents we have.**~~ — **Done 2026-05-18/19.** Five PDFs (WHO Hypertension Treatment Guide, BHBM mHypertension, BHBM mTobaccoCessation, Gambia NCD Policy 2012-2016, Gambia cessation guidelines 2016) ingested through the production dataprep pipeline. Sources now in `corpus/sources/`. See [`PHASE2-PROTOTYPE.md`](../PHASE2-PROTOTYPE.md) §3.1 for the per-file chunk table. | Single biggest patient-quality lever per the LLM-judge run. | Jan (RAG owner) | ✅ |
...
| M5 | ~~**Finish the WhatsApp bot end-to-end.**~~ — **Done.** Now vendored into the main repo at `components/whatsapp-bot/` (was at `/Users/peter/git/GenieAI_whatsapp_bot`). Inbound webhook → RAG → outbound flow working against the Meta Cloud API; conversation persistence via Drizzle ORM + PostgreSQL; signature-verified payloads; internal push API. Node.js/TypeScript stack is a documented deviation from the Django stack named in the submission text. | WhatsApp is named as the PRIMARY outreach channel in the submission. Cannot demo "multi-channel" without it working. | Peter | ✅ |
