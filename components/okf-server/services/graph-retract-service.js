...
/**
 * D-C (#981, David's Stage-1 ruling): drop the repo's per-repo GRAPHS ONLY —
 * the graph definitions + their member collections (+ any orphaned tables).
 * This is NOT the delete cascade: meta rows, files docs, versions and bundle
 * manifests are untouched (a retracted repository stays visible and editable;
 * re-ingest rebuilds the graph born-right from the meta rows).
 * @param {string} repo_id
 * @returns {Promise<string[]>} the dropped entity names (graph defs + collections)
 */
async function dropRepoGraphsForRepo(repo_id) {
  ...
  for (const graph of candidates) {
    // 1. CASCADE DROP (dropCollections=true — same ArangoDB hard rule as the
    //    delete cascade: the definition goes first, errorNum 1942 otherwise).
    try {
      ...
      if (!NOT_FOUND(err)) {
        logger.warn('Graph teardown: graph cascade drop failed', { repo_id, graph, error: err.message });
      }
    }
    ...
    for (const suffix of GRAPH_SUFFIXES) {
      ...
      try {
        ...
        if (!NOT_FOUND(err)) {
          logger.warn('Graph teardown: collection drop failed', { repo_id, name, error: err.message });
        }
      }
    }
  }
  logger.info('OKF repo graphs dropped (retract teardown)', {
    repo_id,
...
module.exports = { retractRepoGraph, dropRepoGraphsForRepo };
