...
class UsdaRssService {
  ...
  Future<Map<String, dynamic>> getPestAlerts({
    ...
    try {
      ...
      if (alerts.isEmpty) {
        print('[UsdaRssService] No USDA alerts found, trying alternative sources');
  ...
  Future<List<Map<String, dynamic>>> _fetchFromUSDA() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        }).where((alert) => alert != null).cast<Map<String, dynamic>>().toList();
      }
  ...
  Map<String, dynamic>? _parseRssItem(
    ...
    if (lowerTitle.contains('armyworm') || lowerDesc.contains('armyworm')) {
      ...
      scientificName = 'Hemileia vastatrix';
        ...
        lowerDesc.contains('medfly') || lowerDesc.contains('fruit fly')) {
    ...
    return {
      ...
      'departments': ['El Salvador'], // RSS feeds don't always have specific departments
      'description': _stripHtmlTags(description),
      'recommendations': 'Monitor fields regularly. Contact local agricultural extension for specific treatment recommendations.',
  ...
  Future<Map<String, dynamic>> _getMockData() async {
    ...
    return {
      ...
      'dataSource': isSpanish ? 'Datos estacionales mejorados (modo demo)' : 'Enhanced seasonal data (demo mode)',
      'alerts': alerts,
      'summary': summary,
  ...
  List<Map<String, dynamic>> _generateSeasonalAlerts(int month, int year) {
    ...
    if (armywormSeverity != 'none') {
      ...
      alerts.add({
        ...
        'description': _getFallArmywormDescription(month, armywormSeverity, isSpanish),
        'recommendations': _getFallArmywormRecommendations(armywormSeverity, isSpanish),
    ...
    if (coffeeRustSeverity != 'none') {
      ...
      alerts.add({
        ...
        'departments': affectedDepts,
        'description': _getCoffeeRustDescription(month, coffeeRustSeverity, isSpanish),
        'recommendations': _getCoffeeRustRecommendations(coffeeRustSeverity, isSpanish),
        'firstDetected': _getFirstDetectedDate(month, year),
    ...
    if (whiteflySeverity != 'none') {
      ...
      alerts.add({
        ...
        'departments': affectedDepts,
        'description': _getWhiteflyDescription(month, whiteflySeverity, isSpanish),
        'recommendations': _getWhiteflyRecommendations(whiteflySeverity, isSpanish),
        'firstDetected': _getFirstDetectedDate(month, year),
    ...
    if (lateBlightSeverity != 'none') {
      ...
      alerts.add({
        ...
        'departments': affectedDepts,
        'description': _getLateBlightDescription(month, lateBlightSeverity, isSpanish),
        'recommendations': _getLateBlightRecommendations(lateBlightSeverity, isSpanish),
        'firstDetected': _getFirstDetectedDate(month, year),
        'source': isSpanish ? 'CENTA - Centro Nacional de Tecnología Agropecuaria y Forestal' : 'CENTA - National Center for Agricultural and Forestry Technology',
    ...
    if (month >= 11 || month <= 2) {
      ...
      alerts.add({
        ...
        'firstDetected': DateTime(month <= 2 ? year - 1 : year, 11, 1).toIso8601String().split('T')[0],
        'source': 'PROCAFE',
        'link': 'https://www.procafe.org.sv/',
  ...
  String _getSeasonalSeverity(
    ...
    if (secondaryPeakStart != null && secondaryPeakEnd != null) {
      if (secondaryPeakStart <= secondaryPeakEnd) {
        inSecondaryPeak = currentMonth >= secondaryPeakStart && currentMonth <= secondaryPeakEnd;
      } else {
        inSecondaryPeak = currentMonth >= secondaryPeakStart || currentMonth <= secondaryPeakEnd;
      }
    }
    ...
    if (peakStart <= peakEnd) {
      inOffSeason = currentMonth == peakStart - 2 || currentMonth == peakEnd + 2;
    } else {
      inOffSeason = currentMonth == peakStart - 2 || currentMonth == peakEnd + 2;
    }
  ...
  List<String> _getDepartmentsForPest(String pestType, int month) {
    ...
    switch (pestType) {
      ...
      case 'coffee_rust':
        ...
        return ['Santa Ana', 'Ahuachapán', 'Sonsonate', 'La Libertad',
  ...
  String _getFirstDetectedDate(int month, int year) {
    ...
    final date = DateTime(year, month, 1).subtract(Duration(days: weeksAgo * 7));
  ...
  String _getFallArmywormDescription(int month, String severity, bool isSpanish) {
