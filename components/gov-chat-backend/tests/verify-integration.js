...
async function verifyIntegration() {
  try {
    ...
    if (queryService.analyticsService) {
      console.log(
        "✅ Analytics service successfully injected into query service",
      );
    } else {
    ...
    if (analyticsRecords.length > 0) {
      console.log(
        `✅ Found ${analyticsRecords.length} records in analytics for query ${testQuery._key}`,
      );
      console.log(
        "Sample record:",
        JSON.stringify(analyticsRecords[0], null, 2).substring(0, 200) + "...",
      );
    } else {
      console.error(
        `❌ No analytics records found for query ${testQuery._key}`,
      );
    }
    ...
    if (feedbackRecords.length > 0) {
      console.log(
        `✅ Found ${feedbackRecords.length} feedback records in analytics for query ${testQuery._key}`,
      );
      console.log(
        "Sample feedback record:",
        JSON.stringify(feedbackRecords[0], null, 2).substring(0, 200) + "...",
      );
    } else {
