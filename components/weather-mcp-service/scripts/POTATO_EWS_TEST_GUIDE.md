...
The actual risk logic is in the weather MCP service.
...
5. `main.py` exposes the HTTP endpoints used by the test and backend.
...
```text
components/weather-mcp-service/scripts/.env
```
...
So the `.env` file is loaded from the same directory as the script, even when the script is started from another directory or from inside Docker.
...
```bash
docker exec -it weather-mcp-standalone python3 /app/scripts/test_potato_ews.py --scenario heat
docker exec -it weather-mcp-standalone python3 /app/scripts/test_potato_ews.py --scenario combined
docker exec -it weather-mcp-standalone python3 /app/scripts/test_potato_ews.py --scenario normal
```
...
```bash
docker exec -it weather-mcp-standalone python3 /app/scripts/test_potato_ews.py
```
