...
const EXPORT_CONFIG = {
  ...
  filename:
    process.env.EXPORT_FILENAME ||
    `serviceCategoriesAndServices_export_${new Date().toISOString().slice(0, 19).replace(/:/g, "-")}.json`,
...
async function validateCollections() {
  try {
    ...
    const collections = {
      ...
      serviceCategoryTranslationsEdge: db.collection(
    ...
    console.log(
      `  - serviceCategories: ${existence[0] ? "EXISTS" : "MISSING"}`,
    );
    console.log(`  - services: ${existence[1] ? "EXISTS" : "MISSING"}`);
    console.log(`  - categoryServices: ${existence[2] ? "EXISTS" : "MISSING"}`);
    console.log(
      `  - serviceCategoryTranslations: ${existence[3] ? "EXISTS" : "MISSING"}`,
    );
    console.log(
      `  - serviceCategoryTranslationsEdge: ${existence[4] ? "EXISTS" : "MISSING"}`,
    );
    console.log(
      `  - serviceTranslations: ${existence[5] ? "EXISTS" : "MISSING"}`,
    );
    console.log(
      `  - serviceTranslationsEdge: ${existence[6] ? "EXISTS" : "MISSING"}`,
    );
    ...
    if (!existence[2]) {
      console.log(
        "⚠ Warning: categoryServices edge collection does not exist - will export without edges",
      );
    }

    if (!existence[3]) {
      console.log(
        "⚠ Warning: serviceCategoryTranslations collection does not exist - will export without category translations",
      );
    }

    if (!existence[4]) {
      console.log(
        "⚠ Warning: serviceCategoryTranslationsEdge collection does not exist - will export without category translation edges",
      );
    }

    if (!existence[5]) {
      console.log(
        "⚠ Warning: serviceTranslations collection does not exist - will export without service translations",
      );
    }

    if (!existence[6]) {
      console.log(
        "⚠ Warning: serviceTranslationsEdge collection does not exist - will export without service translation edges",
      );
    }
    ...
    let categoryServicesInfo = null,
      categoryServicesCount = { count: 0 };
    let serviceCategoryTranslationsInfo = null,
      serviceCategoryTranslationsCount = { count: 0 };
    let serviceCategoryTranslationsEdgeInfo = null,
      serviceCategoryTranslationsEdgeCount = { count: 0 };
    let serviceTranslationsInfo = null,
      serviceTranslationsCount = { count: 0 };
    let serviceTranslationsEdgeInfo = null,
      serviceTranslationsEdgeCount = { count: 0 };
    ...
    if (existence[3]) {
      [serviceCategoryTranslationsInfo, serviceCategoryTranslationsCount] =
        await Promise.all([
    ...
    if (existence[4]) {
      [
        serviceCategoryTranslationsEdgeInfo,
        serviceCategoryTranslationsEdgeCount,
      ] = await Promise.all([
    ...
    if (existence[6]) {
      [serviceTranslationsEdgeInfo, serviceTranslationsEdgeCount] =
        await Promise.all([
    ...
    console.log(
      `  - serviceCategories: ${categoriesInfo.type === 2 ? "Document" : "Edge"} collection with ${categoriesCount.count} documents`,
    );
    console.log(
      `  - services: ${servicesInfo.type === 2 ? "Document" : "Edge"} collection with ${servicesCount.count} documents`,
    );
    console.log(
      `  - categoryServices: ${categoryServicesInfo ? (categoryServicesInfo.type === 3 ? "Edge" : "Document") : "Not Present"} collection with ${categoryServicesCount.count} edges`,
    );
    console.log(
      `  - serviceCategoryTranslations: ${serviceCategoryTranslationsInfo ? (serviceCategoryTranslationsInfo.type === 2 ? "Document" : "Edge") : "Not Present"} collection with ${serviceCategoryTranslationsCount.count} documents`,
    );
    console.log(
      `  - serviceCategoryTranslationsEdge: ${serviceCategoryTranslationsEdgeInfo ? (serviceCategoryTranslationsEdgeInfo.type === 3 ? "Edge" : "Document") : "Not Present"} collection with ${serviceCategoryTranslationsEdgeCount.count} edges`,
    );
    console.log(
      `  - serviceTranslations: ${serviceTranslationsInfo ? (serviceTranslationsInfo.type === 2 ? "Document" : "Edge") : "Not Present"} collection with ${serviceTranslationsCount.count} documents`,
    );
    console.log(
      `  - serviceTranslationsEdge: ${serviceTranslationsEdgeInfo ? (serviceTranslationsEdgeInfo.type === 3 ? "Edge" : "Document") : "Not Present"} collection with ${serviceTranslationsEdgeCount.count} edges`,
    );
    ...
    return {
      ...
      categoryServices: existence[2]
        ? {
            collection: collections.categoryServices,
            count: categoryServicesCount.count,
          }
        : null,
      serviceCategoryTranslations: existence[3]
        ? {
            collection: collections.serviceCategoryTranslations,
            count: serviceCategoryTranslationsCount.count,
          }
        : null,
      serviceCategoryTranslationsEdge: existence[4]
        ? {
            collection: collections.serviceCategoryTranslationsEdge,
            count: serviceCategoryTranslationsEdgeCount.count,
          }
        : null,
      serviceTranslations: existence[5]
        ? {
            collection: collections.serviceTranslations,
            count: serviceTranslationsCount.count,
          }
        : null,
      serviceTranslationsEdge: existence[6]
        ? {
            collection: collections.serviceTranslationsEdge,
            count: serviceTranslationsEdgeCount.count,
          }
        : null,
    };
  } catch (error) {
...
async function exportServiceCategoriesAndServices(dbConfig) {
  try {
    ...
    const [serviceCategoriesDocuments, servicesDocuments] = await Promise.all([
      serviceCategoriesCursor.all(),
      servicesCursor.all(),
    ]);

    console.log(
      `  ✓ Retrieved ${serviceCategoriesDocuments.length} serviceCategories documents`,
    );
    ...
    if (categoryServicesExists) {
      ...
      console.log(
        `  ✓ Retrieved ${categoryServicesDocuments.length} categoryServices edges`,
      );
    } else {
    ...
    if (serviceCategoryTranslationsExists) {
      ...
      console.log(
        `  ✓ Retrieved ${serviceCategoryTranslationsDocuments.length} serviceCategoryTranslations documents`,
      );
    } else {
      console.log(
        "  ⚠ Skipping serviceCategoryTranslations - collection does not exist",
      );
    }
    ...
    if (serviceCategoryTranslationsEdgeExists) {
      ...
      console.log(
        `  ✓ Retrieved ${serviceCategoryTranslationsEdgeDocuments.length} serviceCategoryTranslationsEdge edges`,
      );
    } else {
      console.log(
        "  ⚠ Skipping serviceCategoryTranslationsEdge - collection does not exist",
      );
    }
    ...
    if (serviceTranslationsExists) {
      ...
      console.log(
        `  ✓ Retrieved ${serviceTranslationsDocuments.length} serviceTranslations documents`,
      );
    } else {
      console.log(
        "  ⚠ Skipping serviceTranslations - collection does not exist",
      );
    }
    ...
    if (serviceTranslationsEdgeExists) {
      ...
      console.log(
        `  ✓ Retrieved ${serviceTranslationsEdgeDocuments.length} serviceTranslationsEdge edges`,
      );
    } else {
      console.log(
        "  ⚠ Skipping serviceTranslationsEdge - collection does not exist",
      );
    }

    console.log(`Export data summary:`);
    console.log(
      `  - ServiceCategories: ${serviceCategoriesDocuments.length} documents`,
    );
    console.log(`  - Services: ${servicesDocuments.length} documents`);
    console.log(
      `  - CategoryServices: ${categoryServicesDocuments.length} edges`,
    );
    console.log(
      `  - ServiceCategoryTranslations: ${serviceCategoryTranslationsDocuments.length} documents`,
    );
    console.log(
      `  - ServiceCategoryTranslationsEdge: ${serviceCategoryTranslationsEdgeDocuments.length} edges`,
    );
    console.log(
      `  - ServiceTranslations: ${serviceTranslationsDocuments.length} documents`,
    );
    console.log(
      `  - ServiceTranslationsEdge: ${serviceTranslationsEdgeDocuments.length} edges`,
    );
    console.log(
      `  - Total: ${serviceCategoriesDocuments.length + servicesDocuments.length + categoryServicesDocuments.length + serviceCategoryTranslationsDocuments.length + serviceCategoryTranslationsEdgeDocuments.length + serviceTranslationsDocuments.length + serviceTranslationsEdgeDocuments.length} items`,
    );
    ...
    if (serviceCategoriesDocuments.length > 0) {
      console.log(
        ...
        JSON.stringify(serviceCategoriesDocuments[0], null, 2),
      );
    }
    if (servicesDocuments.length > 0) {
      console.log(
        "Sample service:",
        JSON.stringify(servicesDocuments[0], null, 2),
      );
    }
    if (categoryServicesDocuments.length > 0) {
      console.log(
        "Sample categoryServices edge:",
        JSON.stringify(categoryServicesDocuments[0], null, 2),
      );
    }
    if (serviceCategoryTranslationsDocuments.length > 0) {
      console.log(
        "Sample serviceCategoryTranslations:",
        JSON.stringify(serviceCategoryTranslationsDocuments[0], null, 2),
      );
    }
    if (serviceCategoryTranslationsEdgeDocuments.length > 0) {
      console.log(
        "Sample serviceCategoryTranslationsEdge:",
        JSON.stringify(serviceCategoryTranslationsEdgeDocuments[0], null, 2),
      );
    }
    if (serviceTranslationsDocuments.length > 0) {
      console.log(
        "Sample serviceTranslations:",
        JSON.stringify(serviceTranslationsDocuments[0], null, 2),
      );
    }
    if (serviceTranslationsEdgeDocuments.length > 0) {
      console.log(
        "Sample serviceTranslationsEdge:",
        JSON.stringify(serviceTranslationsEdgeDocuments[0], null, 2),
      );
    }
    ...
    const exportData = {
      metadata: {
        ...
        documentCounts: {
          ...
          serviceCategoryTranslations:
            serviceCategoryTranslationsDocuments.length,
          serviceCategoryTranslationsEdge:
            serviceCategoryTranslationsEdgeDocuments.length,
          serviceTranslations: serviceTranslationsDocuments.length,
          serviceTranslationsEdge: serviceTranslationsEdgeDocuments.length,
        },
        totalDocuments:
          serviceCategoriesDocuments.length +
          servicesDocuments.length +
          categoryServicesDocuments.length +
          serviceCategoryTranslationsDocuments.length +
          serviceCategoryTranslationsEdgeDocuments.length +
          serviceTranslationsDocuments.length +
          serviceTranslationsEdgeDocuments.length,
      ...
      data: {
        ...
        serviceCategoryTranslationsEdge:
          serviceCategoryTranslationsEdgeDocuments,
    ...
    console.log(
      `  - Total items: ${serviceCategoriesDocuments.length + servicesDocuments.length + categoryServicesDocuments.length + serviceCategoryTranslationsDocuments.length + serviceCategoryTranslationsEdgeDocuments.length + serviceTranslationsDocuments.length + serviceTranslationsEdgeDocuments.length}`,
    );
...
async function validateExportedData(filePath) {
  try {
    ...
    if (!exportData.metadata || !exportData.data) {
      throw new Error(
        "Invalid export file structure - missing metadata or data",
      );
    }

    if (!exportData.data.serviceCategories || !exportData.data.services) {
      throw new Error(
        "Missing serviceCategories or services data in export file",
      );
    }
    ...
    if (!exportData.data.categoryServices) {
      console.log(
        "⚠ categoryServices data not present in export (collection may not exist in source)",
      );
    }
    if (!exportData.data.serviceCategoryTranslations) {
      console.log(
        "⚠ serviceCategoryTranslations data not present in export (collection may not exist in source)",
      );
    }
    if (!exportData.data.serviceCategoryTranslationsEdge) {
      console.log(
        "⚠ serviceCategoryTranslationsEdge data not present in export (collection may not exist in source)",
      );
    }
    if (!exportData.data.serviceTranslations) {
      console.log(
        "⚠ serviceTranslations data not present in export (collection may not exist in source)",
      );
    }
    if (!exportData.data.serviceTranslationsEdge) {
      console.log(
        "⚠ serviceTranslationsEdge data not present in export (collection may not exist in source)",
      );
    }

    if (
      !Array.isArray(exportData.data.serviceCategories) ||
      !Array.isArray(exportData.data.services)
    ) {
    ...
    if (
      exportData.data.categoryServices &&
      !Array.isArray(exportData.data.categoryServices)
    ) {
      throw new Error("categoryServices data is not an array");
    }
    if (
      exportData.data.serviceCategoryTranslations &&
      !Array.isArray(exportData.data.serviceCategoryTranslations)
    ) {
      throw new Error("serviceCategoryTranslations data is not an array");
    }
    if (
      exportData.data.serviceCategoryTranslationsEdge &&
      !Array.isArray(exportData.data.serviceCategoryTranslationsEdge)
    ) {
      throw new Error("serviceCategoryTranslationsEdge data is not an array");
    }
    if (
      exportData.data.serviceTranslations &&
      !Array.isArray(exportData.data.serviceTranslations)
    ) {
      throw new Error("serviceTranslations data is not an array");
    }
    if (
      exportData.data.serviceTranslationsEdge &&
      !Array.isArray(exportData.data.serviceTranslationsEdge)
    ) {
    ...
    if (invalidCategories.length > 0) {
      console.error(
        `✗ Found ${invalidCategories.length} invalid serviceCategory documents:`,
        invalidCategories.slice(0, 5),
      );
      return false;
    }
    ...
    if (invalidServices.length > 0) {
      console.error(
        `✗ Found ${invalidServices.length} invalid service documents:`,
        invalidServices.slice(0, 5),
      );
      return false;
    }
    ...
    if (invalidEdges.length > 0) {
      console.error(
        `✗ Found ${invalidEdges.length} invalid categoryServices edges:`,
    ...
    if (invalidCategoryTranslations.length > 0) {
      console.error(
        `✗ Found ${invalidCategoryTranslations.length} invalid serviceCategoryTranslations documents:`,
        invalidCategoryTranslations.slice(0, 5),
      );
      return false;
    }
    ...
    if (invalidCategoryTranslationEdges.length > 0) {
      console.error(
        `✗ Found ${invalidCategoryTranslationEdges.length} invalid serviceCategoryTranslationsEdge edges:`,
        invalidCategoryTranslationEdges.slice(0, 5),
      );
      return false;
    }
    ...
    if (invalidServiceTranslations.length > 0) {
      console.error(
        `✗ Found ${invalidServiceTranslations.length} invalid serviceTranslations documents:`,
        invalidServiceTranslations.slice(0, 5),
      );
      return false;
    }
    ...
    if (invalidServiceTranslationEdges.length > 0) {
      console.error(
        `✗ Found ${invalidServiceTranslationEdges.length} invalid serviceTranslationsEdge edges:`,
        invalidServiceTranslationEdges.slice(0, 5),
      );
      return false;
    }
    ...
    console.log(
      `  - ServiceCategories: ${exportData.data.serviceCategories.length} valid documents`,
    );
    console.log(
      `  - Services: ${exportData.data.services.length} valid documents`,
    );
    console.log(
      `  - CategoryServices: ${exportData.data.categoryServices ? exportData.data.categoryServices.length : 0} valid edges`,
    );
    console.log(
      `  - ServiceCategoryTranslations: ${exportData.data.serviceCategoryTranslations ? exportData.data.serviceCategoryTranslations.length : 0} valid documents`,
    );
    console.log(
      `  - ServiceCategoryTranslationsEdge: ${exportData.data.serviceCategoryTranslationsEdge ? exportData.data.serviceCategoryTranslationsEdge.length : 0} valid edges`,
    );
    console.log(
      `  - ServiceTranslations: ${exportData.data.serviceTranslations ? exportData.data.serviceTranslations.length : 0} valid documents`,
    );
    console.log(
      `  - ServiceTranslationsEdge: ${exportData.data.serviceTranslationsEdge ? exportData.data.serviceTranslationsEdge.length : 0} valid edges`,
    );
...
async function executeExport(dbConfig) {
  console.log(
    "=== ArangoDB ServiceCategories, Services & Translations Data Export ===\n",
  );
  ...
  try {
    ...
    if (
      collections.serviceCategories.count === 0 &&
      collections.services.count === 0
    ) {
    ...
    console.log("\n=== Export Summary ===");
    console.log(
      `✓ Export completed ${isValid ? "successfully" : "with warnings"}`,
    );
    console.log(`✓ File: ${exportResult.filePath}`);
    console.log(
      `✓ ServiceCategories: ${exportResult.documentCounts.serviceCategories} documents`,
    );
    console.log(
      `✓ Services: ${exportResult.documentCounts.services} documents`,
    );
    console.log(
      `✓ CategoryServices: ${exportResult.documentCounts.categoryServices} edges`,
    );
    console.log(
      `✓ ServiceCategoryTranslations: ${exportResult.documentCounts.serviceCategoryTranslations} documents`,
    );
    console.log(
      `✓ ServiceCategoryTranslationsEdge: ${exportResult.documentCounts.serviceCategoryTranslationsEdge} edges`,
    );
    console.log(
      `✓ ServiceTranslations: ${exportResult.documentCounts.serviceTranslations} documents`,
    );
    console.log(
      `✓ ServiceTranslationsEdge: ${exportResult.documentCounts.serviceTranslationsEdge} edges`,
    );
    ...
    console.log("\n=== Usage ===");
    console.log(
      "Use the generated file with the import script to restore all collections to another database.",
    );
