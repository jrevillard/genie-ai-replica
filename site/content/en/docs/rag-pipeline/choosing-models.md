...
| 2 | **Chunk Labeling** | `vllm` (same) | `VLLM_LLM_MODEL_ID` | Main Inference, Graph Extraction | Low — internal pipeline |
| 3 | **Graph Extraction** | `vllm` (same) | `VLLM_LLM_MODEL_ID` | Main Inference, Labeling | Low — internal pipeline |
...
Labeling uses `VLLM_ENDPOINT` + `VLLM_LLM_MODEL_ID` — the **same** vLLM instance as main inference. There is no separate configuration for a labeling-specific model. If your main inference model is too small for labeling (e.g., Granite 2B), consider using the `embedding` or `bm25` labeling strategies instead — see [Data labelling strategy]({{< relref "data-labeling" >}}) and the [Alternative Labeling Strategies](#alternative-labeling-strategies) block below.
...
Graph extraction uses the same `VLLM_ENDPOINT` + `VLLM_LLM_MODEL_ID` as main inference and labeling. There is no separate configuration. **The model you choose for `VLLM_LLM_MODEL_ID` must handle all three roles.**
...
|-------|--------|-------------|-----------|---------|-------|
| **google/translategemma-4b-it** | 4B | ~8 GB | 100+ | Gemma Terms of Use | Fine-tuned specifically for translation via vLLM. **Best dedicated choice.** Commented in the `env` template as an alternative. |
| **google/gemma-3-4b-it** | 4B | ~8 GB | 100+ | Gemma Terms of Use | The docker-compose default (`VLLM_TRANSLATION_MODEL_ID`); general-purpose instruction model that handles translation well. |
| **google/translategemma-12b-it** | 12B | ~24 GB | 100+ | Gemma Terms of Use | Higher-quality translation; needs 24 GB+ GPU. |
...
| Provider | Model | License | Notes |
|----------|-------|---------|-------|
| **Google** | Gemini 2.0 Flash | Google Cloud ToS (paid) | Excellent multilingual. Point `VLLM_TRANSLATION_ENDPOINT` at a Gemini-compatible proxy. |
| **OpenAI** | GPT-4o-mini | OpenAI ToS (paid) | Good translation quality, low cost. Point `VLLM_TRANSLATION_ENDPOINT` at an OpenAI-compatible proxy. |
| **DeepL** | DeepL API | DeepL ToS (paid) | Best-in-class translation quality. **Requires separate integration** — not currently wired in via `VLLM_TRANSLATION_ENDPOINT`. |
...
The translation system supports these language codes (defined in the backend's language maps; the same whitelist the UI uses at `components/gov-chat-frontend/src/i18n/locales/`):
`ar`, `bn`, `de`, `en`, `es`, `fr`, `id`, `man`, `pt`, `ru`, `st`, `sw`, `th`, `zh`
...
Send a chat request through the pipeline (anonymous mode works against the
ChatQnA service directly; against the backend you need a Keycloak JWT).
Confirm a non-empty `text` field returns and `source_documents` is
populated; check the corresponding trace in VictoriaTraces for span
errors.

```bash
# Direct ChatQnA probe (no JWT required):
curl -sk -X POST http://localhost:8888/v1/chatqna \
  -H 'Content-Type: application/json' \
  -d '{"messages":[{"role":"user","content":"<known question>"}], "stream": false}' \
  | jq '.text, (.source_documents | length)'

# Through the backend (requires a Keycloak JWT — see Server API Testing):
curl -sk -X POST http://localhost:3000/api/chat \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"messages":[{"role":"user","content":"<known question>"}]}'
```

If `source_documents` is empty, the reranker (or retriever) is failing —
inspect `reranker.strategy` on the reranker microservice span in
VictoriaTraces (and the `is_grounded` boolean on the SSE metadata
envelope, not on a chatqna span).
...
| Model | VRAM Used | Quality |
|-------|-----------|---------|
| **google/translategemma-4b-it** | ~8 GB | Excellent |
...
├── YES → Use google/translategemma-4b-it
└── NO
...
| `VLLM_GPU_UTILIZATION` | `0.55` | GPU memory fraction (docker-compose default; per-profile overrides in `env.rtx6000` = `0.6`, `env.t4` = `0.4`) |
| `VLLM_MAX_MODEL_LEN` | `65536` | Max context window |
| `VLLM_DTYPE` | `half` | Weight precision (docker-compose default; `env.t4` overrides to `half`) |
...
| `TEI_EMBED_MODEL` | (must match above) | TEI embedding model. Python-side env var consumed by the retriever (`genie-ai-overlay/retriever/config.py:238`); default `BAAI/bge-base-en-v1.5`. Not a docker-compose variable. |
...
| Variable | Default | Purpose |
|----------|---------|---------|
| `CONTENT_EXTRACTION_METHOD` | `opea` | Extraction engine: `opea` (in-code default) or `docling` |
...
**Symptom:** `torch.cuda.is_available()` returns `False` inside the dataprep
container even though `nvidia-smi` works on the host — `python -c "import
torch; print(torch.cuda.is_available())"` returns `False` from
`docker exec ... python3`.

**Cause:** `pip install torch` without an explicit index URL pulls the
latest PyTorch (e.g. CUDA 13.0 builds) which may not match the host
driver (e.g. CUDA 12.8). The container's CUDA runtime is then unable to
talk to the driver.

**Fix:** Pin PyTorch to a CUDA build that matches the host driver. Add
`--index-url https://download.pytorch.org/whl/cu128` (or matching your
host) to the `torch` install line in
`genie-ai-overlay/dataprep/Dockerfile-dataprep_genie-ai` and rebuild the
dataprep image.
