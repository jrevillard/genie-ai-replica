...
from app.core.models import RiskAssessment, UnifiedForecast
...
class StorageLayer:
    ...
    def upsert_seasonal_forecast(self, doc: dict) -> str:
        ...
        try:
            ...
            logger.error(
                "[STORAGE] upsert_seasonal_forecast failed for %s: %s", key, exc
            )
            raise
        return key
    ...
    def upsert_seasonal_assessment(self, assessment: dict) -> str:
        ...
        try:
            ...
            logger.error(
                "[STORAGE] upsert_seasonal_assessment failed for %s: %s", key, exc
            )
            raise
        return key
    ...
    def get_seasonal_assessment(
        ...
        try:
            ...
            logger.warning(
                "[STORAGE] get_seasonal_assessment failed for %s: %s", key, exc
            )
            return None
    ...
    def record_drought_alert_sent(self, location: str, tier: int, channel: str) -> None:
        col = self._db.collection("alerts_sent")
        col.insert(
            {
                "location": location,
    ...
    # ------------------------------------------------------------------
    # BAMIS special bulletins
    # ------------------------------------------------------------------

    def upsert_special_bulletin(self, bulletin: dict) -> tuple[str, bool]:
        """
        Store a BAMIS special bulletin row.
        Returns (document_key, is_new). A bulletin is considered new when its
        key has not been seen before, which lets the scheduler push once.
        """
        key = _norm_key(bulletin["source_id"])
        col = self._db.collection("special_bulletins")
        now = datetime.now(timezone.utc).isoformat()
        doc = {"_key": key, **bulletin, "updated_at": now}

        try:
            if col.has(key):
                existing = dict(col.get(key))
                doc["first_seen_at"] = existing.get("first_seen_at", now)
                doc["notified_at"] = existing.get("notified_at")
                col.replace(doc)
                return key, False
            ...
            logger.error(
                "[STORAGE] upsert_special_bulletin failed for %s: %s", key, exc
            )
            raise

    def mark_special_bulletin_notified(self, source_id: str) -> None:
        key = _norm_key(source_id)
        col = self._db.collection("special_bulletins")
        if col.has(key):
            col.update(
                {"_key": key, "notified_at": datetime.now(timezone.utc).isoformat()}
            )

    def count_special_bulletins(self) -> int:
        aql = """
            RETURN LENGTH(special_bulletins)
        """
        cursor = self._db.aql.execute(aql)
        docs = list(cursor)
        return int(docs[0] or 0) if docs else 0

    def get_recent_special_bulletins(self, limit: int = 10) -> list[dict]:
        aql = """
            FOR doc IN special_bulletins
              SORT doc.published_at DESC, doc.first_seen_at DESC
              LIMIT @limit
              RETURN doc
        """
        cursor = self._db.aql.execute(aql, bind_vars={"limit": limit})
        return [dict(d) for d in cursor]

    def was_special_bulletin_alert_sent(self, source_id: str) -> bool:
        key = _norm_key(source_id)
        col = self._db.collection("special_bulletins")
        if not col.has(key):
            return False
        doc = dict(col.get(key))
        return bool(doc.get("notified_at"))
