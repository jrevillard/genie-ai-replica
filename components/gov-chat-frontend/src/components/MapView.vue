<template>
  <div class="map-overlay-container">
    <button class="map-back-btn" type="button" @click="$emit('back')">&larr; {{ $t('map.backToChat') }}</button>
    <div v-if="name" class="map-location-label">{{ name }}</div>
...
export default {
  ...
  data() {
    return { map: null, renderedLayerIds: [] };
  },

  watch: {
    lat(v) {
      this.map?.flyTo({ center: [this.lon, v], zoom: this.zoom });
    },
    lon(v) {
      this.map?.flyTo({ center: [v, this.lat], zoom: this.zoom });
    },
  ...
  mounted() {
    // Runtime config first (docker-entrypoint.sh writes config.js), so the token
    // can change without rebuilding the bundle; build-time env kept as fallback.
    mapboxgl.accessToken = window.APP_CONFIG?.mapboxToken || process.env.VUE_APP_MAPBOX_TOKEN || '';
  ...
  methods: {
    ...
    _clearRenderedLayers() {
      if (!this.map) return;
      this.renderedLayerIds.forEach((id) => {
        if (this.map.getLayer(id)) this.map.removeLayer(id);
      });
      // Deduplicate source ids before removal (each source backs two layers)
      const sourceIds = [...new Set(this.renderedLayerIds.map((id) => id.replace(/-fill$|-line$/, '')))];
      sourceIds.forEach((srcId) => {
        if (this.map.getSource(srcId)) this.map.removeSource(srcId);
      });
      this.renderedLayerIds = [];
    },
    ...
    _renderGeoJsonLayers(layers) {
      ...
      layers.forEach((layer) => {
        ...
        this.renderedLayerIds.push(fillId, lineId);
