  ...
  <div class="okf-step-validate">
    ...
    <div class="okf-step-validate__groups">
      <details v-for="g in issueGroups" :key="g.code" :open="g.count > 0" class="okf-step-validate__group">
        ...
        <p v-if="g.count === 0" class="okf-step-validate__group-empty">{{ translate('okf.validation.none', 'None') }}</p>
    ...
    <p v-if="!expert" class="okf-step-validate__expert-hint">
      {{ translate('okf.validation.expertHint', 'Switch to Expert mode to see raw validation JSON, filter by severity, and override checks.') }}
    </p>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    headline() {
      ...
      if (blockers > 0) return this.translate('okf.validation.headline.blockers', `${blockers} blocking issue(s) — fix before publishing`);
      if (warnings > 0) return this.translate('okf.validation.headline.warnings', `${warnings} thing(s) need your review`);
    ...
    summaryLine() {
      ...
      return this.translate(
        ...
        '{clean} clean · {warnings} needs review · {blockers} blocking'
      ).replace('{clean}', clean).replace('{warnings}', warnings).replace('{blockers}', blockers);
    }
  }
};
</script>
...
<style scoped>
.okf-step-validate { display: flex; flex-direction: column; gap: var(--space-md); }
.okf-step-validate__health { display: flex; flex-direction: column; align-items: center; gap: var(--space-sm); padding: var(--space-md); background: var(--bg); border: 1px solid var(--border); border-radius: var(--radius-md); }
.okf-step-validate__headline { margin: 0; font-size: var(--text-md); font-weight: 600; }
.okf-step-validate__count { margin: 0; color: var(--muted); font-size: var(--text-sm); }
.okf-step-validate__groups { display: flex; flex-direction: column; gap: var(--space-xs); }
...
.okf-step-validate__group summary { display: flex; justify-content: space-between; align-items: center; cursor: pointer; font-weight: 500; }
.okf-step-validate__group-count { background: var(--accent-muted); color: var(--accent); padding: 2px 10px; border-radius: 100px; font-size: var(--text-xs); }
.okf-step-validate__group-empty { margin: var(--space-xs) 0 0 0; color: var(--muted); font-size: var(--text-sm); }
.okf-step-validate__group-list { margin: var(--space-xs) 0 0 0; padding-left: var(--space-md); display: flex; flex-direction: column; gap: 2px; font-size: var(--text-sm); }
.okf-step-validate__group-code { color: var(--muted); font-family: var(--font-mono); font-size: var(--text-xs); margin-left: var(--space-xs); }
.okf-step-validate__expert-hint { margin: 0; padding: var(--space-sm) var(--space-md); background: var(--bg); border: 1px dashed var(--border); border-radius: var(--radius-sm); color: var(--muted); font-size: var(--text-sm); }
</style>
