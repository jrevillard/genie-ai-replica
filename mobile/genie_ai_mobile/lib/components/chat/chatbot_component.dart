...
class ChatBotComponentState extends State<ChatBotComponent> {
  ...
  Future<void> _toggleVoiceInput() async {
    ...
    if (_isVoiceListening) {
      ...
      if (spokenText.isNotEmpty) {
        unawaited(_sendMessage(spokenText));
      }
      return;
    }
    ...
    final started = await _voiceService.startListening(
      ...
      onError: (error) {
        debugPrint('[CHATBOT] Voice input error: $error');
      },
    );

    if (!mounted) return;
    if (!started) {
      NotificationService.error('Voice input is unavailable on this device');
      return;
    }
  ...
  String _cleanAssistantResponse(dynamic rawResponse) {
    ...
    return text
        .replaceAll(RegExp(r'</?MEWA\s*>', caseSensitive: false), '')
  ...
  _MapRequest? _extractMapRequest(String text) {
    ...
    final fallback = normalized
        .replaceAll(
          RegExp(r'show|me|the|map|of|for|please', caseSensitive: false),
          ' ',
        )
  ...
  String _cleanMapPlace(String raw) {
    return raw
        .replaceAll(RegExp(r'[?.!,;:]+$'), '')
        .replaceAll(RegExp(r'^\s*(of|for|in|at)\s+', caseSensitive: false), '')
        .trim();
  }
  ...
  void _quickHelpPressed(Map<String, dynamic> button) {
    ...
    unawaited(_sendMessage(visibleText, hiddenPrompt: hiddenPrompt));
  }
  ...
  Future<void> saveConversation({
    ...
    if (_messages.isEmpty) {
      if (showNotification) NotificationService.info("Nothing to save");
      return;
    }
                  ...
                  children: [
                    ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final bool isUser = msg['role'] == 'user';
                        final bool hasMap = msg['map'] != null;
