  ...
  test('audit row written (actor = sub string) — carries the per-bundle totals', async () => {
    ...
    expect(writeAudit).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        repo_id: REPO,
        source_ip: '127.0.0.1',
        description: expect.stringContaining('Imported 1 concept(s)'),
...
describe('IMPORT CONCURRENCY (perf — David 2026-09-05: importing must be much faster)', () => {
  test('OKF_IMPORT_CONCURRENCY=2 imports every concept with an exact summary (parallel folds)', async () => {
    process.env.OKF_IMPORT_CONCURRENCY = '2';
    piiService.scanConcept.mockImplementation(
      () => new Promise((resolve) => setTimeout(() => resolve({ pii_state: 'clean' }), 20))
    );
    const concepts = ['Alpha', 'Beta', 'Gamma', 'Delta'].map((t) => conceptInput(t));
    const summary = await ingestService.ingestRepoConcepts(REPO, { concepts }, ACTOR);
    delete process.env.OKF_IMPORT_CONCURRENCY;
    expect(summary.total).toBe(4);
    expect(summary.parsed).toBe(4);
    expect(summary.created).toBe(4);
    expect(summary.enqueued).toBe(4);
    expect(summary.pii).toEqual({ clean: 4, hit: 0, error: 0 });
    expect(summary.enqueue_errors).toEqual([]);
  });

  test('default (unset) stays SEQUENTIAL — summary identical to the historical shape', async () => {
    piiService.scanConcept.mockResolvedValue({ pii_state: 'clean' });
    const summary = await ingestService.ingestRepoConcepts(
      REPO,
      { concepts: [conceptInput('One'), conceptInput('Two')] },
      ACTOR
    );
    expect(summary.total).toBe(2);
...
describe('TYPE INFERENCE contract (David 2026-09-05 — categorization at import)', () => {
  test('classification request types an UNTYPED concept through the pool (4b persists it)', async () => {
    conceptMeta.upsertConceptMeta.mockClear(); // my describe has no beforeEach — clear the accumulated calls
    parserService.parseConcept.mockImplementation(async (markdown, ctx) => ({
      concept_id: ctx.path.replace(/.md$/, ''),
      repo_id: ctx.repo_id,
      path: ctx.path,
      frontmatter: { title: 'Battle of Waterloo' }, // NO type — the placeholder class
      body: '# Battle of Waterloo\nThe battle ended the war in 1815.',
      sources: [],
      links: []
    }));
    await ingestService.ingestRepoConcepts(
      REPO,
      { concepts: [conceptInput('Waterloo')], classification: 'heuristics' },
      ACTOR
    );
    const written = conceptMeta.upsertConceptMeta.mock.calls[0][1];
    expect(written.frontmatter.type).toBe('event');
  });

  test('an AUTHORIAL type is replaced by the taxonomy verdict at import (David 2026-09-08)', async () => {
    conceptMeta.upsertConceptMeta.mockClear();
    parserService.parseConcept.mockImplementation(async (markdown, ctx) => ({
      concept_id: ctx.path.replace(/.md$/, ''),
      repo_id: ctx.repo_id,
      path: ctx.path,
      frontmatter: { title: 'Some Event', type: 'entity' },
      body: 'The war of 1883 founding treaty.',
      sources: [],
      links: []
    }));
    await ingestService.ingestRepoConcepts(
      REPO,
      { concepts: [conceptInput('Authored')], classification: 'llm' },
      ACTOR
    );
    const written = conceptMeta.upsertConceptMeta.mock.calls[0][1];
    // TYPE TAXONOMY: the type is ALWAYS topic|entity|process|event|source —
    // the content's verdict replaces the authorial label.
    expect(['topic', 'entity', 'process', 'event', 'source']).toContain(written.frontmatter.type);
  });
});

// ── D-G (#985): curation provenance — David's activity-log ruling ──
describe('ingestService curation provenance (D-G #985)', () => {
  beforeEach(() => jest.clearAllMocks());

  test('curation {method, resolved_by, label_source} is attached and sent to the meta upsert', async () => {
    parserService.parseConcept.mockImplementation(async (markdown, ctx) => ({
      concept_id: ctx.path.replace(/.md$/, ''),
      repo_id: ctx.repo_id,
      path: ctx.path,
      frontmatter: { title: 'Some Event' }, // no type -> heuristic classification runs
      body: 'The war of 1883 founding treaty.',
      sources: [],
      links: []
    }));
    await ingestService.ingestRepoConcepts(
      REPO,
      { concepts: [conceptInput('Evt')], classification: 'heuristics' },
      ACTOR
    );
    const written = conceptMeta.upsertConceptMeta.mock.calls[0][1];
    expect(written.curation).toMatchObject({
      method: 'heuristics',
      resolved_by: 'heuristics',
      label_source: 'none'
    });
  });

  test('the authorial type is REPLACED by the taxonomy verdict; authorial labels stay author-sourced (David 2026-09-08)', async () => {
    parserService.parseConcept.mockImplementation(async (markdown, ctx) => ({
      concept_id: ctx.path.replace(/.md$/, ''),
      repo_id: ctx.repo_id,
      path: ctx.path,
      frontmatter: { title: 'Some Event', type: 'entity', labels: ['t:9'] },
      body: 'The war of 1883 founding treaty.',
      sources: [],
      links: []
    }));
    ...
    const written = conceptMeta.upsertConceptMeta.mock.calls[0][1];
    // TYPE TAXONOMY: the import-time engine classifies EVERY concept — the
    // old authorial 'entity' is replaced by the content's verdict.
    expect(['topic', 'entity', 'process', 'event', 'source']).toContain(written.frontmatter.type);
    expect(written.frontmatter.type).toBe('event'); // the war/treaty body classifies as event
    expect(written.curation).toMatchObject({
      method: 'llm', // the REQUESTED strategy is recorded
      label_source: 'author' // authorial LABELS are still author-sourced
    });
  });

  test('summary folds curation_counts and the audit row carries method + counts', async () => {
    parserService.parseConcept.mockImplementation(async (markdown, ctx) => ({
      concept_id: ctx.path.replace(/.md$/, ''),
      repo_id: ctx.repo_id,
      path: ctx.path,
      frontmatter: { title: 'X' }, // heuristics classify
      body: 'The war of 1883 founding treaty.',
      sources: [],
      links: []
    }));
    ...
    const summary = await ingestService.ingestRepoConcepts(
      REPO,
      { concepts: [conceptInput('A'), conceptInput('B')], classification: 'heuristics' },
      ACTOR
    );
    expect(summary.curation_counts).toEqual({ heuristics: 2 });
    const row = writeAudit.mock.calls.map((c) => c[0]).find((r) => r.action === 'repo.import');
    expect(row.details.curation.method).toBe('heuristics');
    expect(row.details.curation.counts).toEqual({ heuristics: 2 });
    expect(row.details.curation.heuristics).toMatchObject({ labeled: 0, fallbacks: 0 });
    expect(row.description).toContain('curation method: heuristics (heuristics=2)');
    expect(row.description).toContain('curation pass (heuristics): labeled=0');
  });

  test('no classification requested -> no curation, audit row unchanged in shape', async () => {
    parserService.parseConcept.mockImplementation(async (markdown, ctx) => ({
      concept_id: ctx.path.replace(/.md$/, ''),
      repo_id: ctx.repo_id,
      path: ctx.path,
    ...
    const { writeAudit } = require('../services/audit-service');
    const summary = await ingestService.ingestRepoConcepts(REPO, { concepts: [conceptInput('C')] }, ACTOR);
    expect(summary.curation_counts).toEqual({});
    expect(conceptMeta.upsertConceptMeta.mock.calls[0][1].curation).toBeUndefined();
    const row = writeAudit.mock.calls.map((c) => c[0]).find((r) => r.action === 'repo.import');
    expect(row.details.curation.method).toBeNull();
    expect(row.description).not.toContain('curation method');
  });
});
