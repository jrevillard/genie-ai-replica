...
class TestEnsureGraphCollections:
    """P0 (David, 2026-09-09, verbatim: "why the fuck are u creating
    collections... the underlying collections can be created automatically
    during the graph creation process"): the manual create_collection loop is
    GONE — the named-graph registration auto-creates its members. The ensure
    path now only (re)registers the graph definition, with a self-heal that
    re-registers when a member collection is missing (crash debris)."""

    def _bare(self, has_graph, member_exists):
        instance = GenieArangoDataprep.__new__(GenieArangoDataprep)
        db = MagicMock()
        db.has_graph.return_value = has_graph
        db.has_collection.side_effect = lambda name: member_exists
        instance.db = db
        return instance, db

    def test_registration_creates_members_via_the_graph_api(self):
        instance, db = self._bare(has_graph=False, member_exists=False)
        instance._ensure_graph_collections("OKF_repo1")
        db.create_collection.assert_not_called()  # David: never manual pre-creation
        db.create_graph.assert_called_once()
        edge_defs = db.create_graph.call_args.kwargs["edge_definitions"]
        assert {d["edge_collection"] for d in edge_defs} == {
            "OKF_repo1_HAS_SOURCE",
            "OKF_repo1_LINKS_TO",
        }

    def test_healthy_graph_is_a_noop(self):
        instance, db = self._bare(has_graph=True, member_exists=True)
        instance._ensure_graph_collections("OKF_repo1")
        db.create_graph.assert_not_called()
        db.delete_graph.assert_not_called()

    def test_missing_member_reregisters_the_definition(self):
        # Crash debris: graph exists, a member collection is missing. The
        # DEFINITION is dropped (collections untouched) and re-registered —
        # auto-creation fills exactly the gaps. Still no manual create_collection.
        instance, db = self._bare(has_graph=True, member_exists=False)
        instance._ensure_graph_collections("OKF_repo1")
        db.delete_graph.assert_called_once()
        db.create_graph.assert_called_once()
        db.create_collection.assert_not_called()

    def test_has_collection_failure_never_raises(self):
        instance = GenieArangoDataprep.__new__(GenieArangoDataprep)
        db = MagicMock()
        db.has_collection.side_effect = Exception("arango hiccup")
        instance.db = db
