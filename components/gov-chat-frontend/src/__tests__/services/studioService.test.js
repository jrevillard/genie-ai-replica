...
describe('studioService', () => {
  ...
  describe('getDraft', () => {
    it('GETs /okf/repos/:id/draft silently (absent draft = expected empty state)', async () => {
      mockGet.mockResolvedValue({ data: { step: 3 } });
      await studioService.getDraft('r-1');
      expect(mockGet).toHaveBeenCalledWith('/okf/repos/r-1/draft', {}, { silent: true });
      expect(mockGet.mock.calls[0][0].startsWith('/api/')).toBe(false);
    });
  });

  describe('saveDraft', () => {
    it('PUTs /okf/studio_drafts/:id silently (404 = expected until 10.5 lands)', async () => {
      mockPut.mockResolvedValue({ data: { ok: true } });
      await studioService.saveDraft('r-1', { step: 4 });
      expect(mockPut).toHaveBeenCalledWith('/okf/studio_drafts/r-1', { step: 4 }, { silent: true });
      expect(mockPut.mock.calls[0][0].startsWith('/api/')).toBe(false);
    });
  });

  describe('clearDraft', () => {
    it('DELETEs /okf/studio_drafts/:id silently (no /api prefix)', async () => {
      mockDelete.mockResolvedValue({});
      await studioService.clearDraft('r-1');
      expect(mockDelete).toHaveBeenCalledWith('/okf/studio_drafts/r-1', { silent: true });
