...
- **Strategy**: `CONTEXTUAL_STRATEGY=per_chunk` (default — one call/chunk, section-tailored context, the Anthropic recipe) or `doc_level` (one call/doc, same context on every chunk — N× cheaper). Choose `doc_level` where ingest cost matters more than section-level precision.
...
- **Composable with retrieval hybrid**: BM25 lexical retrieval + RRF fusion is **shipped and on by default** (`RETRIEVER_HYBRID_RETRIEVAL_ENABLED=true`), running over the same contextualized text for the full `dense + sparse → reranker` recipe. The two features are independent (contextual retrieval off + hybrid on = plain lexical hybrid).
