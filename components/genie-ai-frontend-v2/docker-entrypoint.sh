...
# Cloud-native runtime config: regenerate /app/dist/config.js at every container
# start so API URL / proxy host can change without rebuilding the image.
...
# Any new template variable in nginx.conf must:
