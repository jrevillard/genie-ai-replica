  ...
  try {
    ...
    importData.serviceCategories.forEach((doc, index) => {
      const missingFields = categoryRequiredFields.filter(
        (field) => !Object.prototype.hasOwnProperty.call(doc, field) || doc[field] === null || doc[field] === undefined
      );
      ...
      try {
        ...
        for (let j = 0; j < batch.length; j++) {
          try {
            ...
            if (
              ['categoryServices', 'serviceCategoryTranslationsEdge', 'serviceTranslationsEdge'].includes(
                collectionName
              )
            ) {
...
async function verifyImport(collections, originalData) {
  try {
    ...
    const categoryTranslationSampleKeys = originalData.serviceCategoryTranslations
    ...
    const categoryTranslationEdgeSamples = originalData.serviceCategoryTranslationsEdge.slice(
      0,
    ...
    const serviceTranslationEdgeSamples = originalData.serviceTranslationsEdge.slice(
      0,
    ...
    const categoryTranslationSuccess =
      categoryTranslationSampleSize === 0 || categoryTranslationVerifiedCount === categoryTranslationSampleSize;
    const categoryTranslationEdgeSuccess =
      categoryTranslationEdgeSampleSize === 0 ||
      categoryTranslationEdgeVerifiedCount === categoryTranslationEdgeSampleSize;
    const serviceTranslationSuccess =
      serviceTranslationSampleSize === 0 || serviceTranslationVerifiedCount === serviceTranslationSampleSize;
...
async function executeImport(dbConfig, importConfig) {
  ...
  try {
    ...
    const success =
      (importResult.totals.importedCount > 0 || importResult.totals.skippedCount > 0) &&
