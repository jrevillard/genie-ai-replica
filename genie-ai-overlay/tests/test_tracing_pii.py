...
# Circular-reference guard for `_redact_body`.
#
# Pre-fix: a self-referential dict or list in `logRecord.body` would
# recurse forever inside `_redact_body` and raise `RecursionError`,
# crashing the OTel log pipeline mid-emit. Common in practice when an
# uncaught exception's `__context__`/`__cause__` chains form a cycle
# (Python's stdlib always does this for `raise X from None` chains).
def test_handles_self_referential_body_dict(pipeline):
    """A dict whose value references itself must not recurse forever."""
    redactor, inner = pipeline
    body = {"headers": {"authorization": "Bearer abc123def456ghi789jkl012mno345pqr"}}
    body["self"] = body  # direct cycle
    record = _FakeLogRecord(body=body)
    # Pre-fix this raised RecursionError. Post-fix it returns "[CIRCULAR]"
    # for the cyclic branch and redacts the sensitive non-cyclic branch.
    redactor.on_emit(_FakeLogData(record))
    out_body = inner.received[0]["body"]
    assert out_body["self"] == "[CIRCULAR]"
    # The body-walker uses _BODY_PATTERNS which replaces "Bearer <token>"
    # with the more specific "[REDACTED_BEARER]" placeholder (distinct
    # from the attribute-key "[REDACTED]" used by redactAttributes).
    assert out_body["headers"]["authorization"] == "[REDACTED_BEARER]"


def test_handles_indirect_cycle_a_to_b_to_a(pipeline):
    redactor, inner = pipeline
    a, b = {}, {}
    a["b"] = b
    b["a"] = a  # indirect cycle
    record = _FakeLogRecord(body=a)
    redactor.on_emit(_FakeLogData(record))
    out = inner.received[0]["body"]
    assert out["b"]["a"] == "[CIRCULAR]"


def test_handles_self_referential_list(pipeline):
    redactor, inner = pipeline
    items = []
    items.append(items)  # list referencing itself
    record = _FakeLogRecord(body={"items": items})
    redactor.on_emit(_FakeLogData(record))
    out = inner.received[0]["body"]
    assert out["items"][0] == "[CIRCULAR]"


def test_redact_body_does_not_leak_seen_set_between_calls(pipeline):
    """The `_seen` set is created fresh on every top-level call. Otherwise
    two unrelated objects sharing `id()` across calls would be falsely
    flagged as circular (rare but possible under CPython's id recycling)."""
    redactor, inner = pipeline
    # First call: trigger a cycle so the seen set is exercised.
    cycle = {}
    cycle["self"] = cycle
    record1 = _FakeLogRecord(body=cycle)
    redactor.on_emit(_FakeLogData(record1))

    # Second call: completely unrelated object, must NOT be flagged.
    plain = {"a": 1, "b": {"c": 2}}
    record2 = _FakeLogRecord(body=plain)
    redactor.on_emit(_FakeLogData(record2))
    out = inner.received[-1]["body"]
