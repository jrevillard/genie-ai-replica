...
const testSuite = [
  ...
  // --- from chat-history-service.js (59 tests) ---
  {
    ...
    aql: `FOR cat IN serviceCategories FILTER cat._key == @categoryId RETURN cat.nameEN`,
    expectedSql: `SELECT cat.nameEN FROM serviceCategories LET cat = @this WHERE cat._key == :categoryId`
  },
  {
    description: '[CHS-2] createConversation count messages',
    aql: `FOR msg IN messages FILTER msg.conversationId == @generatedKey RETURN msg`,
    expectedSql: `SELECT msg FROM messages LET msg = @this WHERE msg.conversationId == :generatedKey`
  },
  {
    description: '[CHS-3] addMessage get latest sequence',
    aql: `FOR msg IN messages FILTER msg.conversationId == @conversationId SORT msg.sequence DESC LIMIT 1 RETURN msg.sequence`,
    expectedSql: `SELECT msg.sequence FROM messages LET msg = @this WHERE msg.conversationId == :conversationId ORDER BY msg.sequence DESC LIMIT 1`
  },
  {
    description: '[CHS-4] getConversation messagesCursor - REFINED',
    aql: `FOR msg IN messages FILTER msg.conversationId == @conversationId SORT msg.sequence ASC LET queryLink = ( FOR edge IN queryMessages FILTER edge._to == CONCAT('messages/', msg._key) FOR q IN queries FILTER q._id == edge._from RETURN q._key )[0] RETURN MERGE(msg, { queryId: queryLink })`,
    expectedSql: `SELECT msg, (SELECT q._key FROM queryMessages LET edge = @this JOIN queries ON q._id = edge._from WHERE edge._to == concat('messages/', msg._key))[0] AS queryLink FROM messages LET msg = @this WHERE msg.conversationId == :conversationId ORDER BY msg.sequence ASC`
  },
  {
    description: '[CHS-5] getConversation get category details - REFINED',
    aql: `FOR edge IN conversationCategories FILTER edge._from == @conversationId FOR cat IN serviceCategories FILTER cat._id == edge._to RETURN { _id: cat._id, nameEN: cat.nameEN, relevanceScore: edge.relevanceScore }`,
    expectedSql: `SELECT cat._id AS _id, cat.nameEN AS nameEN, edge.relevanceScore AS relevanceScore FROM conversationCategories as edge JOIN serviceCategories as cat ON cat._id = edge._to WHERE edge._from == :conversationId`
  },
  {
    description: '[CHS-6] getConversation get owners - REFINED',
    aql: `FOR edge IN userConversations FILTER edge._to == @conversationId FOR user IN users FILTER user._id == edge._from RETURN { _id: user._id, role: edge.role }`,
    expectedSql: `SELECT user._id AS _id, edge.role AS role FROM userConversations as edge JOIN users as user ON user._id = edge._from WHERE edge._to == :conversationId`
  },
  {
    description: '[CHS-7] getUserConversations main query - REFINED',
    ...
    expectedSql: `SELECT conversation FROM (SELECT expand(outE('userConversations').in) FROM :userId) LET conversation = @this WHERE conversation.isArchived == false ORDER BY conversation.updated DESC LIMIT :limit OFFSET :offset`
  },
  {
    description: '[CHS-8] getUserConversations count query - REFINED',
    aql: `RETURN LENGTH( FOR edge IN userConversations FILTER edge._from == @userId LET conversation = DOCUMENT(edge._to) FILTER conversation.isArchived == false RETURN 1 )`,
    expectedSql: `SELECT COUNT(*) FROM (SELECT 1 FROM (SELECT expand(outE('userConversations').in) FROM :userId) LET conversation = @this WHERE conversation.isArchived == false)`
  },
  {
    description: '[CHS-9] getConversationMessages main query',
    aql: `FOR msg IN messages FILTER msg.conversationId == @conversationId SORT msg.sequence DESC LIMIT @offset, @limit LET queryInfo = ( FOR edge IN queryMessages FILTER edge._to == CONCAT('messages/', msg._key) RETURN 1 )[0] RETURN MERGE(msg, { queryInfo: queryInfo })`,
    expectedSql: `SELECT msg, (SELECT 1 FROM queryMessages LET edge = @this WHERE edge._to == concat('messages/', msg._key))[0] AS queryInfo FROM messages LET msg = @this WHERE msg.conversationId == :conversationId ORDER BY msg.sequence DESC LIMIT :limit OFFSET :offset`
  },
  {
    description: '[CHS-10] getConversationMessages count query',
    aql: `FOR msg IN messages FILTER msg.conversationId == @conversationId COLLECT WITH COUNT INTO total RETURN total`,
    expectedSql: `SELECT COUNT(*) AS total FROM messages LET msg = @this WHERE msg.conversationId == :conversationId`
  },
  {
    description: '[CHS-11] updateConversation remove category edges - REFINED',
    aql: `FOR edge IN conversationCategories FILTER edge._from == @conversationId REMOVE edge IN conversationCategories`,
    expectedSql: `DELETE FROM conversationCategories WHERE _from == :conversationId`
  },
  {
    description: '[CHS-12] markMessagesAsRead update specific',
    aql: `FOR msgId IN @messageIdsJson UPDATE { _key: msgId, readStatus: true } IN messages FILTER OLD.conversationId == "convId" AND OLD.readStatus == false RETURN NEW`,
    expectedSql: `FOR msgId IN :messageIdsJson UPDATE messages SET readStatus = true WHERE _key = msgId AND OLD.conversationId == "convId" AND OLD.readStatus == false RETURN AFTER`
  },
  {
    description: '[CHS-13] markMessagesAsRead update all',
    aql: `FOR msg IN messages FILTER msg.conversationId == "convId" AND msg.readStatus == false UPDATE msg WITH { readStatus: true } IN messages RETURN NEW`,
    expectedSql: `UPDATE messages SET readStatus = true WHERE conversationId == "convId" AND readStatus == false RETURN AFTER`
  },
  {
    description: '[CHS-14] markMessagesAsRead update last viewed',
    aql: `FOR edge IN userConversations FILTER edge._from == 'users/123' AND edge._to == 'conversations/abc' UPDATE edge WITH { lastViewedAt: @currentTime } IN userConversations`,
    expectedSql: `UPDATE userConversations SET lastViewedAt = :currentTime WHERE _from == 'users/123' AND _to == 'conversations/abc'`
  },
  {
    description: '[CHS-15] getConversationOwnerId',
    aql: `FOR edge IN userConversations FILTER edge._to == @conversationId AND edge.role == 'owner' RETURN SUBSTRING(edge._from, 6)`,
    expectedSql: `SELECT SUBSTRING(edge._from, 6) FROM userConversations LET edge = @this WHERE edge._to == :conversationId AND edge.role == 'owner'`
  },
  {
    description: '[CHS-16] deleteConversation permission check',
    aql: `FOR edge IN userConversations FILTER edge._to == 'conversations/abc' AND edge._from == 'users/123' RETURN edge`,
    expectedSql: `SELECT edge FROM userConversations LET edge = @this WHERE edge._to == 'conversations/abc' AND edge._from == 'users/123'`
  },
  {
    description: '[CHS-17] deleteConversation get message IDs',
    aql: `FOR msg IN messages FILTER msg.conversationId == "convId" RETURN msg._id`,
    expectedSql: `SELECT msg._id FROM messages LET msg = @this WHERE msg.conversationId == "convId"`
  },
  {
    description: '[CHS-18] deleteConversation delete query-message edges - REFINED',
    aql: `FOR edge IN queryMessages FILTER edge._to == @messageId REMOVE edge IN queryMessages`,
    expectedSql: `DELETE FROM queryMessages WHERE _to == :messageId`
  },
  {
    description: '[CHS-19] deleteConversation delete messages - REFINED',
    aql: `FOR msg IN messages FILTER msg.conversationId == "convId" REMOVE msg IN messages RETURN OLD`,
    expectedSql: `DELETE FROM messages WHERE conversationId == "convId" RETURN BEFORE`
  },
  {
    description: '[CHS-20] deleteConversation delete user edges - REFINED',
    aql: `FOR edge IN userConversations FILTER edge._to == 'conversations/abc' REMOVE edge IN userConversations`,
    expectedSql: `DELETE FROM userConversations WHERE _to == 'conversations/abc'`
  },
  {
    description: '[CHS-21] deleteConversation delete category edges - REFINED',
    aql: `FOR edge IN conversationCategories FILTER edge._from == 'conversations/abc' REMOVE edge IN conversationCategories`,
    expectedSql: `DELETE FROM conversationCategories WHERE _from == 'conversations/abc'`
  },
  {
    description: '[CHS-22] findMessagesForQuery - REFINED',
    aql: `FOR edge IN queryMessages FILTER edge._from == 'queries/123' FOR msg IN messages FILTER msg._id == edge._to LET conversation = ( FOR conv IN conversations FILTER conv._key == msg.conversationId RETURN conv )[0] RETURN { message: msg, conversation: conversation }`,
    expectedSql: `SELECT msg AS message, (SELECT conv FROM conversations LET conv = @this WHERE conv._key == msg.conversationId)[0] AS conversation FROM queryMessages as edge JOIN messages as msg ON msg._id = edge._to WHERE edge._from == 'queries/123'`
  },
  {
    description: '[CHS-23] findOriginatingQuery - REFINED',
    aql: `FOR edge IN queryMessages FILTER edge._to == 'messages/abc' FOR q IN queries FILTER q._id == edge._from RETURN { query: q }`,
    expectedSql: `SELECT q AS query FROM queryMessages as edge JOIN queries as q ON q._id = edge._from WHERE edge._to == 'messages/abc'`
  },
  {
    description: '[CHS-24] linkQueryToConversation existing link check',
    aql: `FOR edge IN queryMessages FILTER edge._from == @queryId AND edge._to == @messageId RETURN edge`,
    expectedSql: `SELECT edge FROM queryMessages LET edge = @this WHERE edge._from == :queryId AND edge._to == :messageId`
  },
  {
    description: '[CHS-25] linkQueryToConversation category exists check',
    aql: `FOR edge IN conversationCategories FILTER edge._from == @conversationId AND edge._to == @categoryId RETURN edge`,
    expectedSql: `SELECT edge FROM conversationCategories LET edge = @this WHERE edge._from == :conversationId AND edge._to == :categoryId`
  },
  {
  ...
  {
    description: '[CHS-27] searchConversations - REFINED',
    aql: `FOR edge IN userConversations FILTER edge._from == 'users/123' FOR conv IN conversations FILTER conv._id == edge._to AND LIKE(LOWER(conv.title), "%term%") RETURN { conversation: conv }`,
    expectedSql: `SELECT conv AS conversation FROM (SELECT expand(outE('userConversations').in) FROM users WHERE _key = '123') LET conv = @this WHERE LIKE(lower(conv.title), "%term%")`
  },
  {
    description: '[CHS-28] getUserConversationStats - REFINED',
    aql: `LET userConvs = ( FOR edge IN userConversations FILTER edge._from == 'users/123' FOR conv IN conversations FILTER conv._id == edge._to RETURN conv ) LET totalCount = LENGTH(userConvs) RETURN { total: totalCount }`,
    expectedSql: `WITH userConvs AS (SELECT conv FROM (SELECT expand(outE('userConversations').in) FROM users WHERE _key = '123') LET conv = @this), totalCount AS (size(userConvs)) SELECT totalCount AS total`
  },
  {
    description: '[CHS-29] getRecentConversations - REFINED',
    aql: `FOR edge IN userConversations FILTER edge._from == 'users/123' FOR conv IN conversations FILTER conv._id == edge._to AND conv.isArchived == false SORT conv.updated DESC LIMIT 5 RETURN conv`,
  ...
  {
    description: '[CHS-30] getFolder conversations - REFINED',
    aql: `FOR edge IN folderConversations FILTER edge._from == 'folders/abc' FOR conv IN conversations FILTER conv._id == edge._to SORT conv.updated DESC RETURN conv`,
  ...
  {
    description: '[CHS-31] getFolder owners - REFINED',
    aql: `FOR edge IN userFolders FILTER edge._to == 'folders/abc' FOR user IN users FILTER user._id == edge._from RETURN user`,
    expectedSql: `SELECT user FROM (SELECT expand(inE('userFolders').out) FROM folders WHERE _key = 'abc') LET user = @this`
  },
  {
    description: '[CHS-32] getFolder child folders',
    aql: `FOR folder IN folders FILTER folder.parentFolderId == 'abc' SORT folder.order ASC, folder.created ASC RETURN folder`,
    expectedSql: `SELECT folder FROM folders LET folder = @this WHERE folder.parentFolderId == 'abc' ORDER BY folder.order ASC, folder.created ASC`
  },
  {
    description: '[CHS-33] getUserFolders base query',
    aql: `FOR edge IN userFolders FILTER edge._from == @userId LET folder = DOCUMENT(edge._to) RETURN { _id: folder._id }`,
    expectedSql: `SELECT folder._id AS _id FROM (SELECT expand(outE('userFolders').in) FROM :userId) LET folder = @this`
  },
  {
    description: '[CHS-34] getUserFolders folder keys query',
    aql: `FOR folder IN folders FILTER folder._id IN @folderIds RETURN folder`,
    expectedSql: `SELECT folder FROM folders LET folder = @this WHERE folder._id IN :folderIds`
  },
  {
    description: '[CHS-35] getUserFolders main query',
    aql: `FOR edge IN userFolders FILTER edge._from == @userId LET folder = DOCUMENT(edge._to) LET conversationCount = LENGTH( FOR convEdge IN folderConversations FILTER convEdge._from == folder._id RETURN 1 ) RETURN { name: folder.name, conversationCount: conversationCount }`,
    expectedSql: `SELECT folder.name AS name, conversationCount AS conversationCount FROM (SELECT expand(outE('userFolders').in) FROM :userId) LET folder = @this WITH conversationCount AS (SELECT COUNT(*) FROM (SELECT 1 FROM folderConversations LET convEdge = @this WHERE convEdge._from == folder._id))`
  },
  {
    description: '[CHS-36] deleteFolder permission query',
    aql: `FOR edge IN userFolders FILTER edge._to == 'folders/abc' AND edge._from == 'users/123' RETURN edge`,
    expectedSql: `SELECT edge FROM userFolders LET edge = @this WHERE edge._to == 'folders/abc' AND edge._from == 'users/123'`
  },
  {
    description: '[CHS-37] deleteFolder conversation link query',
    aql: `FOR edge IN folderConversations FILTER edge._from == 'folders/abc' RETURN edge`,
    expectedSql: `SELECT edge FROM folderConversations LET edge = @this WHERE edge._from == 'folders/abc'`
  },
  {
    description: '[CHS-38] deleteFolder delete user edge - REFINED',
    aql: `FOR edge IN userFolders FILTER edge._to == 'folders/abc' REMOVE edge IN userFolders`,
    expectedSql: `DELETE FROM userFolders WHERE _to == 'folders/abc'`
  },
  {
    description: '[CHS-39] deleteFolder get child folders',
    aql: `FOR folder IN folders FILTER folder.parentFolderId == 'abc' RETURN folder._key`,
    expectedSql: `SELECT folder._key FROM folders LET folder = @this WHERE folder.parentFolderId == 'abc'`
  },
  {
  ...
  },
  {
    ...
    aql: `FOR edge IN userConversations FILTER edge._to == 'conversations/xyz' AND edge._from == 'users/123' RETURN edge`,
    expectedSql: `SELECT edge FROM userConversations LET edge = @this WHERE edge._to == 'conversations/xyz' AND edge._from == 'users/123'`
  },
  {
    description: '[CHS-42] addConversationToFolder existing link',
    aql: `FOR edge IN folderConversations FILTER edge._to == 'conversations/xyz' RETURN edge`,
    expectedSql: `SELECT edge FROM folderConversations LET edge = @this WHERE edge._to == 'conversations/xyz'`
  },
  {
  ...
  },
  {
    ...
    aql: `FOR edge IN folderConversations FILTER edge._from == 'folders/abc' AND edge._to == 'conversations/xyz' RETURN edge`,
    expectedSql: `SELECT edge FROM folderConversations LET edge = @this WHERE edge._from == 'folders/abc' AND edge._to == 'conversations/xyz'`
  },
  {
    description: '[CHS-45] searchFolders - REFINED',
    aql: `FOR edge IN userFolders FILTER edge._from == 'users/123' FOR folder IN folders FILTER folder._id == edge._to AND LIKE(LOWER(folder.name), "%term%") RETURN folder`,
    expectedSql: `SELECT folder FROM (SELECT expand(outE('userFolders').in) FROM users WHERE _key = '123') LET folder = @this WHERE LIKE(lower(folder.name), "%term%")`
  },
  {
  ...
  {
    description: '[CHS-49] findConversationFolder - REFINED',
    aql: `FOR edge IN folderConversations FILTER edge._to == 'conversations/xyz' LET folder = DOCUMENT(edge._from) RETURN folder`,
    expectedSql: `SELECT folder FROM (SELECT expand(inE('folderConversations').out) FROM conversations WHERE _key = 'xyz') LET folder = @this`
  },
  {
  ...
  },
  {
    ...
    aql: `FOR folder IN folders FILTER folder._key == 'abc' RETURN folder.parentFolderId`,
    expectedSql: `SELECT folder.parentFolderId FROM folders LET folder = @this WHERE folder._key == 'abc'`
  },
  {
    description: '[CHS-59] update { _key, ...} in messages',
    aql: `UPDATE { _key: "123", readStatus: true } IN messages`,
    expectedSql: `UPDATE messages SET readStatus = true WHERE _key = "123"`
  },
...
class TestRunner {
  ...
  async initializeDbService() {
    ...
    try {
      ...
      console.error(
        '\x1b[31m%s\x1b[0m',
        `FATAL: Could not get DB connection from service. Integration tests will be skipped.`
      );
  ...
  analyzeFailure(expected, actual) {
    ...
    if (issuesFound === 0) {
      console.log(
        '     \x1b[33m- General structure or logic mismatch. Review the query structure and clause order.\x1b[0m'
      );
    }
