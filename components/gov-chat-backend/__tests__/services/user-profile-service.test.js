...
describe("UserProfileService", () => {
  ...
  describe("getUserProfile", () => {
    ...
    it("should propagate DB errors", async () => {
      ...
      await expect(
        userProfileService.getUserProfile("invalid"),
  ...
  describe("userExists", () => {
    ...
    it("should throw on non-404 errors", async () => {
      ...
      await expect(userProfileService.userExists("user-1")).rejects.toThrow(
        "DB down",
      );
    });
  });
  ...
  describe("protected field stripping", () => {
    it("should strip JIT_PROTECTED_FIELDS from profileData", async () => {
      ...
      expect(Object.prototype.hasOwnProperty.call(updateCall[1], "email")).toBe(
        false,
      );
      expect(
        Object.prototype.hasOwnProperty.call(updateCall[1], "firstName"),
      ).toBe(false);
    });
  });
  ...
  describe("storeFile", () => {
    it("should write file from buffer and return URL", async () => {
      ...
      const result = await userProfileService.storeFile(
        file,
        "user-1",
        "personalIdentification-photo",
      );
  ...
  describe("deleteUserFiles", () => {
    it("should remove user directory when it exists", async () => {
      ...
      expect(mockFsTracker.rmCalls[0][1]).toEqual({
        recursive: true,
        force: true,
      });
    });
  ...
  describe("resetUserData", () => {
    ...
    it("should throw NotFoundError when user not found", async () => {
      mockUsers.document.mockRejectedValueOnce(new Error("Not found"));
      await expect(
        userProfileService.resetUserData("missing"),
      ).rejects.toThrow();
    });
