...
async function startWorkers({ tokenRepository, broadcastRepository, fcmSender }) {
  ...
  const broadcastWorker = new Worker(BROADCAST_QUEUE, (job) => processor.processBroadcast(job), {
    connection,
    concurrency: 2,
    settings: { backoffStrategy: fullJitterBackoff }
  });

  const chunkWorker = new Worker(CHUNK_QUEUE, (job) => processor.processChunk(job), {
    connection,
    concurrency,
  ...
  await notificationQueue.maintenanceQueue
    .add(
      'reap-stale-tokens',
      {},
      {
