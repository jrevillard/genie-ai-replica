...
<template>
  <div class="okf-dashboard">
      ...
      <section v-for="lane in lanes" :key="lane.key" class="okf-dashboard__lane">
        ...
        >
          ...
          <span class="okf-dashboard__card-row">
            ...
            <span class="okf-dashboard__card-count">{{ (r.concept_count || 0) }} {{ translate('okf.dashboard.topics', 'topics') }}</span>
            <span class="okf-dashboard__card-stage">{{ stageLabel(r) }}</span>
          </span>
          <span v-if="isStale(r)" class="okf-dashboard__card-stale">{{ translate('okf.dashboard.stale', 'stale') }}</span>
        </button>
      </section>
    </div>
    ...
    <nav v-if="selectedIds.length >= 2" class="okf-dashboard__bulk" aria-label="Bulk actions">
      ...
      <DsButton variant="primary" @click="bulkOpen = true">{{ translate('okf.dashboard.bulk.publish', 'Publish selected') }}</DsButton>
    ...
    <DsDialog
      ...
      @action="onBulkAction"
    >
      <p>{{ translate('okf.dashboard.bulk.body', 'Once published, downstream chat answers can use them. Each repository below lists its final outcome.') }}</p>
      <DsTable :columns="bulkColumns" :rows="selectedRows">
        <template #cell-name="{ row }">{{ row.name || row.repo_id }}</template>
        <template #cell-status="{ row }">
          <DsStatusTag :variant="row.lifecycle_state === 'review' || row.lifecycle_state === 'approve' ? 'pending' : 'info'">
      ...
      <p v-if="bulkResults.length" class="okf-dashboard__bulk-results">
        ...
        <span v-for="r in bulkResults" :key="r.repo_id" :class="{ 'okf-dashboard__bulk-results-ok': r.ok, 'okf-dashboard__bulk-results-fail': !r.ok }">
...
export default {
  ...
  computed: {
    ...
    expertMode() { return this.isExpert; },
    totalCount() {
    ...
    bulkActions() {
      return [
        ...
        { key: 'publish', label: this.translate('okf.dashboard.bulk.publishConfirm', `Publish ${this.selectedIds.length}`), variant: 'primary' }
      ];
    }
  },
  mounted() {
...
<style scoped>
.okf-dashboard { display: flex; flex-direction: column; gap: var(--space-md); }
.okf-dashboard__bar { display: flex; justify-content: space-between; align-items: center; }
.okf-dashboard__bar-left { display: flex; gap: var(--space-sm); align-items: baseline; }
.okf-dashboard__title { margin: 0; font-size: var(--text-md); font-weight: 600; }
...
.okf-dashboard__filters { display: flex; gap: var(--space-sm); }
.okf-dashboard__lanes { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: var(--space-md); }
.okf-dashboard__lane {
  background: var(--bg);
...
.okf-dashboard__lane-header { display: flex; justify-content: space-between; align-items: center; margin: 0 0 var(--space-sm); }
.okf-dashboard__lane-header h4 { margin: 0; font-size: var(--text-sm); font-weight: 600; }
...
.okf-dashboard__empty { color: var(--muted); font-size: var(--text-sm); padding: var(--space-md); text-align: center; }
.okf-dashboard__card {
  display: flex;
...
.okf-dashboard__card:hover { border-color: var(--accent); background: var(--accent-muted); }
.okf-dashboard__card--selected { border-color: var(--accent); background: var(--accent-muted); }
.okf-dashboard__card-row { display: flex; align-items: center; gap: var(--space-xs); }
.okf-dashboard__card-checkbox { margin: 0; }
.okf-dashboard__card-name { font-weight: 600; }
.okf-dashboard__card-count { font-size: var(--text-xs); color: var(--muted); }
.okf-dashboard__card-stage { margin-left: auto; font-size: var(--text-xs); color: var(--muted); }
...
.okf-dashboard__bulk-results { margin: var(--space-md) 0 0 0; font-size: var(--text-sm); display: flex; gap: var(--space-sm); flex-wrap: wrap; }
.okf-dashboard__bulk-results-ok { color: var(--success); }
.okf-dashboard__bulk-results-fail { color: var(--danger); }
</style>
