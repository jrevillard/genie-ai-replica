...
describe("PATCH /api/chat/conversations/:conversationId", () => {
  it("should update conversation and return 200", async () => {
    ...
    const response = await authPatch("/api/chat/conversations/conv-1", {
      title: "Updated",
    });
  ...
  it("should pass next on service error", async () => {
    chatHistoryService.updateConversation.mockRejectedValue(
      new Error("DB error"),
    );
...
describe("POST /api/chat/conversations/:conversationId/messages/read", () => {
  it("should mark messages as read", async () => {
    ...
    const response = await authPost(
      "/api/chat/conversations/conv-1/messages/read",
      {
    ...
    expect(response.status).toBe(200);
    expect(chatHistoryService.markMessagesAsRead).toHaveBeenCalledWith(
      "conv-1",
      ["msg-1", "msg-2"],
    );
  });
...
describe("POST /api/chat/folders", () => {
  it("should create folder and return 201", async () => {
    chatHistoryService.getUserFolders.mockResolvedValue([]);
    chatHistoryService.createFolder.mockResolvedValue({
      _key: "f-1",
      name: "New Folder",
    });
...
describe("PATCH /api/chat/folders/:folderId", () => {
  it("should update folder and return 200", async () => {
    chatHistoryService.updateFolder.mockResolvedValue({
      _key: "f-1",
      name: "Updated",
    });

    const response = await authPatch("/api/chat/folders/f-1", {
      name: "Updated",
    });
  ...
  it("should return 400 when creating circular parent reference", async () => {
    ...
    expect(response.body.message).toBe(
      "Cannot move a folder to its own subfolder",
    );
  });
...
describe("POST /api/chat/folders/reorder", () => {
  it("should reorder folders and return 200", async () => {
    chatHistoryService.reorderFolders.mockResolvedValue({
      updatedFolders: 2,
      success: true,
    });
...
describe("POST /api/chat/folders/:folderId/conversations/:conversationId", () => {
  it("should add conversation to folder", async () => {
    chatHistoryService.addConversationToFolder.mockResolvedValue({
      success: true,
    });

    const response = await authPost(
      "/api/chat/folders/f-1/conversations/conv-1",
    );
...
describe("DELETE /api/chat/folders/:folderId/conversations/:conversationId", () => {
  it("should remove conversation from folder", async () => {
    chatHistoryService.removeConversationFromFolder.mockResolvedValue({
      success: true,
    });
    ...
    expect(response.status).toBe(200);
    expect(
      chatHistoryService.removeConversationFromFolder,
    ).toHaveBeenCalledWith("f-1", "conv-1", mockUser.iss_sub, mockUser._key);
  });
...
describe("GET /api/chat/conversations/:conversationId/folder", () => {
  ...
  it("should return 404 when conversation not in any folder", async () => {
    ...
    expect(response.status).toBe(404);
    expect(response.body.message).toBe(
      "Conversation not found or not in any folder",
    );
