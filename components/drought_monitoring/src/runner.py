    ...
    for location, (lat, lon) in DISTRICT_COORDS.items():
        try:
            assessment = _run_one_district(
                storage, location, lat, lon, days, reports_dir
            )
            if assessment:
                stored += 1
                ...
                if tier > 0:
                    logger.warning(
                        ...
                        location,
                        tier,
                        assessment.get("tier_label"),
                        assessment.get("drought_score", 0),
                        4,
                    )
                else:
...
def _run_one_district(
    ...
    from drought_report.generate_report import analyse_trend, generate_pdf_report
    ...
    for band in ALL_BANDS:
        ...
        if res.flag:
            ...
            triggers.append(
                f"{label} at {pct:.0f}% of safe level"
                if pct
                else f"{label} below threshold"
            )
