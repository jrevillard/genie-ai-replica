...
- 2026-08-12: Code-review fixes applied. Decisions resolved: red-run banked as build-surface proof (assertion-level red → mutation probe + evidence stage), deleted red-run job documented, ingest chunk-only accepted, contract jobs scoped to retriever+dataprep, in-image import-skip tightened. 12 patches applied + 4 deferred to ledger: pytest in CI, AQL real-builder test, telemetry scan fix, docarray strict import, fake-HTTP restore + sync response, junit path, per-hop wire kwargs, budget timeouts, guard/extractor fixes, dataprep monkeypatch, telemetry test rename, label-roundtrip un-skipped. Verified: ruff clean, `pytest contracts/` 19 passed/14 skipped (in-image skips). Docs updated: `.claude/rules/TESTING.md`, `site/content/en/docs/core/source-tree-analysis.md`.

### Review Findings

#### Resolved (2026-08-12)

- [x] [Review][Resolved] Red-run evidence: BANKED the build-surface red as the milestone-(d) proof — `red-run-v1.5-bare.md` amended to scope the red (build surface only) and point the assertion-level red at story 2.7 (mutation probe) + 3.3 (`verify:evidence` red-run logs). No suite red-run in this story (no runnable v1.5 image without re-graft).
- [x] [Review][Resolved] Deleted `contract-red-run:retriever-v1.5` job: DOCS UPDATED — `red-run-v1.5-bare.md` + `contracts/README.md` now record the one-shot red-run and name 2.7/3.3 as successors; job not restored (intentionally one-shot).
- [x] [Review][Resolved] T3 ingest scope: CHUNK-ONLY ACCEPTED for milestone (d); `INGEST_CONFIG` recorded as the hand-typed production snapshot; the baseline-config-derivation gap noted (rag-baseline carries no chunk config). Round-trip retrieve belongs to the parity work (3.1).
- [x] [Review][Resolved] Per-module contract jobs: SCOPED to retriever + dataprep for milestone (d); chatqna/reranker contract jobs get wired in their re-graft stories (2.4 reranker, 2.6 chatqna) where pytest is installed for those images.
- [x] [Review][Resolved] Bare `except ImportError → skip`: TIGHTENED — the three module imports (label_filter:39, ingest:74, e2e:38) now re-raise when `in_image_comps_importable()` is True, so a genuine in-image import break goes red; skip only when the module is genuinely absent from the image.

#### Patch

- [x] [Review][Patch] `contract:retriever-arango` cannot run — no pytest in the images [.gitlab-ci.yml:755-756] — CI command runs `python -m pytest` with no install step and no Dockerfile adds pytest; retriever image fails with `No module named pytest` every run (dataprep image gets pytest via `requirements.lock`). Fix: install pytest in the container command or add a test layer / contract build stage to the module Dockerfiles. — applied (2026-08-12)
- [x] [Review][Patch] AQL filter-clause tests are tautologies (AC:4 green-on-green) [genie-ai-overlay/contracts/test_contract_label_filter.py:81-111] — `test_aql_filter_clause_constructed_for_labels` builds its own f-string clause and asserts on it; `test_aql_filter_clause_absent_when_no_labels` assigns `""` itself. Never invokes the real builder (`genieai_retriever_arangodb.py:796-810`). Fix: call the real clause-building path with known labels and assert the produced AQL. — applied (2026-08-12)
- [x] [Review][Patch] Telemetry test is self-validating (AC:4 green-on-green) [genie-ai-overlay/contracts/test_contract_telemetry.py:33-59] — rglobs `genie-ai-overlay/**/*.py` which includes `contracts/_harness.py`; `_harness.py:200-208` defines the exact `EXPECTED_SPAN_NAMES` literals, so `found` is always all 7 and the test can never fail. Fix: exclude `contracts/` (and `.venv`/`.ruff_cache`/`reports`) from the scan, or assert the actual tracer call sites. — applied (2026-08-12)
- [x] [Review][Defer] filter_clause drop surface guarded only by source-grep [genie-ai-overlay/contracts/test_contract_label_filter.py:114-128] — asserts `"filter_clause=" in inspect.getsource(cls.invoke)`; passes if the literal appears in a comment and cannot catch a runtime drop. Fix: behavioral assertion (stubbed vector store) that the excluded-document set is correct. — deferred: the retriever re-graft re-scopes label-filter behavior
- [x] [Review][Defer] Streaming metadata shape test is hardcoded [genie-ai-overlay/contracts/test_contract_e2e_pipeline.py:67-90] — parses its own `data: {...}` literal; never calls the real `_stream_with_metadata`, so a streaming-format rename on the bump passes green. Fix: drive the real stream with a fake session and assert the emitted metadata event. — deferred: the chatqna re-graft exercises the real stream
- [x] [Review][Defer] E2E graph test asserts only `result is not None` [genie-ai-overlay/contracts/test_contract_e2e_pipeline.py:92-157] — a silent early-exit / short-circuit also returns non-None; does not substantiate "pipeline REACHES the LLM node". Fix: assert the observable result surface. — deferred: the re-graft work re-asserts against real module surfaces
- [x] [Review][Defer] E2E omits confidence distribution + abstention + response schema [genie-ai-overlay/contracts/test_contract_e2e_pipeline.py] — T6 lists all four surfaces; only label roundtrip, streaming shape, and graph-schedule are asserted. Fix: add the missing assertions. — deferred: the parity evaluation regression set covers confidence/abstention
- [x] [Review][Patch] Docarray-rename sensitivity is a try/except fallback that passes both versions [test_contract_orchestrator_wire.py:150-154, test_contract_ingest.py:87-91, test_contract_e2e_pipeline.py:142-145, test_contract_nfrp_budgets.py:72-77] — `try: opea_docarray except ImportError: docarray`; the rename hack applies identically on v1.3/v1.5 so a broken `opea_docarray` still passes via fallback (AC:4 surface). Fix: assert the resolved module is the expected one and fail if the primary import fails. — applied (2026-08-12)
- [x] [Review][Patch] `install_fake_aiohttp` mutates `sys.modules` globally, never restores; sync `requests` gets coroutines [genie-ai-overlay/contracts/_harness.py:146-179] — patches `aiohttp.ClientSession` + `requests.post/get` without restore; `_FakeResponse.json()` is an async coroutine, so a synchronous `requests.post(...).json()` caller receives a coroutine. Fix: restore in fixture teardown; make the response usable by both async and sync callers. — applied (2026-08-12)
- [x] [Review][Patch] `contract:unit` junit artifact path is wrong [.gitlab-ci.yml:830-847] — `cd`s to `genie-ai-overlay/contracts`, writes `reports/contract-report.xml` there, but declares artifact `reports/contract-report.xml` (resolved from `$CI_PROJECT_DIR`); job fails at artifact collection even on a green run. Fix: write to a project-root-relative path or declare the nested path. — applied (2026-08-12)
- [x] [Review][Patch] Wire-test kwargs snapshot is per-hop overwrite + tests the generic orchestrator, not GENIE wiring [genie-ai-overlay/contracts/test_contract_orchestrator_wire.py:89-91] — `captured["align_inputs"] = dict(kwargs)` overwritten on every `align_inputs` call, so only the last hop's kwargs are checked (a drop on an earlier hop is invisible); the graph registers generic remote `MicroService`s, not chatqna's `_build_rag_graph`. Fix: accumulate per-hop kwargs and assert every hop received them. — applied (2026-08-12)
- [x] [Review][Patch] Budget tests have no timeout on `asyncio.run` [genie-ai-overlay/contracts/test_contract_nfrp_budgets.py:97-112, 115-141] — a hung `schedule()`/chunker stalls CI instead of failing within budget. Fix: wrap in `asyncio.wait_for` with the budget. — applied (2026-08-12)
- [x] [Review][Patch] `in_image_comps_importable` keys off hardcoded path markers [genie-ai-overlay/contracts/_harness.py:55-71] — markers `/app/comps`, `/home/user/comps` match current images, but a future vendor-path change silently skips the whole suite green. Fix: validate `comps.__file__` shape (ends in `/comps/__init__.py` with `cores/` present). — applied (2026-08-12)
- [x] [Review][Patch] `extract_dashboard_services` treats template/regex values as service names [genie-ai-overlay/contracts/_harness.py:227-239] — `service_name=~"$service"` lands `$service` in the set. Fix: filter out `$`-prefixed and regex-metachar values. — applied (2026-08-12)
- [x] [Review][Patch] `_dataprep_instance` mutates module-global `CONTENT_EXTRACTION_METHOD` without restore; `obj.tracer` is dead [genie-ai-overlay/contracts/test_contract_ingest.py:77-79] — `_load_and_chunk` uses the module-global `tracer`, not `self.tracer`. Fix: restore the env in teardown; drop the dead attribute set. — applied (2026-08-12)
- [x] [Review][Patch] `test_dashboard_referenced_spans_are_emitted` is misnamed — checks services, not spans [genie-ai-overlay/contracts/test_contract_telemetry.py:62-79] — despite the name it only cross-checks the dashboard service set, duplicating the harness test. Fix: rename/clarify, or make it assert span names. — applied (2026-08-12)
