...
def script_project_root() -> Path | None:
    ...
    if (
        len(parents) >= 3
        and parents[0].name == "scripts"
        and parents[1].name == "_bmad"
    ):
        return parents[2]
    return None
...
def warn_on_masked_override(
    chosen: Path, rejected: list[Path], skill_name: str
) -> None:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Resolve skill customization using four-layer TOML merge (skill default, global user, project team, project user)."
    )
    parser.add_argument(
