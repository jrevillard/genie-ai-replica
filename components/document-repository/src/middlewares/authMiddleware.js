...
const authenticateToken = async (req, res, next) => {
  ...
  try {
    ...
    if (!decoded) {
      logger.warn('[AUTH DEBUG] Decoded token is null or undefined');
    ...
    try {
      ...
      if (user.disabled === true) {
        logger.warn(`[AUTH DEBUG] User account is disabled: ${decoded.userId}`);
      ...
      logger.warn(`[AUTH DEBUG] Failed to fetch user details: ${userError.message}`, { stack: userError.stack });
...
const authorizeRole = (allowedRoles) => {
    return async (req, res, next) => {   
      ...
      try {
        ...
        if (!hasAllowedRole) {
          logger.warn(`[AUTH MIDDLEWARE] User role "${userRole}" not in allowed roles: [${allowedRoles.join(', ')}]`);
