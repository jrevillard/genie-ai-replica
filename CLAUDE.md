...
- `DEFAULT_LOCATION` / `DEFAULT_LAT` / `DEFAULT_LON` - Shared fallback place for every climate service (backend weather, web alert banner, weather-mcp-service, drought-monitoring, warning_system_engine, geo-inference-worker) when a request names no place and geolocation is unavailable. Unset = `Dhaka` / `23.8103` / `90.4125`. `DEFAULT_LOCATION_DISTRICT` names the administrative district a sub-district fallback belongs to (e.g. `Sapahar` → `Naogaon`) so BMD CAP warnings for that district also target it.
- `DELINEATION_*` - Field delineation in geo-inference-worker (Fields of The World via ftw-tools 2.0): `DELINEATION_ENGINE` (`ftw` default, or `delineate-anything`), `DELINEATION_MODEL` (registry name; default `FTW_PRUE_EFNET_B5_CCBY`), `DELINEATION_RADIUS_KM` (half-width of the searched square, default 2.5) capped by `DELINEATION_MAX_RADIUS_KM` (5), `DELINEATION_YEAR` (crop-calendar year, default previous year), `DELINEATION_STAC_HOST` (`mspc`/`earthsearch`), `DELINEATION_SCENE_MODE` (`latest` default: newest cloud-free pass within `DELINEATION_MAX_AGE_DAYS`, 42; or `crop-calendar`), `DELINEATION_SUPER_RESOLUTION` (`true` default: ESA SEN2SR 10 m → 2.5 m before FTW). See `components/geo-inference-worker/README.md`.
