...
<template>
  <div class="okf-step">
    ...
    <div class="okf-step__summary">
      <p>
        <strong>{{ translate('okf.steps.review.repo', 'Repository') }}:</strong> {{ (draft && draft.name) || '—' }}
      </p>
      <p>
        <strong>{{ translate('okf.steps.review.topics', 'Topics') }}:</strong> {{ (draft && draft.concept_count) || 0 }}
      </p>
      <p>
        <strong>{{ translate('okf.steps.review.labels', 'Labels') }}:</strong> {{ labelList }}
      </p>
      <p>
        <strong>{{ translate('okf.steps.review.sources', 'Sources') }}:</strong> {{ sourceList }}
      </p>
    </div>
  </div>
</template>
