...
- **Dataprep defensively creates the ArangoDB vector index on the embedding field (#1002):** New `ensure_vector_index()` helper in `genieai_dataprep_arangodb.py` runs both on service startup and post-ingest. Idempotent and dim/metric/nLists-aware: skip if the canonical `idx_embedding_<dim>` already matches, drop+recreate on dim/metric/nLists drift (e.g. an embedder model swap from bge-base 768-dim to bge-large 1024-dim), and also drops the legacy `vector_index` that `langchain-arangodb` would otherwise lazy-create with `nLists=1` (defeats ANN) and without dim-staleness detection. Telemetry via the `dataprep.ensure_vector_index` OTel span; status events routed through `_write_ingestion_log` so they appear in the UI. Defensive only — behaviour under the default `RETRIEVER_ARANGO_USE_APPROX_SEARCH=false` is unchanged.
