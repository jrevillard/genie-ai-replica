  ...
  try {
    ...
    return {
      ...
      categoryServices: existence[2]
        ? { collection: collections.categoryServices, count: categoryServicesCount.count }
        : null,
      serviceCategoryTranslations: existence[3]
        ? { collection: collections.serviceCategoryTranslations, count: serviceCategoryTranslationsCount.count }
        : null,
      serviceCategoryTranslationsEdge: existence[4]
        ? { collection: collections.serviceCategoryTranslationsEdge, count: serviceCategoryTranslationsEdgeCount.count }
        : null,
      serviceTranslations: existence[5]
        ? { collection: collections.serviceTranslations, count: serviceTranslationsCount.count }
        : null,
      serviceTranslationsEdge: existence[6]
        ? { collection: collections.serviceTranslationsEdge, count: serviceTranslationsEdgeCount.count }
        : null
    };
  } catch (error) {
...
async function exportServiceCategoriesAndServices(dbConfig) {
  try {
    ...
    const jsonData = EXPORT_CONFIG.prettyPrint ? JSON.stringify(exportData, null, 2) : JSON.stringify(exportData);
