...
const securityScanService = {
  ...
  async runSecurityScan(logsService) {
    ...
    try {
      ...
      const { vulnerabilities, failedLogins, suspiciousActivities } =
      ...
      logger.info(
        `Security scan completed in ${(Date.now() - startTime) / 1000}s`,
      );
      return scanResult;
    } catch (error) {
      logger.error(`Error in runSecurityScan: ${error.message}`, {
        stack: error.stack,
      });
      throw error;
    }
  ...
  async processLogsInParallel(logsService) {
    ...
    const vulnerabilityPatterns = [
      ...
      },
      {
        ...
        recommendation:
          "Ensure sensitive files are not exposed and access is blocked.",
        service: "http",
      },
      {
        type: "ip_blocked",
        ...
        recommendation:
          "Review blocked IPs for false positives and enhance rate limiting.",
        service: "system",
      },
      {
      ...
      },
      {
        ...
        severity: "medium",
        regex:
          /collection\.save failed.*expecting both `_from` and `_to` attributes/i,
      ...
      },
      {
        ...
        severity: "low",
        regex:
          /Blocked access to sensitive path:\s*(\/\.well-known\/appspecific\/com\.chrome\.devtools\.json)/i,
        description:
          "Attempt to access non-critical configuration file detected",
      ...
      },
      {
        ...
        recommendation:
          "Review for probing attempts and ensure proper routing.",
        service: "http",
      },
      {
    ...
    try {
      console.log(
        `Starting unified log scan for period ${startDate} to ${endDate}`,
      );
      const allLogFiles = await logsService.getLogFilesInRange(
        startDate,
        endDate,
        true,
      );
      const validLogFiles = (
        await Promise.all(
          allLogFiles.map(async (file) => {
            if (file.endsWith(".gz") && !(await this.isGzipValid(file)))
              return null;
            if (
              !file.match(
                /(combined|error)-\d{4}-\d{2}-\d{2}\.log(\.gz|\.\d+\.gz)?$/,
              )
            )
              return null;
            return file;
      ...
      for (let i = 0; i < validLogFiles.length; i += concurrencyLimit) {
        ...
        const batchPromises = batch.map((file) =>
          this.processFile(file, startTime, {
            ...
            logger.error(
              `Error processing file ${file} in worker: ${err.message}`,
            );
            return {
          ...
          for (const severity of ["critical", "medium", "low"]) {
            for (const vuln of result.vulnerabilities[severity]) {
              ...
              if (finalIssueMap.has(aggregationKey)) {
                ...
                if (vuln.lastSeen > existingIssue.lastSeen)
                  existingIssue.lastSeen = vuln.lastSeen;
              } else {
        ...
        console.debug(
          `Batch processed. Total lines so far: ${totalLinesProcessed}`,
        );
      }
      ...
      suspiciousActivities =
        this.removeDuplicateLogEntries(suspiciousActivities);

      return { vulnerabilities, failedLogins, suspiciousActivities };
    } catch (error) {
      logger.error(`Error in processLogsInParallel: ${error.message}`, {
        stack: error.stack,
      });
      throw error;
    }
  ...
  async checkLogsForIssues(logsService) {
    logger.info(
      "Legacy checkLogsForIssues called. Checking cache or running full scan.",
    );
  ...
  async checkFailedLogins(logsService) {
    logger.info(
      "Legacy checkFailedLogins called. Checking cache or running full scan.",
    );
    const cached = await this.checkCachedResults();
  ...
  async checkSuspiciousActivities(logsService) {
    logger.info(
      "Legacy checkSuspiciousActivities called. Checking cache or running full scan.",
    );
    const cached = await this.checkCachedResults();
  ...
  parseLogLine(line, file, lineNumber, invalidLogStream) {
    function extractUrl(message) {
      const urlMatch = message.match(
        /https?:\/\/[^\s]+|(GET|POST|PUT|DELETE)\s+([^\s]+)/i,
      );
    ...
    if (fallbackMatch) {
      ...
      const timestamp = DateTime.fromFormat(datetime, "yyyy-MM-dd HH:mm:ss", {
        zone: "utc",
      });
      if (!timestamp.isValid) {
        if (invalidLogStream)
  ...
  async getLastScanDetails() {
    try {
      ...
      logger.error(`Error in getLastScanDetails: ${error.message}`, {
        stack: error.stack,
      });
      throw error;
    }
  ...
  async scanForVulnerabilities() {
    ...
    try {
      ...
      logger.error(`Error in scanForVulnerabilities: ${error.message}`, {
        stack: error.stack,
      });
      return vulnerabilities;
    }
    ...
    try {
      ...
      if (!headers["strict-transport-security"])
        missingHeaders.push({
          ...
          severity: "medium",
          description:
            "HSTS header not set, increasing risk of protocol downgrade attacks",
          recommendation:
      ...
      if (!headers["x-content-type-options"])
        missingHeaders.push({
          ...
          severity: "medium",
          description:
            "X-Content-Type-Options header not set, increasing risk of MIME type confusion attacks",
      ...
      if (!headers["x-frame-options"])
        missingHeaders.push({
          ...
          severity: "medium",
          description:
            "X-Frame-Options header not set, increasing risk of clickjacking attacks",
      ...
      if (!headers["referrer-policy"])
        missingHeaders.push({
          ...
          severity: "low",
          description:
            "Referrer-Policy header not set, potentially leaking referrer information",
          recommendation:
            "Add Referrer-Policy: no-referrer-when-downgrade header",
        });
    ...
    try {
      ...
      if (headers["server"] && headers["server"].includes("/"))
        leakageIssues.push({
          ...
          recommendation:
            "Configure server to remove version information from Server header",
        });
      ...
      logger.error(
        `Error checking server information leakage: ${error.message}`,
      );
      return [];
    }
  },
  ...
  async checkTimestampDisclosure() {
    try {
        ...
        try {
          ...
          if (matches && matches.length > 0) {
            disclosureIssues.push({
              ...
              recommendation:
                "Format timestamps as ISO strings or human-readable dates before sending to client",
            });
          }
        } catch (err) {
          console.debug(
            `Skipping timestamp check for ${endpoint}: ${err.message}`,
          );
          continue;
        }
      }
      ...
      console.debug(
        `Timestamp disclosure issues found: ${disclosureIssues.length}`,
      );
        ...
        try {
          ...
          if (response.status !== 404) {
            foundFiles.push({
              ...
              recommendation:
                "Block access to hidden files and development artifacts",
            });
          }
        } catch (err) {
          if (err.response && err.response.status !== 404) {
            foundFiles.push({
              ...
              recommendation:
                "Verify server configuration for handling hidden files",
            });
          }
        }
      }
  ...
  async loginIssues(logsService) {
    ...
    try {
      ...
      const allKeywords = [
        ...new Set([...loginKeywords, ...suspiciousKeywords]),
      ];
      ...
      try {
        ...
        console.debug(
          `Found ${results.logs?.length || 0} logs matching keywords`,
        );
        ...
        if (results.logs && results.logs.length > 0) {
          for (const log of results.logs) {
            ...
            const loginMatch = loginKeywords.find((keyword) =>
              messageLower.includes(keyword.toLowerCase()),
            );
            const suspiciousMatch = suspiciousKeywords.find((keyword) =>
              messageLower.includes(keyword.toLowerCase()),
            );
      ...
      const uniqueSuspiciousIssues =
        this.removeDuplicateLogEntries(suspiciousIssues);
      ...
      logger.error(`Error checking logs: ${error.message}`, {
        stack: error.stack,
      });
      return {
  ...
  generateRecommendations(loginIssues, suspiciousActivities, vulnerabilities) {
    ...
    if (loginIssues.count > 0) {
      const disabledAccountCount = loginIssues.details.filter((issue) =>
      ...
      if (disabledAccountCount > 0) {
        recommendations.push({
          ...
          action:
            "Review account status in user management and verify if account disabling is legitimate",
        });
      }
      recommendations.push({
        severity: "medium",
        ...
        action:
          "Consider implementing account lockout policies and multi-factor authentication",
      });
    }
    ...
    if (vulnerabilities.medium.length > 0) {
      const sensitiveFileAccess = vulnerabilities.medium.filter(
        (v) => v.type === "sensitive_file_access",
      );
      ...
      const dbIssues = vulnerabilities.medium.filter(
        (v) => v.type.includes("database") || v.type === "db_error",
      );
      if (dbIssues.length > 0) {
        recommendations.push({
          ...
          action:
            "Review database configuration, connections, and query handling",
        });
      }

      const jwtIssues = vulnerabilities.medium.filter(
        (v) => v.type.includes("jwt") || v.type === "token_issue",
      );
      if (jwtIssues.length > 0) {
        recommendations.push({
          ...
          action:
            "Review token expiration settings and refresh token implementation",
        });
      }

      const headerIssues = vulnerabilities.medium.filter((v) =>
        v.type.includes("header"),
      );
      if (headerIssues.length > 0) {
        recommendations.push({
          ...
          action:
            "Configure server to add proper security headers for all responses",
        });
      }
      ...
      if (leakageIssues.length > 0) {
        recommendations.push({
          ...
          action:
            "Configure server to prevent leaking version information and hide internal details",
        });
      }

      const corsIssues = vulnerabilities.medium.filter((v) =>
        v.type.includes("cross_domain"),
      );
      if (corsIssues.length > 0) {
        recommendations.push({
          ...
          action:
            "Restrict CORS to only allow trusted domains instead of wildcard (*) origin",
        });
      }

      const ipBlockedIssues = vulnerabilities.medium.filter(
        (v) => v.type === "ip_blocked",
      );
      if (ipBlockedIssues.length > 0) {
        recommendations.push({
          ...
          action:
            "Investigate blocked IPs for malicious activity and review rate limiting policies",
        });
      }

      const authFailureIssues = vulnerabilities.medium.filter(
        (v) => v.type === "auth_failure_401",
      );
      if (authFailureIssues.length > 0) {
        recommendations.push({
          ...
          action:
            "Enhance authentication mechanisms and monitor for brute force attacks",
        });
      }
    }

    if (vulnerabilities.low.length > 0) {
      const nonCriticalFileAccess = vulnerabilities.low.filter(
        (v) => v.type === "non_critical_file_access",
      );
      if (nonCriticalFileAccess.length > 0) {
        recommendations.push({
          ...
          action:
            "Verify if these files need to be publicly accessible or should be blocked",
        });
      }
      ...
      if (missingResources.length > 0) {
        recommendations.push({
          ...
          action:
            "Update application to remove references to non-existent endpoints or implement the missing resources",
        });
      }

      const registrationIssues = vulnerabilities.low.filter(
        (v) => v.type === "registration_failure",
      );
      if (registrationIssues.length > 0) {
        recommendations.push({
          ...
          action:
            "Implement CAPTCHA or rate limiting to prevent automated registration abuse",
        });
      }

      const logLimitIssues = vulnerabilities.low.filter(
        (v) => v.type === "log_limit_exceeded",
      );
    ...
    recommendations.push({
      ...
      action:
        "Implement regular security audits, keep dependencies updated, and consider penetration testing",
    });
  ...
  async saveScanResults(results) {
    ...
    try {
      ...
      console.log(
        `Saving scan results to directory: ${dataDir}, file: ${resultsPath}`,
      );
...
if (!isMainThread) {
  ...
  try {
    ...
    console.error(
      `Error creating invalid log stream for ${file}: ${err.message}`,
    );
  }
