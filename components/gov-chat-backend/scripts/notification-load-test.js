...
async function run() {
  if (!SECRET) {
    console.error('NOTIFICATION_BROADCAST_SECRET is required');
    process.exit(1);
  }
  const district = arg('district', null);
  const body = broadcastBody(
    process.argv.includes('--all') || !district ? {} : { districts: [district], location: district }
  );
  ...
  for (;;) {
    ...
    process.stdout.write(
      `\r[${Math.round((Date.now() - t0) / 1000)}s] status=${s.status} sent=${s.counts?.sent} failed=${s.counts?.failed} pruned=${s.counts?.pruned} chunks=${s.counts?.chunksDone}/${s.counts?.chunksTotal}   `
    );
    if (['completed', 'partial', 'failed'].includes(s.status)) {
      ...
      console.log(
        `  sent+failed==matched: ${sent + failed === matched ? 'PASS' : `FAIL (${sent}+${failed}!=${matched})`}`
      );
      process.exit(0);
    }
