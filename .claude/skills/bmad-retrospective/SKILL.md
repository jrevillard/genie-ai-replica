...
4. **Load config** by running `uv run {project-root}/_bmad/scripts/resolve_config.py --project-root {project-root}`. If the command fails, read the merge logic in `{project-root}/_bmad/scripts/resolve_config.py` and apply it yourself. Resolve `project_name`, `user_name`, `communication_language`, `document_output_language`, `user_skill_level`, `planning_artifacts`, `implementation_artifacts`, `output_folder`, and `date` (system datetime). Speak all output in `{communication_language}`; write all documents in `{document_output_language}`. Never state time estimates — AI has changed development speed, so hour/day/week predictions are noise.
