<!-- OkfStudioDashboard.vue — Story #978 lifecycle edition (David, 2026-08-28).
  Five lifecycle lanes — Import / In review / Published / Ingested /
      ...
      <section v-for="lane in lanes" :key="lane.key" class="okf-dashboard__lane">
        ...
        <div v-for="r in reposInLane(lane.key)" :key="r.repo_id" class="okf-dashboard__card-wrap">
          ...
          >
            ...
            <span class="okf-dashboard__card-row">
              <!-- BUILDING GATE (David, 2026-09-02; import/RAG boundary
                2026-09-03): an animated build state — the import runs in the
                background and the card stays in this lane until it completes. -->
              <!-- P0-UI (David, 2026-09-08): a genuinely-running drain spins
                   too — the chip alone left the card looking frozen. -->
              <DsSpinner v-if="isBuilding(r) || isRedraining(r)" size="sm" :aria-label="buildingAria(r)" />
              ...
              <!-- P0-UI (David, 2026-09-08): a publish-state repo whose drain
                   is running shows this chip INSTEAD of any Building/Import
                   presentation. Covers the serving re-drain AND the
                   between-state after a re-ingest click. -->
              <DsPill v-if="isRedraining(r)" variant="info" class="okf-dashboard__redrain">{{
                translate('okf.dashboard.stage.redraining', 'Re-draining…')
              }}</DsPill>
              <!-- P0-UI: an honest FAILED drain (65's honest row-failures). -->
          ...
          <div class="okf-dashboard__card-actions" role="group" :aria-label="actionsAria(r)">
            <!-- Building: NO lifecycle action is offered (the server refuses
              transitions with 409 BUILD_IN_PROGRESS while an import runs) —
              a disabled chip tells the steward to wait. An ingest drain on a
              published repo shows Ingesting… instead. -->
            <DsButton v-if="isImporting(r)" variant="secondary" small disabled>
              {{ translate('okf.dashboard.card.building', 'Building…') }}
            </DsButton>
            <DsButton v-else-if="isDraining(r)" variant="secondary" small disabled>
              {{ translate('okf.dashboard.card.ingesting', 'Ingesting…') }}
            </DsButton>
...
const LANES = [
  { key: 'draft', label: 'Import', emptyText: 'Nothing importing yet' },
...
// Subject-area filter options load from the Knowledge Hierarchy
// (loadSubjectAreaOptions) — the hard-coded list is gone (David,
// 2026-09-04). 'All subject areas' is the ever-present first option.
const DOMAIN_FILTER_DEFAULT = [{ value: '', label: 'All subject areas' }];
...
export default {
  ...
  data() {
    return {
      ...
      domainFilterOptions: DOMAIN_FILTER_DEFAULT,
      actionBusy: false,
      exportBusy: false,
      actionError: '',
      publishAsk: null,
  ...
  mounted() {
    ...
    // THE 11-HOUR GHOST FIX (David, 2026-09-05): a left-open dashboard kept
    // showing "Building…" for hours after the import finished — chips and
    // lanes read the Vuex store, which never refreshed itself. While ANY repo
    // is building, poll the repo list so cards flip on their own.
  ...
  methods: {
    /** 5s store poll — UNCONDITIONAL while the dashboard is mounted (David,
     * 2026-09-08, standing directive "no clever gates": every frozen-card
     * complaint this week traced to gate starvation — a gate keyed on the
     * STALE store objects can never see a state it doesn't already show).
     * One list query per 5s; cards project whatever the fresh doc says
     * (isBuilding/isRedraining/failed all keep working, fed live data). */
    async pollBuilding() {
    ...
    // P0-UI: an honest FAILED drain (65's honest row-failures) — the card
    // must say so instead of spinning forever. A failed row keeps the poll
    // alive so the card flips the moment a re-ingest or settle lands.
    isDrainFailed(r) {
      return !!(r && r.rag_ingestion && r.rag_ingestion.status === 'failed');
    },
    // SUBJECT AREAS follow the Knowledge Hierarchy Categories (David,
    // 2026-09-04): the filter loads its options from the curated tree and
    // keeps any legacy hard-coded repo domains visible as "(legacy)".
    async refreshDomainOptions() {
      try {
        ...
        if (Array.isArray(opts) && opts.length) {
          this.domainFilterOptions = [...DOMAIN_FILTER_DEFAULT, ...opts];
        }
      } catch {
        /* keep the previous options — the filter never blanks */
      }
    },
    refreshAll() {
      this.$store.dispatch('okf/fetchRepos', { stage: 'all' }).catch(() => {});
    ...
    stageLabel(r) {
      // P0-UI (David's re-test, 2026-09-08): SERVING TRUTH FIRST — a
      // publish-state repo can never read as 'Building…', whatever a stale
      // drain flag says. Building is import-phase (pre-publish) only.
    ...
    isImporting(r) {
      return !!(r.conversion && !['done', 'failed'].includes(r.conversion.status));
    },
    isDraining(r) {
    ...
    isRedraining(r) {
      return okfRepoOps.isRedraining(r);
    },
    ...
    onCardEnter(r, evt) {
      if (!this.hasStatusPopup(r)) return;
