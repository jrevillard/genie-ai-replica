...
export function applyThemeToAxes(svg, theme) {
  ...
  svg.selectAll('.domain, .tick line').attr('stroke', theme.borderColor)
...
export function createBarGradient(defs, id, theme) {
  ...
  if (theme.isDarkMode) {
    ...
    gradient.append('stop').attr('offset', '0%').attr('stop-color', '#4a8bbf')

    gradient.append('stop').attr('offset', '100%').attr('stop-color', '#2d6fa7')
  } else {
    // Lighter gradient for light mode
    gradient.append('stop').attr('offset', '0%').attr('stop-color', '#62d9a6')

    gradient.append('stop').attr('offset', '100%').attr('stop-color', '#2da676')
  }
...
export function createThemedText(svg, text, options, theme) {
  ...
  const { x = 0, y = 0, fontSize = 12, fontWeight = 'normal', textAnchor = 'start', className = '' } = options || {}
