...
// - use configuration from appConfig.js
...
class DatabaseService {
  constructor() {
    ...
    const databaseName = appConfig.database.databaseName;
    const username = appConfig.database.username;
    const password = appConfig.database.password;
    ...
    this._defaultConfig = {
      ...
      agentOptions: {
        keepAlive: true,
        keepAliveMsecs: appConfig.database.connectionSettings?.agentOptions?.keepAliveMsecs || 120000,
        maxSockets: appConfig.database.connectionSettings?.agentOptions?.maxSockets || 50,
        maxFreeSockets: appConfig.database.connectionSettings?.agentOptions?.maxFreeSockets || 20,
        timeout: appConfig.database.connectionSettings?.agentOptions?.timeout || 180000,
        freeSocketTimeout: appConfig.database.connectionSettings?.agentOptions?.freeSocketTimeout || 300000
      },
      timeout: appConfig.database.connectionSettings?.timeout || 180000
    };
  ...
  async getConnection(name = 'default', config = {}) {
    ...
    logger.info(`[DB_CONNECTION] ARANGO_URL: ${config}`);
