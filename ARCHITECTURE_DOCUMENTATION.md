# GENIE.AI — Platform Architecture Documentation

**Canonical source:** [`docs/architecture.md`](docs/architecture.md). This file at the repository root reproduces that document for submissions, printing, or export (for example to PDF or Word). When the architecture changes, update `docs/architecture.md` first, then refresh this copy if needed.
...
This ensures ArangoDB always reflects the current state from the identity provider. For detailed user management procedures, see the [Keycloak Admin Guide](docs/keycloak-admin-guide.md).
...
For configuration details, see the [External IdP Integration Guide](docs/external-idp-integration-guide.md).
...
- [Keycloak Admin Guide](docs/keycloak-admin-guide.md) -- Realm configuration, user management, client setup
- [Docker Compose Setup](docs/docker-compose-setup.md) -- Local development deployment with Docker Compose
- [Docker Swarm Setup](docs/docker-swarm-setup.md) -- Production deployment with Docker Swarm and Ansible
- [External IdP Integration Guide](docs/external-idp-integration-guide.md) -- Connecting Google, Microsoft, and SAML identity providers
- [E2E Tests](docs/e2e-tests/README.md) -- End-to-end test procedures for authentication and session lifecycle
