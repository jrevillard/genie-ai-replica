...
def _write_event(events_dir, name, event):
    ...
    if _is_link_like(events_dir):
        raise OSError(
            f"refusing to write events into a redirected directory: {events_dir}"
        )
    ...
    create = (
        os.O_WRONLY | os.O_CREAT | os.O_EXCL | o_nofollow | getattr(os, "O_BINARY", 0)
    )
...
def main() -> int:
    ...
    events_dir = os.environ.get("BMAD_LOOP_EVENTS_DIR") or os.path.join(
        run_dir, "events"
    )
    try:
