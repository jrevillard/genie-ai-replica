...
# Modifications made by the International Telecommunication Union (ITU) 2025
...
def load_pdf(pdf_path):
    ...
    text_per_page = []  
    position_based_candidates = []  

    header_threshold = 50  
    footer_threshold_ratio = 50  
    ...
    for i in range(doc.page_count):
        ...
        # Identify text in the header/footer position
        for block in text_blocks:
            if len(block) >= 5:
                x0, y0, x1, y1, text, *_ = block  

                if y0 < header_threshold or y1 > footer_threshold:
                    position_based_candidates.append(text.strip())  

        text_per_page.append([block[4] for block in text_blocks if len(block)>=5])

    line_counts = Counter(position_based_candidates)

    # Define a threshold (e.g., text must appear on >60% of pages)
    frequency_threshold = int(doc.page_count * 0.6)
    common_lines = {line for line, count in line_counts.items() if count > frequency_threshold}

    cleaned_text = [
        "\n".join([line for line in text if line not in common_lines])
        for text in text_per_page
    ]

    combined_result = "".join(cleaned_text)
...
def load_html(html_path):
    ...
    for tag in soup.find_all(["h1", "h2", "h3", "h4", "h5", "h6", "p", "li"]):
        cleaned_text = " ".join(tag.get_text().split()).strip() 
        if cleaned_text:  
            extracted_text.append(cleaned_text)
    
    structured_data = partition_html(text=raw_html)
    structured_text = " ".join([clean_text(item.text) for item in structured_data if item.text])
    final_text = "\n\n".join(extracted_text) + "\n\n" + structured_text
