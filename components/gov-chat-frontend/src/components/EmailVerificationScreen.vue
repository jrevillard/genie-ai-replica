...
export default {
  ...
  watch: {
    theme(newTheme) {
      console.log('[VERIFY] Theme prop updated:', newTheme, 'source: prop change', new Date().toISOString())
      this.applyTheme()
    },
  },
  created() {
    ...
    if (this.$i18n) {
      ...
      console.log('[VERIFY] Verifying translation:', this.$t('verification.verifying'))
    }
  ...
  mounted() {
    ...
    console.log('[VERIFY] Title text content:', title ? title.textContent : 'not found')
    console.log('[VERIFY] Title computed color:', title ? window.getComputedStyle(title).color : 'not found')
    const subtitle = document.querySelector('.heading')
    console.log('[VERIFY] Subtitle computed color:', subtitle ? window.getComputedStyle(subtitle).color : 'not found')
    const icon = document.querySelector('.app-logo')
    console.log('[VERIFY] Icon source:', icon ? icon.querySelector('img')?.src : 'fallback')
    console.log(
    ...
    console.log('[VERIFY] Message computed color:', message ? window.getComputedStyle(message).color : 'not found')
  ...
  methods: {
    ...
    applyTheme() {
      console.log('[VERIFY] Applying theme:', this.theme, new Date().toISOString())
      const currentTheme = document.documentElement.getAttribute('data-theme')
      if (currentTheme !== this.theme) {
        console.warn('[VERIFY] Theme mismatch: component theme=', this.theme, 'vs DOM theme=', currentTheme)
      }
      themeManager.setTheme(this.theme)
      const title = document.querySelector('.verify-card .app-name')
      console.log(
