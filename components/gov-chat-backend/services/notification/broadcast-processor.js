...
class BroadcastProcessor {
  ...
  async processBroadcast(job) {
    ...
    if (matched === 0) {
      ...
      await this.broadcasts.finalize(
        key,
        { matched: 0, chunksTotal: 0, chunksDone: 0, sent: 0, failed: 0, pruned: 0 },
        {}
      );
      return { matched: 0 };
    }
  ...
  async processChunk(job) {
    ...
    if (result.abort) {
      await this.broadcasts.markFailed(key, {
        code: 'app/invalid-credential',
        message: 'Firebase credentials rejected'
      });
