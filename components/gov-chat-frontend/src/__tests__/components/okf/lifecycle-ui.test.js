...
const { mount, enableAutoUnmount } = require('@vue/test-utils');
// Unmount every wrapper after each test — BuildProgressCard's 5s refresh
// interval otherwise leaks across tests and consumes the next test's
// mockResolvedValueOnce queue (flaky cross-test data bleed).
...
jest.mock('@/services/okfRepoOps', () => {
  ...
  const impl = {
    ...
    // UI must react to REAL shapes, not a jest.fn(). P0-UI: publish is
    // NEVER building (serving truth / re-drain shown via the chip instead).
    isBuilding: (repo) =>
      !!repo &&
      repo.lifecycle_state !== 'publish' &&
      ((repo.conversion && !['done', 'failed'].includes(repo.conversion.status)) ||
        !!(repo.rag_ingestion && repo.rag_ingestion.status === 'draining')),
    // Mirrors okfRepoOps.isRedraining (P0-UI): publish-state repo whose
    // drain is genuinely running — keys on the drain flags, not ingested_at.
    isRedraining: (repo) =>
      !!repo &&
      repo.lifecycle_state === 'publish' &&
      (repo.rag_drain_active === true || !!(repo.rag_ingestion && repo.rag_ingestion.status === 'draining')),
    // Subject Areas load from the Knowledge Hierarchy — canned for the
    // dashboard mount (no network in jsdom).
    loadSubjectAreaOptions: jest.fn().mockResolvedValue([{ value: 'Water Supply', label: 'Water Supply' }])
  };
...
describe('OkfStudioDashboard — five lifecycle lanes', () => {
  ...
  it('renders all five lanes with their repos in the right one', async () => {
    ...
    expect(headers).toEqual(['Import', 'In review', 'Published', 'Ingested', 'Retracted']);
...
describe('BUILDING GATE (David, 2026-09-02) — building repos stay In progress', () => {
  ...
  it('laneFor follows LIFECYCLE TRUTH — building never overrides it (P0-UI, 2026-09-08)', async () => {
    const store = buildStore();
    const wrapper = mount(OkfStudioDashboard, { global: { mocks: { $store: store }, stubs: STUBS } });
    const DRAFT_BUILDING = {
      repo_id: 'db1',
      name: 'DraftBuilder',
    ...
    await seedRepos(store, [BUILDING, INDEXING, DONE, DRAFT_BUILDING]);
    await flush();
    const lanes = wrapper.findAll('.okf-dashboard__lane');
    // review/approve states sit in In review EVEN IF a conversion/drain flag
    // is stale — the old building pinning threw serving repos into Import.
    const inReview = lanes[1].text();
    expect(inReview).toContain('Builder');
    expect(inReview).toContain('DoneRepo');
    expect(inReview).toContain('Indexer');
    // a DRAFT repo with an active conversion is genuinely importing → Import
    const inProgress = lanes[0].text();
    expect(inProgress).toContain('DraftBuilder');
    const importCards = lanes[0].findAll('.okf-dashboard__card-wrap');
    expect(importCards.length).toBe(1); // only the draft builder, not the review-state one
  });
  ...
  it('hovering a building card opens the live progress metrics card (David, 2026-09-03)', async () => {
    ...
    const text = wrapper.text();
    // IMPORT phase: conversion metrics only — no RAG "concepts indexed" metric
    expect(text).not.toContain('concepts indexed');
    ...
    expect(text).toContain('stays in Import');
  });

  it('an approved repo with an active RAG drain shows index progress (import/RAG boundary)', async () => {
    const store = buildStore();
    const wrapper = mount(OkfStudioDashboard, { global: { mocks: { $store: store }, stubs: STUBS } });
    await seedRepos(store, [
      {
        ...
        rag_ingestion: { status: 'draining', concepts_total: 40, concepts_done: 30 }
      }
    ]);
    await flush();
    repoOkfService.get.mockResolvedValueOnce({
    ...
    const card = wrapper.findAll('.okf-dashboard__card-wrap').find((c) => c.text().includes('Draining'));
    await card.find('.okf-dashboard__card').trigger('mouseenter');
    await flush();
    await flush();
    const text = wrapper.text();
    ...
    expect(text).toContain('concepts indexed');
    expect(text).toContain('RAG index is building');
  });

  it('the drain popup shows the DRAIN start, never the frozen import start (David 222h, 2026-09-08)', async () => {
    const store = buildStore();
    const wrapper = mount(OkfStudioDashboard, { global: { mocks: { $store: store }, stubs: STUBS } });
    const doc = {
      repo_id: 'old-start',
      ...
      lifecycle_state: 'publish',
      concept_count: 5,
      conversion: { status: 'done', started_at: new Date(Date.now() - 9 * 24 * 3600 * 1000).toISOString() },
      rag_drain_active: true,
      rag_ingestion: {
        status: 'draining',
        concepts_total: 5,
    ...
    await seedRepos(store, [doc]);
    await flush();
    repoOkfService.get.mockResolvedValueOnce(doc);
    const card = wrapper.findAll('.okf-dashboard__card-wrap').find((c) => c.text().includes('OldStart'));
    await card.find('.okf-dashboard__card').trigger('mouseenter');
    await flush();
    await flush();
    const text = wrapper.text();
    expect(text).toContain('Started 2 min ago');
  ...
  it('a published repo mid-ingest shows Ingesting… + the drain popup (David, 2026-09-04)', async () => {
    ...
    const draining = {
      ...
      concept_count: 40,
      rag_drain_active: true,
      rag_ingestion: { status: 'draining', concepts_total: 40, concepts_done: 25 }
    };
    await seedRepos(store, [draining]);
    await flush();
    repoOkfService.get.mockResolvedValueOnce({ ...draining });
    const card = wrapper.findAll('.okf-dashboard__card-wrap').find((c) => c.text().includes('Draining'));
    expect(card.findAll('button').some((b) => b.text() === 'Ingesting…')).toBe(true);
    // P0-UI: publish-state repos NEVER render as Building/Import — the
    // mid-ingest between-state shows Published + the Re-draining chip.
    expect(card.text()).toContain('Published v2');
    expect(card.text()).toContain('Re-draining…');
    expect(card.text()).not.toContain('Building…');
    await card.find('.okf-dashboard__card').trigger('mouseenter');
    await flush();
    await flush();
    const text = wrapper.text();
    expect(text).toContain('Ingestion progress');
    expect(text).toContain('25 / 40');
    expect(text).toContain('concepts indexed');
  });

  it('an Ingested (serving) repo carries the serving-status popup (David, 2026-09-04)', async () => {
    const store = buildStore();
    const wrapper = mount(OkfStudioDashboard, { global: { mocks: { $store: store }, stubs: STUBS } });
    const serving = {
      repo_id: 'sv1',
      ...
      ingested_graph_name: 'OKF_server_v2',
      concept_count: 12,
      rag_ingestion: { status: 'completed', concepts_total: 12, concepts_done: 12 }
    };
    await seedRepos(store, [serving]);
    await flush();
    repoOkfService.get.mockResolvedValueOnce({ ...serving });
    const card = wrapper.findAll('.okf-dashboard__card-wrap').find((c) => c.text().includes('Server'));
    await card.find('.okf-dashboard__card').trigger('mouseenter');
    await flush();
    await flush();
    const text = wrapper.text();
    expect(text).toContain('Serving status');
    expect(text).toContain('Serving');
    expect(text).toContain('OKF_server_v2');
    expect(text).toContain('v2');
    expect(text).toContain('12 / 12');
    expect(text).toContain('serving RAG traffic');
  });
