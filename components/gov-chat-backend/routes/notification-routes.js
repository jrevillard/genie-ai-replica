...
// This deployment authenticates with Keycloak OIDC; the PolisenseAI branch's
// standalone auth-middleware does not exist here. Same contract: reject
// unauthenticated callers on the user-facing register/unregister routes.
...
module.exports = (notificationService) => {
  ...
  router.post('/register', keycloakAuthMiddleware.authenticate, async (req, res) => {
    try {
      const body = req.body || {};
      // req.user is the provisioned ArangoDB user document (keycloak-auth-middleware),
      // so _key is the canonical identity - the same key user-routes uses. The
      // caller may NOT supply userId: accepting body.userId here let any logged-in
      // user register a push token against someone else's account.
