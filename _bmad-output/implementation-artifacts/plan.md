...
| Date | What happened |
|------|---------------|
| 2026-09-06 (26) | **3-9's promised deferral closed + tracker hygiene.** `TestDLQReplay` (7 tests) delivers the DLQ-replay direct coverage 3-9's review deferred to 3-11 — drives `_replay_dlq` through real XRANGE-shaped data (replay+removal w/ restored timestamps, poll/other-feed entries untouched, retry-cap parking pins the destroyed-payload bug, failed replay increments retries, garbage retries→0, Redis failure never escapes). Implementation held unchanged — only coverage was missing. Tracker corrections: **3-4's line was stale** (`backlog # not in stream_ingestor` — it shipped+reviewed 2026-09-05 with 3 criticals fixed); **2-10 → deferred** per standing D5. Overlay **842 green**. **Active scope now fully caught up:** everything else is review (merge-gated on !279), 1-7 bump-gated, 2-10/5-1 deferred. Open: ADR 0004 sign-off, 4-8 deep-link call, push (14+ local commits). |
