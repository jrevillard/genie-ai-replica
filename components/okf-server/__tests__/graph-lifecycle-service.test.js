...
describe('graph-lifecycle-service (versioned serving graph)', () => {
  ...
  test('promoteGraph with NO graph content creates NO definition (born at ingest — David 2026-09-09)', async () => {
    // No seeding: sources never materialized with data — the mock treats
    // empty/absent as not-exists. Nothing moves, nothing is created: the
    // graph is born at drain time (dataprep ensure-graph), never before.
    const out = await promoteGraph(repoDoc({ lifecycle_state: 'retracted' }));
    expect(out).toBe('OKF_kenya-government-services_v2');
...
describe('BORN AT INGEST (David 2026-09-09): no empty graph shells', () => {
  test('renameForRepoNameChange of a never-ingested repo spawns NO shell definition', async () => {
    // No seedGraph: the repo has no graph content (never ingested). The
    // rename is pure metadata — no collections move, no definition is created.
    mockDb.query.mockResolvedValueOnce({ all: async () => [] }); // drain-authority read
    const repo = repoDoc({ name: 'Fresh Wiki Crawl', version: 1, lifecycle_state: 'draft' });
    const out = await renameForRepoNameChange(repo, 'Wiki Crawl');
    expect(out).toMatch(/^OKF_fresh-wiki-crawl_v\d+$/);
    expect(mockDb.createGraph).not.toHaveBeenCalled();
  });

  test('renameForRepoNameChange of a RETRACTED repo (graph dropped) also spawns NO shell', async () => {
    mockDb.query.mockResolvedValueOnce({ all: async () => [] });
    const repo = repoDoc({ name: 'Retracted Wiki', version: 2, lifecycle_state: 'retracted' });
    const out = await renameForRepoNameChange(repo, 'Wiki');
    expect(out).toMatch(/^OKF_retracted-wiki_v\d+$/);
    expect(mockDb.createGraph).not.toHaveBeenCalled();
  });
});
