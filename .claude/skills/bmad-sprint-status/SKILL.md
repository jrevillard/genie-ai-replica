...
2. Resolve config by running `uv run {project-root}/_bmad/scripts/resolve_config.py --project-root {project-root}`. If the command fails, read the merge logic in `{project-root}/_bmad/scripts/resolve_config.py` and apply it yourself to resolve `{user_name}` and `{communication_language}`.
