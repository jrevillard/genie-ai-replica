...
INSTRUCTION_FORMAT = """
You are a safety classifier. Your task is to analyze the input text and determine if it contains any harmful or malicious content.
To perform this task you first evaluate if the input text includes any of the below:
- Hate speech or discrimination
- Harassment or bullying
- Threats or incitement to violence
- Sexually explicit material
- Malicious code, dangerous instructions or phishing attempts
If it does, you return your response in JSON format with a single key "is_the_text_safe" and a value "no"
If it does not, you return your response in JSON format with a single key "is_the_text_safe" and a value "yes"
Please analyse and classify this input text:
"""
...
class OpeaGuardrailsGemmaGuard(OpeaComponent):
    ...
    async def invoke(self, input: Union[GeneratedDoc, TextDoc]):
        ...
        if isinstance(input, GeneratedDoc):
            message = INSTRUCTION_FORMAT + '\nINPUT:\n' + input.prompt + " " + input.text + '\nOUTPUT:\n'
            if logflag:
                logger.info(f"Input text for guardrails check: {message}")
        else:
            message = INSTRUCTION_FORMAT + '\nINPUT:\n' + input.text + '\nOUTPUT:\n'
            if logflag:
                logger.info(f"Input text for guardrails check: {message}")
        ...
        while yes_or_no != 'yes' and yes_or_no != 'no':
            response_input_guard = await asyncio.to_thread(self.llm_guard.invoke, message)
            print('🍓🍓🍓🍓🍓🍓🍓🍓🍓🍓🍓🍓🍓🍓🍓')
            print(response_input_guard)
            print(type(response_input_guard))
            print('🍓🍓🍓🍓🍓🍓🍓🍓🍓🍓🍓🍓🍓🍓🍓')
            if hasattr(response_input_guard, "content"):
                try:
                    data = json.loads(response_input_guard.content)
                    yes_or_no = data.get('is_the_text_safe', None).lower()
                except:
                    continue
            else:
                continue
    ...
    def check_health(self) -> bool:
        ...
        try:
            ...
            if isinstance(response, str):
                ...
                return False
            return "is_the_text_safe" in data
